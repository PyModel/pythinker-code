...
- `apps/pythinker-web`: the browser web UI, a peer to the TUI. Vue 3 + Vite + vue-i18n; talks to the server over REST + WebSocket under `/api/v1`. It must not depend on `@pymodel/agent-core` (wire types are re-implemented locally). See `apps/pythinker-web/AGENTS.md`.

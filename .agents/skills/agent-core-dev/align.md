...
| Errors | `from '../../errors'` (central `PythinkerError`, `ErrorCodes`) | `from '#/_base/errors'` + domain co-located `XxxError` — see errors.md |
...
| `agent/goal/`, `agent/plan/`, `agent/dynamic_workflow/`, `agent/cron/`, `agent/background/` | `goal`, `plan`, `dynamic_workflow`, `cron`, `background`, `subagentHost` |
...
import { PythinkerError, ErrorCodes } from '../../errors';

// v2
import { createDecorator, type ServiceIdentifier } from '#/_base/di/instantiation';
import { Disposable } from '#/_base/di/lifecycle';
import { IInstantiationService } from '#/_base/di/instantiation';
import { PythinkerError, type ErrorCode } from '#/_base/errors';
```
...
**Errors** — move any shared error into a co-located `XxxError extends PythinkerError` with a registered `code` (errors.md). Do not keep throwing v1's central error codes from a v2 domain.
...
```ts
// v1
import { TestInstantiationService } from '@pymodel/agent-core/di/test';

...
If any fail → wrap in a **facade** (a Service that takes ids, returns data, throws `PythinkerError`) and expose the facade. The repo already ships a wire-shaped facade in `rpc/core-api.ts` (`CoreAPI` / `SessionAPI` / `AgentAPI`) behind `IAgentRPCService` / `ISessionRPCService` — prefer building the HTTP edge on top of it rather than re-deriving a new one.
...
| `dynamic_workflow` | `isActive` | IDynamicWorkflowService.isActive | GET |
| `dynamic_workflow` | `enter` / `exit` | IDynamicWorkflowService.* | POST |

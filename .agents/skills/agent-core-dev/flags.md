...
- `src/app/flag/flagService.ts` — `FlagService` impl + `MASTER_ENV` (`PYTHINKER_CODE_EXPERIMENTAL_FLAG`); reads definitions from `IFlagRegistry` and overrides from `IConfigService`; self-registers at App scope.
...
1. Master env `PYTHINKER_CODE_EXPERIMENTAL_FLAG` truthy → every flag on.
2. Per-feature `def.env` (e.g. `PYTHINKER_CODE_EXPERIMENTAL_MY_FEATURE`) → forces on/off.
...
export const myFeatureFlag: FlagDefinitionInput = {
  ...
  title: 'My feature',
  description: '...',
  env: 'PYTHINKER_CODE_EXPERIMENTAL_MY_FEATURE',
  default: false,
  surface: 'both',
};
...
- `env` must start with `PYTHINKER_CODE_EXPERIMENTAL_`, be unique, and not equal `PYTHINKER_CODE_EXPERIMENTAL_FLAG`.
...
- `env` must start with `PYTHINKER_CODE_EXPERIMENTAL_`, be unique, and not equal `PYTHINKER_CODE_EXPERIMENTAL_FLAG`; `id` must not be `flag`.

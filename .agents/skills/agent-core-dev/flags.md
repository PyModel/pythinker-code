...
- `src/flag/flagService.ts` — `FlagService` impl + `MASTER_ENV` (`PYTHINKER_CODE_EXPERIMENTAL_FLAG`) + `EXPERIMENTAL_SECTION` (`experimental`); reads definitions from `IFlagRegistry`; self-registers at App scope.

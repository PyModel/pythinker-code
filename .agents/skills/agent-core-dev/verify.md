...
Run from the package (or with `--filter @pymodel/agent-core-v2`):

- `pnpm --filter @pymodel/agent-core-v2 lint:imports` — import-boundary guard (`scripts/check-import-boundaries.mjs`). Catches v1 imports (`@pymodel/agent-core`) and kosong subtree violations.
- `pnpm --filter @pymodel/agent-core-v2 typecheck` — `tsc -p tsconfig.json --noEmit`.
- `pnpm --filter @pymodel/agent-core-v2 test` — `vitest run`.
...
- Do not list internal packages in a changeset when the change enters the CLI bundle — list `@pymodel/pythinker-code` and describe the real change.

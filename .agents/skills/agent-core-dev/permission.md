...
2. **Heterogeneous policies.** Some check a tool-name set, some count same-batch `AgentDynamicWorkflow` calls, some run a hook, some inspect the plan state machine — no uniform `(sub, obj, act)` shape.
...
| tool-batch exclusivity | `dynamic_workflow` domain — `onBeforeExecuteTool` veto listener | harness constraint (off-chain) |
...
1. ~~**Sink domain dimensions.**~~ **Done** — plan guard/review, goal-start review, dynamic_workflow batch exclusivity, and btw deny-all moved out of the chain into their owning domains as `onBeforeExecuteTool` veto listeners (immediate `veto` / `allow` / `pass` statements plus cold `waitUntil` factories for approval round-trips); the shared approval round-trip was extracted to `IAgentToolApprovalService`; `registerPolicy` was removed (btw was its only production user). The chain now holds 12 risk-adjudication nodes only.

# Operations: running the pythinker-webbridge daemon

Read this when the user asks about pythinker-webbridge itself — install, start, stop, restart, upgrade, "is it running", "why isn't the extension connected", moving it to another port. Mid-task recovery (the daemon becomes unreachable while you are driving the browser) is covered in SKILL.md and needs nothing from here.
...
Everything below lives under `~/.pythinker-webbridge/` (Windows: `%USERPROFILE%\.pythinker-webbridge\`).

## What is on disk

- `bin/pythinker-webbridge` (`bin\pythinker-webbridge.exe`) — the binary. It is not on PATH, so call it by this path. `command not found` means it is not installed.
...
| `uninstall [-y]` | Stops the daemon and deletes `~/.pythinker-webbridge/` — binary, config, logs, everything | Only when the user explicitly asks |
...
3. Ask the user to point the extension at it once: Pythinker side panel → Settings → Local agent remote control → Connection address → `ws://127.0.0.1:<port>/ws`. The extension remembers it.
...
- `update_available` `{current, latest, command}` — a newer release exists and the extension's major.minor allows it; `command` is `pythinker-webbridge upgrade`.

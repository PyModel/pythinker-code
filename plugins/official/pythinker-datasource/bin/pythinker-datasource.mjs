...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Get the current API documentation for one Pythinker data source before calling a specific API. For a simple lookup, choose exactly one specialized source; do not inspect fallback or comparison sources unless the user explicitly asks for a cross-source comparison.',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
...
async function loadAccessToken() {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      throw new Error(`Failed to parse Pythinker Code credentials file: ${error.message}`);
    }
    throw error;
  }
...
async function callPythinkerTool(method, params, trace = {}) {
  const { pythinkerHome, token: initialToken } = await loadAccessToken();
  ...
  try {
    const request = async (accessToken) => {
      const response = await fetch(API_URL, {
        method: 'POST',
        headers: await buildHeaders(pythinkerHome, accessToken, toolCallId),
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error('Pythinker Code access_token was rejected. Run /login again and retry.');
      }

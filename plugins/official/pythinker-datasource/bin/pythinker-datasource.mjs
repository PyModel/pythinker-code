#!/usr/bin/env node
// Stdio MCP server for pythinker-datasource.
//
// Speaks newline-delimited JSON-RPC 2.0 on stdin/stdout per the MCP "stdio"
// transport. Implements the minimal surface the Pythinker Code host calls:
//   - initialize
...
const VERSION = '3.2.0';
const DEFAULT_PYTHINKER_CODE_OAUTH_HOST = 'https://auth.pythinker.com';
const DEFAULT_PYTHINKER_CODE_BASE_URL = 'https://api.pythinker.com/coding/v1';
...
const TOOLS = [
  {
    ...
    description:
      "Dispatch a call to any registered data source's API via the Pythinker Code gateway. Always call get_data_source_desc(name) first to learn that source's available APIs and required params, then construct this call with api_name and params taken from that description.",
    inputSchema: {
      type: 'object',
      properties: {
        data_source_name: {
  ...
  },
  {
    ...
    description:
      'Get the current API documentation for one Pythinker data source before calling a specific API.',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
  ...
  switch (method) {
    case 'initialize':
      return {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'pythinker-datasource', version: VERSION },
      };
    case 'ping':
      return {};
    case 'tools/list':
...
async function runTool(params) {
  ...
  try {
    ...
    const response = await callPythinkerTool(handler.method, built, trace);
...
function appendTrace(text, trace) {
  ...
  return `${text}\n\n[pythinker-datasource] ${parts.join(' · ')}`;
}

function resolvePythinkerHome() {
  const explicit = process.env.PYTHINKER_CODE_HOME?.trim();
  return explicit && explicit.length > 0 ? explicit : path.join(homedir(), '.pythinker-code');
}

function datasourceApiUrl() {
  const explicit = process.env.PYTHINKER_DATASOURCE_API_URL?.trim();
  if (explicit !== undefined && explicit.length > 0) return explicit;
  return `${pythinkerCodeBaseUrl()}/tools`;
}

function pythinkerCodeBaseUrl() {
  return (process.env.PYTHINKER_CODE_BASE_URL ?? DEFAULT_PYTHINKER_CODE_BASE_URL).replace(/\/+$/, '');
}

function pythinkerCodeOAuthHost() {
...
function resolvePythinkerCodeCredentialName() {
  const oauthHost = pythinkerCodeOAuthHost();
  const baseUrl = pythinkerCodeBaseUrl();
  if (
    oauthHost === normalizeEndpoint(DEFAULT_PYTHINKER_CODE_OAUTH_HOST) &&
    baseUrl === DEFAULT_PYTHINKER_CODE_BASE_URL
  ) {
    return 'pythinker-code';
  }
...
async function loadAccessToken() {
  const pythinkerHome = resolvePythinkerHome();
  const credentialsFile = path.join(
    pythinkerHome,
    'credentials',
    `${resolvePythinkerCodeCredentialName()}.json`,
  );
  let parsed;
  try {
    ...
    if (isNotFound(err)) {
      throw new Error(
        `Pythinker Code credentials file not found: ${credentialsFile}\nRun /login in Pythinker Code first.`,
      );
    }
    if (err instanceof SyntaxError) {
      throw new Error(`Failed to parse Pythinker Code credentials file: ${err.message}`);
    }
    throw err;
  }

  if (!isRecord(parsed)) {
    throw new Error(`Invalid Pythinker Code credentials file: ${credentialsFile}`);
  }
  const token = typeof parsed.access_token === 'string' ? parsed.access_token : '';
  if (token.length === 0) {
    throw new Error('Pythinker Code credentials do not contain access_token. Run /login again.');
  }
  const expiresAt = typeof parsed.expires_at === 'number' ? parsed.expires_at : 0;
  if (expiresAt > 0 && expiresAt <= Math.floor(Date.now() / 1000)) {
    throw new Error('Pythinker Code access_token has expired. Run /login again and retry.');
  }
  return { pythinkerHome, token };
}

async function callPythinkerTool(method, params, trace = {}) {
  const { pythinkerHome, token } = await loadAccessToken();
  ...
  try {
    const response = await fetch(API_URL, {
      method: 'POST',
      headers: await buildHeaders(pythinkerHome, token, toolCallId),
...
async function buildHeaders(pythinkerHome, token, toolCallId) {
  return {
    Authorization: `Bearer ${token}`,
    'Content-Type': 'application/json',
    'X-Msh-Tool-Call-Id': toolCallId,
    'X-Msh-Platform': asciiHeader(process.env.PYTHINKER_MSH_PLATFORM ?? 'pythinker-code-cli'),
    'X-Msh-Version': asciiHeader(process.env.PYTHINKER_MSH_VERSION ?? VERSION),
    'X-Msh-Device-Name': asciiHeader(process.env.PYTHINKER_MSH_DEVICE_NAME ?? hostname()),
    'X-Msh-Device-Model': asciiHeader(process.env.PYTHINKER_MSH_DEVICE_MODEL ?? deviceModel()),
    'X-Msh-Os-Version': asciiHeader(process.env.PYTHINKER_MSH_OS_VERSION ?? release()),
    'X-Msh-Device-Id': asciiHeader(process.env.PYTHINKER_MSH_DEVICE_ID ?? (await createDeviceId(pythinkerHome))),
    'User-Agent': `pythinker-datasource/${VERSION}`,
  };
}

async function createDeviceId(pythinkerHome) {
  const deviceIdPath = path.join(pythinkerHome, 'device_id');
  try {
  ...
  const id = randomUUID();
  try {
    await mkdir(pythinkerHome, { recursive: true, mode: 0o700 });

...
Pythinker Code CLI supports three MCP server connection methods:
...
- **User level**: `~/.pythinker-code/mcp.json` (or `$PYTHINKER_CODE_HOME/mcp.json`), shared across projects
- **Project level**: `.pythinker-code/mcp.json` in the working directory, effective only for the current repository

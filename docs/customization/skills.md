...
Agent Skills are a lightweight mechanism for extending model capabilities in Pythinker Code CLI. A Skill is a Markdown document with YAML frontmatter that describes a specialized area of knowledge or a workflow: a project's code style guidelines, a PR review process, or a commit message format.
...
│   ├── SKILL.md            # Main file
│   └── checklist.md        # Supporting file, referenced via ${PYTHINKER_SKILL_DIR}
...
- A flat Skill has no directory of its own, so `${PYTHINKER_SKILL_DIR}` points at the skills directory itself — switch to the directory form whenever the Skill needs supporting files.
...
- `${PYTHINKER_SKILL_DIR}`: The directory containing the current Skill file
...
Pythinker Code CLI scans four tiers by scope; more specific scopes take higher priority: **Project > User > Extra > Built-in**

**User level** (applies to all projects):
- `$PYTHINKER_CODE_HOME/skills/` (default: `~/.pythinker-code/skills/`)
- `~/.agents/skills/`

The Pythinker-specific user Skill directory moves with `PYTHINKER_CODE_HOME`, so isolated data roots also get isolated Pythinker-specific Skills. The generic `~/.agents/skills/` directory stays under the real OS home so it can be shared across tools.
...
**Built-in Skills** are distributed with the CLI and have the lowest priority. They provide out-of-the-box workflows for common tasks: configuring MCP servers, customizing the TUI theme, and editing config files. See [Built-in skill commands](../reference/slash-commands.md#built-in-skill-commands) for the full list. Those describing Pythinker Code itself can be turned off with the top-level [`builtin_product_skills`](../configuration/config-files.md#top-level-fields) field.
...
Save this as `$PYTHINKER_CODE_HOME/skills/review-pr/SKILL.md` (or `~/.pythinker-code/skills/review-pr/SKILL.md` when `PYTHINKER_CODE_HOME` is unset), place the checklist at `references/checklist.md` in the same directory, and after starting a new session you can invoke it with `/skill:review-pr #1234`, where `#1234` is expanded into `$pr_ref`.

...
Built-in tools are the tool set provided by Pythinker Code CLI alongside its core engine — no MCP server installation required. The Agent automatically selects and calls these tools based on the task at hand during each conversation; users can inspect the details of each tool call through the approval interface.
...
- `run_in_background`: whether to run as a background task; background tasks default to a 10-minute timeout (no timeout by default in print mode `pythinker -p`)
...
| `AgentDynamicWorkflow` | Auto-allow in dynamic_workflow mode; otherwise requires approval | Launch item-based subagents or resume existing subagents |
...
The entire feature is experimental and off by default. Enable it with `PYTHINKER_CODE_EXPERIMENTAL_NOTIFY_USER=1`, `[experimental] notify_user = true` in `config.toml`, or `/experiments` before creating a TUI session. Sessions created while it is disabled have neither the tool nor its prompt guidance.

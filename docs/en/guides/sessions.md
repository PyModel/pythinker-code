...
As a conversation grows, Pythinker Code CLI automatically compresses the message history when the context approaches the window limit, freeing up token space. You can also trigger compression manually at any time:
...
The browser buffers the ZIP before saving it, so web exports are limited to 64 MiB. For a larger session, use `pythinker export <sessionId>` or the TUI `/export-debug-zip` command.
...
- [pythinker command reference](../reference/pythinker-command.md) — complete parameter reference for `--continue`, `--session`, `export`, and other commands

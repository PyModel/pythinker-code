...
Start Pythinker Code CLI with remote control enabled by running `pythinker rc` in a terminal — it generates a link that can remotely control this machine. Scan the QR code with your phone to open the link, or visit it directly on another device. After opening the link, log in with the same Pythinker account as in your local Pythinker Code CLI to check on task progress, handle approvals, continue conversations, or start new sessions. Tasks always run on your machine — the web page is just a remote window.
...
- **Pythinker Code CLI installed**: see [Getting started](../guides/getting-started.md)
- **Logged in to your Pythinker account with a paid membership**: Remote Control requires a paid membership and is not available to free users
- **Machine stays awake and online**: Remote Control depends on a persistent connection between your machine and the Pythinker service; remote sessions are unavailable after shutdown, sleep, or network loss
...
- **`pythinker rc`** (alias `pythinker remote`): start Remote Control directly
- **`pythinker web --remote-control`**: equivalent to `pythinker rc` — starts the local web interface and exposes it to the public internet at the same time
...
![Terminal output after starting pythinker rc: QR code and connection status](../../media/pythinker-rc-banner.jpg)
...
- Remote Control cannot be combined with `--dangerous-bypass-auth`, and it only binds to the loopback address (`--host` LAN sharing is not supported — remote access goes through the Pythinker relay service)
...
2. Log in with the same Pythinker account as on the machine.
...
- **Can't find the terminal**: the single-instance lock file `~/.pythinker-code/server/rc.json` records the process pid and the link in use (the error from starting a second instance prints both as well) — run `kill <pid>`
...
| Session sync | Via the Pythinker service |
...
## What's the difference between Remote Control and Pythinker Code Web?

[Pythinker Code Web](../guides/web.md) is the graphical interface on your machine or LAN; Remote Control extends it to any device on the public internet:

| | Pythinker Code Web | Remote Control |
| --- | --- | --- |
| Access scope | `localhost`, or the LAN with `--host` | Any device on the public internet (via the Pythinker relay) |
| How to start | Run `pythinker web` in a terminal | `pythinker rc`, `pythinker web --remote-control`, or `/remote-control` in the CLI |
| Authentication | Local token | Log in with the same Pythinker account |
...
For the web interface's features, see [Using Pythinker Code in the browser](../guides/web.md).

## Security and permissions

### How remote devices authenticate

A remote device must log in with the same Pythinker account as the machine to view and control sessions. Your devices are never exposed to other accounts, and there is no public link that works without logging in.
...
4. **Account match**: is the web side logged in with the same Pythinker account
5. **Firewall and proxy**: is your corporate network or proxy blocking `code-rc.kimi.com`

## Next steps

- [Using Pythinker Code in the browser](../guides/web.md) — Remote Control opens the same web interface; learn what the interface itself can do

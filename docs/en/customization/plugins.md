...
- **Official**: Pythinker-maintained marketplace plugins
- **Curated**: Third-party plugins from Pythinker partners in the default marketplace
...
Official plugins are plugins and built-in product capabilities maintained by Pythinker. There are currently three:

- **[Pythinker Datasource](#pythinker-datasource)**: Query financial market data, financial news, macroeconomic indicators, corporate registration records, academic literature, Chinese laws and regulations, and official data from intergovernmental organizations in natural language
- **[Pythinker Pythinker Browser Extension](#pythinker-webbridge)**: Let AI drive your own browser to get web tasks done
- **[Pythinker Computer Use](#pythinker-computer-use)**: Let AI operate your desktop apps (macOS and Windows)
...
::: info Note
Pythinker Pythinker Browser Extension installs in two parts: after the steps above, you also need to [install the browser extension](#install-the-browser-extension) before it works.
:::
...
### Pythinker Datasource <Badge type="tip" text="v3.4.0" />

Pythinker Datasource is the official Pythinker Code data plugin, letting you query financial market data, financial news, macroeconomic indicators, corporate registration records, academic literature, Chinese laws and regulations, and official data from intergovernmental organizations in natural language. No manual API calls or data accounts required.
...
You must first complete OAuth login with a Pythinker Code account via `/login`; data queries consume your Pythinker Code plan quota.

#### How to use

1. Describe your need in natural language, and Pythinker Code will automatically invoke the data capabilities
2. Explicitly trigger the data query skill with `/skill:pythinker-datasource`
...
### Pythinker Pythinker Browser Extension <Badge type="tip" text="v1.11.3" />

Pythinker Pythinker Browser Extension lets AI drive your browser directly: not an emulator, not a crawler, but the browser you use every day, with your login sessions and cookies. AI can open pages, read content, click buttons, fill in forms, and take screenshots just like you do, taking repetitive web operations off your hands. See the [Pythinker Pythinker Browser Extension site](https://www.kimi.com/features/webbridge) for a product overview.

#### Install the browser extension

After installing via `/plugins`, you also need the Pythinker Pythinker Browser Extension extension in your browser before AI can drive it. There are two ways to install it:

**Option 1: Install from a store (recommended)**

Open the [Chrome Web Store](https://chromewebstore.google.com/detail/pythinker-webbridge/fldmhceldgbpfpkbgopacenieobmligc) or [Edge Add-ons](https://microsoftedge.microsoft.com/addons/detail/pythinker-webbridge/bnlffdbcfnanfbknnlaflhlhkocccckg) page and click Add.

**Option 2: Install manually**

Use this when you can't reach the stores:

1. [Download the extension package](https://pythinker-web-img.pymodel.cn/webbridge/latest/extension/pythinker-webbridge-extension.zip) and unzip it
...
3. Click **Load unpacked** in the top-left corner and select the unzipped `pythinker-webbridge-extension` folder

   ![Load the unpacked extension](../../media/webbridge-load-unpacked.jpeg)

4. Once installed, the Pythinker Pythinker Browser Extension icon appears in the browser toolbar. Seeing the icon means the installation succeeded, and AI can start working on web pages for you.

   ![The Pythinker Pythinker Browser Extension icon in the browser toolbar](../../media/webbridge-install-success.jpeg)
...
### Pythinker Computer Use <Badge type="tip" text="v0.5.4" />

Pythinker Computer Use lets AI operate your desktop apps directly, clicking, dragging, scrolling, and typing. The macOS version works silently in the background without taking over your mouse (a few popup actions may still bring an app to the foreground); see [the notes below](#notes-for-the-windows-version) for how the Windows version differs.

#### Authorization (macOS)

The first time you use Pythinker Computer Use after installation, it shows an authorization window. Just follow the prompts:
...
2. Turn on the **Pythinker Code** switch under "Connect local agents", then restart Pythinker Code for it to take effect

<div style="max-width: 380px; margin: 0 auto;">

![Pythinker Computer Use authorization window](../../media/pythinker-computer-use-auth.jpeg)
...
- **Matching privilege level**: If the target app runs as administrator, PythinkerCU must run at the same privilege level
...
When both files exist, `pythinker.plugin.json` takes precedence.
...
System-prompt contributions take effect on every Pythinker Code surface: the interactive TUI, `pythinker -p`, and `pythinker web` all run on the v2 engine.
...
In the manifest (`pythinker.plugin.json`), the `commands` field points to where the command files live:
...
```text
/pythinker-finance:report TSLA
```

Pythinker replaces `$ARGUMENTS` in the body with `TSLA`, then runs the prompt. The three details below cover each step.
...
Commands are prefixed with the plugin id (their namespace) and registered as `<plugin>:<command>`, so the command above is actually `/pythinker-finance:report`. This keeps same-named commands from different plugins from colliding.
...
- The hook process receives two extra environment variables: `PYTHINKER_CODE_HOME` and `PYTHINKER_PLUGIN_ROOT` (the plugin root directory).

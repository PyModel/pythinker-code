...
- **[Pythinker Datasource](#pythinker-datasource)**: Query financial market data, macroeconomic indicators, corporate registration records, academic literature, and Chinese laws and regulations in natural language
...
### Pythinker Datasource <Badge type="tip" text="v3.3.0" />

Pythinker Datasource is the official Pythinker Code data plugin, letting you query financial market data, macroeconomic indicators, corporate registration records, academic literature, and Chinese laws and regulations in natural language — no manual API calls or data accounts required.
...
System-prompt contributions take effect on both agent engines. The interactive TUI, `pythinker -p`, and `pythinker web` use the v2 engine by default; setting `PYTHINKER_CODE_LEGACY_FLAG=1` routes the local CLI surfaces to the legacy engine.

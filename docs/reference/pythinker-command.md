...
When resuming a session, you can override its saved permission or plan mode by adding `--auto`, `--yolo`, or `--plan`. For example, `pythinker --continue --auto` resumes the latest session and switches it to Never Ask mode.

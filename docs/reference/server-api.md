...
The local server started by `pythinker web` exposes two programmatic surfaces: a REST API (`/api/v1`, plus `/api/v2/sessions` and `/api/v2/mcp`) and a WebSocket event stream (`/api/v1/ws`). This page is the protocol reference for both. For how to start the server and its command-line options, see the [pythinker command](./pythinker-command.md#pythinker-web) reference; for an end-to-end walkthrough, see [Drive a session over the API](#drive-a-session-over-the-api) below.
...
The default address is `http://127.0.0.1:58627`. When the port is taken, the server retries with the next port (up to 100 times); use `--port` / `--host` to change the bind. Multiple instances can coexist under the same home directory; running instances register under `~/.pythinker-code/server/instances/`.
...
How to carry it: REST uses the `Authorization: Bearer <token>` header; the WebSocket upgrade accepts the same header or the subprotocol `pythinker-code.bearer.<token>`. Token generation and rotation are covered in [Using Pythinker Code in the browser: Getting started](../guides/web.md#getting-started).
...
Resolves which Pythinker region this client belongs to. The answer is derived locally, not probed over the network: an OAuth host pinned by environment or config wins first, then the configured OAuth key, then the region marker file in the home directory; the default is `mainland-cn`.
...
| `type` | string | Wire protocol: `pythinker` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
| `type` | body | string | **Required.** Wire protocol: `pythinker` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
| `dynamic_workflow_mode` | boolean | Enter or exit dynamic_workflow mode |
...
Requests a generated title from the session's prompts and applies it, broadcasting `session.meta.updated`. Without a title-generation backend the route returns unavailable. Without `force`, a session that already has a custom or generated title is reported unavailable instead of being overwritten.
...
- `40401`: session not found
- `40923`: generation unavailable — no title backend, no prompt content yet, an existing title without `force`, or the backend request failed
...
On success, `data` is `{ busy, model?, thinking_level, permission, plan_mode, dynamic_workflow_mode, context_tokens, max_context_tokens?, context_usage? }`: `busy` reports an active turn, `model` / `thinking_level` / `permission` are the effective agent settings, `plan_mode` / `dynamic_workflow_mode` are the mode flags, and `context_tokens` with `max_context_tokens` and `context_usage` (0–1) describe context-window consumption.
...
Exports the session together with diagnostic logs as a zip attachment (`pythinker-session-<id>.zip`). The response is a binary stream, not a JSON envelope — capabilities and failure semantics are covered under [Binary and streaming endpoints](#binary-and-streaming-endpoints).
...
The schema also accepts `metadata`, `plan_mode`, `dynamic_workflow_mode`, `goal_objective`, and `goal_control`, but the submit route currently does not apply them. Each `content` part is an object discriminated by `type`:
...
| `persist` | body | boolean | Defaults to `true`: appends to `workspace.additional_dir` in `<project root>/.pythinker-code/local.toml`. With `false`, the directory only joins the in-memory ephemeral set shared by all sessions of the workspace |
...
- [Using Pythinker Code in the browser](../guides/web.md) — start the server and use Pythinker Code in a browser
- [pythinker command](./pythinker-command.md#pythinker-web) — all `pythinker web` command-line options

...
`pythinker acp` switches Pythinker Code CLI to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive Pythinker's sessions, prompts, and tool calls.

```sh
pythinker acp
```

Once started, the command prints no banner and immediately waits for the ACP client to send an `initialize` request on stdin. Logs are written to stderr (as well as the diagnostic log under `~/.pythinker-code/logs/`), so the ACP channel itself stays clean.

::: tip Who calls this?
You typically do not need to run `pythinker acp` manually — this command is the subprocess entry point for IDEs. For IDE-side configuration, see [Using in IDEs](../guides/ides.md).
:::
...
**Summary: the ACP server implements the core (2/3) and session (11/11) agent-side surface, 10/11 client reverse-RPC methods, and the `session/set_model` extension. Not implemented: `providers/*`, `nes/*`, `document/*`, and `elicitation/complete` — requests for them return `methodNotFound`.**

### Core agent-side — IDE → agent (2 / 3)

| Method | Implemented | Description |
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'Pythinker Code CLI', version }`, capability matrix, and an empty `authMethods` list: Pythinker Code authenticates with the configured provider credentials, not with an ACP login |
| `authenticate` | Yes | No login method is advertised; gated calls return `auth_required (-32000)` until a usable provider credential is configured |
| `logout` | No | Provider credentials are managed with `pythinker provider`, not over ACP |

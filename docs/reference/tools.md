...
**`NotifyUser`** lets the main agent and subagents send a short progress update using a single `message` parameter with light Markdown. The TUI Updates panel groups messages by agent. Press `Ctrl-N` to focus the panel, then `←`/`→` to switch channels and `↑`/`↓` to page messages. The next main-agent turn clears the panel. Failed, interrupted, or suppressed notifications do not enter the panel.

The feature is experimental and off by default. Enable it with `PYTHINKER_CODE_EXPERIMENTAL_NOTIFY_USER=1`, `[experimental] notify_user = true` in `config.toml`, or `/experiments` before creating a TUI session. Sessions created while it is disabled have neither the tool nor its prompt guidance. If a session was created with the feature disabled, start a new session to use Updates.

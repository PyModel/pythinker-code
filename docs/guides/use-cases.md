...
When taking over an unfamiliar repository, a good first step is to use `pythinker --plan` or press `Shift-Tab` to enter Plan mode, so the agent outputs a research plan before touching anything:

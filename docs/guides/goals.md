...
1. Use goals for work with a clear finish line and verifiable evidence.
    ...
    Pythinker Code can inspect test output, change files, rerun checks, and decide when the goal is complete.
...
- **complete**: the objective is done, Pythinker Code clears the goal, and the agent summarizes how it completed the work
...
Upcoming goals are not visible to the agent while the current goal is running. When the current goal completes, Pythinker Code starts the first upcoming goal in the same way as users enter `/goal <objective>`.
...
If the current goal is paused, canceled, or blocked, Pythinker Code does not start the next upcoming goal. When a goal blocks and upcoming goals exist, the TUI reminds you that they wait for completion.

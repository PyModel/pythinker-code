...
Pythinker Code CLI has three places where runtime parameters can be influenced: the config file, command-line options, and environment variables. They are not a simple priority stack: the three serve different scenarios and have non-overlapping scopes:
...
This distinction matters: many users run `export PYTHINKER_API_KEY=xxx` in the shell expecting the CLI to pick it up automatically, but it does not. See [Provider credentials](#provider-credentials) below for why.
...
2. **Runtime switches**: A small set of variables like `PYTHINKER_DISABLE_TELEMETRY` directly shut down the corresponding subsystem. Even if `config.toml` has `telemetry = true`, a truthy value for this variable disables telemetry. The semantics are "additionally disable", not "ordinary override".
3. **Runtime endpoints and diagnostics**: Variables like `CUSTOM_OAUTH_HOST`, `CUSTOM_API_BASE_URL`, and `PYTHINKER_LOG_LEVEL` are read when the OAuth or logging subsystems initialize. For the full list, see [Environment variables](./env-vars.md).
...
::: warning
**Ordinary runtime parameters do not fall back to shell environment variables.** Provider `api_key` / `base_url` are read only from `config.toml` (including the `[providers.<name>.env]` sub-table) and do not fall back to `export`-ed shell variables. The only exception is the explicit `PYTHINKER_MODEL_*` channel; see [Define a model from environment variables](./env-vars.md#define-a-model-from-environment-variables-pythinker_model_).
:::
...
2. The matching key inside the `[providers.<name>.env]` sub-table (`PYTHINKER_API_KEY`, `ANTHROPIC_API_KEY`, etc.): consulted only when `api_key` is empty

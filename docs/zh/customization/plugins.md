...
Plugins 把可复用的 Pythinker Code CLI 能力打包成可安装单元——可以添加 [Agent Skills](./skills.md)、在会话启动时自动加载指定 Skill，也可以声明 MCP servers 来提供真实工具能力。适合把工作流共享给团队、连接外部服务，或从官方 marketplace 安装扩展。

Pythinker Code CLI 对 plugin 采用保守的加载策略：安装 plugin 时不会执行其中的 Python、Node.js、Shell、hook 或命令脚本。
...
Plugin 管理器会展示每个安装的来源和信任徽章：`pythinker-official`（来自官方地址）、`curated`（来自精选地址）、`third-party`（其他所有情况）。
...
Pythinker Datasource 是 Pythinker Code 官方数据插件，让你通过自然语言直接查询金融行情、宏观经济、企业工商、学术文献和中国法律法规，无需手动调用接口或申请任何数据账号。
...
2. 找到 **Pythinker Datasource**，按 `Space` 安装

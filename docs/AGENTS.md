...
| MCP | MCP | yes | yes |
| Pythinker Code CLI | Pythinker Code CLI | yes | yes |
...
| | Pythinker Code platform | Kimi Open Platform |
|---|---|---|
...
| API key entry | [Pythinker Code console](https://www.kimi.com/code/console) | [platform.kimi.com](https://platform.kimi.com) |

Rules:
- When documenting Pythinker Code CLI or VS Code: always use `api.kimi.com/coding/…`. Never write `api.moonshot.cn` in this context.
...
- **Callout titles**: Use short category titles for callout blocks (`::: tip`, `::: warning`, `::: info`, `::: danger`). Put the detailed description in the block content, not the title.
  ...
  - ✓ `::: warning 注意` + content `部分 \`.agents\` 资源不受 \`PYTHINKER_CODE_HOME\` 影响。...`
  - ✗ `::: warning 不影响 .agents` (title too long, should be in content)
  - ✗ `::: tip .agents 路径独立于 PYTHINKER_CODE_HOME` (title too long)
...
Pythinker Code CLI requires Node.js 24.15.0 or later. We recommend using pnpm for installation and management.

If you haven't installed pnpm yet, please refer to the pnpm installation docs first. Install Pythinker Code CLI:
...
- **Platform URLs**: `api.kimi.com/coding/…` for Pythinker Code platform; `api.moonshot.cn/v1` for Open Platform — never mix the two

...
1. Install from the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=moonshot-ai.pythinker-code)
...
The extension runs the Pythinker Code Node SDK in the VS Code Extension Host. When
the extension and the Pythinker Code terminal app resolve to the same
`PYTHINKER_CODE_HOME`, they share `config.toml`, MCP configuration, login state, and
sessions. The system-level `PYTHINKER_CODE_HOME` environment variable is supported;
...
legacy data it finds. Migration copies or merges data into the current Pythinker Code
home and does not delete the legacy source. Legacy Pythinker Code OAuth and MCP OAuth
...
Official doc for Pythinker Code can be found at [www.kimi.com/code/docs](https://www.kimi.com/code/docs/en/pythinker-code-for-vscode/guides/getting-started.html)

...
The `0.5.x` VS Code extension launched a separately installed Python Pythinker CLI
...
editor and Pythinker Code.

Version `0.6.0` moves the extension into this monorepo under `apps/vscode` and
runs the stable TypeScript v1 engine through `@pymodel/pythinker-code-sdk` in the
...
- Share Pythinker Code configuration, authentication, MCP configuration, and
  sessions with the TUI when both processes resolve the same Pythinker Code home.
...
- Adding TUI-only features such as goals, cron, dynamic_workflow, or BTW.
...
flowchart LR
  ...
  SDK["@pymodel/pythinker-code-sdk<br/>PythinkerHarness and Session"]
  Core["v1 agent-core"]
  Home["Pythinker Code home<br/>config, auth, MCP, sessions"]
...
`pythinker-code-vscode/0.6.0`. The version has one source of truth and is not copied
into runtime code or packaging scripts.

### Package boundaries

- `apps/vscode` depends on `@pymodel/pythinker-code-sdk`.
- `apps/vscode` must not depend directly on `@pymodel/agent-core`.
...
| Webview lifecycle | `apps/vscode/src/PythinkerWebviewProvider.ts` |
| Webview RPC boundary | `apps/vscode/src/bridge-handler.ts`, `apps/vscode/src/handlers` |
| SDK host | `apps/vscode/src/runtime/pythinker-runtime.ts` |
...
### Shared Pythinker Code home

The SDK resolves the home directory using the normal Pythinker Code rules:

1. system-level `PYTHINKER_CODE_HOME`, when set;
2. otherwise `~/.pythinker-code`.

The extension does not add a separate `pythinker.homeDir` setting and does not pass
...
- `sessions/`
- `session_index.jsonl`
- other SDK-owned Pythinker Code data
...
The old `pythinker.environmentVariables` setting existed to populate the environment
...
- proxy and other process-level variables are inherited from the Extension
  Host environment.
- a legacy `PYTHINKER_SHARE_DIR` in the removed setting is consulted only as an
...
Migration is opt-in and uses `@pymodel/migration-legacy` for detection and
translation. The extension coordinates prompts and reports but does not
maintain another config/session translator.

### Sources and target

- default source: `~/.pythinker`;
- optional additional source: a valid legacy `PYTHINKER_SHARE_DIR` from the removed
  VS Code setting;
- target: the SDK-resolved Pythinker Code home.
...
`Pythinker Code: Migrate Legacy Data` remains available for manual runs and retries.

The shared marker `.migrated-to-pythinker-code` can contain multiple target homes.
This prevents duplicate migration when the TUI migrated the same source first,
while still allowing a different `PYTHINKER_CODE_HOME` to be migrated later.
...
resolved Pythinker Code home and session ID. The home namespace prevents sessions
...
3. Shared config and sessions live in the SDK-resolved Pythinker Code home; editor

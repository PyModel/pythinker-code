  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const [loginStatus, pythinkerConfig] = await Promise.all([bridge.checkLoginStatus(), bridge.getModels()]);
        if (cancelled) {
          return;
        }

        console.log("[AppInit] Login status:", loginStatus, "pythinkerConfig:", pythinkerConfig);

        setIsLoggedIn(loginStatus.loggedIn);
        initModels(pythinkerConfig.models, pythinkerConfig.defaultModel, pythinkerConfig.defaultThinking, pythinkerConfig.defaultThinkingEffort);

        const modelsCount = pythinkerConfig.models?.length ?? 0;
        ...
        if (requiresManagedProviderLogin(pythinkerConfig.models, pythinkerConfig.defaultModel, loginStatus.loggedIn)) {
          setState({ status: "not-logged-in", errorMessage: null, modelsCount });
          return;
        }

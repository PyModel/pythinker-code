...
const PYTHINKER_SESSIONS_KEY = ["pythinkerSessions"] as const;
...
export function SessionList({ onClose }: SessionListProps) {
  ...
  const { data: pythinkerSessions = NO_SESSIONS, isPending: loading } = useQuery({
    queryKey: PYTHINKER_SESSIONS_KEY,
    queryFn: () => bridge.getAllPythinkerSessions(),
  });
  ...
  const filteredSessions = useMemo(() => {
    if (!searchQuery.trim()) return pythinkerSessions;
    const q = searchQuery.toLowerCase();
    return pythinkerSessions.filter((s) => s.brief.toLowerCase().includes(q));
  }, [pythinkerSessions, searchQuery]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData<SessionInfo[]>(PYTHINKER_SESSIONS_KEY, (prev) => prev?.filter((s) => s.id !== deleteTarget.id) ?? []);
    } catch (error) {

...
class Bridge {
  ...
  saveExtensionConfig(patch: Partial<ExtensionConfig>) {
    return this.call<{ ok: boolean }>(Methods.SaveExtensionConfig, patch);
  }
  ...
  getSessions() {
    return this.call<SessionInfo[]>(Methods.GetSessions);
  }

  getAllSessions() {
    return this.call<SessionInfo[]>(Methods.GetAllSessions);
  }
  ...
  loadSessionHistory(sessionId: string) {
    return this.call<UIStreamEvent[]>(Methods.LoadSessionHistory, { sessionId });
  }

  deleteSession(sessionId: string) {
    return this.call<{ ok: boolean }>(Methods.DeleteSession, { sessionId });
  }

  forkSession(sessionId: string, turnIndex: number) {
    return this.call<{ sessionId: string } | null>(Methods.ForkSession, { sessionId, turnIndex });
  }

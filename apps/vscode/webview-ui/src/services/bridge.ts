...
class Bridge {
  ...
  constructor() {
    ...
    if (typeof acquireVsCodeApi === "function") {
      ...
      console.warn("[Pythinker Bridge] Running outside VS Code, using mock");
      this.vscode = {
        postMessage: (msg) => console.log("[Pythinker Mock]", msg),
        getState: () => undefined,
        setState: () => {},
      };
    }
  ...
  getModels() {
    return this.call<PythinkerConfig>(Methods.GetModels);
  }
  ...
  getPythinkerSessions() {
    return this.call<SessionInfo[]>(Methods.GetPythinkerSessions);
  }

  getAllPythinkerSessions() {
    return this.call<SessionInfo[]>(Methods.GetAllPythinkerSessions);
  }
  ...
  loadSessionHistory(sessionId: string) {
    return this.call<UIStreamEvent[]>(Methods.LoadPythinkerSessionHistory, { pythinkerSessionId: sessionId });
  }

  deleteSession(sessionId: string) {
    return this.call<{ ok: boolean }>(Methods.DeletePythinkerSession, { sessionId });
  }

  forkSession(sessionId: string, turnIndex: number) {
    return this.call<{ sessionId: string } | null>(Methods.ForkPythinkerSession, { sessionId, turnIndex });
  }

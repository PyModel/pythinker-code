...
const REQUIRED_WEBVIEW_FILES = [
  'dist/webview.js',
  'dist/pythinker-banner-dark.svg',
  'dist/pythinker-banner-light.svg',
  'dist/pythinker-logo.png',
];
...
export async function verifyVsix(vsixPath, target, options = {}) {
  const extractionRoot = await mkdtemp(join(tmpdir(), 'pythinker-vsix-audit-'));
  try {
...
function verifyForbiddenFiles(files) {
  for (const file of files) {
    ...
    if (
      lower.includes('pythinker-agent-sdk') ||
      lower.includes('download-cli') ||
      lower.includes('/bin/pythinker/') ||
      /(^|\/)uv(?:\.exe)?$/.test(lower)
    ) {

...
const defaultCachePath = join(tmpdir(), "pythinker-vscode-test-cache");
...
export async function runExtensionHostSmoke(options = {}) {
  ...
  try {
    ...
    await writeFile(join(paths.workspace, "README.md"), "# Pythinker VSIX Extension Host smoke\n", "utf8");
    ...
    await runTests({
      ...
      extensionTestsEnv: {
        PYTHINKER_CODE_HOME: paths.pythinkerHome,
        PYTHINKER_VSCODE_SMOKE_OS_HOME: paths.osHome,
        PYTHINKER_VSCODE_SMOKE_REPORT: paths.report,
        PYTHINKER_VSCODE_SMOKE_VSIX: basename(vsixPath),
      },
    });
...
function defaultVsixPath() {
  ...
  return join(appDir, "artifacts", "vsix", `pythinker-code-${process.platform}-${arch}.vsix`);
}
...
async function writeHarnessManifest(directory) {
  await writeFile(
    ...
    JSON.stringify({
      name: "pythinker-vscode-extension-host-smoke",
      displayName: "Pythinker VSCode Extension Host Smoke",

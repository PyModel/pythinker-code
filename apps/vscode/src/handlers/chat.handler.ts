...
const streamChat: Handler<StreamChatParams, { done: boolean }> = async (params, ctx) => {
  if (!ctx.workDir) {
    ...
    void vscode.window.showWarningMessage("Pythinker: Please open a folder first.", "Open Folder").then((action) => {
...
function emitCaughtError(
  ...
  const code = isPythinkerError(error) ? error.code : "internal";

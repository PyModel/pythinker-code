...
export const sessionHandlers: Record<string, Handler<any, any>> = {
  [Methods.GetPythinkerSessions]: async (_, ctx): Promise<SessionInfo[]> => {
    if (!ctx.workDir) return [];
    return (await ctx.harness.listSessions({ workDir: ctx.workDir })).map(toSessionInfo);
  },

  [Methods.GetAllPythinkerSessions]: async (_, ctx): Promise<SessionInfo[]> => {
  ...
  [Methods.LoadPythinkerSessionHistory]: async (params: LoadHistoryParams, ctx) => {
    if (!ctx.workDir || !isSessionId(params.pythinkerSessionId)) return [];
    const runtime = await ctx.resumeSession(params.pythinkerSessionId);
    ...
    try {
      ...
      void Promise.resolve(
        vscode.window.showWarningMessage(
          "Pythinker: This conversation opened, but its file change history is unavailable.",
          "Show Logs",
        ),
      )
        .then((action) => {
  ...
  [Methods.DeletePythinkerSession]: async (params: DeleteSessionParams, ctx): Promise<{ ok: boolean }> => {
  ...
  [Methods.ForkPythinkerSession]: async (params: ForkSessionParams, ctx) => {

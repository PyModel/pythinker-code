...
export const sessionHandlers: Record<string, Handler<any, any>> = {
  [Methods.GetSessions]: async (_, ctx): Promise<SessionInfo[]> => {
    if (!ctx.workDir) return [];
    return (await ctx.harness.listSessions({ workDir: ctx.workDir })).map(toSessionInfo);
  },

  [Methods.GetAllSessions]: async (_, ctx): Promise<SessionInfo[]> => {
  ...
  [Methods.LoadSessionHistory]: async (params: LoadHistoryParams, ctx) => {
    if (!ctx.workDir || !isSessionId(params.sessionId)) return [];
    const runtime = await ctx.resumeSession(params.sessionId);
  ...
  [Methods.DeleteSession]: async (params: DeleteSessionParams, ctx): Promise<{ ok: boolean }> => {
  ...
  [Methods.ForkSession]: async (params: ForkSessionParams, ctx) => {

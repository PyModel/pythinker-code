...
import type { McpServerConfig as SdkMcpServerConfig, McpTestResult } from "@pymodel/pythinker-code-sdk";
...
export const mcpHandlers: Record<string, Handler<any, any>> = {
    ...
    await vscode.window.withProgress(
      {
        ...
        title: `Pythinker: Authenticating "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          ...
          await vscode.window.showErrorMessage(`Pythinker: OAuth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.ResetAuthMCP]: async ({ name }: NameParams, ctx) => {
    await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: `Pythinker: Resetting auth for "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          ...
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`Pythinker: Reset auth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );

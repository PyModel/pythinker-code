...
export interface LegacyMigrationManagerOptions {
  ...
  /** Defaults to the legacy pythinker-cli home (`~/.pythinker`). Injectable for isolated tests. */
  readonly defaultSourceHome?: string;
  /** Defaults to the legacy pythinker-cli plans dir (`~/.pythinker/plans`). Injectable for isolated tests. */
  readonly plansSourceDir?: string;
  /** First workspace root. Used only to resolve a relative legacy PYTHINKER_SHARE_DIR. */
  readonly workspaceRoot?: string | null;
  /** The removed `pythinker.environmentVariables` VS Code setting, read once for migration. */
...
/**
 * Coordinates legacy pythinker-cli migration for the VS Code host while keeping all
...
export class LegacyMigrationManager {
  ...
  constructor(options: LegacyMigrationManagerOptions) {
    ...
    this.defaultSourceHome = resolve(options.defaultSourceHome ?? join(homedir(), ".pythinker"));
    this.workspaceRoot =
      ...
      prompt:
        sources.length === 0
          ...
          : {
              ...
              message:
                "Legacy Pythinker data was found. Migrate config, MCP servers, history, skills, and sessions into Pythinker Code? Your old data will be kept.",
              actions: [
    ...
    if (shareDir.kind === "invalid") {
      ...
      if (isAbsolute(shareDir.value)) {
        ...
        warnings.push({
          code: "invalid-share-dir",
          message:
            "The legacy PYTHINKER_SHARE_DIR is relative, but no workspace is open; this migration source was ignored.",
        });
      } else {
        ...
        warnings.push({
          ...
          sourceHome,
          message: `The legacy relative PYTHINKER_SHARE_DIR was resolved against the workspace: ${sourceHome}`,
        });
      }
      ...
      if (sourceHome !== undefined && samePath(sourceHome, this.targetHome)) {
        warnings.push({
          ...
          sourceHome,
          message: "The legacy PYTHINKER_SHARE_DIR resolves to the Pythinker Code home and was ignored.",
        });
      } else if (
...
function readLegacyShareDir(
  ...
  if (
    ...
    return {
      kind: "invalid",
      message: "The legacy pythinker.environmentVariables setting is invalid and was ignored.",
    };
  }

  const value = (environmentVariables as Record<string, unknown>)["PYTHINKER_SHARE_DIR"];
  if (value === undefined) return { kind: "missing" };
  if (typeof value !== "string" || value.trim().length === 0) {
    return {
      kind: "invalid",
      message: "The legacy PYTHINKER_SHARE_DIR must be a non-empty string and was ignored.",
    };
  }
  ...
  for (const source of sources) {
    for (const failure of source.failures) {
      if (failure.code === "run-failed") {
        actions.push(
          `Fix access to ${source.source.sourceHome} or the Pythinker Code home, then run “Pythinker Code: Migrate Legacy Data” again.`,
        );
      } else {
        actions.push(
    ...
    if (summary.config.wroteSiblingDueToConflict) {
      actions.push("Review and merge config.migrated-from-pythinker-cli.toml.");
    }
    if (summary.config.wroteTuiSibling) {
      actions.push("Review and merge tui.migrated-from-pythinker-cli.toml.");
    }
    if (summary.mcp.wroteSiblingDueToConflict) {
      actions.push("Review and merge mcp.migrated-from-pythinker-cli.json.");
    }
...
function runMessage(
  ...
  if (status === "nothing-to-migrate") return "No legacy Pythinker data needs migration.";
  if (status === "failed") {

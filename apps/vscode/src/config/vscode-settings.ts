...
// oxlint-disable-next-line unicorn/no-typeof-undefined -- __EXTENSION_VERSION__ is a build-time esbuild define, not a real binding outside the tsdown build; `typeof` is the only safe probe.
...
export const LEGACY_ENGINE_ENV = "PYTHINKER_CODE_LEGACY_FLAG";

const TRUTHY_ENV_VALUES = new Set(["1", "true", "yes", "on"]);

/**
 * The single engine-selection decision for the whole extension. A truthy
 * `PYTHINKER_CODE_LEGACY_FLAG` wins over the `pythinker.useAgentCoreV1` setting, so
...
function getConfig() {
  return vscode.workspace.getConfiguration("pythinker");
}
...
export const VSCodeSettings = {
  ...
  async updateExtensionConfig(params: Partial<ExtensionConfig>): Promise<void> {
...
export function onSettingsChange(callback: (changedKeys: string[]) => void): vscode.Disposable {
  return vscode.workspace.onDidChangeConfiguration((e) => {
    if (!e.affectsConfiguration("pythinker")) {
      return;
    }
    ...
    const changedKeys = keys.filter((key) => e.affectsConfiguration(`pythinker.${key}`));

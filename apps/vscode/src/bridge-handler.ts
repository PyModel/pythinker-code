...
import { PythinkerRuntime } from "./runtime/pythinker-runtime";
...
export class BridgeHandler {
  ...
  readonly runtime: PythinkerRuntime;
  ...
  constructor(
    ...
    try {
      ...
      const rollbackHint = useAgentCoreV1
        ? ""
        : " You can roll back to the legacy engine: enable the 'pythinker.useAgentCoreV1' setting and reload the window.";
      throw new Error(
        `Failed to start the Pythinker engine: ${error instanceof Error ? error.message : String(error)}.${rollbackHint}`,
        { cause: error },
      );
    }
  ...
  private trace(id: string, method: string, durationMs: number, ok: boolean): void {
    ...
    console.debug(`[pythinker-vscode] ${line}`);
    this.writeLog(line);
  }
  ...
  private logRuntimeError(message: string, error?: unknown): void {
    ...
    console.error(`[pythinker-vscode] ${line}`);
    this.writeLog(line);
  }
}

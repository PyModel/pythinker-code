 ...
 * Wiring: the real composition root; VS Code, pythinker-telemetry, oauth, and SDK config/auth boundaries are mocked.
...
const host = vi.hoisted(() => {
  ...
  return {
    state,
    createPythinkerDeviceId: vi.fn((_homeDir: string, options?: { onFirstLaunch?: () => void }) => {
    ...
    resolvePythinkerRegion: vi.fn(() => "global" as const),
    ...
    resolveConfigPath: vi.fn(() => "/pythinker-home/config.toml"),
    resolvePythinkerHome: vi.fn(() => "/pythinker-home"),
...
vi.mock("@pymodel/pythinker-code-oauth", () => ({
  ...
  resolvePythinkerRegion: host.resolvePythinkerRegion,
}));

/**
 * Scenario: VS Code discovers and runs legacy pythinker-cli migration without touching a real home.
 ...
 * Run: pnpm --filter pythinker-code test -- legacy-migration.manager.test.ts
 */
...
describe("legacy migration manager (discovery and migration coordination)", () => {
  ...
  it("does not duplicate data when the TUI already migrated the same source", async () => {
    ...
    await writeFile(existingSession, '{"custom":{"imported_from_pythinker_cli":true}}');
    await writeSharedMarker(rig.sourceHome, rig.targetHome);
  ...
  it("conservatively suppresses the prompt when the shared marker is corrupt", async () => {
    const rig = await createRig();
    await writeLegacyConfig(rig.sourceHome);
    await writeFile(join(rig.sourceHome, ".migrated-to-pythinker-code"), "not-json");
  ...
  it("discovers a relative legacy PYTHINKER_SHARE_DIR from the workspace and warns about its resolution", async () => {
    const rig = await createRig({
      legacyEnvironmentVariables: { PYTHINKER_SHARE_DIR: "legacy-pythinker" },
    });
    const shareHome = join(rig.workspaceRoot, "legacy-pythinker");
  ...
  it("migrates both the default home and the extra legacy PYTHINKER_SHARE_DIR source", async () => {
    const rig = await createRig({
      legacyEnvironmentVariables: { PYTHINKER_SHARE_DIR: "legacy-pythinker" },
    });
    await writeLegacyConfig(rig.sourceHome);
  ...
  it("ignores legacy environment variables other than PYTHINKER_SHARE_DIR", async () => {
    const rig = await createRig({
      legacyEnvironmentVariables: {
        PYTHINKER_CODE_HOME: join(tmpdir(), "must-not-be-read"),
  ...
  it("ignores a non-string legacy PYTHINKER_SHARE_DIR with a clear warning", async () => {
    const rig = await createRig({
      legacyEnvironmentVariables: { PYTHINKER_SHARE_DIR: 42, HTTPS_PROXY: "https://example.test" },
    });
  ...
  it("ignores a relative legacy PYTHINKER_SHARE_DIR when no workspace can resolve it", async () => {
    const rig = await createRig({
      workspaceRoot: null,
      legacyEnvironmentVariables: { PYTHINKER_SHARE_DIR: "legacy-pythinker" },
    });
  ...
  it("reports legacy OAuth login as requiring a new login without treating it as migratable data", async () => {
    ...
    await writeFile(join(rig.sourceHome, "credentials", "pythinker-code.json"), "{}");
...
async function createRig(options: RigOptions = {}): Promise<{
  ...
  const sourceHome = join(root, ".pythinker");
  const targetHome = join(root, ".pythinker-code");
...
async function writeSharedMarker(sourceHome: string, targetHome: string): Promise<void> {
  await mkdir(sourceHome, { recursive: true });
  await writeFile(
    join(sourceHome, ".migrated-to-pythinker-code"),

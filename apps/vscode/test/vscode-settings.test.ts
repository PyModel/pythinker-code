 ...
 * PYTHINKER_CODE_LEGACY_FLAG override the setting, and ignore non-truthy env values.
 * Wiring: the real VSCodeSettings module; the vscode configuration store is a
 * mutable in-memory fake.
 * Run: pnpm --filter pythinker-code exec vitest run test/vscode-settings.test.ts
 */
...
describe("VSCodeSettings.useAgentCoreV1", () => {
  it("reads the pythinker.useAgentCoreV1 setting", () => {

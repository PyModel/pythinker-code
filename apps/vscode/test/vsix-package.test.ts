...
describe('VSIX package CLI (target planning and validation)', () => {
  it('plans all six supported targets when no target is supplied', async () => {
    const outputDir = await makeTempDir('pythinker-package-plan-');
    ...
    expect(result.stdout).toContain('pythinker-code-darwin-x64.vsix');
    expect(result.stdout).toContain('pythinker-code-darwin-arm64.vsix');
    expect(result.stdout).toContain('pythinker-code-linux-x64.vsix');
    expect(result.stdout).toContain('pythinker-code-linux-arm64.vsix');
    expect(result.stdout).toContain('pythinker-code-win32-x64.vsix');
    expect(result.stdout).toContain('pythinker-code-win32-arm64.vsix');
  });

  it('accepts a Windows ARM target when the output path contains spaces', async () => {
    const root = await makeTempDir('pythinker-package-windows-');
...
describe('Extension Development Host setup (isolated local state)', () => {
  it('creates the complete isolated directory layout for a debug launch', async () => {
    ...
    await expect(directoryExists(join(baseDir, 'pythinker-home'))).resolves.toBe(true);
  });

  it('refuses to clear a directory without the dedicated safety suffix', async () => {
    const unsafeDir = await makeTempDir('pythinker-dev-unsafe-');
...
async function makeVsixFixture(target: string): Promise<string> {
  const root = await makeTempDir('pythinker-vsix-fixture-');
  ...
  await Promise.all([
    ...
    writeFile(join(distDir, 'webview.js'), 'globalThis.__pythinkerWebview = true;\n'),
    writeFile(join(distDir, 'pythinker-banner-dark.svg'), '<svg />'),
    writeFile(join(distDir, 'pythinker-banner-light.svg'), '<svg />'),
    writeFile(join(distDir, 'pythinker-logo.png'), 'fixture'),
    writeFile(join(resourcesDir, 'pythinker-icon-storefront.png'), 'fixture'),
    writeFile(join(resourcesDir, 'pythinker-icon.svg'), '<svg />'),
  ]);
  return root;
}

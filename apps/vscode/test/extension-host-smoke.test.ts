 ...
 * Host cannot discover the developer's real legacy Pythinker home.
...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      options.extensionTestsEnv.PYTHINKER_VSCODE_SMOKE_REPORT,
...
describe('installed VSIX Extension Host smoke', () => {
  ...
  it('gives the harness separate Pythinker and operating-system homes before activation', async () => {
    const fixture = await makeFixture();
    ...
    expect(env.PYTHINKER_CODE_HOME).not.toBe(env.PYTHINKER_VSCODE_SMOKE_OS_HOME);
    expect(env.PYTHINKER_VSCODE_SMOKE_OS_HOME).toContain('os-home');
  });
...
async function makeFixture(): Promise<{ vsixPath: string; cachePath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'pythinker-extension-host-smoke-'));
  tempDirs.push(root);
  const cachePath = join(root, 'cache');
  const vsixPath = join(root, 'pythinker-code-test.vsix');

...
const EXTENSION_ID = "moonshot-ai.pythinker-code";
const EXPECTED_COMMANDS = [
  "pythinker.clearAllState",
  "pythinker.focusInput",
  "pythinker.insertMention",
  "pythinker.logout",
  "pythinker.migrateLegacyData",
  "pythinker.newConversation",
  "pythinker.openInSideBar",
  "pythinker.openInTab",
  "pythinker.resetPythinker",
  "pythinker.showLogs",
];

exports.run = async function run() {
  const isolatedHome = process.env.PYTHINKER_VSCODE_SMOKE_OS_HOME;
  ...
  assert.ok(process.env.PYTHINKER_CODE_HOME, "PYTHINKER_CODE_HOME must point at the isolated test home");
  ...
  const config = vscode.workspace.getConfiguration("pythinker");
  ...
  await vscode.commands.executeCommand("pythinker.openInTab");
  await waitFor(() => {
    return vscode.window.tabGroups.all.some((group) =>
      group.tabs.some((tab) =>
        tab.input instanceof vscode.TabInputWebview && isPythinkerPanelViewType(tab.input.viewType)));
  }, 5_000, () => `Pythinker Webview tab did not open; tabs=${describeTabs()}`);

  await vscode.commands.executeCommand("pythinker.showLogs");
  await vscode.commands.executeCommand("pythinker.resetPythinker");
  ...
  assert.ok(process.env.PYTHINKER_VSCODE_SMOKE_REPORT, "Extension Host report path must be provided");
  await writeFile(
    process.env.PYTHINKER_VSCODE_SMOKE_REPORT,

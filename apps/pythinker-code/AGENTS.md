# apps/pythinker-code Development Guide

This file only contains rules local to `apps/pythinker-code`. For cross-repo rules, see the root `AGENTS.md`.
...
`apps/pythinker-code` is the terminal UI / CLI app. The entry chain is:

`src/main.ts` -> `src/cli/commands.ts` -> `src/cli/run-shell.ts` -> SDK `PythinkerHarness` -> `src/tui/pythinker-tui.ts`
...
- `PythinkerTUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `controllers`, `commands`, `components`, `reverse-rpc`, or `utils` first.
- `controllers` own the heavy, independently-testable slices (event routing, streaming render, session replay, tasks browser, editor keyboard, auth). Event-routing and rendering logic belong here, not on the `PythinkerTUI` class.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `PythinkerTUI`, but complex logic should continue to sink downward.
...
- `apps/pythinker-code` may only use core capabilities through `@pymodel/pythinker-code-sdk`. Do not import engine packages directly in app code.
...
- Functions with no state / UI side effects do not belong as private methods on the `PythinkerTUI` class; put them in external utils.

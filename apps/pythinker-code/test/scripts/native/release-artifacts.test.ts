...
const executableName = process.platform === 'win32' ? 'pythinker.exe' : 'pythinker';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    for (const name of [
      ...
      `pythinker-code-${target}.tar.gz.sha256`,
      'manifest.json',
    ]) {
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, PYTHINKER_CODE_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `pythinker-code-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  pythinker-code-${target}.zip\n`,
    );
  });
  ...
  it('produces compressed artifacts and a manifest paired with the bare binary', async () => {
    ...
    await execFileAsync(process.execPath, [manifestScript, artifactsDir, '@pymodel/pythinker-code@0.5.0']);

    const zstBytes = readFileSync(resolve(artifactsDir, `pythinker-code-${target}.zst`));
    expect(zstdDecompressSync(zstBytes).toString('utf-8')).toBe(binaryContent);
    const tarballPath = resolve(artifactsDir, `pythinker-code-${target}.tar.gz`);
    expect(existsSync(tarballPath)).toBe(true);
    expect(readFileSync(`${tarballPath}.sha256`, 'utf-8')).toBe(
      `${sha256(readFileSync(tarballPath))}  pythinker-code-${target}.tar.gz\n`,
    );
    ...
    expect(manifest).toEqual({
      ...
      platforms: {
        [target]: {
          filename: `pythinker-code-${target}`,
          checksum: sha256(Buffer.from(binaryContent)),
          compressed: { filename: `pythinker-code-${target}.zst`, checksum: sha256(zstBytes) },
        },
      },
    });
  });
  ...
  it('keeps the .exe suffix in Windows manifest filenames', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'pythinker-manifest-win32-'));
    try {
      const binaryContent = Buffer.from('fake windows binary');
      const zip = new ZipFile();
      zip.addBuffer(binaryContent, 'pythinker.exe');
      zip.end();
      await pipeline(
        zip.outputStream,
        createWriteStream(join(releaseDir, 'pythinker-code-win32-x64.zip')),
      );
      await writeFile(
        join(releaseDir, 'pythinker-code-win32-x64.zip.sha256'),
        `${'b'.repeat(64)}  pythinker-code-win32-x64.zip\n`,
      );
  ...
  it('fails when no zip sidecars exist', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'pythinker-manifest-empty-'));
    try {
      await expect(

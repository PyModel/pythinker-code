...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `pythinker-code-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `pythinker-code-${target}.zip.sha256`), { force: true });
  });
  ...
  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'pythinker-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'pythinker-code-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'pythinker-code-darwin-arm64.zip.sha256'),

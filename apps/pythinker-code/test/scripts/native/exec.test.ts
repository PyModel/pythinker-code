...
describe('commandForExecFile', () => {
  it('returns command as-is on non-Windows', () => {
    ...
    expect(result).toEqual({ command: 'postject', args: ['pythinker', 'NODE_SEA_BLOB', './blob'] });
  });

  it('returns command as-is on Windows for non-batch files', () => {
    const result = commandForExecFile('postject.exe', ['pythinker.exe'], 'win32');
    expect(result).toEqual({ command: 'postject.exe', args: ['pythinker.exe'] });
  });
  ...
  it('wraps .cmd files through cmd.exe on Windows', () => {
    ...
    expect(result.args).toEqual([
      ...
      '/s',
      '/c',
      '""postject.cmd" "pythinker.exe" "NODE_SEA_BLOB""',
    ]);

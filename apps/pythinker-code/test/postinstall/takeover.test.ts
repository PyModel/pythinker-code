...
async function makeEnv(): Promise<TestEnv> {
  ...
  await writeFile(join(ownRoot, 'package.json'), '{"name":"@pymodel/pythinker-code"}', 'utf-8');
  await writeFile(join(ownRoot, 'main.mjs'), '// pythinker-code\n', 'utf-8');
  await chmod(join(ownRoot, 'main.mjs'), 0o755);
  await symlink(join(ownRoot, 'main.mjs'), join(ownBin, 'pythinker'));
  return {
...
async function makeLegacyShim(dirName: string): Promise<{ dir: string; shim: string }> {
  ...
  const shim = join(dir, 'pythinker');
  await writeFile(shim, '#!/bin/sh\n# setuptools entry point for pythinker_cli\n', 'utf-8');
  await chmod(shim, 0o755);
  return { dir, shim };
}

describe.runIf(POSIX)('shim takeover (POSIX fixtures)', () => {
  it('renames a single legacy shim to pythinker-legacy and verifies the takeover', async () => {
    ...
    expect(existsSync(join(legacy.dir, 'pythinker-legacy'))).toBe(true);
    expect(existsSync(legacy.shim)).toBe(false);
    await expect(readFile(join(legacy.dir, 'pythinker-legacy'), 'utf-8')).resolves.toContain(
      'pythinker_cli',
    );
  ...
  it('preserves the first of two legacy shims and deletes the second', async () => {
    ...
    expect(existsSync(join(first.dir, 'pythinker-legacy'))).toBe(true);
    expect(existsSync(join(second.dir, 'pythinker'))).toBe(false);
    expect(existsSync(join(second.dir, 'pythinker-legacy'))).toBe(false);
  });

  it('consolidates onto an existing legacy pythinker-legacy', async () => {
    const env = await makeEnv();
    const legacy = await makeLegacyShim('uvbin');
    await writeFile(
      join(legacy.dir, 'pythinker-legacy'),
      '#!/bin/sh\n# older pythinker_cli entry point\n',
      'utf-8',
    );
  ...
  it('leaves a user-managed pythinker-legacy untouched (delete-only)', async () => {
    const env = await makeEnv();
    const legacy = await makeLegacyShim('uvbin');
    await writeFile(join(legacy.dir, 'pythinker-legacy'), 'my own wrapper\n', 'utf-8');
    ...
    expect(existsSync(legacy.shim)).toBe(false);
    await expect(readFile(join(legacy.dir, 'pythinker-legacy'), 'utf-8')).resolves.toBe(
      'my own wrapper\n',
    );
  });
  ...
  it('a failed preserve attempt gives the next shim its own preserve attempt', async () => {
    ...
    const outcomes = await executeTakeover([
      {
        kind: 'renameable',
        shimPath: join(root, 'gone', 'pythinker'),
        target: join(root, 'gone', 'pythinker-legacy'),
        detection: { shimPath: join(root, 'gone', 'pythinker'), realPath: '' },
      },
      {
        kind: 'renameable',
        shimPath: second.shim,
        target: join(second.dir, 'pythinker-legacy'),
    ...
    expect(existsSync(join(second.dir, 'pythinker-legacy'))).toBe(true);
  });
  ...
  it('aborts with kind=foreign when an unrecognized pythinker wins resolution', async () => {
    const env = await makeEnv();
    const foreignDir = join(root, 'homebin');
    await mkdir(foreignDir, { recursive: true });
    await writeFile(join(foreignDir, 'pythinker'), '#!/bin/sh\necho mine\n', 'utf-8');
    await chmod(join(foreignDir, 'pythinker'), 0o755);
    const legacy = await makeLegacyShim('uvbin');
  ...
  it('verifies none when every pythinker is gone after execution', async () => {
    const env = await makeEnv();
...
describe('windows forms (platform injection, host-agnostic)', () => {
  it('expands PATHEXT candidates for pythinker', () => {
    const candidates = executableCandidates('pythinker', 'win32');
    expect(candidates).toContain('pythinker');
    expect(candidates).toContain('pythinker.exe');
    expect(candidates).toContain('pythinker.cmd');
    expect(executableCandidates('pythinker', 'linux')).toEqual(['pythinker']);
  });

  it('preserves the extension in the rename target', () => {
    expect(renameTargetFor('C:\\Users\\me\\.local\\bin\\pythinker.exe', 'win32')).toBe(
      'C:\\Users\\me\\.local\\bin\\pythinker-legacy.exe',
    );
    expect(renameTargetFor('C:\\Users\\me\\.local\\bin\\pythinker', 'win32')).toBe(
      'C:\\Users\\me\\.local\\bin\\pythinker-legacy',
    );
    expect(renameTargetFor('/home/me/.local/bin/pythinker', 'linux')).toBe(
      '/home/me/.local/bin/pythinker-legacy',
    );
  });

  it('classifies system-owned dirs from drive-letter and UNC forms', async () => {
    await expect(isSystemOwnedDir('C:\\Program Files\\pythinker\\pythinker.exe', 'win32')).resolves.toBe(true);
    await expect(isSystemOwnedDir('c:\\programdata\\uv\\pythinker.exe', 'win32')).resolves.toBe(true);
    await expect(isSystemOwnedDir('C:\\Users\\me\\.local\\bin\\pythinker.exe', 'win32')).resolves.toBe(false);
    await expect(isSystemOwnedDir('D:\\tools\\pythinker.exe', 'win32')).resolves.toBe(false);
    await expect(isSystemOwnedDir('\\\\server\\share\\tools\\pythinker.exe', 'win32')).resolves.toBe(false);
  });
});

...
vi.mock('@pymodel/agent-core-v2', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@pymodel/agent-core-v2')>();
  return {
    ...actual,
...
vi.mock('@pymodel/pythinker-code-oauth', async () => {
  ...
  return {
    ...actual,
    createPythinkerDefaultHeaders: mocks.createPythinkerDefaultHeaders,
...
describe('runV2Print', () => {
  ...
  it('binds the profile named by --agent-file when --agent is absent', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pythinker-agent-file-'));

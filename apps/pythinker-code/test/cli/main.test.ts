...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      ...
      expect(process.title).toBe('pythinker-code');
    } finally {
  ...
  it('initializes and flushes telemetry around the upgrade command', async () => {
    ...
    expect(mocks.createPythinkerHarness).toHaveBeenCalledWith(expect.objectContaining({
      homeDir: '/tmp/pythinker-home',
      telemetry: {
        track: mocks.track,
  ...
  it('formats Pythinker startup errors with structured fields', () => {
    const error = new PythinkerError(

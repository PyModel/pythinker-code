...
function parse(argv: string[]): CLIOptions {
  ...
  program.parse(['node', 'pythinker', ...argv]);
...
describe('CLI options parsing', () => {
  ...
  describe('--model / -m', () => {
    it('parses -m as a model override', () => {
      expect(parse(['-m', 'pythinker-code/k2']).model).toBe('pythinker-code/k2');
    });

    it('parses --model=value as a model override', () => {
      expect(parse(['--model=pythinker-code/k2.5']).model).toBe('pythinker-code/k2.5');
    });

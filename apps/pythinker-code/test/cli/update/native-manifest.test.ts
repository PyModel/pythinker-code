...
describe('url helpers', () => {
  it('builds the manifest and binary URLs from the binaries base', () => {
    expect(nativeManifestUrl(VERSION)).toBe(`${PYTHINKER_CODE_CDN_BINARIES_BASE}/${VERSION}/manifest.json`);
    expect(nativeBinaryUrl(VERSION, 'pythinker-code-win32-x64.zip')).toBe(
      `${PYTHINKER_CODE_CDN_BINARIES_BASE}/${VERSION}/pythinker-code-win32-x64.zip`,
    );
  });
});

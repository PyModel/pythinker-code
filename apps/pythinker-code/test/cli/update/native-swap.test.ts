...
import { PYTHINKER_CODE_UPDATE_REEXEC_ENV } from '#/constant/app';
...
describe('maybeRelaunchWithStagedNativeUpdate', () => {
  ...
  beforeEach(async () => {
    workDir = await mkdtemp(join(tmpdir(), 'pythinker-swap-test-'));
  ...
  it('does nothing when the re-exec guard env is set', async () => {
    ...
    const env = { [PYTHINKER_CODE_UPDATE_REEXEC_ENV]: '1' };
    ...
    expect(env[PYTHINKER_CODE_UPDATE_REEXEC_ENV]).toBeUndefined();
  ...
  it('swaps in the staged exe, re-execs with the original argv and forwards the exit code', async () => {
    ...
    expect((calls[1]?.options['env'] as Record<string, string>)[PYTHINKER_CODE_UPDATE_REEXEC_ENV]).toBe('1');
  ...
  it('falls back to a pid-named backup when the plain .bak cannot be removed', async () => {
    ...
    expect(names.toSorted()).toEqual(['pythinker', 'pythinker.bak']);
  ...
  it('leaves a staged update in place when automatic updates are disabled by env', async () => {
    ...
    const relaunched = await maybeRelaunchWithStagedNativeUpdate(
      makeDeps(exePath, {
        spawnImpl,
        env: { PATH: '/usr/bin', PYTHINKER_CODE_NO_AUTO_UPDATE: '1' },
      }),
    );
  ...
  it('applies a manually staged update even when automatic updates are disabled by env', async () => {
    // The opt-out targets automatic updates; an explicit `pythinker upgrade`

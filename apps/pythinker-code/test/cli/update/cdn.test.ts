...
import { pythinkerCodeCdnLatestJsonUrl, pythinkerCodeCdnLatestUrl } from '#/constant/app';
...
describe('fetchLatestFromCdn', () => {
  it('parses latest.json and returns the manifest', async () => {
    const f = mockRoutedFetch({ [pythinkerCodeCdnLatestJsonUrl()]: { body: MANIFEST_BODY } });
  ...
  it('ignores unknown manifest fields (lenient parsing)', async () => {
    ...
    const f = mockRoutedFetch({ [pythinkerCodeCdnLatestJsonUrl()]: { body } });
  ...
  for (const [name, route] of fallbackCases) {
    it(`falls back to plain /latest when ${name}`, async () => {
      const f = mockRoutedFetch({
        [pythinkerCodeCdnLatestJsonUrl()]: route,
        [pythinkerCodeCdnLatestUrl()]: { body: '1.9.0\n' },
      });
      await expect(fetchLatestFromCdn(f)).resolves.toEqual({
        latest: '1.9.0',
        manifest: null,
  ...
  it('throws when both latest.json and plain /latest fail', async () => {
    const f = mockRoutedFetch({
      [pythinkerCodeCdnLatestJsonUrl()]: { status: 500 },
      [pythinkerCodeCdnLatestUrl()]: { status: 500 },
    });
  ...
  it('propagates the plain /latest error when the fallback also breaks', async () => {
    const f = mockRoutedFetch({
      [pythinkerCodeCdnLatestJsonUrl()]: new Error('json down'),
      [pythinkerCodeCdnLatestUrl()]: { body: 'not-a-version' },
    });
  ...
  it('falls back to plain /latest when latest.json hangs past the request timeout', async () => {
    ...
    try {
      const f = vi.fn(async (input: string | URL, init?: RequestInit) => {
        if (String(input) === pythinkerCodeCdnLatestJsonUrl()) {
        ...
        if (String(input) === pythinkerCodeCdnLatestUrl()) {

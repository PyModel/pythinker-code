/**
 * Tests for the `pythinker web` Commander wiring and its subcommands.
 *
 ...
 * output, browser opening, and the rotate-token / deprecated `pythinker server kill`
...
describe('pythinker web', () => {
  ...
  it('routes `pythinker server` and any legacy subcommand to a deprecation notice', async () => {
    for (const argv of [
      ['node', 'pythinker', 'server'],
      ['node', 'pythinker', 'server', 'run', '--port', '1'],
      ['node', 'pythinker', 'server', 'status'],
      ['node', 'pythinker', 'server', 'ps', '--json'],
    ]) {
      const program = makeProgram();
      let stderr = '';
      ...
      expect(stderr).toContain('`pythinker server` has been deprecated and no longer works.');
      expect(stderr).toContain('pythinker web');
      expect(stderr).toContain('pythinker server kill');
...
describe('`pythinker web` ready banner', () => {
  it('prints the TUI-style ready panel once listening', async () => {
    ...
    // the Pythinker sprite stays next to the title.
    ...
    expect(plain.indexOf('Pythinker server ready')).toBeLessThan(plain.indexOf('Local:'));
  ...
  it('uses the TUI dark palette for the ready banner', async () => {
    ...
    expect(out).toContain(color.bold.hex(darkColors.primary)('Pythinker server ready'));
...
describe('ready banner reflects the bind class', () => {
  it('lists Local + Network addresses for a 0.0.0.0 bind (Vite-style)', async () => {
    ...
    expect(raw).toContain('Pythinker server ready');
...
describe('`pythinker web` option threading', () => {
  ...
  it('prints the one-line ready line instead of the full banner with a non-default --log-level', async () => {
    ...
    const plain = stripAnsi(readStdout());
    expect(plain).toContain('Pythinker server: http://127.0.0.1:58627/#token=tok');
    expect(plain).not.toContain('Pythinker server ready');
...
describe('server web asset directory resolution', () => {
  ...
  it('returns the assets dir when it is built, dev mode or not', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-web-assets-'));
    try {
      writeFileSync(join(dir, 'index.html'), '<html></html>');
      expect(serverWebAssetsDir({}, dir)).toBe(dir);
      expect(serverWebAssetsDir({ PYTHINKER_CODE_DEV_SERVER: '1' }, dir)).toBe(dir);
    } finally {
  ...
  it('tolerates missing assets in dev mode (API-only server)', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-web-assets-'));
    try {
      expect(serverWebAssetsDir({ PYTHINKER_CODE_DEV_SERVER: '1' }, dir)).toBeUndefined();
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe('`pythinker server kill` (deprecated, legacy servers only)', () => {
  ...
  it('prints "No running legacy Pythinker server." and sends no signal when no lock exists', async () => {
    const { handleLegacyKillCommand } = await import('#/cli/sub/web/legacy-kill');
    const { deps, writes, signals } = makeLegacyKillDeps({ readLock: async () => undefined });

    await handleLegacyKillCommand(deps);

    expect(writes.join('')).toContain('No running legacy Pythinker server.');
...
describe('readLegacyLock', () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pythinker-legacy-lock-'));
  });
  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
  });
...
describe('resolveServerToken', () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pythinker-server-token-'));
  });
  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
  });
...
describe('`pythinker web rotate-token`', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pythinker-rotate-'));
  ...
  it('writes a new token to server.token and prints it', async () => {
    ...
    await program.parseAsync(['node', 'pythinker', 'web', 'rotate-token']);

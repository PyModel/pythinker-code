...
const mocks = vi.hoisted(() => ({
  ...
  resolvePythinkerHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/pythinker-export-home'),
...
vi.mock('@pymodel/pythinker-code-sdk', async (importOriginal) => {
  ...
  return {
    ...
    createPythinkerHarness: (...args: unknown[]) => {
      ...
      const homeDir = options?.homeDir ?? '/tmp/pythinker-export-home';
...
afterEach(() => {
  ...
  mocks.resolvePythinkerHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/pythinker-export-home',
  );
...
describe('pythinker export', () => {
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'pythinker', 'export', '--no-include-global-log', '-y']);
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'pythinker', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'pythinker', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
    ...
    expect(mocks.pythinkerHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/pythinker-export-home' }),
    );

 ...
 * `pythinker dashboard`
 *
 * Verifies the CLI layer for the session dashboard: home + auto-port
 * resolution, browser open vs `--no-open`, and the session deep-link path.
 * Uses injected deps so no real port is bound and the real dashboard server is
 * never started.
 */

import { describe, it, expect, vi } from 'vitest';

import { handleDashboard, type DashboardDeps } from '#/cli/sub/dashboard';
...
function makeDeps(over: Partial<DashboardDeps> = {}): {
  ...
  const deps: DashboardDeps = {
    getHomeDir: () => '/home/k',
    startDashboardServer: vi.fn(async (o) => ({
      port: 41234,
      host: '127.0.0.1',
      url: 'http://127.0.0.1:41234/',
      close: async () => {},
      _opts: o,
    })) as unknown as DashboardDeps['startDashboardServer'],
    ...
    stderr: { write: () => true },
    exit: vi.fn() as unknown as DashboardDeps['exit'],
    ...over,
  };
...
describe('handleDashboard', () => {
  it('starts the server with the home dir + auto port and opens the browser', async () => {
    ...
    await handleDashboard(deps, { open: true });
  ...
  it('reports a clean error and exits when the server fails to start', async () => {
    ...
    await handleDashboard(deps, { open: true, port: 4321 });
    expect(errored.join('')).toContain('Failed to start pythinker dashboard');

...
const ENABLED = process.env['PYTHINKER_E2E'] === '1';
...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'pythinker-cli-log-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'pythinker-cli-log-work-'));
  oldHome = process.env['PYTHINKER_CODE_HOME'];
  oldLogLevel = process.env['PYTHINKER_LOG_LEVEL'];
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['PYTHINKER_CODE_HOME'] = oldHome;
  }
  if (oldLogLevel === undefined) {
    delete process.env['PYTHINKER_LOG_LEVEL'];
  } else {
    process.env['PYTHINKER_LOG_LEVEL'] = oldLogLevel;
  }
...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    const harness = createPythinkerHarness({
      homeDir,
      identity: createPythinkerCodeHostIdentity('0.1.1'),
    });
    try {
      ...
      await runPythinkerExport([session.id, '-o', defaultZip]);
      ...
      await runPythinkerExport([session.id, '-o', noGlobalZip, '--no-include-global-log']);
...
async function runPythinkerExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`pythinker export exited ${code}: ${stderr.join('')}`);
    },
  });
  await program.parseAsync(['node', 'pythinker', 'export', ...args]);
}

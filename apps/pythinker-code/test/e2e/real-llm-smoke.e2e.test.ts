/**
 * Real-LLM smoke. Opt-in only — runs when `PYTHINKER_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `PythinkerHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   PYTHINKER_E2E_REAL     — set to "1" to enable this suite
 *   PYTHINKER_E2E_MODEL    — model alias override (default: config's default)
 *   PYTHINKER_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   PYTHINKER_E2E_WORKDIR  — workspace directory (default: /tmp/pythinker-e2e)
 */
...
const DEFAULT_WORKDIR = '/tmp/pythinker-e2e';
const TURN_TIMEOUT_MS = 60_000;

const ENABLED = process.env['PYTHINKER_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through PythinkerHarness',
    async () => {
      const workDir = process.env['PYTHINKER_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['PYTHINKER_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['PYTHINKER_E2E_MODEL'];

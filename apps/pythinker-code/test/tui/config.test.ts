...
beforeEach(() => {
  vi.stubEnv('PYTHINKER_CODE_TUI_FULL_SCREEN', '');
    ...
    await saveTuiConfig(
      {
        ...
        disableFeedbackSurvey: DEFAULT_TUI_CONFIG.disableFeedbackSurvey,
        editorCommand: null,
...
describe('TUI config status_line round-trip', () => {
  it('preserves an active status_line across save and reload', async () => {
    await saveTuiConfig(
      {
        ...DEFAULT_TUI_CONFIG,
        statusLine: { items: ['model', 'git'], command: '~/.pythinker-code/statusline.sh' },
      },
      filePath,
    );
...
describe('TUI config tui_mode env migration', () => {
  it('migrates PYTHINKER_CODE_TUI_FULL_SCREEN=1 into tui_mode when the key is absent', async () => {
  ...
  it('only honors the exact value 1 like the old gate', async () => {
    vi.stubEnv('PYTHINKER_CODE_TUI_FULL_SCREEN', 'true');
    writeFileSync(filePath, 'theme = "dark"\n', 'utf-8');

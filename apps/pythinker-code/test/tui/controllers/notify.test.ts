import type { Event, ResumedSessionState } from '@pymodel/pythinker-code-sdk';
import {
  Container,
  Text,
  TuiMainScreen,
...
describe('NotifyController', () => {
  ...
  it('mirrors Agent and AgentDynamicWorkflow delegations into the panel immediately', () => {
    const h = makeHarness();
    h.emit('tool.call.started', {
      toolCallId: 'a1',
      name: 'Agent',
  ...
  it('counts resumed subagents in AgentDynamicWorkflow delegation entries', () => {
    const h = makeHarness();
    h.emit('tool.call.started', {
      toolCallId: 'a1',
      name: 'AgentDynamicWorkflow',

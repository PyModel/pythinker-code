...
let home: string;
const originalHome = process.env['PYTHINKER_CODE_HOME'];

beforeEach(() => {
  home = join(tmpdir(), `pythinker-themes-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
afterEach(() => {
  ...
  if (originalHome === undefined) {
    ...
    process.env['PYTHINKER_CODE_HOME'] = originalHome;
  }
});

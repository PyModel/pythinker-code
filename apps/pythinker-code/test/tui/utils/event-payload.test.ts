...
describe('error payload formatting', () => {
  ...
  it('shows concise provider filter text from PythinkerError details', () => {
    const error = new PythinkerError(ErrorCodes.PROVIDER_API_ERROR, filteredThinkOnlyMessage, {
      details: {
        finishReason: 'filtered',
        rawFinishReason: 'content_filter',
      },

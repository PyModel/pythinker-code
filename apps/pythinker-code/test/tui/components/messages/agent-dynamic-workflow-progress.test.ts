...
import {
  ...
  type AgentDynamicWorkflowProgressOptions,
  agentDynamicWorkflowDescriptionFromArgs,
  agentDynamicWorkflowGridHeightForTerminalRows,
  agentDynamicWorkflowItemsFromArgs,
  agentDynamicWorkflowPartialItemsCountFromArguments,
  agentDynamicWorkflowPartialItemsFromArguments,
  calculateAgentDynamicWorkflowGridLayout,
...
function createComponent(
  ...
  return new AgentDynamicWorkflowProgressComponent({
...
function renderLines(component: AgentDynamicWorkflowProgressComponent, width = 100): string[] {
  return renderText(component, width).split('\n');
}

function registerSubagents(component: AgentDynamicWorkflowProgressComponent, count: number): void {
...
function startSubagents(component: AgentDynamicWorkflowProgressComponent, count: number): void {
...
describe('calculateAgentDynamicWorkflowGridLayout', () => {
  ...
  it('adds text columns before falling back to compact bars', () => {
    const textLayout = calculateAgentDynamicWorkflowGridLayout({
      width: 120,
      height: 4,
      count: 20,
    });
    const compactLayout = calculateAgentDynamicWorkflowGridLayout({
      width: 117,
      height: 4,
      count: 20,
    });
  ...
  it('derives the grid height left inside the AgentDynamicWorkflow block', () => {
    expect(agentDynamicWorkflowGridHeightForTerminalRows(undefined)).toBeUndefined();
    expect(agentDynamicWorkflowGridHeightForTerminalRows(10)).toBe(4);
    expect(agentDynamicWorkflowGridHeightForTerminalRows(20, 5)).toBe(9);
    expect(agentDynamicWorkflowGridHeightForTerminalRows(4)).toBe(0);
  });
});

describe('AgentDynamicWorkflowProgressComponent', () => {
  ...
  it('repaints from the active palette when the theme changes', () => {
    ...
    try {
      ...
      const titleOf = (): string => {
        const line = component.render(100).find((l) => strip(l).includes('Agent DynamicWorkflow'));
  ...
  it('renders blank padding around the block without a bottom divider', () => {
    ...
    expect(lines[0]).toBe(' ');
    expect(lines[1]).toContain('Agent DynamicWorkflow');
  ...
  it('reserves one blank column on the right edge', () => {
    ...
    expect(rendered.some((line) => line.includes('Agent DynamicWorkflow'))).toBe(true);
  ...
  it('applies no-index AgentDynamicWorkflow result statuses by tag order', () => {
    const component = createComponent();
  ...
  it('strips nested AgentDynamicWorkflow prefixes from failure details', () => {
    const component = createComponent();
    ...
    component.applyResult([
      '<agent_dynamic_workflow_result>',
      '<summary>failed: 1</summary>',
      '<subagent index="1" agent_id="agent-1" outcome="failed">agent_dynamic_workflow: failed',
  ...
  it('renders completed summaries from AgentDynamicWorkflow result output', () => {
    const component = createComponent();
  ...
  it('counts partial items before each string is complete', () => {
    expect(
      agentDynamicWorkflowPartialItemsCountFromArguments('{"items":["src/a.ts","src/b'),
    ).toBe(2);
    expect(
      agentDynamicWorkflowPartialItemsCountFromArguments('{"items":["src/a.ts","src/\\"b.ts","src/c'),
    ).toBe(3);
    expect(
      agentDynamicWorkflowPartialItemsFromArguments('{"items":["src/a.ts","src/\\"b.ts","src/c'),
  ...
  it('maps subagents by structured dynamic_workflow indexes when descriptions include issue references', () => {
    const component = createComponent({
      description: 'Fix #123',
    });
  ...
  it('extracts description and item list from AgentDynamicWorkflow args', () => {
    const args = {
      description: 'Review changed files',
      items: ['src/a.ts', 123],
    };

    expect(agentDynamicWorkflowDescriptionFromArgs(args)).toBe('Review changed files');
    expect(agentDynamicWorkflowItemsFromArgs(args)).toEqual(['src/a.ts', '123']);
  });
});

...
describe('built-in slash command registry', () => {
  ...
  it('keeps dynamic_workflow mode changes and dynamic_workflow tasks idle-only', () => {
    const dynamic_workflow = findBuiltInSlashCommand('dynamic_workflow');
    expect(dynamic_workflow).toBeDefined();
    expect((dynamic_workflow as PythinkerSlashCommand).experimentalFlag).toBeUndefined();
    expect(resolveSlashCommandAvailability(dynamic_workflow!, 'on')).toBe('idle-only');
    expect(resolveSlashCommandAvailability(dynamic_workflow!, 'off')).toBe('idle-only');
    expect(resolveSlashCommandAvailability(dynamic_workflow!, 'Ship feature X')).toBe('idle-only');
  });

...
import { resetCapabilitiesCache, setCapabilities, type Component } from '@pymodel/pi-tui';
...
describe('plugins command capability surface', () => {
  ...
  it('routes built-in entries through capabilities only on v2', () => {
    ...
    expect(
      isCapabilityEntry(v2.host, { id: 'pythinker-cu', source: 'capability:pythinker-cu', builtIn: true } as never),
    ).toBe(true);
    expect(
    ...
    expect(
      isCapabilityEntry(v2.host, { id: 'pythinker-cu', source: 'https://example.test/plugin.zip' } as never),
    ).toBe(false);
    // A forged capability: source without the parser-proof flag is a plain row.
    expect(
      isCapabilityEntry(v2.host, { id: 'pythinker-cu', source: 'capability:pythinker-cu' } as never),
    ).toBe(false);

    const v1 = fakeHost({});
    expect(
      isCapabilityEntry(v1.host, { id: 'pythinker-cu', source: 'capability:pythinker-cu', builtIn: true } as never),
    ).toBe(false);
  });
  ...
  it('removePlugin notes that capability runtimes are left untouched', async () => {
    ...
    await removePlugin(host, 'pythinker-cu');
    expect(statuses.some((s) => s.includes('Removed pythinker-cu'))).toBe(true);
    ...
    expect(statuses.some((s) => s.includes('Restart Pythinker Code before reinstalling'))).toBe(true);
  ...
  it('keeps the runtime note for the Windows backing plugin id', async () => {
    const { host, statuses } = fakeHost({ engineV2: true });
    await removePlugin(host, 'pythinker-cu-win');
    expect(statuses.some((s) => s.includes('Removed pythinker-cu-win'))).toBe(true);
    expect(statuses.some((s) => s.includes('runtime binaries were left untouched'))).toBe(true);
  });
  ...
  it('treats only the default catalog (and the dev server) as injectable', () => {
    ...
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL: 'http://127.0.0.1:60056/marketplace.json',
        PYTHINKER_CODE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER: '1',
      }),
    ).toBe(true);
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL: 'https://example.test/marketplace.json',
      }),
    ).toBe(false);
  ...
  it('starts a capability install only when none is running', async () => {
    ...
    await installCapabilityFromPanel(
      idle.host,
      fakePanel().panel,
      { id: 'pythinker-cu', displayName: 'Pythinker Computer Use', source: 'capability:pythinker-cu' } as never,
    );
    expect(idle.installCapability).toHaveBeenCalledWith('pythinker-cu');
  });
  ...
  it('renders visible clickable store URLs after WebBridge installs in a hyperlink-capable terminal', async () => {
    ...
    expect(rendered).toContain(
      '\u001B]8;;https://chromewebstore.google.com/detail/pythinker-webbridge/fldmhceldgbpfpkbgopacenieobmligc\u001B\\',
    );
  ...
  it('renders full store URLs after WebBridge installs in a terminal without hyperlinks', async () => {
    ...
    expect(unwrappedVisibleText(transcriptEntries)).toContain(
      'https://chromewebstore.google.com/detail/pythinker-webbridge/fldmhceldgbpfpkbgopacenieobmligc',
    );
  });
  ...
  it('shows the engine error when a background capability install fails', async () => {
    ...
    expect(statuses).toContain(
      'Pythinker Computer Use installation failed: Authenticode signature is not valid',
    );

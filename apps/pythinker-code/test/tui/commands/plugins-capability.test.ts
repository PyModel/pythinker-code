...
describe('plugins command capability surface', () => {
  ...
  it('routes built-in entries through capabilities', () => {
    expect(
      isCapabilityEntry({ id: 'pythinker-cu', source: 'capability:pythinker-cu', builtIn: true } as never),
    ).toBe(true);
    expect(
      isCapabilityEntry({
    ...
    expect(
      isCapabilityEntry({ id: 'pythinker-cu', source: 'https://example.test/plugin.zip' } as never),
    ).toBe(false);
    // A forged capability: source without the parser-proof flag is a plain row.
    expect(isCapabilityEntry({ id: 'pythinker-cu', source: 'capability:pythinker-cu' } as never)).toBe(
      false,
    );
  });

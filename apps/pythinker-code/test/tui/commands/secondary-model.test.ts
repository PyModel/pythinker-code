 ...
 * Run: pnpm -C apps/pythinker-code exec vitest run test/tui/commands/secondary-model.test.ts
 */
import type { ModelAlias, ThinkingEffort } from '@pymodel/pythinker-code-sdk';
...
describe('handleSecondaryModelCommand', () => {
  ...
  it('warns with the env-overridden effective binding instead of the picked model', async () => {
    // PYTHINKER_SECONDARY_MODEL / PYTHINKER_SECONDARY_EFFORT win over the persisted
    ...
    expect(message).toContain('PYTHINKER_SECONDARY_MODEL=cheap');
    expect(message).toContain('PYTHINKER_SECONDARY_EFFORT=low');

...
describe('buildExportMarkdown', () => {
  ...
  it('builds complete markdown with frontmatter and overview', () => {
    ...
    expect(md).toContain('# Pythinker Session Export');
  ...
  it('renders an uploaded image daemon ref as [image] in the exported user message', () => {
    // An uploaded image persists as a self-contained `pythinker-file://` part —
    ...
    const msgs: ContextMessage[] = [
      {
        ...
        content: [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'pythinker-file://f_1?path=%2FUsers%2Falice%2Fmedia%2Ff_1.png' },
          },
        ],
        toolCalls: [],
        origin: { kind: 'user' },
    ...
    expect(md).not.toContain('pythinker-file');

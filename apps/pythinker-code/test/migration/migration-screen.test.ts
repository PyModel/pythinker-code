...
function makePlan(over: Partial<MigrationPlan> = {}): MigrationPlan {
  return {
    ...
    hasMcp: true,
    hasUserHistory: true,
    oauthCredentials: ['pythinker-code.json'],
    workdirs: [],
    detectedPlugins: [],
...
describe('MigrationScreenComponent — ask phase', () => {
  it('ask1 renders the intro block and three options', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.pythinker',
      targetHome: '/y/.pythinker-code',
      onComplete: () => {},
    });
    const out = render(c);
    expect(out).toContain('Migrate from pythinker-cli');
  ...
  it('ask1 summary does not mention pythinker-cli login (oauth is not a migrated kind)', async () => {
    // OAuth credentials are deliberately never migrated, so the pre-migration
    // summary must not list "pythinker-cli login" alongside the real migratable
    ...
    const out = render(c);
    expect(out).not.toContain('pythinker-cli login');
...
describe('MigrationScreenComponent — progress phase', () => {
  it('renders a step checklist and the session counter when in progress', () => {
    ...
    expect(out).toContain('Migrating from pythinker-cli');
...
describe('MigrationScreenComponent — result phase', () => {
  it('renders the report summary including plugin notices', () => {
    ...
    expect(out).toContain('2 pythinker-cli plugins');
  });
  ...
  it('surfaces conflict and failure warnings on the result screen', () => {
    ...
    const out = c.render(80).join('\n');
    expect(out).toContain('config.migrated-from-pythinker-cli.toml');
    expect(out).toContain('mcp.migrated-from-pythinker-cli.json');
  ...
  it('lists sibling-file contents in the config-fallback warning so the user knows what to merge', () => {
    ...
    // to `config.migrated-from-pythinker-cli.toml` instead, the result screen must
  ...
  it('lists kept config settings on the result screen when pythinker-cli differed', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.pythinker',
      targetHome: '/y/.pythinker-code',
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedModels: [],
            droppedKeys: [],
            configConflicts: ['default_model', 'providers.pythinker'],
            wroteSiblingDueToConflict: false,
            wroteTuiSibling: false,
            migratedHooks: 0,
            droppedHooks: 0,
    ...
    expect(out).toContain('default_model · providers.pythinker');
  });

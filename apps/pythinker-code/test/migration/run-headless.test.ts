...
async function writeRealSession(workdir: string, uuid: string): Promise<void> {
  ...
  await writeFile(
    ...
    JSON.stringify({ work_dirs: [{ path: workdir, pyaos: 'local', last_session_id: uuid }] }),
    'utf-8',
  );
  await writeFile(
...
describe('runHeadlessMigrate', () => {
  ...
  it('migrates config and sessions, writes the report and the completion marker', async () => {
    ...
    const marker = JSON.parse(
      await readFile(join(sourceHome(), '.migrated-to-pythinker-code'), 'utf-8'),
    );
  ...
  it('exits incomplete and writes no marker when a session fails', async () => {
    ...
    await expect(
      readFile(join(sourceHome(), '.migrated-to-pythinker-code'), 'utf-8'),
    ).rejects.toThrow();
  });

  it('honors PYTHINKER_SHARE_DIR as the source and keeps skills on the default home', async () => {
    ...
    const code = await runHeadlessMigrate(
      { configOnly: false },
      { env: { PYTHINKER_SHARE_DIR: shareDir }, userHome: home, targetHome: target, write },
    );
    expect(code).toBe(MIGRATE_HEADLESS_EXIT.success);
    const out = lines.join('\n');
    expect(out).toContain(`source: ${shareDir} (PYTHINKER_SHARE_DIR)`);

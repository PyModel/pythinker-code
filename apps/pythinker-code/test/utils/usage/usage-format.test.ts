  ...
  it('renders one row per window the backend served, in payload order', () => {
    ...
    expect(rows).toEqual([
      ...
      { name: 'Weekly limit', usedRatio: 0.2, resetAt: '2026-09-17T00:00:00Z', breakdown: undefined },
      {
        ...
        breakdown: { pythinkerRatio: 0.15, codeRatio: 0.25 },
      },
    ]);
  });
  ...
  it('skips entries the backend omitted', () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        usedRatio: 0.4,
        resetAt: undefined,
        breakdown: { pythinkerRatio: 0.4, codeRatio: 0 },
      },
    ]);
  });

  it('clamps the pythinker share against float noise and over-100 code shares', () => {
    const rows = quotaUsageRows({
      usages: {
    ...
    expect(rows[0]?.breakdown).toEqual({ pythinkerRatio: 0, codeRatio: 0.25 });
  });
});

...
import { resolvePythinkerCodeOAuthKey } from '@pythoughts/pythinker-code-oauth';
import { describe, expect, it } from 'vitest';

const REPO_ROOT = join(import.meta.dirname, '../../../..');
const SERVER_ENTRY = join(REPO_ROOT, 'plugins/official/pythinker-datasource/bin/pythinker-datasource.mjs');
const SKILL_PATH = join(REPO_ROOT, 'plugins/official/pythinker-datasource/SKILL.md');

describe('pythinker-datasource MCP server', () => {
  it('defaults agent-facing instructions to English', async () => {
    const skill = await readFile(SKILL_PATH, 'utf8');

    expect(skill).toContain('Answer in English by default');
    expect(skill).not.toMatch(/language the user used/iu);
  });

  it('exposes the same two generic tools as the Python plugin', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pythinker-datasource-plugin-'));
    const pythinkerHome = join(tempDir, 'pythinker-home');
    let child: ChildProcessWithoutNullStreams | undefined;

    try {
      await mkdir(join(pythinkerHome, 'credentials'), { recursive: true });
      await writeFile(
        join(pythinkerHome, 'credentials', 'pythinker-code.json'),
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          PYTHINKER_CODE_HOME: pythinkerHome,
        },
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          PYTHINKER_CODE_HOME: pythinkerHome,
          PYTHINKER_DATASOURCE_API_URL: `http://127.0.0.1:${address.port}`,
        },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('uses env-scoped credentials and derives the datasource URL from PYTHINKER_CODE_BASE_URL', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pythinker-datasource-plugin-'));
    const pythinkerHome = join(tempDir, 'pythinker-home');
    const requests: unknown[] = [];
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      const scopedCredential = pythinkerCodeEnvCredentialName({ oauthHost, baseUrl });
      ...
      await writeFile(
        join(pythinkerHome, 'credentials', `${scopedCredential}.json`),
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          PYTHINKER_CODE_HOME: pythinkerHome,
          PYTHINKER_CODE_BASE_URL: baseUrl,
          PYTHINKER_CODE_OAUTH_HOST: oauthHost,
          PYTHINKER_DATASOURCE_API_URL: undefined,
        },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('registers yuandian_law in the get_data_source_desc enum', async () => {
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        cwd: REPO_ROOT,
        env: { ...process.env, PYTHINKER_CODE_HOME: pythinkerHome },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('appends a request-id / tool-call-id trace line to tool results', async () => {
    ...
    try {
      ...
      expect(text).toContain('[pythinker-datasource] request-id: backend-req-test · tool-call-id:');
    } finally {
      child?.stdin.end();
      child?.kill();
      await closeServer(server);
...
// truth in @pythoughts/pythinker-code-oauth. The credential file name is the OAuth
// key with its `oauth/` prefix stripped.
function pythinkerCodeEnvCredentialName(options: {
  readonly oauthHost: string;
  readonly baseUrl: string;
}): string {
  return resolvePythinkerCodeOAuthKey(options).replace(/^oauth\//, '');
}

 ...
 * installs a fresh `pythinker` shim into the global bin dir; this script
 * removes any pre-existing `pythinker` shim left behind by the previous
 * Python CLI (installed via `uv tool install`, `pipx install`,
 * `pip install`, etc.) that would otherwise shadow ours via PATH
 * ordering. The renamed shim is kept as `pythinker-legacy` so users can
 ...
 *   - Does not touch a `pythinker` we don't recognize as the previous
 ...
 *     rename target like `pythinker.exe` → `pythinker-legacy.exe`).
 *
 * ## Code layout
 *
 ...
 *     `pythinker`-vs-`pythinker-legacy` classification, the rename / unlink
 *     primitives.
 ...
 *      The `bin.pythinker` mapping in `package.json` tells the manager to
 *      install a `pythinker` shim under its global bin directory.
 ...
 *      twice.
 *   5. Detect EVERY previous Python `pythinker-cli` shim on the detection
 *      PATH (`detectLegacyShims`). Returns `[]` for fresh-install /
 *      no-op. Multiple results happen when the user has installed
 *      `pythinker-cli` through more than one Python tool (uv + pipx, or
 ...
 *      `findFirstResolvablePythinker` walks PATH treating the actionable
 ...
 *        - `foreign` → some `pythinker` we don't recognize (a user's own
 *          file) wins. Surface `logForeignPythinkerInTheWay` asking the
 *          user to delete or rename their own file; touch nothing.
 *        - `none` → no `pythinker` on PATH at all (our shim's bin dir
 ...
 *      touch becomes `pythinker-legacy` (preserves what `pythinker` referred
 *      to before this install). Each subsequent shim is `unlink`ed —
 *      keeping it as a dormant duplicate adds no value. If the
 *      first shim's `pythinker-legacy` target is already user-managed,
 *      we delete `pythinker` anyway (still achieves takeover) and tell
 *      the user we couldn't preserve a fallback. Extension is
 *      preserved on Windows (`pythinker.exe` → `pythinker-legacy.exe`).
...
async function main() {
  ...
  // we don't claim "pythinker works now" when the shim only sits in the
  // installer's env).
  const paths = await postinstallPaths();

  // Step 4: detect EVERY previous Python `pythinker-cli` shim on the
  ...
  //     needed)
  //   - an unrelated `pythinker` we don't recognize (a user's own wrapper
  ...
  const blocker = await findFirstResolvablePythinker(
    ownRoot,
    paths.reachability,
    actionableShimPaths,
  ...
  if (blocker.kind !== 'own') {
    if (blocker.kind === 'blocked-legacy') {
      ...
      logForeignPythinkerInTheWay(blocker.path, pm);
    } else {
  ...
  // we can touch becomes `pythinker-legacy` (preserves what the user's
  // `pythinker` used to refer to). Every subsequent shim is just
...
main().catch((err) => {
  ...
  notify(`[pythinker-code] postinstall warning: ${message}`);
});

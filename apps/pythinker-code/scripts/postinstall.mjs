 ...
 *      twice.
 *   5. `planTakeover`: detect EVERY previous Python `pythinker-cli` shim
 ...
 *        - a foreign `pythinker` wins → `logForeignPythinkerInTheWay`.
 ...
 *      preserved becomes `pythinker-legacy`; each subsequent shim is
...
async function main() {
  ...
  if (plan.kind === 'foreign') {
    logForeignPythinkerInTheWay(plan.path, pm);
    return;
  }
  ...
  // `pythinker-legacy`, delete the rest).

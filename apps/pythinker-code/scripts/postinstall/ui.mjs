 ...
 *     `pythinker-legacy`. The "`pythinker` now launches the new CLI" claim is
 *     safe to make here because the orchestrator already verified
 *     reachability after this set of removals.
 *   - "Consolidated" — first shim's `pythinker-legacy` already pointed at
 *     a legacy file (re-migration case); we deleted the duplicate
 *     source and kept the existing target. Same end state, different
 *     mechanism.
 *   - "Couldn't preserve as pythinker-legacy" — first shim's `pythinker-legacy`
 *     slot was a user-managed file; we deleted the source `pythinker` to
...
export function logMigrationDone(outcomes, pm) {
  const reinstallCmd = pmGlobalInstallCommand(pm, '@pymodel/pythinker-code');
  const {
    renames,
    consolidates,
  ...
  const lines = [successHeading('pythinker now runs the new version'), ''];

  if (renames.length > 0) {
    lines.push(pad('   Renamed your old pythinker so you can still run it as'));
    lines.push(pad('   pythinker-legacy:'));
  ...
  if (consolidates.length > 0) {
    lines.push(pad('   Removed an extra copy of your old pythinker (pythinker-legacy'));
    lines.push(pad('   was already set up here from before):'));
    for (const c of consolidates) {
      lines.push(pathInBox(c.shimPath));
      lines.push(pathInBox('  (pythinker-legacy is at ' + c.target + ')'));
    }
    lines.push('');
  }

  if (skippedForeignTarget.length > 0) {
    lines.push(pad('   Removed your old pythinker (a file you created was already'));
    lines.push(pad('   using the name pythinker-legacy, so we left it alone):'));
  ...
  if (deletes.length > 0) {
    ...
    lines.push(pad('   new pythinker if we left them):'));
  ...
  if (blockedHarmless.length > 0) {
    ...
    lines.push(pad('   they won\'t run instead of the new pythinker:'));
  ...
  //      available as `pythinker-legacy`. Standard takeover footer.
  //   2. Only skippedForeignTarget (no rename, no consolidate, no
  //      blockedHarmless): user has a file they made called
  //      `pythinker-legacy`, so we couldn't save the old CLI under that
  ...
  if (preservedSomewhere) {
    lines.push(
      pad('   Now typing `pythinker` runs the new version. To run the old'),
      pad('   version, type `pythinker-legacy` instead. Your settings from'),
      pad('   the old version will be moved over the first time you'),
      pad('   run `pythinker`.'),
      '',
      pad('   Note: if you reinstall the old pythinker later (e.g. with'),
      pad('   `uv tool`, `pip`, or `pipx`), it will put `pythinker` back.'),
      pad('   Run this command again to switch to the new one:'),
      pathInBox(reinstallCmd),
      '',
      pad('   If typing `pythinker` still runs the old version, open a new'),
      pad('   terminal window — your current one may have remembered'),
      pad('   the old path.'),
    );
  } else if (skippedForeignTarget.length > 0) {
    lines.push(
      pad('   Now typing `pythinker` runs the new version. Your settings'),
      pad('   from the old version will be moved over the first time'),
      pad('   you run `pythinker`.'),
      '',
      pad('   We couldn\'t save the old pythinker as `pythinker-legacy` because'),
      pad('   that name was already taken by a file you\'d created.'),
      pad('   If you need the old pythinker back, install it again with'),
      pad('   `uv tool install pythinker-cli` (or pipx / pip).'),
      '',
      pad('   If typing `pythinker` still runs the old version, open a new'),
      pad('   terminal window — your current one may have remembered'),
      pad('   the old path.'),
...
export function logMigrationBlocked(blocked, actionable, pm) {
  ...
  const lines = [
    warningHeading('Can\'t switch to the new pythinker yet'),
    '',
    pad('   There\'s an old pythinker on your computer that we can\'t change.'),
    pad('   As long as it\'s there, typing `pythinker` will still run the old'),
  ...
  if (actionable.length > 0) {
    lines.push(
      '',
      pad('   We also found these old pythinker files. We could remove them'),
...
/**
 * The reachability check found a `pythinker` ahead of our shim on PATH
 ...
 * user can decide.
 */
export function logForeignPythinkerInTheWay(foreignPath, pm) {
  const reinstallCmd = pmGlobalInstallCommand(pm, '@pymodel/pythinker-code');
  emit(
    renderBox([
      warningHeading('Can\'t switch to the new pythinker yet'),
      '',
      pad('   There\'s another file called `pythinker` on your computer that\'s'),
      pad('   not the new CLI and not the old one — it looks like'),
      pad('   something you set up yourself. As long as it\'s there,'),
      pad('   typing `pythinker` will run it instead of the new version.'),
      '',
      pad('   We found it at:'),
      pathInBox(foreignPath),
      '',
      pad('   To use the new pythinker, delete or rename that file, then'),
      pad('   install again:'),
      pathInBox(reinstallCmd),
      '',
      pad('   Nothing on your computer was changed.'),
 ...
 * verification: `pythinker` still resolves to a legacy/foreign file (or to
...
export function logMigrationIncomplete(input, pm) {
  ...
  const lines = [warningHeading('Couldn\'t finish switching to the new pythinker'), ''];

  if (verify.kind === 'blocked-legacy') {
    lines.push(
      pad('   Typing `pythinker` still runs the old version. This file is in'),
    ...
    lines.push(
      pad('   Typing `pythinker` runs a file we don\'t recognize (not the new'),
    ...
    lines.push(
      pad('   Typing `pythinker` currently finds nothing at all — the old'),
  ...
  const changed = [
    ...
    ...consolidates.map((c) => c.shimPath + '  (removed; pythinker-legacy kept)'),
  ...
  if (verify.kind === 'blocked-legacy') {
    ...
    lines.push(
      pad('   Open a new terminal and check `which pythinker`. If it finds'),
  ...
  if (renames.length > 0 || consolidates.length > 0) {
    lines.push(
      pad('   The old version is still available as `pythinker-legacy`.'),
    );
  }
...
/**
 * The legacy `pythinker` was found, but the directory where the package
 * manager placed the new `pythinker` shim is not on the user's PATH.
 * Renaming the legacy shim now would leave the user with NO reachable
 * `pythinker` command — the new one would still not be discoverable by
...
export function logNewCliNotOnPath(detection, pm) {
  ...
  emit(
    renderBox([
      warningHeading('New pythinker is installed, but your terminal can\'t find it'),
      '',
      pad('   The old pythinker is still here:'),
      pathInBox(detection.shimPath),
      '',
      pad('   The new pythinker was installed by ' + pm + ', but it landed in a'),
      pad('   folder your terminal doesn\'t search. (Your terminal looks'),
      pad('   for commands in folders listed in your PATH.) If we removed'),
      pad('   the old pythinker now, typing `pythinker` wouldn\'t find anything.'),
      '',
      pad('   Add the new pythinker\'s folder to your PATH (and save the change'),
      pad('   in your ' + rcLabel + ' so it sticks), then install again:'),
      pathInBox(newPathHint),
      pathInBox(reinstallCmd),
      '',
      pad('   The old pythinker is still where it was.'),
    ]),
  );
}

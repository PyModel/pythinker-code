 ...
 *     pipx-installed `pythinker-cli` has two legacy shims in different
 ...
 *     decide whether an existing `pythinker-legacy` is itself a legacy CLI
 ...
 * with a misleading "pythinker now launches the new CLI" notice in front of
...
const LEGACY_BIN = 'pythinker';
const LEGACY_RENAME = 'pythinker-legacy';
const PYTHON_MARKER = 'pythinker_cli';
...
//            (~45 KiB) and the `pythinker_cli` module name sits embedded
...
/**
 * Expand `pythinker` into the set of filenames that resolve as executables
 * on this platform. POSIX → just `['pythinker']`. Windows → adds every
 * `PATHEXT` extension (so we find `pythinker.exe`, `pythinker.cmd`, etc).
 */
...
/**
 * Walk `pathString` and return every legacy `pythinker` shim along the
...
export async function detectLegacyShims(ownRoot, pathString) {
  ...
  for (const dir of pathEntries(pathString)) {
    for (const name of candidates) {
      ...
      // Defence-in-depth: never touch a `pythinker` that resolves into our
      // own installed package. The `pythinker_cli` marker check below
...
/**
 * Does the file at `p` look like the legacy Python `pythinker-cli`?
 *
 ...
 * resolved file contains the `pythinker_cli` module name. Used to decide
 * whether an existing `pythinker-legacy` is itself a legacy shim (safe to
 * drop the duplicate `pythinker`) or a user-managed file we must not
 * clobber.
 */
 ...
 * extension so a Windows `C:\…\pythinker.exe` ends up at `pythinker-legacy.exe`
 * rather than an extension-less `pythinker-legacy` that `pythinker.exe -- legacy`
 * shells won't run.
 */
 ...
 *   `sudo pip install pythinker-cli` → `/usr/local/bin/`.
 *
 ...
 * which shim becomes pythinker-legacy) before any filesystem writes
 * happen.
 *
 ...
 *                        existing `pythinker-legacy` as the canonical
 ...
 *                        "preserve original pythinker" invariant fails for
 ...
 *   - we're consolidating onto an existing legacy `pythinker-legacy`, or

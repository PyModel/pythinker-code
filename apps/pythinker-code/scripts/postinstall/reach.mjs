/**
 * Where the user actually types things, and what `pythinker` resolves to
 * from there.
 *
 * Covers:
 *
 ...
 *     `pythinker` into, not just the installer's environment.
 ...
 *   - The reachability check ({@link findFirstResolvablePythinker}) —
 ...
 *     unknown/foreign `pythinker` (e.g. a user-written wrapper).
 *
...
/**
 * Expand a basename like `pythinker` into the set of filenames the OS
 * would actually match on PATH.
 *
 * On POSIX: just `['pythinker']`.
 *
 * On Windows: `['pythinker', 'pythinker.exe', 'pythinker.cmd', …]` — every
 * extension in `PATHEXT`. Without this, our PATH walk would miss
 * the typical `pythinker.exe` shim produced by `uv tool install` on
 * Windows.
 */
 ...
 *     --location=global @pythoughts/pythinker-code` — verified on npm
 *     11.13.0.
 ...
 * directory so symlinked install layouts (e.g. `<prefix>/bin/pythinker`
...
//     do on POSIX)
const PACKAGE_NAME_MARKERS = ['@pythoughts/pythinker-code', '@pythoughts\\pythinker-code'];
 ...
 * `pythinker` to, AFTER the to-be-removed shims in `actionableShimPaths`
 * are pretended gone. Returns the first match by PATH order:
 *
 *   - { kind: 'own' }                     — our shim wins.
 *   - { kind: 'blocked-legacy', shim }    — a legacy `pythinker` we
 *     detected but couldn't touch (a "blocked" shim) wins.
 *   - { kind: 'foreign', path }           — something else wins: a
 *     `pythinker` we didn't recognize as a legacy CLI and didn't generate
 *     ourselves (e.g. a user-managed wrapper script in `~/bin`).
 *   - { kind: 'none' }                    — no `pythinker` resolves at all.
 *
 ...
 * unknown `pythinker`.
 */
export async function findFirstResolvablePythinker(
  ownRoot,
  pathString,
  actionableShimPaths,
 ...
 * `pythinker` into. A `~/.zshrc`-only PATH entry, a `~/.bash_profile`
...
export async function userShellPath() {
  ...
  return new Promise((resolve) => {
    ...
    const probe =
      'printf "<<<PYTHINKER_PATH_BEGIN>>>%s<<<PYTHINKER_PATH_END>>>\\n" "$PATH"';
    ...
    child.on('close', (code) => {
      ...
      const match = stdout.match(
        /<<<PYTHINKER_PATH_BEGIN>>>([\s\S]*?)<<<PYTHINKER_PATH_END>>>/,
      );
 ...
 *     because either source may contain a legacy `pythinker` we want to
 ...
 *     the user will actually type `pythinker` into. We do NOT union here:

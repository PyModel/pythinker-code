 ...
 *
 * Input dir must contain files matching: pythinker-code-<target>.zip.sha256
...
if (sumFiles.length === 0) {
  console.error(`No pythinker-code-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // pythinker-code-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^pythinker-code-/, '').replace(/\.zip$/, '');

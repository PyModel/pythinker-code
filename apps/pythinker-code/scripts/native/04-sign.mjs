...
function azureSigningEnv() {
  ...
  if (!endpoint || !accountName || !profileName) {
    fail(
      'PYTHINKER_AZURE_TRUSTED_SIGNING=true requires AZURE_TRUSTED_SIGNING_ENDPOINT, ' +
...
export async function runSignStep({ identity = '-', keychainPath = null } = {}) {
  ...
  if (process.platform === 'win32' && process.env.PYTHINKER_AZURE_TRUSTED_SIGNING === 'true') {

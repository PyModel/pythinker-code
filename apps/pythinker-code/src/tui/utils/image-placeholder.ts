 ...
 *     `pythinker-file://<id>` image part — the engine's prompt intake materializes
 ...
 *   - video placeholders expand to a bare `pythinker-file://<id>` video part:
...
export function extractMediaAttachments(
  ...
  while ((match = PLACEHOLDER_REGEX.exec(text)) !== null) {
    ...
    if (attachment.kind === 'video') {
      // The paste was uploaded to the daemon file store in the background:
      // reference it by a bare `pythinker-file://` url — the engine's prompt
      ...
      if (attachment.fileId !== undefined) {
        ...
        // (v2): reference them by a bare `pythinker-file://` url — the engine's
...
export function refreshExpiringImageFileRefs(
  ...
  const next = parts.map((part) => {
    ...
    if (attachmentId === undefined || !part.imageUrl.url.startsWith('pythinker-file://')) return part;
...
export function makeExtractionResendable(
  ...
  for (const part of extraction.parts) {
    ...
    if (snapshot === undefined || !part.imageUrl.url.startsWith('pythinker-file://')) {
      parts.push(part);
      continue;
    }
    parts.push({
...
function imagePartMatchesAttachment(
  ...
  if (url.startsWith('pythinker-file://')) {
 ...
 * `pythinker-file://` url — the engine's prompt intake materializes the session

 ...
 * `ImageAttachment.original`) and video placeholders to `pythinker-file://`
 * daemon references (the paste was uploaded to the daemon file store in
 * the background, exactly like an uploaded image).
 *
 * Scope is per-`PythinkerTUI` instance. Reloads (`/new`, `/clear`,
...
export interface ImageAttachment {
   ...
   * (v2 engine only). Submit-time expansion then emits a `pythinker-file://`
...
export interface VideoAttachment {
   ...
   * time. Submit-time expansion emits a `pythinker-file://` video reference;

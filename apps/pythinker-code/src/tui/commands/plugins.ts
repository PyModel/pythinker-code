...
import { Spacer } from '@pymodel/pi-tui';
...
function isDefaultMarketplaceCatalog(
  ...
  if (env[PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL_ENV] === undefined) return true;
  return env['PYTHINKER_CODE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER'] === '1';
}
...
async function installCapabilityFromPanel(
  ...
  if (entry.id === 'pythinker-webbridge') {
...
const WEBBRIDGE_POST_INSTALL_MARKDOWN = [
  '*Two steps left to use Pythinker Pythinker Browser Extension:*',

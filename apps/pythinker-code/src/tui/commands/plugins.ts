...
import type { CapabilityStatus, PluginInfo, PluginSummary, Session } from '@pymodel/pythinker-code-sdk';
...
async function showPluginsPicker(
  ...
  const panel = new PluginsPanelComponent({
    ...
    catalogIsDefault:
      ...
      process.env[PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL_ENV] === undefined,
...
async function loadMarketplaceCatalog(
  ...
  try {
    ...
    const isDefaultCatalog =
      source === undefined && process.env[PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL_ENV] === undefined;
...
async function confirmInstallTrust(
  ...
  // Pythinker-built official plugins are trusted implicitly; anything else requires
...
function isCapabilityId(host: SlashCommandHost, id: string): boolean {
  return host.engineV2 && (id === 'pythinker-cu' || id === 'pythinker-webbridge');
}
...
async function installCapabilityFromPanel(
  ...
  if (result.state !== 'ready') {
    ...
    if (result.id === 'pythinker-cu' && result.steps.some((step) => step.id === 'permissions' && step.state !== 'ok')) {
      host.showStatus(
  ...
  if (skillShadow?.detail !== undefined) {
    host.showStatus(
      `A user-installed pythinker-webbridge skill is shadowing the managed plugin. Remove it manually: ${skillShadow.detail}`,
      'warning',
    );
  }
  host.showStatus(PLUGIN_RELOAD_HINT, 'warning');
...
async function removePlugin(host: SlashCommandHost, id: string): Promise<void> {
  ...
  if (isCapabilityId(host, id)) {
    host.showStatus(
      'Note: the runtime binaries were left untouched, but Pythinker Code plugin wiring is disabled for new sessions. Reinstall any time from the Official tab.',
    );
  }
  host.showStatus(PLUGIN_RELOAD_HINT, 'warning');
}

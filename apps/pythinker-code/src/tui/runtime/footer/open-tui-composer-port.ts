...
export type ComposerIntent =
  ...
  | { readonly kind: 'steer' }
  | { readonly kind: 'cycle-thinking-effort' }
...
const COMPOSER_ACTIONS: ReadonlySet<string> = new Set([
  ...
  'chat:cancel',
  // 'chat:cycleMode' is deprecated (kept for schema back-compat only; see
  // keybindings.ts) but still routed here so a legacy user rebind doesn't
...
export class OpenTuiComposerPort implements ComposerPort {
  ...
  public handleKey(key: ComposerKey): readonly ComposerIntent[] {
    ...
    if (
      ...
      return [{ kind: 'cycle-thinking-effort' }];
    }
  ...
  private handleConfiguredKey(
    ...
    const handled = this.keybindings.dispatchKeyId(
      ...
      ['Chat', 'Global'],
      {
        ...
        'chat:cycleMode': () => {
          intents = [{ kind: 'cycle-thinking-effort' }];
        },
        'chat:thinkingToggle': () => {
          intents = [{ kind: 'cycle-thinking-effort' }];
        },

/**
 * Renders an assistant message using the Pythinker markdown theme.
 *
...
export class AssistantMessageComponent implements Component {
  ...
  updateContent(text: string): void {
    ...
    if (displayText.trim().length > 0) {
      this.contentContainer.addChild(new Markdown(displayText.trim(), 0, 0, createPythinkerMarkdownTheme()));
    }
  }
  ...
  invalidate(): void {
    ...
    if (this.lastText.trim().length > 0) {
      this.contentContainer.addChild(
        new Markdown(this.lastText.trim(), 0, 0, createPythinkerMarkdownTheme()),
      );
    }
  }

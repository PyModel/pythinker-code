...
import {
  AgentDynamicWorkflowProgressEstimator,
...
const AGENT_DYNAMIC_WORKFLOW_LEFT_INDENT = ' ';
...
type AgentDynamicWorkflowPhase = AgentDynamicWorkflowProgressEstimatorPhase;
...
interface AgentDynamicWorkflowMember {
  readonly id: string;
  agentId?: string;
  phase: AgentDynamicWorkflowPhase;
  ticks: number;
  itemText: string;
...
interface AgentDynamicWorkflowSnapshot {
  readonly phase: AgentDynamicWorkflowPhase;
  readonly ticks: number;
  readonly latestModelText: string;
  readonly phaseElapsedMs: number;
}

interface AgentDynamicWorkflowResultStatus {
...
export interface AgentDynamicWorkflowResultSummary {
...
interface AgentDynamicWorkflowSummary {
...
export interface AgentDynamicWorkflowGridLayout {
...
export interface AgentDynamicWorkflowProgressOptions {
  readonly description: string;
  readonly requestRender?: () => void;
  readonly availableGridHeight?: () => number | undefined;
}

const PHASE_LABELS: Record<AgentDynamicWorkflowPhase, string> = {
...
export class AgentDynamicWorkflowProgressComponent implements Component {
  ...
  updateArgs(
    ...
    const description = agentDynamicWorkflowDescriptionFromArgs(args);
    if (description.length > 0 || this.description.length === 0) {
      this.description = description;
    }
    const fullRows = [...agentDynamicWorkflowResumeItemsFromArgs(args), ...agentDynamicWorkflowItemsFromArgs(args)];
    const partialRows = streamingArguments === undefined
      ? []
      : [
          ...agentDynamicWorkflowPartialResumeItemsFromArguments(streamingArguments),
          ...agentDynamicWorkflowPartialItemsFromArguments(streamingArguments),
        ];
    if (
      fullRows.length > 0 ||
      partialRows.length > 0 ||
      (streamingArguments !== undefined && agentDynamicWorkflowWorkItemsStartedFromArguments(streamingArguments))
    ) {
      this.itemsStarted = true;
    }
    const fullPromptTemplate = agentDynamicWorkflowPromptTemplateFromArgs(args);
    const partialPromptTemplate =
      streamingArguments === undefined
        ? ''
        : agentDynamicWorkflowPartialPromptTemplateFromArguments(streamingArguments);
  ...
  markDynamicWorkflowFailed(failureText?: string): void {
    this.failed = true;
    this.aborted = false;
    const nowMs = Date.now();
    for (const member of this.members) {
  ...
  applyResult(output: string): boolean {
    const statuses = parseAgentDynamicWorkflowResultStatuses(output);
  ...
  private indentLines(lines: readonly string[], width: number): string[] {
    const contentWidth = Math.max(
      0,
      width - visibleWidth(AGENT_DYNAMIC_WORKFLOW_LEFT_INDENT) - AGENT_DYNAMIC_WORKFLOW_RIGHT_GAP,
    );
    return lines.map((line) =>
      truncateToWidth(
        AGENT_DYNAMIC_WORKFLOW_LEFT_INDENT + truncateToWidth(line, contentWidth),
        width,
      )
    );
  }
  ...
  private renderGrid(
    ...
    snapshots: readonly AgentDynamicWorkflowSnapshot[],
    nowMs: number,
  ): string[] {
    const layout = calculateAgentDynamicWorkflowGridLayout({
      width,
  ...
  private renderCellContent(
    ...
    const bar = brailleBar(
      (progressEstimate?.displayTicks ?? 0),
      snapshot.phase,
      layout.barCells,
      this.colors,
  ...
  ): AgentDynamicWorkflowMember | undefined {
    const existing = this.findMemberByAgentId(agentId);
    if (existing !== undefined) return existing;

    if (dynamicWorkflowIndex !== undefined && Number.isInteger(dynamicWorkflowIndex) && dynamicWorkflowIndex > 0) {
      this.ensureMemberCount(dynamicWorkflowIndex);
      const byIndex = this.members[dynamicWorkflowIndex - 1];
  ...
  private promoteToRunning(member: AgentDynamicWorkflowMember, nowMs?: number, setTicks = false): void {
  ...
  private completeMember(member: AgentDynamicWorkflowMember, nowMs: number, completedText?: string): void {
  ...
  private failMember(member: AgentDynamicWorkflowMember, nowMs: number, failureText?: string): void {
  ...
  private cancelMember(member: AgentDynamicWorkflowMember, nowMs: number): void {
...
function createMembers(count: number, phase: AgentDynamicWorkflowPhase): AgentDynamicWorkflowMember[] {
...
function terminalPhaseElapsedMs(member: AgentDynamicWorkflowMember, nowMs: number): number {
...
export function agentDynamicWorkflowItemsFromArgs(args: Record<string, unknown>): string[] {
  const items = args['items'];
  if (!Array.isArray(items)) return [];
  return items.map(String);
}

function agentDynamicWorkflowResumeItemsFromArgs(args: Record<string, unknown>): string[] {
...
export function agentDynamicWorkflowPartialItemsCountFromArguments(argumentsText: string): number {
  return agentDynamicWorkflowPartialItemsFromArguments(argumentsText).length;
}

function agentDynamicWorkflowWorkItemsStartedFromArguments(argumentsText: string): boolean {
  return /"items"\s*:/.test(argumentsText) || /"resume_agent_ids"\s*:/.test(argumentsText);
}

export function agentDynamicWorkflowPartialItemsFromArguments(argumentsText: string): string[] {
...
function agentDynamicWorkflowPartialResumeItemsFromArguments(argumentsText: string): string[] {
...
export function agentDynamicWorkflowDescriptionFromArgs(args: Record<string, unknown>): string {
  const description = args['description'];
  return typeof description === 'string' ? description : '';
}

function agentDynamicWorkflowPromptTemplateFromArgs(args: Record<string, unknown>): string {
  const promptTemplate = args['prompt_template'];
  return typeof promptTemplate === 'string' ? promptTemplate : '';
}

function agentDynamicWorkflowPartialPromptTemplateFromArguments(argumentsText: string): string {
...
export function agentDynamicWorkflowResultSummaryFromOutput(output: string): AgentDynamicWorkflowResultSummary {
  const statuses = parseAgentDynamicWorkflowResultStatuses(output);
  let completed = 0;
  let failed = 0;
  let aborted = 0;
...
function parseAgentDynamicWorkflowResultStatuses(output: string): AgentDynamicWorkflowResultStatus[] {
  const xmlStatuses = parseAgentDynamicWorkflowXmlResultStatuses(output);
  if (xmlStatuses.length > 0) return xmlStatuses;
  return parseAgentDynamicWorkflowLegacyResultStatuses(output);
}
...
function parseAgentDynamicWorkflowXmlResultStatuses(output: string): AgentDynamicWorkflowResultStatus[] {
...
function parseAgentDynamicWorkflowLegacyResultStatuses(output: string): AgentDynamicWorkflowResultStatus[] {
  return forEachAgentBlock(output, (block, index) => {
    ...
    return {
      ...
      completedText: status === 'completed' ? parseAgentDynamicWorkflowCompletedText(block) : undefined,
      failureText: status === 'failed' ? parseAgentDynamicWorkflowFailureText(block) : undefined,
    };
  });
}

function parseAgentDynamicWorkflowCompletedText(block: string): string | undefined {
  const marker = '\n[summary]\n';
...
function parseAgentDynamicWorkflowFailureText(block: string): string | undefined {
...
function textGridLayout(
  ...
  gapWidth: number,
  idWidth: number,
): AgentDynamicWorkflowGridLayout {
  return {
    renderText: true,
...
export function calculateAgentDynamicWorkflowGridLayout(
  input: AgentDynamicWorkflowGridLayoutInput,
): AgentDynamicWorkflowGridLayout {
  const count = Math.max(0, Math.floor(input.count));
  const width = Math.max(0, Math.floor(input.width));
  const height = Math.max(0, Math.floor(input.height));
  const idWidth = agentDynamicWorkflowGridIdWidth(count);
...
export function agentDynamicWorkflowGridHeightForTerminalRows(
  rows: number | undefined,
  followingRows = 0,
): number | undefined {
  if (rows === undefined || !Number.isFinite(rows)) return undefined;
  const rowsAfterDynamicWorkflow = Number.isFinite(followingRows)
    ? Math.max(0, Math.floor(followingRows))
    : 0;
  return Math.max(0, Math.floor(rows) - rowsAfterDynamicWorkflow - AGENT_DYNAMIC_WORKFLOW_NON_GRID_LINES);
}

function agentDynamicWorkflowGridIdWidth(count: number): number {
...
function summarizeSnapshots(snapshots: readonly AgentDynamicWorkflowSnapshot[]): AgentDynamicWorkflowSummary {
  let completed = 0;
  let failed = 0;
  let cancelled = 0;
...
function statusBarCounts(members: readonly AgentDynamicWorkflowMember[]): StatusBarCount[] {
...
function statusBarPhase(phase: AgentDynamicWorkflowPhase): StatusBarPhase {
  const map: Record<AgentDynamicWorkflowPhase, StatusBarPhase> = {
    pending: 'queued',
    queued: 'queued',
    suspended: 'suspended',
    running: 'working',
...
function normalizeFailureText(text: string | undefined): string | undefined {
  ...
  const nestedFailureText = nestedAgentDynamicWorkflowFailureText(text);
  const normalized = stripAgentDynamicWorkflowPrefix(collapseWhitespace(nestedFailureText ?? text));
  return normalized.length > 0 ? normalized : undefined;
}

function nestedAgentDynamicWorkflowFailureText(text: string): string | undefined {
  const xmlFailureText = nestedAgentDynamicWorkflowXmlFailureText(text);
  if (xmlFailureText !== undefined) return nestedAgentDynamicWorkflowFailureText(xmlFailureText) ?? xmlFailureText;

  if (!/^\s*agent_dynamic_workflow:\s*failed\b/m.test(text)) return undefined;
  const match = /^\s*subagent error:\s*([\s\S]*?)(?=\n\[agent \d+\]\n|$)/m.exec(text);
  if (match === null) return undefined;
  const failureText = match[1];
  if (failureText === undefined) return undefined;
  return nestedAgentDynamicWorkflowFailureText(failureText) ?? failureText;
}

function nestedAgentDynamicWorkflowXmlFailureText(text: string): string | undefined {
  if (!/<agent_dynamic_workflow_result\b/.test(text)) return undefined;
  const failed = parseAgentDynamicWorkflowXmlResultStatuses(text).find((entry) => {
    return entry.status === 'failed' && entry.failureText !== undefined;
  });
  return failed?.failureText;
}

function stripAgentDynamicWorkflowPrefix(text: string): string {
  return text.replace(/^agent_dynamic_workflow:\s*(?:failed|completed)?\s*/i, '').trim();
}

...
import {
  ...
  isDynamicWorkflowResult,
} from './dynamic-workflow-mission-control';
...
function isDismissedAskUserQuestionResult(output: string): boolean {
  let parsed: unknown;
  try {
...
const PATH_KEYS = new Set(['path', 'file_path', 'notebook_path']);
...
function formatKeyArgument(
  ...
  const displayValue =
    (toolName === 'Read' || toolName === 'NotebookEdit') && PATH_KEYS.has(key)
...
export class ToolCallComponent extends Container {
  ...
  private readonly markdownTheme = createPythinkerMarkdownTheme();
  private readonly subagentThinkingPreview = new MarkdownPreviewComponent('', {
   ...
   * `args.plan` is empty. `PythinkerTUI` calls `setPlanInfo` with
  ...
  // when PythinkerTUI routes a `subagent.event` with this tool call
  ...
  private bashStatusPulseTimer: ReturnType<typeof setInterval> | undefined;
  private bashStatusPulseVisible = true;
  ...
  dispose(): void {
    ...
    this.stopBashStatusPulseTimer();
  }

  /**
   * Injects plan body/path asynchronously. Only ExitPlanMode cards use
   * this: plan-file mode leaves `args.plan` empty, so `PythinkerTUI` fetches
  ...
  // ── Subagent API (called by PythinkerTUI event routing) ───────────────
  ...
  private syncBashStatusPulseTimer(): void {
    if (!this.isRunningBash()) {
      this.stopBashStatusPulseTimer();
      return;
    }
    if (this.ui === undefined || this.bashStatusPulseTimer !== undefined) {
      return;
    }
    this.bashStatusPulseVisible = true;
    this.bashStatusPulseTimer = setInterval(() => {
      if (!this.isRunningBash()) {
        this.stopBashStatusPulseTimer();
        return;
      }
      this.bashStatusPulseVisible = !this.bashStatusPulseVisible;
      this.headerText.setText(this.buildHeader());
      this.ui?.requestRender();
    }, BASH_STATUS_PULSE_INTERVAL_MS);
    this.bashStatusPulseTimer.unref?.();
  }

  private stopBashStatusPulseTimer(): void {
    if (this.bashStatusPulseTimer !== undefined) {
      clearInterval(this.bashStatusPulseTimer);
      this.bashStatusPulseTimer = undefined;
    }
    this.bashStatusPulseVisible = true;
  }

  private isRunningBash(): boolean {
    return (
      this.toolCall.name === 'Bash' &&
      this.result === undefined &&
      this.toolCall.truncated !== true
    );
  }
  ...
  private buildHeader(): string {
    ...
    } else if (toolCall.name === 'Bash' && !this.bashStatusPulseVisible) {
      // Preserve the two-cell marker width while the calm running pulse is off.
      bullet = '  ';
    } else {
    ...
    if (toolCall.name === 'AskUserQuestion') {
      ...
      if (isFinished && !isError && !isBackgroundAsk) {
        if (result !== undefined && isDismissedAskUserQuestionResult(result.output)) {
          return currentTheme.boldFg('warning', `${ABORTED_MARK} Dismissed`);
        }
        return currentTheme.boldFg('success', `${SUCCESS_MARK}Answered`);
      }
    ...
    const toolLabel =
      decoded !== null
        ? `${currentTheme.boldFg('textStrong', decoded.toolName)}${currentTheme.dim(` · MCP/${decoded.serverName}`)}`
        : currentTheme.boldFg('textStrong', displayToolName(toolCall.name));
  ...
  private buildSubagentBlock(): void {
    ...
    for (const sub of this.finishedSubCalls) {
      ...
      const nameCol = currentTheme.fg('textStrong', sub.name);
    ...
    for (const [id, call] of this.ongoingSubCalls) {
      ...
      const nameCol = currentTheme.fg('textStrong', call.name);
      const argCol = keyArg ? currentTheme.dim(` (${keyArg})`) : '';
      void id;
  ...
  private buildSingleSubagentHeader(): string {
    ...
    const label = currentTheme.boldFg('textStrong', labelText);
    ...
    const isolationPlain = this.toolCall.args['isolation'] === 'worktree' ? ' · worktree' : '';
    const isolationText = isolationPlain.length > 0 ? currentTheme.dim(isolationPlain) : '';
    const stats = currentTheme.dim(this.formatSingleSubagentStatsText());
    return `${bullet}${label} ${status}${descriptionText}${isolationText}${stats}`;
  }
  ...
  private buildSingleSubagentBlock(): void {
    ...
    if (
      ...
      // Scroll Markdown thinking within a fixed two-row, width-aware window.
      this.subagentThinkingPreview.setText(this.subagentThinkingText.trimEnd());
      this.addChild(this.subagentThinkingPreview);
    }
  ...
  private formatSubToolActivity(verb: string, activity: SubToolActivity): string {
    ...
    const nameCol = currentTheme.fg('textStrong', activity.name);
    const argCol = keyArg ? currentTheme.dim(` (${keyArg})`) : '';
  ...
  private buildCallPreview(): void {
    ...
    if (name === 'Write') {
      ...
      for (const [i, line] of shown.entries()) {
        ...
        this.addChild(new TruncatedText(`  ${lineNum}${line.replaceAll('\t', '   ')}`));
      }
      ...
      for (const line of lines) {
        this.addChild(new TruncatedText(`  ${line.replaceAll('\t', '   ')}`));
      }
    }
  }
  ...
  private buildContent(): void {
    ...
    if (this.toolCall.name === 'DynamicWorkflow' && isDynamicWorkflowResult(result.output)) {
      this.buildDynamicWorkflowResultSummary(result);
      return;
    }
  ...
  private buildDynamicWorkflowResultSummary(result: ToolResultBlockData): void {
    const summary = dynamicWorkflowResultSummaryFromOutput(result.output);
    ...
    if (segments.length > 0) {
      this.addChild(new Text(`${dim('Dynamic Workflow: ')}${segments.join(dim(' · '))}`, 2, 0));
      return;
    }
    ...
    this.addChild(new Text(`${dim('Dynamic Workflow: ')}${currentTheme.fg(colorToken, label)}`, 2, 0));
  }
  ...
  private renderAskUserQuestionResult(output: string): boolean {
    ...
    for (const [question, answer] of Object.entries(answers as Record<string, unknown>)) {
      ...
      const answerText =
        typeof answer === 'string' ? answer : (serializedAnswer ?? String(answer));
      this.addChild(
        new MarkdownPreviewComponent(question, {
          firstPrefix: () => `  ${currentTheme.dim('┌ Q')} `,
          continuationPrefix: () => `  ${currentTheme.dim('│')}   `,
        }),
      );
      this.addChild(
        new MarkdownPreviewComponent(answerText, {
          firstPrefix: () => `  ${currentTheme.fg('success', '└ ✓')} `,
          continuationPrefix: '      ',
        }),
      );
    }

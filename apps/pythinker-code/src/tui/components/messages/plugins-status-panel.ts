...
export function buildPluginsInfoLines(input: PluginsInfoPanelInput): readonly string[] {
  ...
  const trustLine = (() => {
    ...
    if (label === 'official') {
      return `${muted('Trust:')}  ${success(OFFICIAL_BADGE)} ${muted('(Pythinker-built and -maintained)')}`;
    }
    if (label === 'curated') {
      return `${muted('Trust:')}  ${primary(CURATED_BADGE)} ${muted('(Pythinker-reviewed, upstream-maintained)')}`;
    }

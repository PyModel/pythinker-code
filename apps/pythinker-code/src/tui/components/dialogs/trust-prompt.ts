  ...
  render(width: number): string[] {
    ...
    const notice =
      this.opts.gatedMcpServers.length > 0
        ? `Pythinker Code loads project-level MCP servers (.mcp.json, .pythinker-code/mcp.json) only in trusted folders. They run as local processes on your machine. This folder defines: ${this.opts.gatedMcpServers.join(', ')}.`
        : 'Pythinker Code loads project-level MCP servers (.mcp.json, .pythinker-code/mcp.json) only in trusted folders. They run as local processes on your machine.';

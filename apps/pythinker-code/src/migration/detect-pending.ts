...
export async function detectPendingMigration(
  ...
  if (input.ignoreMarker !== true) {
    if (migrationAlreadyTargeted(join(sourceHome, '.migrated-to-pythinker-code'), targetHome)) {
      return null;
    }
    if (existsSync(join(targetHome, '.skip-migration-from-pythinker-cli'))) return null;
  }
  ...
  // only data is `credentials/*.json` has nothing to offer — pythinker-code's own
...
/**
 * True when the legacy `.migrated-to-pythinker-code` marker records a migration
 * into *this* target home. A marker written for a different `PYTHINKER_CODE_HOME`

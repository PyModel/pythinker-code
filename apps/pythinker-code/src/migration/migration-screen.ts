...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  private renderResult(width: number): string[] {
    ...
    if (r !== undefined) {
      ...
      lines.push('', chalk.hex(colors.textMuted)(' Old data kept at ~/.pythinker/ — pythinker-cli still works.'));
    }
    lines.push('', chalk.hex(colors.textMuted)(' ⏎ continue to pythinker-code'), chalk.hex(colors.primary)('─'.repeat(width)));
    return lines.map((l) => truncateToWidth(l, width));
  }
  ...
  private renderAsk(width: number): string[] {
    ...
    if (this.phase === 'ask1') {
      lines.push(chalk.hex(colors.text)(' Found an existing pythinker-cli installation:'), chalk.hex(colors.textMuted)(`   ${summarizePlan(this.opts.plan)}`), '');
    }

 ...
 * conventions in `apps/pythinker-code/src/tui/components/dialogs/choice-picker.ts`.
 *
 ...
 */
import { Container, matchesKey, Key, truncateToWidth, type Focusable } from '@pymodel/pi-tui';
...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `pythinker migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `pythinker migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "pythinker migrate".'));
      lines.push('');
      lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to pythinker-code'));
    ...
    if (r !== undefined) {
      ...
      if (r.notices.detectedPlugins.length > 0) {
        lines.push(
          chalk.hex(colors.warning)(
            `  ⚠ ${r.notices.detectedPlugins.length} pythinker-cli plugins — not yet supported for migration`,
          ),
        );
      }
      ...
      // safely be held by two installs at once). pythinker-code's normal auth flow
      ...
      // landed in a *.migrated-from-pythinker-cli.* sibling or that sessions failed.
      ...
      if (sum.config.wroteSiblingDueToConflict) {
        ...
        // migrated content went to `config.migrated-from-pythinker-cli.toml` and
      ...
      lines.push('');
      lines.push(
        chalk.hex(colors.textMuted)(` Old data kept at ${this.opts.sourceHome} — pythinker-cli still works.`),
      );
    }
    lines.push('');
    lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to pythinker-code'));
  ...
  private renderProgress(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(colors.primary).bold(' Migrating from pythinker-cli'),
      '',
    ];
  ...
  private renderAsk(width: number): string[] {
    ...
    const lines: string[] = [
      chalk.hex(colors.primary)('─'.repeat(width)),
      chalk.hex(colors.primary).bold(' Migrate from pythinker-cli'),
      '',
    ];
    if (this.phase === 'ask1') {
      lines.push(chalk.hex(colors.text)(' Found an existing pythinker-cli installation:'));
...
function stepFor(phase: Phase, plan: MigrationPlan): StepDef {
  if (phase === 'ask1') {
    return {
      title: 'Migrate this data to pythinker-code?',
      options: [

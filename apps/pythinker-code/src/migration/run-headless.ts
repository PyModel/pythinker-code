...
export async function runHeadlessMigrate(
  ...
  const resolved: HeadlessMigrateDeps = {
    ...
    targetHome: deps?.targetHome ?? resolvePythinkerHome(),
    write: deps?.write ?? defaultWrite,
  };
  const log = (msg: string): void => {
    resolved.write(`[pythinker-migrate ${timestamp()}] ${msg}`);
  };

  const source = resolveLegacySourceHome(resolved.env, resolved.userHome, resolved.cwd);
  log(`source: ${source.sourceHome} (${source.origin === 'share-dir' ? 'PYTHINKER_SHARE_DIR' : 'default ~/.pythinker'})`);
  if (source.skillsSourceHome !== undefined) {
    log(`skills source: ${source.skillsSourceHome} (pythinker-cli skills are not relocated by PYTHINKER_SHARE_DIR)`);
  }
  ...
  const plansSourceHome = join(resolved.userHome, '.pythinker', 'plans');
...
function logPlan(plan: MigrationPlan, log: (msg: string) => void): void {
  ...
  if (plan.detectedPlugins.length > 0) {
    log(`pythinker-cli plugins (not migrated): ${plan.detectedPlugins.join(', ')}`);
  }
}
...
function logReport(
  ...
  if (c.wroteSiblingDueToConflict) {
    log(`config: live config.toml unparseable — migrated copy at config.migrated-from-pythinker-cli.toml (${c.siblingContents.providers.length} providers, ${c.siblingContents.models.length} models, ${c.siblingContents.hooks} hooks)`);
  }
  if (c.wroteTuiSibling) {
    log('config: tui.toml conflicted — migrated copy at tui.migrated-from-pythinker-cli.toml');
  }
  const m = sum.mcp;
  log(
    `mcp: merged=[${m.mergedServers.join(', ')}]` +
      ...
      (m.wroteSiblingDueToConflict ? ' wrote mcp.migrated-from-pythinker-cli.json' : '') +
  ...
  if (scope.sessions) {
    log(
      `sessions: scanned=${s.bucketsScanned} attempted=${s.sessionsAttempted} migrated=${s.sessionsMigrated}` +
        ...
        (s.bucketsSkippedNonlocalPyaos > 0 ? ` buckets-skipped-nonlocal-pyaos=${s.bucketsSkippedNonlocalPyaos}` : '') +

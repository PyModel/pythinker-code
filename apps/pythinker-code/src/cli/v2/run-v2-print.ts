/**
 * Native v2 `pythinker -p` (print mode) runner.
 *
 ...
 *
 * Selected by `runPrompt` when `PYTHINKER_CODE_EXPERIMENTAL_FLAG` is set.
 */
...
export async function runV2Print(
  ...
  const identity = createPythinkerCodeHostIdentity(version);
  const hostHeaders = createPythinkerDefaultHeaders({ homeDir, ...identity });
...
/**
 * Apply the print-mode (`pythinker -p`) background-resource policy after the main

/**
 * Experimental v2 config.toml validation for `pythinker doctor`.
 *
 * Loaded lazily (dynamic import) by the doctor command only when the
 * agent-core-v2 master switch (`PYTHINKER_CODE_EXPERIMENTAL_FLAG`) is on, so the
...
import { collectKeyDeprecations } from '@pymodel/agent-core-v2/app/config/deprecations';
import {
  camelToSnake,
  describeTomlSyntaxError,
  isPlainObject,
  transformTomlData,
} from '@pymodel/agent-core-v2/app/config/toml';
 ...
 * `PythinkerConfigValidationIssue` list), so the doctor formatter renders v2

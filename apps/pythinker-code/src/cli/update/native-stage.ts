...
import { PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME } from '#/constant/app';
...
const StagedNativeUpdateSchema = z
  .object({
     ...
     * `pythinker upgrade` (vs the passive background downloader): manual stages
...
export function stagedExeFileName(version: string, platform: NodeJS.Platform): string {
  return platform === 'win32' ? `pythinker-${version}.exe` : `pythinker-${version}`;
}
 ...
 * infix (`pythinker-<version>.<pid>.<epoch-ms>.<n>[.exe]`). Once published, a
...
function uniqueStagedExeFileName(version: string, platform: NodeJS.Platform): string {
  ...
  stageTempCounter += 1;
  return platform === 'win32' ? `pythinker-${version}${infix}.exe` : `pythinker-${version}${infix}`;
}
 ...
 * persisted. Used when an explicit `pythinker upgrade` adopts a payload the
...
async function cleanupStagingOrphans(stagingDir: string): Promise<void> {
  ...
  const keep = new Set<string>([PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME]);
  for (const entry of entries) {
    // The current record and every swap claim pin the exe they reference.
    if (
      entry !== PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME &&
      !entry.startsWith(`${PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME}.swap-`)
    ) {
      continue;
    }
    keep.add(entry);
...
export interface StageNativeUpdateOptions {
  ...
  /** True when the stage answers an explicit user-initiated `pythinker upgrade`. */
 ...
 * disk (repeat `pythinker upgrade`, or foreground/background overlap). **Throws**

 ...
 *
 * `pythinker upgrade` must NOT go through this gate — it selects directly from the
 * raw latest version.
 */
...
export function resolveUpdateDeviceId(): string {
  return readPythinkerDeviceId(resolvePythinkerHome()) ?? randomUUID();
}
 ...
 * truthy values as `PYTHINKER_CODE_NO_AUTO_UPDATE`) — the update preflight runs
 * before the harness exists, so the core flag registry is not consulted.
 * `PYTHINKER_CODE_NO_AUTO_UPDATE` still wins: disabling updates beats opting in.
 */
export function isRolloutBypassedByExperimentalEnv(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  const value = (env['PYTHINKER_CODE_EXPERIMENTAL_FLAG'] ?? '').trim().toLowerCase();

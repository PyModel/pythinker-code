 ...
 * prefix (`pythinker.config.bak`, …) belongs to the user. A `.bak` still mapped
...
async function cleanupStaleSwapClaims(exePath: string): Promise<boolean> {
  ...
  for (const entry of entries) {
    if (!entry.startsWith(`${PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME}.swap-`)) continue;
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = deps.spawnImpl(deps.exePath, deps.argv.slice(2), {
        stdio: 'inherit',
        env: { ...deps.env, [PYTHINKER_CODE_UPDATE_REEXEC_ENV]: '1' },
...
export async function maybeRelaunchWithStagedNativeUpdate(
  ...
  if (isTruthy(deps.env[PYTHINKER_CODE_UPDATE_REEXEC_ENV])) {
    // Read-once guard: drop it so this session's children (and any nested
    // pythinker launches from them) do not inherit the swap skip.
    delete deps.env[PYTHINKER_CODE_UPDATE_REEXEC_ENV];
    return false;
  }
  if (swapInProgress) {

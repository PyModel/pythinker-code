...
export async function fetchLatestVersionFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, pythinkerCodeCdnLatestUrl(), timeoutMs);
  if (!response.ok) {
...
async function fetchUpdateManifestFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, pythinkerCodeCdnLatestJsonUrl(), timeoutMs);
  if (!response.ok) {

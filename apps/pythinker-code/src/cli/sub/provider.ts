/**
 * `pythinker provider` sub-command — non-interactive provider management.
 *
 * Mirrors the TUI `/provider` flow (apps/pythinker-code/src/tui/commands/provider.ts)
 ...
 * (apps/pythinker-code/src/tui/utils/refresh-providers.ts) groups by URL, retries
...
export interface ProviderDeps {
  readonly getHarness: () => PythinkerHarness;
...
export async function handleProviderAdd(
  ...
  // `core-impl.ts removePythinkerProvider`), so calling it inside the apply loop
...
export async function handleCatalogAdd(
  ...
  if (opts.defaultModel !== undefined && !models.some((m) => m.id === opts.defaultModel)) {
    deps.stderr.write(
      `Model "${opts.defaultModel}" is not in provider "${providerId}". Run "pythinker provider catalog list ${providerId}" to see available ids.\n`,
    );
    deps.exit(1);
  }
...
export function registerProviderCommand(parent: Command, deps?: Partial<ProviderDeps>): void {
  ...
  provider
    ...
    .option('--api-key <key>', 'Registry API key. Falls back to PYTHINKER_REGISTRY_API_KEY.')
...
function resolveDeps(overrides: Partial<ProviderDeps> = {}): ResolvedProviderDeps {
  let harness: PythinkerHarness | undefined;
...
function resolveApiKey(flag: string | undefined, env: NodeJS.ProcessEnv): string | undefined {
  ...
  const fromEnv = env['PYTHINKER_REGISTRY_API_KEY'];

 ...
 * clients can drive a pythinker-code session on the new engine. This is the v2
 * counterpart to `pythinker acp` (which runs the legacy `@pymodel/acp-adapter`
 * over the SDK harness).
 *
 * Wire-up mirrors `pythinker acp` for the parts that are host-independent:
 ...
 *  - `PYTHINKER_CODE_HOME` (if set) is forwarded into `authMethods[0].env` so the
 ...
 * lazy dynamic import so the default CLI / `pythinker acp` module graph stays free
 * of the experimental v2 engine — mirroring the `pythinker server run` v2 routing
...
export function registerAcpV2Command(parent: Command): void {
  parent
    .command('acp-v2')
    .description(
      'Run pythinker-code as an Agent Client Protocol (ACP) server over stdio (experimental agent-core-v2 engine).',
    )
    .option(
      '--login',

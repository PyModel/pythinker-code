/**
 * `pythinker web` — run the local server in the foreground and open the web UI.
 *
...
/**
 * `pythinker web` — runs the local server in-process, attached to the current
 ...
 * and the web UI is expected to come from the pythinker-web Vite dev server.
...
export function serverWebAssetsDir(
  ...
  if (env['PYTHINKER_CODE_DEV_SERVER'] === '1' && !existsSync(join(dir, 'index.html'))) {
    return undefined;
  }
  return dir;
}

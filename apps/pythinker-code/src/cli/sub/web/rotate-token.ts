/**
 * `pythinker web rotate-token` — generate a new persistent server token.
 *
 * Rewrites `<PYTHINKER_CODE_HOME>/server.token` (0600, atomic). The previous token
...
import { getLiveServerInstance, rotateServerToken } from '@pymodel/agent-gateway';

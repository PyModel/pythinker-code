...
export async function handleVis(deps: VisDeps, opts: VisOptions): Promise<void> {
  ...
  try {
    ...
    deps.stderr.write(`Failed to start pythinker vis: ${msg}\n`);
    return deps.exit(1);
  }
  ...
  deps.stdout.write(`pythinker vis is running at ${server.url}\n`);

/**
 * Hidden `pythinker __update_download <version>` sub-command: the self-spawned
 ...
 * `pythinker upgrade` in this loop.
 *
...
export async function runUpdateDownloadCommand(
  ...
  if (lock === null) {
    ...
    if (holderVersion === version) {
      ...
      out.write(
        `A download of Pythinker Code ${version} is already in progress; waiting for it to finish…\n`,
      );
      const wait = await waitForStagedUpdate(version, process.execPath, manual);
      if (wait.status === 'staged') {
        out.write(`Pythinker Code ${version} is downloaded; it applies on the next start.\n`);
        return 0;
      }
  ...
  const label = `Downloading Pythinker Code ${version} (${process.platform}-${process.arch})…`;
  ...
  try {
    ...
    if (result.status === 'already-staged') {
      out.write(`Pythinker Code ${version} is already downloaded; it applies on the next start.\n`);
    }
    return 0;
  } catch (error) {
    if (out.isTTY) out.write('\n');

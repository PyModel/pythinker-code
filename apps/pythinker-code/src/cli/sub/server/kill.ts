/**
 * `pythinker server kill` — terminate the running server.
 *
...
export function registerKillCommand(server: Command): void {
  server
    .command('kill')
    .description('Stop the running Pythinker server (graceful API + forced PID kill).')
    .action(async () => {
      try {
...
export async function handleKillCommand(deps: KillCommandDeps): Promise<void> {
  ...
  if (!lock) {
    deps.stdout.write('No running Pythinker server.\n');
    return;
  }
  ...
  if (await waitForExit(pid, TERM_GRACE_MS, deps)) {
    deps.stdout.write(`Pythinker server (pid ${String(pid)}) stopped.\n`);
    return;
  }
  ...
  if (await waitForExit(pid, KILL_GRACE_MS, deps)) {
    deps.stdout.write(`Pythinker server (pid ${String(pid)}) killed.\n`);
    return;
  }

  throw new Error(
    `Failed to stop Pythinker server (pid ${String(pid)}); insufficient permissions?`,
  );
}

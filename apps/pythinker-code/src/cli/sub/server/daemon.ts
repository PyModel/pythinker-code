/**
 * `pythinker web` daemon orchestration — parent (spawner) side.
 *
 ...
 *
 *   1. Read `~/.pythinker-code/server/lock`. If it names a *live* daemon, reuse it
 *      (wait for it to be healthy) — never spawn a second one.
 *   2. Otherwise pick a free port (preferred port when available, else an
 *      OS-assigned one) and spawn `pythinker server run --daemon` as a detached
 ...
 * name (e.g. `pythinker`) or the first user argument — never a script path — so we
...
export function resolveDaemonProgram(
  ...
  // `cwd` would produce a bogus path (e.g. `<cwd>/pythinker`) and crash the spawn
...
export async function ensureDaemon(options: EnsureDaemonOptions = {}): Promise<EnsureDaemonResult> {
  ...
  throw new Error(
    `Pythinker server daemon failed to start within ${String(SPAWN_TIMEOUT_MS)}ms. ` +

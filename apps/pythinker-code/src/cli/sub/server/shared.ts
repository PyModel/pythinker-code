/**
 * Shared helpers for `pythinker server …` subcommands.
 *
...
export interface ServerCliOptions {
  ...
  logLevel?: string;
  debugEndpoints?: boolean;
  /** Internal flag set by the daemon spawner (`pythinker web`). */
  daemon?: boolean;
...
export async function ensureServerWebReady(origin: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Server at ${origin} does not serve the Pythinker web UI${reason}. Stop the existing server and rerun \`pythinker server run\`.`,
      { cause: error },
    );
  } finally {
    clearTimeout(timeout);

...
export function addLifecycleCommands(parent: Command, deps: LifecycleCommandDeps = DEFAULT_DEPS): void {
  parent
    .command('install')
    .description('Install the Pythinker server as an OS-managed service (launchd/systemd/schtasks).')
  ...
  parent
    .command('uninstall')
    .description('Uninstall the Pythinker server service.')
    .option('--json', 'Output JSON', false)
  ...
  parent
    .command('start')
    .description('Start the Pythinker server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('stop')
    .description('Stop the Pythinker server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('restart')
    .description('Restart the Pythinker server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('status')
    .description('Show Pythinker server service status and connectivity.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
...
async function runLifecycle(
  ...
  try {
    ...
    await exitLifecycle(deps, 1);
  }
}

/**
 * Exit with `code` after flushing any output written to the real process
 * streams, so error/result output is not lost when the lifecycle command
 * fails.
 */
async function exitLifecycle(deps: LifecycleCommandDeps, code: number): Promise<void> {
  const drains: Promise<void>[] = [];
  if (deps.stdout === process.stdout) drains.push(drainWritable(process.stdout));
  if (deps.stderr === process.stderr) drains.push(drainWritable(process.stderr));
  try {

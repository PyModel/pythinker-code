/**
 * `pythinker server ps` — list clients currently connected to the running server.
 *
 * Talks to the running server over HTTP (`GET /api/v1/connections`) using the
 * single-instance lock (`~/.pythinker-code/server/lock`) to discover its origin —
 * the same way `pythinker web` locates the daemon.
 */
...
export function registerPsCommand(server: Command): void {
  server
    .command('ps')
    .description('List clients currently connected to the running Pythinker server.')
...
async function handlePsCommand(opts: { json?: boolean }): Promise<void> {
  ...
  if (!lock) {
    throw new Error(
      'No running Pythinker server. Start one with `pythinker server run` or `pythinker web`.',
    );
  }

  const origin = serverOrigin(lockConnectHost(lock), lock.port);
  if (!(await isServerHealthy(origin, HEALTH_TIMEOUT_MS))) {
    throw new Error(`Pythinker server at ${origin} is not responding.`);
  }

/**
 * `pythinker server` parent command. Mounts:
 ...
 *
 * The top-level `pythinker web` alias is registered separately via
...
export function registerServerCommand(program: Command): void {
  const server = program
    .command('server')
    .description('Run the local Pythinker server (REST + WebSocket + web UI).');

  buildRunCommand(
    server.command('run').description('Start the Pythinker server (background daemon; use --foreground to attach).'),
    { defaultOpen: false },
  );
  ...
  // daemon (`pythinker web`) over service-ization. The implementation still lives in

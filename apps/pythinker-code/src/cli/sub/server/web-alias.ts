/**
 * `pythinker web` — open the Pythinker web UI.
 *
 * Shares the exact same code path as `pythinker server run`: it is registered via
...
export function registerWebAliasCommand(program: Command): void {
  buildRunCommand(
    program
      .command('web')
      .description('Open the Pythinker web UI (starts a background daemon if needed).'),
    { defaultOpen: true },
  );
}

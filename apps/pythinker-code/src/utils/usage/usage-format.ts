...
import { type ManagedQuota, type ManagedQuotaEntry } from '@pymodel/pythinker-code-oauth';
...
/**
 * The pythinker/code split of the new plan's monthly quota: `codeRatio` is the
 * code-typed share of the monthly total as served, `pythinkerRatio` the
 * remainder, clamped against float noise.
 */
export interface MonthlyUsageBreakdown {
  readonly pythinkerRatio: number;
 ...
 * monthly (with its pythinker/code breakdown) — in payload order. Entries the
...
function monthlyBreakdown(quota: ManagedQuota): MonthlyUsageBreakdown | undefined {
  ...
  const pythinkerRatio = safeUsageRatio(Math.round((total.usedRatio - codeRatio) * 1e6) / 1e6);
  return { pythinkerRatio, codeRatio };
}

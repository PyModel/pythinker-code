...
//
// Debug-only startup phase tracer, enabled with PYTHINKER_STARTUP_TRACE=1.
// Each call appends one `<elapsed-ms> <label>` line to the trace file
// (default /tmp/pythinker-startup-trace.log, override with
// PYTHINKER_STARTUP_TRACE_LOG=<path>), so a slow or BLOCKING startup phase
...
const enabled = process.env['PYTHINKER_STARTUP_TRACE'] !== undefined && process.env['PYTHINKER_STARTUP_TRACE'] !== '';
const logPath = process.env['PYTHINKER_STARTUP_TRACE_LOG'] ?? '/tmp/pythinker-startup-trace.log';

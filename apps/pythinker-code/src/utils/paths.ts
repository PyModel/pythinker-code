...
import {
  PYTHINKER_CODE_BANNER_DIR_NAME,
  PYTHINKER_CODE_BANNER_STATE_FILE_NAME,
  PYTHINKER_CODE_BIN_DIR_NAME,
  PYTHINKER_CODE_CACHE_DIR_NAME,
  PYTHINKER_CODE_DATA_DIR_NAME,
  ...
  PYTHINKER_CODE_NATIVE_STAGING_DIR_NAME,
  PYTHINKER_CODE_PLUGIN_UPDATE_NOTICE_STATE_FILE_NAME,
  PYTHINKER_CODE_RECOMMENDED_EFFORT_STATE_FILE_NAME,
  PYTHINKER_CODE_SURVEY_STATE_FILE_NAME,
  ...
  PYTHINKER_CODE_UPDATE_STATE_FILE_NAME,
} from '#/constant/app';

/**
 * Return the root data directory for Pythinker Code.
 *
 * Priority: `PYTHINKER_CODE_HOME` env var > `~/.pythinker-code`.
 */
export function getDataDir(): string {
  const envDir = process.env[PYTHINKER_CODE_HOME_ENV];
  if (envDir) {
    return envDir;
  }
  return join(homedir(), PYTHINKER_CODE_DATA_DIR_NAME);
}
...
export function getLogDir(): string {
  return join(getDataDir(), PYTHINKER_CODE_LOG_DIR_NAME);
}
...
export function getCacheDir(): string {
  return join(getDataDir(), PYTHINKER_CODE_CACHE_DIR_NAME);
}
...
export function getBinDir(): string {
  return join(getDataDir(), PYTHINKER_CODE_BIN_DIR_NAME);
}
...
export function getUpdateStateFile(): string {
  return join(getDataDir(), PYTHINKER_CODE_UPDATE_DIR_NAME, PYTHINKER_CODE_UPDATE_STATE_FILE_NAME);
}
...
export function getUpdateInstallStateFile(): string {
  return join(getDataDir(), PYTHINKER_CODE_UPDATE_DIR_NAME, PYTHINKER_CODE_UPDATE_INSTALL_STATE_FILE_NAME);
}
 ...
 *
 * Anchored on the running executable (not `~/.pythinker-code/bin`) because the
 * Windows installer honors `PYTHINKER_INSTALL_DIR`, and the swap's atomic renames
 * require the staged binary to sit on the same volume as the exe.
 */
export function getNativeStagingDir(exePath: string): string {
  return join(dirname(exePath), PYTHINKER_CODE_NATIVE_STAGING_DIR_NAME);
}
...
export function getNativeStagedStateFile(exePath: string): string {
  return join(getNativeStagingDir(exePath), PYTHINKER_CODE_NATIVE_STAGED_STATE_FILE_NAME);
}
...
export function getBannerStateFile(): string {
  return join(getCacheDir(), PYTHINKER_CODE_BANNER_DIR_NAME, PYTHINKER_CODE_BANNER_STATE_FILE_NAME);
}

export function getSurveyStateFile(): string {
  return join(getDataDir(), PYTHINKER_CODE_SURVEY_STATE_FILE_NAME);
}

export function getRecommendedEffortStateFile(): string {
  return join(getDataDir(), PYTHINKER_CODE_RECOMMENDED_EFFORT_STATE_FILE_NAME);
}
...
export function getInputHistoryFile(workDir: string): string {
  ...
  return join(getDataDir(), PYTHINKER_CODE_INPUT_HISTORY_DIR_NAME, `${hash}.jsonl`);
}

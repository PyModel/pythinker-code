...
import type { PythinkerConfig, PythinkerConfigPatch, ModelAlias } from '@pymodel/pythinker-code-sdk';
...
export interface ApplyRecommendedEffortDeps {
  ...
  setConfig: (patch: PythinkerConfigPatch) => Promise<unknown>;
  track: (event: string, properties?: TelemetryProperties) => void;
  stateFile?: string;
  now?: () => Date;
}

function eligibleModelEntry(config: PythinkerConfig): ModelAlias | undefined {
  ...
  return isManagedPythinkerCodeBaseUrl(baseUrl) ? entry : undefined;
}

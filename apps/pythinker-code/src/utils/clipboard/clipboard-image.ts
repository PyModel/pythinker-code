...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
  const tmpFile = join(tmpdir(), `pythinker-wsl-clip-${randomUUID()}.png`);
  try {
    ...
    const psScript = [
      ...
      '$path = $env:PYTHINKER_WSL_CLIPBOARD_IMAGE_PATH',
    ...
    const result = runCommand('powershell.exe', ['-NoProfile', '-Command', psScript], {
      ...
      env: { ...process.env, PYTHINKER_WSL_CLIPBOARD_IMAGE_PATH: winPath },
    });

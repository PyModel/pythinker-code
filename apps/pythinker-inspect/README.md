...
1. Start a agent-gateway with the debug surface mounted (repo dev scripts do this
   ...
   bearer auth.
2. `pnpm --filter @pymodel/pythinker-inspect dev` — the Vite dev server proxies
   `/api` to the server (`PYTHINKER_SERVER_URL`, default `http://127.0.0.1:58627`)
   and auto-discovers running instances
   (`~/.pythinker-code/server/instances`); switch servers from the header dropdown.

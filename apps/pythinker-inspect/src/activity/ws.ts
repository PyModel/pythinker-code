 ...
 * `pythinker-code.bearer.<token>` subprotocol (the only credential channel a
...
export class GlobalEventsWs {
  ...
  private connect(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      this.send({
        type: 'client_hello',
        id: `pythinker-inspect-global-${Date.now().toString(36)}`,
        payload: { client_id: 'pythinker-inspect', subscriptions: [] },
      });

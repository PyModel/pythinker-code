...
describe('ProxyChannel.call', () => {
  it('POSTs the command to the service base URL; no body and no header without args/token', async () => {
    ...
    const channel = new ProxyChannel({
      baseUrl: 'http://127.0.0.1:1/api/v1/debug/session/s%201/agent/main/agentLoopService',
      fetch: fetchImpl,
    });
    ...
    expect(calls[0]!.url).toBe(
      'http://127.0.0.1:1/api/v1/debug/session/s%201/agent/main/agentLoopService/getModel',
    );

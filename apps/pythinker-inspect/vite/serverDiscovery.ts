 ...
 * `~/.pythinker-code/server.token` by hand — no new exposure beyond the local dev
 * session.
 */
...
/** home resolution per request: `PYTHINKER_CODE_HOME` env, else `~/.pythinker-code`. */
export function resolvePythinkerHomeDir(env: NodeJS.ProcessEnv = process.env): string {
  const fromEnv = env['PYTHINKER_CODE_HOME'];
...
export interface DiscoverOptions {
  /** The dev-proxy target (`PYTHINKER_SERVER_URL`); merged as a `proxy` entry. */
...
export async function discoverLocalServers(
  ...
  const home = options.homeDir ?? resolvePythinkerHomeDir();
...
export function serverDiscoveryPlugin(options: { proxyTarget: string }): Plugin {
  ...
  return {
    name: 'pythinker-inspect-server-discovery',

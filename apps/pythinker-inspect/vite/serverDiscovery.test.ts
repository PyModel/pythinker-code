...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'pythinker-inspect-discovery-'));
});
...
describe('resolvePythinkerHomeDir', () => {
  it('honors PYTHINKER_CODE_HOME, else falls back to ~/.pythinker-code', () => {
    expect(resolvePythinkerHomeDir({ PYTHINKER_CODE_HOME: '/tmp/kh' })).toBe('/tmp/kh');
    expect(resolvePythinkerHomeDir({})).toBe(join(process.env['HOME'] ?? '', '.pythinker-code'));
  });
});

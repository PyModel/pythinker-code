...
// `pnpm --filter @pymodel/vis-web typecheck` here until a renderer is
...
export const WIRE_RENDERERS: RendererMap = {
  ...
  'dynamic_workflow_mode.exit': {
    tone: 'subagent',
    label: 'dynamic_workflow✓',
    headline: () => ({ main: <Dim>dynamic_workflow mode exited</Dim> }),
  },
  ...
  'subagent.spawned': {
    ...
    headline: (r) => ({
      main: (
        <span className="flex items-center gap-2 min-w-0">
          <Pill tone="subagent" variant="soft">
            {r.dynamicWorkflowIndex === undefined ? 'child' : `member ${r.dynamicWorkflowIndex}`}
          </Pill>

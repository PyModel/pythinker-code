...
function validBundle(): Record<string, string> {
  return {
    'manifest.json': JSON.stringify({
      ...
      pythinkerCodeVersion: '0.20.2',
      workspaceDir: '/home/u/proj',
      title: 'imported demo',
      desktopLogPath: 'logs/pythinker-desktop.log',
    ...
    'logs/pythinker-code.log': '2026-06-01T00:00:00.000Z INFO  hello  k=v\n',
  };
}
...
describe('import-store', () => {
  ...
  it('imports a valid bundle and lists it with manifest metadata', async () => {
    ...
    expect(meta.manifest?.desktopLogPath).toBe('logs/pythinker-desktop.log');
    expect(meta.manifest?.webLogPath).toBe('logs/pythinker-web.jsonl');
    ...
    expect((await stat(join(dir, 'logs', 'pythinker-code.log'))).isFile()).toBe(true);

...
describe('session-store', () => {
  ...
  it('prefers v2 labels for parentAgentId / dynamicWorkflowItem, top-level as fallback', async () => {
    const { home, sessionDir, cleanup: c } = await buildSessionFixture('sample-main');
    cleanup = c;
    const { readFile, writeFile } = await import('node:fs/promises');
    const { join } = await import('node:path');
    ...
    // and puts the real parent / dynamic-workflow label under `labels`.
    state.agents['agent-1'] = {
      type: 'sub',
      parentAgentId: 'main',
      labels: {
        parentAgentId: 'agent-0',
        dynamicWorkflowItem: 'batch item',
        profileName: 'explore',
      },
    };
    await writeFile(statePath, JSON.stringify(state));
    ...
    expect(nested.dynamicWorkflowItem).toBe('batch item');
  ...
  it('normalizes untrusted agent metadata before exposing it', async () => {
    ...
    state.agents['agent-0'].dynamicWorkflowItem = 42;

...
import { estimateTokensForMessages } from '@pymodel/agent-core-v2/llm-adapter/contract/tokens';
import { buildCompactionContinuationText } from '@pymodel/agent-core-v2/agent/contextMemory/compactionHandoff';
...
describe('context-projector', () => {
  ...
  it('tracks dynamic_workflow mode enter/exit', () => {
    ...
    expect(exit.dynamic_workflow.active).toBe(false);
  });
  ...
  it('reads the config snapshot from v2 profile.bind', () => {
    const entries = [
      { lineNo: 1, data: { type: 'profile.bind' as const, agentId: 'main',
          modelAlias: 'k2', profileName: 'agent', thinkingEffort: 'high', systemPrompt: 'You are Pythinker.',
          environmentDisclosure: { cwd: '/repo' }, disallowedTools: [] }, raw: {} },
    ];
    const proj = projectContext(entries as any);
    expect(proj.config).toEqual({
      cwd: '/repo', modelAlias: 'k2', profileName: 'agent',
      thinkingEffort: 'high', systemPrompt: 'You are Pythinker.',
    });
  });

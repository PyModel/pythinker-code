...
describe('context route', () => {
  ...
  it('echoes the new projection fields (contextTokens, goal, dynamic_workflow)', async () => {
    ...
    expect(body).toHaveProperty('dynamic_workflow');

    // The sample fixture's only step.end carries usage 10+5 → contextTokens=15,
    // and has no goal / dynamic_workflow records.
    expect(body['contextTokens']).toBe(15);
    expect(body['goal']).toBeNull();
    expect(body['dynamic_workflow']).toEqual({ active: false });
  });

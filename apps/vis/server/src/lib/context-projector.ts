...
} from '@pymodel/agent-core-v2/agent/contextMemory/compactionHandoff';
import {
  computeUndoCut,
  isFullyUndoable,
  readContextCompactionShapeInput,
} from '@pymodel/agent-core-v2/agent/contextMemory/contextOps';
import { createLoopEventFold } from '@pymodel/agent-core-v2/agent/contextMemory/loopEventFold';
...
export interface ContextProjection {
  ...
  dynamic_workflow: { active: boolean; trigger?: string };
}
 ...
 *
 *  Everything else (append_message, loop events, goal/dynamic_workflow/permission/
...
export function projectContext(
  ...
  let dynamic_workflow: { active: boolean; trigger?: string } = { active: false };
  let microCutoff = 0;
  ...
  for (const entry of entries) {
    ...
    switch (rec.type) {
      ...
      case 'goal.clear':
        goal = null;
        break;
      case 'dynamic_workflow_mode.enter':
        dynamic_workflow = { active: true, trigger: rec.trigger };
        break;
      case 'dynamic_workflow_mode.exit':
        dynamic_workflow = { active: false };
        break;

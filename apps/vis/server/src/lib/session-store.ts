...
interface StateJson {
  ...
  //
  // v2 writes the REAL parent / dynamic-workflow-item label under `labels` (its
 ...
 *  `imported_from_pythinker_cli` hide-filter is intentionally NOT applied — the
...
async function tryReadSummary(
  ...
  if (!imported && state.custom?.['imported_from_pythinker_cli'] === true) return null;

...
export { WIRE_PROTOCOL_VERSION } from '@pymodel/agent-core-v2/wire/migration/migration';
export type {
...
import type {
  ...
  ToolsSetActiveTools,
} from '@pymodel/agent-core-v2/agent/profile/profileOps';
...
import type { DynamicWorkflowModeEnter, DynamicWorkflowModeExit } from '@pymodel/agent-core-v2/features/dynamic_workflow/dynamicWorkflowOps';
...
export interface AgentInfo {
  ...
  wireProtocolVersion: string | null;
  /** Per-item dynamic_workflow work label persisted by the engine for dynamic-workflow-spawned
   *  sub-agents (`AgentMeta.dynamicWorkflowItem`, or `AgentMeta.labels.dynamicWorkflowItem` on
   *  v2-written sessions). `null` when the agent is not a dynamic_workflow item or when

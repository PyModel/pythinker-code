...
const SESSION_LOG_REL = ['logs', 'pythinker-code.log'] as const;
const GLOBAL_LOG_REL = ['logs', 'global', 'pythinker-code.log'] as const;
const HOME_GLOBAL_LOG_REL = ['logs', 'pythinker-code.log'] as const;

export function logsRoute(home: string = PYTHINKER_CODE_HOME): Hono {
  const r = new Hono();
  r.get('/:id/logs', async (c) => {
    ...
    // Either log may have rotated (pythinker-code.log.1, .2, …); discover the active

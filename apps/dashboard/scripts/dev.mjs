#!/usr/bin/env node
// Wraps `concurrently` so the dashboard-server (API) and vite (web) each pick a
...
if (apiPort !== requestedApi) {
  process.stdout.write(`[dashboard] api port ${requestedApi} busy, using ${apiPort} instead\n`);
}
...
if (webPort !== requestedWeb) {
  process.stdout.write(`[dashboard] web port ${requestedWeb} busy, using ${webPort} instead\n`);
}

process.stdout.write(`[dashboard] web → http://localhost:${webPort}  (api on ${apiPort})\n`);

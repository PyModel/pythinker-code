...
export interface ContextProjection {
  ...
  dynamicWorkflow: { active: boolean; trigger?: string };
}
 ...
 *
 *  Everything else (append_message, loop events, goal/dynamic workflow/permission/
...
export function projectContext(
  ...
  let dynamicWorkflow: { active: boolean; trigger?: string } = { active: false };
  let microCutoff = 0;
  ...
  for (const entry of entries) {
    ...
    switch (rec.type) {
      ...
      case 'dynamic_workflow_mode.enter':
        dynamicWorkflow = { active: true, trigger: rec.trigger };
        break;
      case 'dynamic_workflow_mode.exit':
        dynamicWorkflow = { active: false };
        break;
...
// These replicate agent-core's `toolResultOutputForModel` so dashboard's model-view

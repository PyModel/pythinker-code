...
import { startDashboardServer } from '../../src/start';
...
describe('startDashboardServer', () => {
  it('serves the embedded web asset and the API on an auto-picked port', async () => {
    ...
    expect(await page.text()).toContain('<title>dashboard</title>'); // fetch auto-inflates gzip

    const spa = await fetch(`${server.url}sessions/anything`);
    expect(await spa.text()).toContain('<title>dashboard</title>'); // SPA fallback
  ...
  it('rejects instead of hanging when the port is already bound', async () => {
    const first = await startDashboardServer({ port: 0, homeDir: '/tmp/does-not-exist-home' });
    stop = first.close;
    ...
    await expect(
      startDashboardServer({ port: taken, homeDir: '/tmp/does-not-exist-home' }),

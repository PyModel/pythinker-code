...
describe('context-projector', () => {
  ...
  it('projects messages and aggregates usage', async () => {
    ...
    expect(proj.usage.byModel['pythinker-k2']).toEqual({
      inputOther: 10, output: 5, inputCacheRead: 0, inputCacheCreation: 0,
    });

    expect(proj.config.systemPrompt).toBe('You are Pythinker.');
  ...
  it('micro_compaction.apply weights non-ASCII Latin chars as full tokens', () => {
    // ~150 accented Latin chars. Under a naive chars/4 estimate this is ~38 tokens (< 100
    ...
    // non-ASCII-heavy tool result diverging from agent-core no longer happens.
    const accented = 'é'.repeat(150);
    const entries = [
      { lineNo: 1, data: { type: 'context.append_message' as const, message: {
          role: 'tool' as const, toolCallId: 'c0', toolCalls: [],
          content: [{ type: 'text' as const, text: accented }],
        } }, raw: {} },
      { lineNo: 2, data: { type: 'micro_compaction.apply' as const, cutoff: 1 }, raw: {} },
    ];
    const proj = projectContext(entries as any);
  ...
  it('projects dynamic workflow mode enter/exit without the removed swarm field', () => {
    const enter = projectContext([{ lineNo: 1, data: { type: 'dynamic_workflow_mode.enter' as const, trigger: 'task' }, raw: {} }] as any);
    expect(enter.dynamicWorkflow).toEqual({ active: true, trigger: 'task' });
    expect(enter).not.toHaveProperty('swarm');
    const exit = projectContext([
      { lineNo: 1, data: { type: 'dynamic_workflow_mode.enter' as const, trigger: 'task' }, raw: {} },
      { lineNo: 2, data: { type: 'dynamic_workflow_mode.exit' as const }, raw: {} },
    ] as any);
    expect(exit.dynamicWorkflow.active).toBe(false);
  });

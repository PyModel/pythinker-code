...
async function setup(messages: AppMessage[] = []) {
  ...
  let handlers: PythinkerEventHandlers | undefined;
  const eventConn = {
    subscribe: vi.fn(),
    unsubscribe: vi.fn(),
  ...
  const initialConfig: AppConfig = { providers: {}, defaultModel: 'pythinker/default' };
  const api = {
    ...
    connectEvents: vi.fn((nextHandlers: PythinkerEventHandlers) => {
...
describe('usePythinkerWebClient session memory cache', () => {
  it('reads and writes only the dynamic workflow storage key', async () => {
    localStorage.setItem('pythinker-web.dynamic-workflow-mode', 'true');

    const { client } = await setup();
    const dynamicClient = client as unknown as {
      dynamicWorkflowMode: { value: boolean };
      setDynamicWorkflowMode(on: boolean): void;
    };

    expect(dynamicClient.dynamicWorkflowMode.value).toBe(true);
    dynamicClient.setDynamicWorkflowMode(false);
    expect(localStorage.getItem('pythinker-web.dynamic-workflow-mode')).toBe('false');
  });

  it('ignores and leaves the removed storage key untouched', async () => {
  ...
  it('uses the fast spinner class only for high-speed active-session output', async () => {
    vi.useFakeTimers();
    try {
      const { client, getHandlers } = await setup([]);
      await client.createSession('/repo');
  ...
  it('keeps daemon config writes and configChanged events in client state', async () => {
    ...
    await client.updateConfig({ defaultModel: 'pythinker/k2' });

    expect(api.setConfig).toHaveBeenCalledWith({ defaultModel: 'pythinker/k2' });
    expect(client.config.value?.defaultModel).toBe('pythinker/k2');
    expect(client.defaultModel.value).toBe('pythinker/k2');
...
describe('unread persistence across reload', () => {
  it('restores unread dots from storage and clears them on open', async () => {
    try { localStorage.removeItem('pythinker-web.unread'); } catch { /* ignore */ }
    try {

...
import { buildDynamicWorkflowGroups } from '../src/composables/dynamic_workflowGroups';
...
describe('messagesToTurns agent blocks', () => {
  ...
  it('does NOT render a dynamic_workflow (subagents with a dynamicWorkflowIndex) inline — it is a DynamicWorkflowCard', () => {
    const messages: AppMessage[] = [
      {
        id: 'msg_1',
        sessionId: 'ses_1',
        ...
        createdAt: now,
        content: [
          { type: 'toolUse', toolCallId: 'tc_dynamic_workflow', toolName: 'agent_dynamic_workflow', input: { description: 'review', count: 2 } },
        ],
      },
    ];
    const tasks: AppTask[] = [
      {
        ...
        status: 'running', createdAt: now, subagentPhase: 'queued', parentToolCallId: 'tc_dynamic_workflow', dynamicWorkflowIndex: 2,
      },
      {
        id: 'agent_a', sessionId: 'ses_1', kind: 'subagent', description: 'First',
        status: 'completed', createdAt: now, subagentPhase: 'completed', parentToolCallId: 'tc_dynamic_workflow', dynamicWorkflowIndex: 1,
      },
    ];

    // The dynamic_workflow is rendered as its own DynamicWorkflowCard (buildDynamicWorkflowGroups), so it must
    ...
    // ...but it IS surfaced once, as a dynamic_workflow group.
  ...
  it('renders multiple NON-dynamic_workflow subagents (no dynamicWorkflowIndex) as an inline agentGroup', () => {
    const messages: AppMessage[] = [
      {
        id: 'msg_1',
        sessionId: 'ses_1',
    ...
    // Not a dynamic_workflow → no DynamicWorkflowCard.

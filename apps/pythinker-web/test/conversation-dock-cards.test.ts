...
import type { DynamicWorkflowGroup } from '../src/composables/dynamic_workflowGroups';
...
function dynamic_workflowGroup(members: { phase: DynamicWorkflowGroup['members'][number]['phase']; id?: string }[]): DynamicWorkflowGroup {
  ...
  return { id: 'dynamic_workflow_1', members: ms, counts };
}

describe('ConversationPane dynamic_workflow stack', () => {
  it('shows the dynamic_workflow stack while at least one member is active', () => {
    const wrapper = mountPane({
      dynamic workflows: [dynamic_workflowGroup([{ phase: 'working' }, { phase: 'completed' }])],
    });
    expect(wrapper.find('.dynamic-workflow-stack').exists()).toBe(true);
  });

  it('hides the dynamic_workflow stack once all members are completed or failed', () => {
    const wrapper = mountPane({
      dynamic workflows: [dynamic_workflowGroup([{ phase: 'completed' }, { phase: 'failed' }])],
    });

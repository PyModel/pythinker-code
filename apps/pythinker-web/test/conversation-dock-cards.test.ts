...
import type { DynamicWorkflowGroup } from '../src/composables/dynamicWorkflowGroups';
...
function dynamicWorkflowGroup(members: { phase: DynamicWorkflowGroup['members'][number]['phase']; id?: string }[]): DynamicWorkflowGroup {
  const ms: DynamicWorkflowGroup['members'] = members.map((m, i) => ({
    id: m.id ?? `agent_${i + 1}`,
    name: `Agent ${i + 1}`,
    phase: m.phase,
    dynamicWorkflowIndex: i + 1,
  }));
  const counts: DynamicWorkflowGroup['counts'] = { queued: 0, working: 0, suspended: 0, completed: 0, failed: 0 };
  for (const m of ms) counts[m.phase]++;
  return { id: 'workflow_1', members: ms, counts };
}

describe('ConversationPane dynamic workflow stack', () => {
  it('shows the dynamic workflow stack while at least one member is active', () => {
    const wrapper = mountPane({
      dynamicWorkflows: [dynamicWorkflowGroup([{ phase: 'working' }, { phase: 'completed' }])],
    });
    expect(wrapper.find('.dynamic-workflow-stack').exists()).toBe(true);
  });

  it('hides the dynamic workflow stack once all members are completed or failed', () => {
    const wrapper = mountPane({
      dynamicWorkflows: [dynamicWorkflowGroup([{ phase: 'completed' }, { phase: 'failed' }])],
    });
    expect(wrapper.find('.dynamic-workflow-stack').exists()).toBe(false);
  });
});

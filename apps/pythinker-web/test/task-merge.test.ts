// Regression test for the dynamic-workflow/subagent "running" flicker.
//
...
// background store and never returns foreground dynamic workflow subagents. A plain replace
// dropped those WS-delivered subagents on every refresh, so the next event
// re-added them — flickering the dynamic workflow cards once per second. keepLiveSubagents
...
describe('keepLiveSubagents', () => {
  it('keeps a live subagent that the REST list omits (the flicker fix)', () => {
    ...
    const existing = [task('bg_1', 'bash'), task('agent_1', 'subagent', { dynamicWorkflowIndex: 1 } as AppTask)];
  ...
  it('does not duplicate a subagent that REST does return', () => {
    const restBased = [task('agent_1', 'subagent', { dynamicWorkflowIndex: 1 } as AppTask)];
    const existing = [task('agent_1', 'subagent', { dynamicWorkflowIndex: 1 } as AppTask)];

...
//
// Integration test that drives the real usePythinkerWebClient + ConversationPane
...
import type { AppSession, PythinkerEventHandlers, PythinkerWebApi } from '../src/api/types';
...
describe('ConversationPane empty-session send integration', () => {
  it('clears the composer through the real client flow', async () => {
    ...
    expect(localStorage.getItem('pythinker-web.draft.__new__')).toBe(null);

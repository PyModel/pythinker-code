...
import { createInitialState, reduceAppEvent, type PythinkerClientState } from '../src/api/daemon/eventReducer';
...
function play(events: [string, unknown][]): PythinkerClientState {
...
describe('multi-segment thinking', () => {
  it('keeps interleaved thinking/text segments separate within one step', () => {
    const state = play([
      ...
      ['thinking.delta', { delta: 'thought A-1 ' }],
      ['thinking.delta', { delta: 'thought A-2' }],
      ['assistant.delta', { delta: 'answer B' }],
      ['thinking.delta', { delta: 'thought C' }],
      ['assistant.delta', { delta: 'answer D' }],
    ...
    expect(assistant.content).toEqual([
      { type: 'thinking', thinking: 'thought A-1 thought A-2' },
  ...
  it('renders think → tool → think again as two thinking blocks in call order', () => {
    const state = play([
      ['turn.started', { turnId: 1 }],
      ['turn.step.started', { turnId: 1 }],
      ['thinking.delta', { delta: 'check the file first' }],
      ...
      ['turn.step.started', { turnId: 1 }],
      ['thinking.delta', { delta: 'done reading, drafting answer' }],
      ['assistant.delta', { delta: 'final answer' }],
      ['turn.step.completed', { turnId: 1 }],
      ['turn.ended', { turnId: 1, reason: 'completed' }],
    ]);
    ...
    expect(blocks[0]).toEqual({ kind: 'thinking', thinking: 'check the file first' });
    expect(blocks[2]).toEqual({ kind: 'thinking', thinking: 'done reading, drafting answer' });
  });
  ...
  it('does not clobber already-streamed text when thinking starts afterwards', () => {
    const state = play([
      ['turn.started', { turnId: 1 }],
      ['turn.step.started', { turnId: 1 }],
      ['assistant.delta', { delta: 'first line' }],
      ['thinking.delta', { delta: 'extra thought' }],
      ['turn.step.completed', { turnId: 1 }],
      ['turn.ended', { turnId: 1, reason: 'completed' }],
    ]);
    ...
    expect(assistant.content).toEqual([
      ...
      { type: 'thinking', thinking: 'extra thought' },
    ]);
  });

...
describe('draftStorageKey', () => {
  it('uses the session id when present', () => {
    expect(draftStorageKey('abc')).toBe('pythinker-web.draft.abc');
  });

  it('falls back to __new__ when sid is empty/undefined', () => {
    expect(draftStorageKey(undefined)).toBe('pythinker-web.draft.__new__');
    expect(draftStorageKey('')).toBe('pythinker-web.draft.__new__');
  });
});

describe('STORAGE_KEYS', () => {
  it('keeps the legacy key strings unchanged', () => {
    expect(STORAGE_KEYS.theme).toBe('pythinker-web.theme');
    expect(STORAGE_KEYS.activeWorkspace).toBe('pythinker-active-workspace');

...
describe('messagesToTurns', () => {
  ...
  it('renders a live multi-member dynamic_workflow inline as a tool card', () => {
    const turns = messagesToTurns(
      [
        message('u1', 'user', [{ type: 'text', text: 'run a dynamic_workflow' }]),
        message('a1', 'assistant', [
          { type: 'toolUse', toolCallId: 'dynamic-workflow-1', toolName: 'AgentDynamicWorkflow', input: {} },
        ]),
      ],
      [],
      undefined,
    ...
    expect(assistant?.tools).toContainEqual(
      expect.objectContaining({ id: 'dynamic-workflow-1', name: 'AgentDynamicWorkflow', status: 'running' }),
    );
  ...
  it('renders a completed multi-member dynamic_workflow inline as a tool card', () => {
    const turns = messagesToTurns(
      [
        message('u1', 'user', [{ type: 'text', text: 'run a dynamic_workflow' }]),
        message('a1', 'assistant', [
          { type: 'toolUse', toolCallId: 'dynamic-workflow-2', toolName: 'AgentDynamicWorkflow', input: {} },
        ]),
        message('t1', 'tool', [{ type: 'toolResult', toolCallId: 'dynamic-workflow-2', output: 'all done' }]),
      ],
      [],
      undefined,
      false,
    ...
    const assistant = turns.at(-1);
    expect(assistant?.tools).toContainEqual(
      expect.objectContaining({ id: 'dynamic-workflow-2', name: 'AgentDynamicWorkflow', status: 'ok' }),
    );
    expect(assistant?.blocks ?? []).not.toContainEqual(
      expect.objectContaining({ kind: 'agentGroup' }),
    );
    ...
    const turns = messagesToTurns(
      [
        message('u1', 'user', [
          ...
          {
            type: 'text',
            text: `<video path="/Users/me/.pythinker-code/cache/${fileId}.mp4"></video>`,
          },
        ]),
      ],
      [],
  ...
  it('recovers a file attachment from the server’s "Attached file" notice, not raw text', () => {
    ...
    const notice =
      ...
      `/home/u/.pythinker-code/sessions/s_1/attachments/${fileId}-report.pdf — open it with the Read tool`;
    const turns = messagesToTurns(
      [
        message('u1', 'user', [
  ...
  it('recovers a non-clickable chip from an inline-base64 attachment notice (no fileId)', () => {
    ...
    const notice =
      ...
      '/home/u/.pythinker-code/sessions/s_1/attachments/9f86d081884c7d659a2feaa0c55ad015-image.avif — open it with the Read tool';
    const turns = messagesToTurns(
  ...
  it('recovers the full UUID file id from a v2 "Attached file" notice', () => {
    ...
    const notice =
      ...
      `/home/u/.pythinker-code/sessions/s_1/attachments/${fileId}-api-spec-v2.yaml — open it with the Read tool`;
    const turns = messagesToTurns(
      [message('u1', 'user', [{ type: 'text', text: notice }])],
      [],
      (id) => `/api/v1/files/${id}`,
  ...
  it('keeps the video tag as text when no file resolver is provided', () => {
    const tag =
      '<video path="/Users/me/.pythinker-code/cache/f_01KWK39A0ZC8R2ATZEQMD8716C.mp4"></video>';
    const turns = messagesToTurns(
  ...
  it('strips the hidden image-compression caption from a user bubble', () => {
    ...
    const caption =
      ...
      'is saved at "/Users/me/.pythinker-code/files/f_0000000000000000000000000"; if you need fine ' +

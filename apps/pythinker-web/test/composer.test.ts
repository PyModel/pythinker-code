    ...
    messages: {
      en: {
        ...
        commands: {
          ...
          dynamic_workflow: { desc: 'Run with dynamic_workflow' },
...
describe('Composer slash command input', () => {
  it('emits /goal with the typed objective instead of sending it as chat', async () => {
    ...
    await textarea.setValue('/goal dynamic_workflow review the changed files');
    await textarea.trigger('keydown', { key: 'Enter' });

    expect(wrapper.emitted('command')).toEqual([['/goal dynamic_workflow review the changed files']]);
    expect(wrapper.emitted('submit')).toBeUndefined();
  });

  it('emits /dynamic_workflow with the typed task instead of sending it as chat', async () => {
    const wrapper = mountComposer();
    const textarea = wrapper.get('textarea');

    await textarea.setValue('/dynamic_workflow inspect flaky tests');
    await textarea.trigger('keydown', { key: 'Enter' });

    expect(wrapper.emitted('command')).toEqual([['/dynamic_workflow inspect flaky tests']]);
    expect(wrapper.emitted('submit')).toBeUndefined();
  });
...
describe('Composer model dropdown', () => {
  const models: AppModel[] = [
    { id: 'pythinker/k2', provider: 'pythinker', model: 'k2', displayName: 'Kimi K2', maxContextSize: 128000 },
  ...
  it('shows starred models from other providers in the quick-switch dropdown', async () => {
    const wrapper = mountComposer({
      status: { model: 'Kimi K2', modelId: 'pythinker/k2', ctxUsed: 0, ctxMax: 128000, permission: 'manual' },
      models,
...
describe('Composer context indicator', () => {
  const status = { model: 'Kimi K2', modelId: 'pythinker/k2', ctxUsed: 0, ctxMax: 128000, permission: 'manual' };

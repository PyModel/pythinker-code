 ...
 * Run: pnpm --filter @pymodel/pythinker-web exec vitest run test/event-batcher.test.ts
 */
...
vi.mock('../src/api', () => ({
  getPythinkerWebApi: () => clientApiMock,
}));
...
function assistantState(content: AppMessage['content'] = [{ type: 'text', text: '' }]): PythinkerClientState {
...
describe('usePythinkerWebClient (resync integration)', () => {
  it('flushes queued deltas around an authoritative snapshot before live streaming resumes', async () => {
    ...
    let seedCalls = 0;
    const connection: PythinkerEventConnection = {
      subscribe: vi.fn(),
      unsubscribe: vi.fn(),

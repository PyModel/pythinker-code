...
function play(events: [string, unknown][]): { state: PythinkerClientState; appEvents: AppEvent[] } {
...
describe('compaction pipeline', () => {
  ...
  it('a completed event without a prior started still appends a marker', () => {
    ...
    expect(state.compactionBySession[SESSION]).toBeUndefined();
    const msgs = state.messagesBySession[SESSION] ?? [];
    expect(msgs.at(-1)?.metadata?.[COMPACTION_MARKER_METADATA_KEY]).toMatchObject({
      trigger: 'auto',
      tokensBefore: 50000,
      tokensAfter: 8000,
    });

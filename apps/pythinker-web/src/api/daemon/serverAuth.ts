...
// and WebSocket call (the persistent server token, or the PYTHINKER_CODE_PASSWORD
// password). The Web UI obtains that credential in one of two ways:
//   1. From the URL fragment (`#token=<...>`) that `pythinker web` appends when it
...
// at <PYTHINKER_CODE_HOME>/server.token and handed to the browser in the launch URL.
// `pythinker web rotate-token` invalidates a stale copy, and the next 401 clears
// it here.

const STORAGE_KEY = 'pythinker-web.server-credential';
...
export function clearCredential(): void {
  ...
  try {
    ...
    // (e.g. right after `pythinker web rotate-token`).
    const raw = globalThis.localStorage?.getItem(STORAGE_KEY);
    const stored = raw === null || raw === undefined
      ? undefined
      : decodeStoredCredential(raw);

...
const CLIENT_ID_KEY = 'pythinker-web.client-id';
const WEB_CLIENT_NAME = 'pythinker-code-web';
const WEB_CLIENT_UI_MODE = 'web';

export interface PythinkerApiConfig {
...
export function readPythinkerApiConfig(): PythinkerApiConfig {
  return {
    serverHttpUrl: normalizeServerOrigin(import.meta.env.VITE_PYTHINKER_SERVER_HTTP_URL),
...
//  - prod: `pythinker web` serves this built SPA from the server itself, so the
//    server's origin already is the API origin.
// Set VITE_PYTHINKER_SERVER_HTTP_URL to connect directly to an absolute server
 ...
 *  - VITE_PYTHINKER_SERVER_HTTP_URL set → that absolute server origin (direct mode).
...
export function serverEndpointLabel(): string {
  const direct = import.meta.env.VITE_PYTHINKER_SERVER_HTTP_URL;
  if (direct && direct.trim()) return shortOrigin(normalizeServerOrigin(direct));

  const proxy =
    typeof __PYTHINKER_DEV_PROXY_TARGET__ !== 'undefined' ? __PYTHINKER_DEV_PROXY_TARGET__ : '';
...
function webClientVersion(): string {
  return typeof __PYTHINKER_WEB_VERSION__ === 'string' && __PYTHINKER_WEB_VERSION__.trim()

...
//   POST /__pythinker-dev/backend { name } → repoint the /api/v1 proxy
...
export function initialDevBackendState(): DevBackendState | null {
  ...
  const presets =
    typeof __PYTHINKER_DEV_BACKENDS__ !== 'undefined' ? __PYTHINKER_DEV_BACKENDS__ : null;
  if (!presets) return null;
  const current =
    typeof __PYTHINKER_DEV_PROXY_TARGET__ !== 'undefined' && __PYTHINKER_DEV_PROXY_TARGET__
      ? __PYTHINKER_DEV_PROXY_TARGET__
      : presets.default;
...
export async function fetchDevBackendState(): Promise<DevBackendState | null> {
  if (!import.meta.env.DEV) return null;
  try {
    const res = await fetch('/__pythinker-dev/backend');

...
//
// Opt-in: `?debug=1` in the URL or `localStorage["pythinker-web.debug"]="1"`.
...
const SENSITIVE_KEY_RE = /api[_-]?key|authorization|token|secret|password|cookie|credential|headers?|env/i;
...
export function isTraceEnabled(): boolean {
  ...
  if (!enabled) {
    try {
      enabled = localStorage.getItem('pythinker-web.debug') === '1';
    } catch {
...
export function downloadTraceLog(list: readonly TraceEntry[] = entries): void {
  ...
  a.href = url;
  a.download = `pythinker-web-log-${new Date().toISOString().replaceAll(/[:.]/g, '-')}.jsonl`;

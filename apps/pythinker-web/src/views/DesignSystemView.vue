  ...
  <div class="ds-page">
    ...
    <div class="layout">
      ...
      <aside class="sidebar">
        <div class="brand">
          ...
          <div class="brand-name">Pythinker Web</div>
        </div>
        ...
        <div class="content-inner">
          ...
          <section id="overview">
            <div class="hero">
              ...
              <h1>Pythinker Web <span class="grad">Design System</span></h1>
              <p class="lead">
                This document defines the visual language and component specification for Pythinker Web — design tokens, component primitives, the chat interface, theming, and style rules.
                All UI work is grounded in it: unified, restrained, token-driven, and themeable.
              </p>
              <div class="hero-meta">
                <span class="meta-chip"><span class="dot"></span> Scope <b>apps/pythinker-web</b></span>
          ...
          <section id="tokens">
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td class="tk">--color-sidebar-bg</td><td class="val"><span class="swatch" style="background:#e9eaf2"></span>#e9eaf2</td><td class="val"><span class="swatch" style="background:#2c343a"></span>#2c343a</td><td>Sidebar solid composite (row masks / overlays). The visible sidebar surface is the frost pair <code>--color-sidebar-glass</code> + <code>--sidebar-wash</code> blurred by <code>--p-sidebar-backdrop</code></td></tr>
            ...
            <p>Pythinker Web uses two font families: <b>--font-ui</b> (UI and body, Inter first) and <b>--font-mono</b> (code and monospace). Components always reference the variables; do not hard-code font names.</p>
      ...
      ui-monospace, "SF Mono", Menlo, Consolas, monospace;</pre></div>
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td class="tk">--z-modal-dropdown</td><td class="val">500</td><td>modal dropdown / nested confirmation</td></tr>
            ...
            <div class="stage-wrap">
              ...
              <div class="stage p col">
                ...
                <div class="demo-row">
                  ...
                  <span class="p-badge solid">PYTHINKER</span>
                </div>
              ...
              <div class="stage p col">
                <div class="demo-row" style="align-items:flex-start">
                  ...
                  <div class="p-field demo-grow">
                    ...
                    <select class="p-select"><option>Anthropic</option><option>OpenAI</option><option>PyModel</option></select>
                  </div>
                </div>
                <div class="p-field">
    ...
    export function auth(token: string) {
      return verify(token, process.env.JWT_SECRET!);
            ...
            <div class="stage-wrap">
              ...
              <div class="stage p col">
                <div class="p-empty" style="width:100%;border:1px dashed var(--p-line);border-radius:var(--p-r-lg)">
                  ...
                  <div class="em-hint">Click "New chat" to start a conversation with Pythinker</div>
                </div>
              </div>
            </div>
            ...
            <p>The application top bar. Solid by default; the <code>.frost</code> variant is translucent + background blur, used <b>only for sticky navigation bars</b>, and — together with the <b>Sidebar frost layer</b> (see §05/§06) — is a sanctioned exception to the <code>no-glassmorphism</code> rule (see §06).</p>
            <div class="stage-wrap">
          ...
          <section id="chat">
            ...
            <p class="sec-desc">
              The message stream is the core of Pythinker Web. The goal of the overhaul: have the 6 card types (Agent / Tool / Question / Approval / DynamicWorkflow / Todo)
            ...
            <h3 class="sub">Conversation outline · user-prompt line index</h3>
            <p>The desktop outline is a quiet vertical stack of equal 16 × 2px horizontal lines on a 10px rhythm, centered at the chat pane's left edge beside the app sidebar. Each line represents one loaded <b>user prompt</b>; assistant messages and tool calls never become navigation items. The active line is white and the other lines are muted, so the current prompt is clear without a node timeline.</p>
            <table class="dt">
              <thead><tr><th>State</th><th>Visual</th><th>Behavior</th></tr></thead>
              <tbody>
                <tr><td class="tk">default</td><td>Equal short prompt lines centered in one compact stack</td><td>The card and prompt labels stay absent while the line index is collapsed.</td></tr>
                <tr><td class="tk">hover / focus-within</td><td>Prompt-only rows on an opaque raised surface with a menu border and shadow</td><td>The lines disappear. Rows use the shared hover surface; keyboard focus uses <code>--p-focus-ring</code>.</td></tr>
                <tr><td class="tk">active</td><td>White line when collapsed; selected row and accent label when expanded</td><td>The button carries <code>aria-current="location"</code>; the outline keeps it visible in long threads.</td></tr>
                ...
                <tr><td class="tk">constrained</td><td>Centered internal scroll with a reserved reading inset</td><td>The expanded list stays inside the middle half of the viewport and above the measured composer dock. Narrow desktop panes reserve 96px at the reading column's left edge so the collapsed lines remain visible. Hide on mobile, while loading, below two prompts, when the collapsed stack cannot fit, or while a wide table covers the anchor.</td></tr>
              </tbody>
            </table>
            ...
            <div class="stage-wrap">
              ...
              <div class="stage p col" style="align-items:center;background:#fff">
                <div class="p-composer" style="width:100%;max-width:620px">
                  <div class="p-composer-ta ph">Message Pythinker, / to run a command, @ to reference a file…</div>
          ...
          <section id="themes">
            ...
            <p class="sec-desc">
              Pythinker Web uses <b>one unified theme</b>: the same components, fonts, radii, shadows, and surfaces — "reskinning" only changes colors.
          ...
          <section id="rules">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr><td class="tk">no-gradient-text</td><td>gradient text / gradient background (<b>Sidebar frost wash</b> excepted: <code>--color-sidebar-wash</code> paints the vibrancy tint on <code>.side::before</code>)</td><td><span class="pill red">Forbidden</span></td></tr>
                <tr><td class="tk">no-glassmorphism</td><td><code>backdrop-filter: blur</code> (<b>TopBar sticky nav bar</b> and the <b>Sidebar frost layer</b> are the sanctioned exceptions)</td><td><span class="pill amber">TopBar / Sidebar exempt</span></td></tr>
            ...
            <div class="callout good"><span class="ico">✓</span><div>
              <code>backdrop-filter: blur</code> is banned site-wide, with two sanctioned exceptions: the <code>.frost</code> variant of <code>TopBar</code> (sticky navigation bars, readable over scrolling content) and the <b>Sidebar frost layer</b> (<code>.side::before</code> — translucent glass + wash + backdrop blur, applied on the pseudo-layer so the sidebar's non-teleported fixed menus keep their viewport anchoring; same recipe extends to the Onboarding hero). No other component (card, dialog, Toast, panel) may use glassmorphism; violations are flagged under <code>no-glassmorphism</code>.
            </div></div>

            <div class="footer">
              <span>Pythinker Web Design System · v1.0</span>
    ...
    /*         sidebar ↑    ↑handle  ↑conversation  ↑handle ↑right panel (auto) */</pre></div>
            ...
            <p>The sidebar from top to bottom: brand header → New chat → search → grouped list (workspace head + session rows) → settings footer. Controls reuse the §03 primitives as much as possible. The sidebar renders as a <b>frost layer</b>: <code>.side::before</code> composes the translucent <code>--color-sidebar-glass</code> base, the <code>--color-sidebar-wash</code> gradient (cool blue→lavender in light, dark blue-gray vibrancy in dark), and <code>--p-sidebar-backdrop</code> blur — sanctioned extensions to §06. <code>--color-sidebar-bg</code> remains the solid composite (row masks / overlays). The hairline still separates it from the conversation pane. Vertical rhythm: the brand header keeps 12px padding (on macOS desktop it adds 36px top clearance for the traffic lights); rows inside the actions group (New chat + search) stack flush (0 gap, same rhythm as the list rows); adjacent groups are separated by 12px. Row hover uses <code>--sb-hover</code> (= the global <code>--color-hover</code> wash); the selected row uses <code>--color-selected</code> — neutral, never the accent.</p>
            <table class="dt">
              <thead><tr><th>Block</th><th>Use</th><th>Note</th></tr></thead>
              <tbody>
                <tr><td>Brand header</td><td>theme-matched Pythinker banner + collapse IconButton</td><td>the supplied dark/light banner sits left and the collapse IconButton sm is right-aligned inside the header on every platform. On macOS desktop the header adds top clearance for the floating traffic lights and doubles as the window-drag strip (the button opts out of dragging).</td></tr>
          ...
          <section id="a11y">
            ...
            <p class="sec-desc">
              Pythinker Web is a local developer tool; it <b>does not target a specific WCAG conformance level</b>, nor maintain a full screen-reader QA matrix.

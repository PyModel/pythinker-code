...
<!-- First-run onboarding overlay: a short welcome + theme preference.
     Re-openable from the settings popover. -->
<script setup lang="ts">
import { ref } from 'vue';
import { useI18n } from 'vue-i18n';
import type { Theme } from '../composables/usePythinkerWebClient';
...
<template>
  <div class="ob-backdrop">
    <div class="ob-card" role="dialog" aria-modal="true" :aria-label="t('onboarding.title')">
      ...
      <div class="ob-brand">
        <PythinkerLogo size="md" class="ob-logo" />
        <div>
      ...
      <section class="ob-sec">
        ...
        <div class="ob-themes">
          ...
          <button
            type="button"
            class="ob-theme"
            :class="{ on: selectedTheme === 'pythinker' }"
            :aria-pressed="selectedTheme === 'pythinker'"
            @click="selectedTheme = 'pythinker'"
          >
            <span class="ob-theme-prev pythinker" aria-hidden="true">
              <span class="kb u"></span><span class="kb a"></span>
            </span>
            <span class="ob-theme-name">{{ t('theme.pythinker') }}</span>
            <span class="ob-theme-desc">{{ t('onboarding.pythinkerDesc') }}</span>
          </button>
        </div>
      </section>
...
/* Pythinker: flat white canvas, quiet gray bubbles, no blue. color-mix keeps the
   sketch readable in both color schemes without theme-specific values. */
.ob-theme-prev.pythinker { background: var(--bg); border: 1px solid var(--line); box-sizing: border-box; align-items: stretch; }
.ob-theme-prev.pythinker .kb { height: 14px; border-radius: 7px; }
.ob-theme-prev.pythinker .kb.u { width: 60%; align-self: flex-end; background: color-mix(in srgb, var(--ink) 8%, var(--bg)); }
.ob-theme-prev.pythinker .kb.a { width: 80%; background: color-mix(in srgb, var(--ink) 4%, var(--bg)); }

...
const canExpand = computed(() => hasOutput.value || props.tool.status !== 'error');
const open = ref(props.tool.defaultExpanded === true && canExpand.value);
const emptyText = computed(() => {
  if (props.tool.status === 'running') return t('tools.output.waiting');
  if (props.tool.status === 'ok' && !hasOutput.value) return t('tools.output.saved');
  return t('tools.output.empty');
});
...
<template>
  <ToolRow
    ...
    @toggle="toggle"
  >
    <ToolOutputBlock :lines="tool.output" :empty-text="emptyText" />
  </ToolRow>
</template>

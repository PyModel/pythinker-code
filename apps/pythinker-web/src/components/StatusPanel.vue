...
<!-- state (no daemon call). Light only, monospace, Pythinker blue, no emoji. -->
...
const permColor = computed(() => {
  ...
  if (p === 'yolo') return 'var(--warn)';
  if (p === 'auto') return 'var(--err)';
  return 'var(--ink)';
});
...
.row dd.dynamic-workflow-on { color: var(--blue); }

...
import type { DynamicWorkflowGroup } from '../composables/dynamicWorkflowGroups';
...
const props = defineProps<{
  ...
  todos?: TodoView[];
  goal?: AppGoal | null;
  dynamicWorkflows?: DynamicWorkflowGroup[];
  ...
  mobile?: boolean;
  /** Bubble themes (Modern/Pythinker): render chat bubbles at all widths (desktop included). */
  modern?: boolean;
  ...
  /** Git diff line stats for the header diff counter (mirrors pythinker-cli/web). */
...
/** Dynamic Workflow cards are live progress indicators: keep the bottom stack only while
...
const activeDynamicWorkflows = computed<DynamicWorkflowGroup[]>(() => {
  return (
    props.dynamicWorkflows?.filter((group) =>
...
try {
  localStorage.removeItem('pythinker-web.content-align');
} catch {
...
function focusDynamicWorkflow(): void {
  void nextTick(() => {
    const first = panesRef.value?.querySelector<HTMLElement>('.dynamic-workflow-card');
        ...
        <div class="content-wrap" :class="[mobile ? 'align-mobile' : 'align-center']">
          <template v-if="turns.length === 0 && !sessionLoading">
            ...
            <Composer
              ...
              @focus-dynamic-workflow="focusDynamicWorkflow"
...
.dynamic-workflow-stack {
  padding: 0 18px 16px;
}
.content-wrap.align-mobile .dynamic-workflow-stack {
  padding: 0 14px 18px;
}

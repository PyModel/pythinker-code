...
import type { DynamicWorkflowGroup } from '../composables/dynamic_workflowGroups';
...
const props = defineProps<{
  ...
  todos?: TodoView[];
  goal?: AppGoal | null;
  dynamic workflows?: DynamicWorkflowGroup[];
...
/** DynamicWorkflow cards are live progress indicators: keep the bottom stack only while
...
const activeDynamicWorkflows = computed<DynamicWorkflowGroup[]>(() => {
  return (
    props.dynamic workflows?.filter((group) =>
        ...
        <div class="content-wrap" :class="[mobile ? 'align-mobile' : 'align-center']">
          <template v-if="turns.length === 0 && !sessionLoading">
            ...
            <Composer
              ...
              @focus-dynamic_workflow="focusDynamicWorkflow"

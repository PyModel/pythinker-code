...
async function loadStoredKey(): Promise<void> {
  const provider = props.provider;
  if (props.mode !== 'edit' || provider === undefined || !provider.hasApiKey) return;
  try {
...
<template>
  <form class="provider-form" @submit.prevent="save" @input="markDirty">
    ...
    <div v-if="modelsOpen" class="provider-form__models">
      ...
      <template v-for="(model, index) in form.models" :key="index">
        <div class="provider-form__model" :class="{ 'has-error': !!errors.models[index] }">
          ...
          />
          <Input v-model="model.displayName" :placeholder="t('providers.modelNamePlaceholder')" />
          <IconButton
            size="sm"
    ...
    <div class="provider-form__actions">
      ...
      <Button type="submit" variant="primary" :loading="saving">{{ t('providers.save') }}</Button>
    </div>
  </form>
</template>

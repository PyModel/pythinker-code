...
const props = withDefaults(
  defineProps<{
     ...
     * instead of the desktop `user@pythinker $` / `pythinker >` line-turns. Driven by the
     ...
     * starts streaming. Renders an activity-spinner placeholder at the end of the
     * transcript so the user knows the request is in flight.
     */
    sending?: boolean;
    /** Switches the CSS-only working spinner to the faster visual cadence. */
    fastSpinner?: boolean;
    /**
  ...
  { approvals: () => [], bubble: false, mobile: false, running: false, sending: false, fastSpinner: false, compaction: null },
);
...
// Trailing "working" spinner. `sending` is an optimistic flag set on submit and
// kept until the session goes idle, so during a normal turn the spinner shows the
...
// NOTE: the turn-summary line ("N tools invoked…") was removed in f9417af. If it
...
<template>
  ...
  <!-- Same ChatTurn data as desktop, rendered as bubbles. User turns are
       right-aligned soft-blue bubbles (no `user@pythinker $` prefix, no line number);
  ...
  <div v-if="childBubble" class="chat">
    ...
    <!-- Working placeholder — spinner while the turn is in flight (covers
    ...
    <template v-for="(turn, ti) in turns" :key="turn.id">
      ...
      >
        ...
        <div class="tx">
          ...
          <div class="role-row">
            <template v-if="turn.role === 'user'">
              <span class="pr">user@pythinker</span>
              <span class="who"> $ </span>
            </template>
            <template v-else>

...
export const STORAGE_KEYS = {
  ...
  onboarded: 'pythinker-web.onboarded',
  accent: 'pythinker-web.accent',
  colorScheme: 'pythinker-web.color-scheme',
  hiddenWorkspaces: 'pythinker-web.hidden-workspaces',
  collapsedWorkspaces: 'pythinker-web.collapsed-workspaces',
  ...
  contentAlign: 'pythinker-web.content-align',
  theme: 'pythinker-web.theme',
  thinking: 'pythinker-web.thinking',
} as const;

/** Per-session composer draft key. */
export function draftStorageKey(sid: string | undefined): string {
  return `pythinker-web.draft.${sid && sid.length > 0 ? sid : '__new__'}`;
}

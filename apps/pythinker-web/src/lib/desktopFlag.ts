...
//
// Detects whether the web UI is running inside the Pythinker Desktop app, and on
// which platform.
//
// The desktop app shares the local pythinker daemon with the CLI / browser / TUI, so
...
// unreliable. Instead, the desktop app appends `?pythinker_desktop=1&platform=<os>`
// to the URL it loads (see apps/pythinker-desktop/src/main/index.ts); we persist
// those values in sessionStorage so they survive any in-app navigation or
// redirect that drops the query string. The compile-time __PYTHINKER_WEB_DESKTOP__
// is kept as an additional signal for the case where the web bundle itself was
// built for the desktop.

const QUERY_KEY = 'pythinker_desktop';
const PLATFORM_KEY = 'platform';
const STORAGE_KEY = 'pythinker-desktop';
const PLATFORM_STORAGE_KEY = 'pythinker-desktop-platform';
...
function detect(): DesktopEnv {
  // `__PYTHINKER_WEB_DESKTOP__` is injected by Vite `define`, but that replacement
  ...
  let desktop = typeof __PYTHINKER_WEB_DESKTOP__ !== 'undefined' ? __PYTHINKER_WEB_DESKTOP__ : false;
...
/** True when running inside the Pythinker Desktop app (any platform). */

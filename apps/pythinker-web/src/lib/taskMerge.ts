...
/**
 * Append the live-only dynamic workflow subagents that a fresh REST `/tasks` list does not
 * contain.
 *
 * REST `/tasks` lists only the main agent's background-task store — it never
 * returns foreground dynamic workflow subagents (kind `'subagent'`), which arrive purely
 ...
 * them, flickering the dynamic workflow/subagent cards (and their live "currently doing"

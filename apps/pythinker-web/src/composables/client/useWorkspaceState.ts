...
import type { ExtendedState, PromptAttachment } from '../usePythinkerWebClient';
...
export interface UseWorkspaceStateDeps {
  ...
  getEventConn: () => PythinkerEventConnection | null;
...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  async function loadOlderMessages(sessionId: string): Promise<void> {
    ...
    try {
      const page = await getPythinkerWebApi().listMessages(sessionId, {
        beforeId,
  ...
  async function finishSessionGroupLoadInBackground(
    ...
    const sessions = [
      ...mapped.loaded.filter((session) => !deletedSessionIds.has(session.id)),
      ...rawState.sessions.filter((session) => !mappedIds.has(session.id) && !deletedSessionIds.has(session.id)),
  ...
  async function loadMoreSessions(workspaceId: string): Promise<void> {
    ...
    try {
      const page = await getPythinkerWebApi().listSessions({
        workspaceId,
      ...
      const fresh = page.items.filter((s) => !existing.has(s.id) && !deletedSessionIds.has(s.id));
  ...
  async function loadAllSessions(): Promise<void> {
    ...
    const result = await listAllSessionsGlobal().catch((error) => {
      console.warn('[pythinker-web] loadAllSessions failed; search covers only loaded sessions', error);
      return null;
    });
    ...
    const sessions = deletedSessionIds.size > 0
      ? allSessions.filter((s) => !deletedSessionIds.has(s.id))
      : allSessions;
    setSessionsPreservingLiveUsage(sessions);
  ...
  async function refreshServerMeta(): Promise<void> {
    const m = await getPythinkerWebApi()
      .getMeta()
      .catch(() => null);
    if (m === null) return;
  ...
  async function createDraftSession(workspaceId: string): Promise<string | null> {
    ...
    if (draftModes.dynamicWorkflowMode) {
      rawState.dynamicWorkflowModeBySession = { ...rawState.dynamicWorkflowModeBySession, [sid]: true };
      saveDynamicWorkflowModeToStorage();
    }
    ...
    draftModes.dynamicWorkflowMode = false;
   ...
   * plan/dynamic_workflow modes here, because skill activation carries only `args`.
   */
  async function startSessionAndActivateSkill(
    ...
    try {
      ...
      // the draft (plan/dynamic_workflow, plus permission via /auto|/yolo). Persist them
      ...
      const dynamicWorkflowMode = rawState.dynamicWorkflowModeBySession[sid] ?? false;
   ...
   * chat prompt itself carries model / thinking / permissionMode / plan / dynamic_workflow
  ...
  function setDynamicWorkflowMode(on: boolean): void {
    const sid = rawState.activeSessionId;
    if (sid) {
      rawState.dynamicWorkflowModeBySession = { ...rawState.dynamicWorkflowModeBySession, [sid]: on };
      saveDynamicWorkflowModeToStorage();
      void persistSessionProfile({ dynamicWorkflowMode: on });
    } else {
      draftModes.dynamicWorkflowMode = on;
    }
  }

  /** Flip dynamic_workflow mode on/off. In manual permission mode, ask before enabling. */
  async function toggleDynamicWorkflowMode(): Promise<void> {
    const sid = rawState.activeSessionId;
    const current = sid ? (rawState.dynamicWorkflowModeBySession[sid] ?? false) : draftModes.dynamicWorkflowMode;
    const on = !current;
    if (on && rawState.permission === 'manual') {
      const ok = await confirm({
        title: t('workspace.dynamicWorkflowEnableTitle'),
        message: t('workspace.dynamicWorkflowEnableConfirm'),
        variant: 'primary',
      });
      if (!ok) return;
    }
  ...
  /** Persist goal mode for the active session. Unlike plan/dynamic_workflow, this is a
  ...
  async function generateSessionTitle(id: string): Promise<string | null> {
    try {
      const api = getPythinkerWebApi();
      const data = await api.generateSessionTitle(id, { force: true, source: 'digest' });
      return data.title.length > 0 ? data.title : null;
    } catch (error) {
      console.warn('[pythinker-web] generateSessionTitle failed for', id, error);
      return null;
    }
  }
  ...
  async function renameWorkspace(id: string, name: string): Promise<void> {
    ...
    try {
      await getPythinkerWebApi().updateWorkspace(id, { name });
  ...
  async function exportSession(targetSessionId?: string): Promise<boolean> {
    ...
    try {
      ...
      const { blob, fileName } = await getPythinkerWebApi().exportSession(sessionId, webLog);
  ...
  async function restoreSession(id: string): Promise<boolean> {
    try {
      const restored = await getPythinkerWebApi().restoreSession(id);
  ...
  function loadArchivedSessions(input?: { beforeId?: string; pageSize?: number }) {
    return getPythinkerWebApi().listSessions({
      archivedOnly: true,

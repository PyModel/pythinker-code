...
import type { AppMessage, PythinkerEventConnection, ThinkingLevel } from '../../api/types';
...
export function useSideChat(rawState: ExtendedState, deps: UseSideChatDeps) {
   ...
   *  permissionMode / plan / dynamic_workflow so the turn matches the UI regardless of
   ...
   *  controls (model, thinking, permissionMode, plan/dynamic_workflow) the UI shows so a

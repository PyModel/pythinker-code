...
import { buildDynamicWorkflowGroups, countDynamicWorkflowMembers } from './dynamic_workflowGroups';
import type { DynamicWorkflowGroup } from './dynamic_workflowGroups';
import type {
  ActivityState,
  ActivationBadges,
  ApprovalBlock,
...
/** UI theme: 'terminal' = dense line look, 'modern' = bubbles everywhere,
    ...
    pythinkerDark interaction accent, PingFang/Geist type). */
...
// Accent / colour scheme: 'blue' (Pythinker blue, default) or 'mono' (black/white,
...
async function loadTasksForSession(sessionId: string): Promise<void> {
  try {
    ...
    rawState.tasksBySession = {
      ...rawState.tasksBySession,
      // Keep WS-delivered dynamic_workflow subagents that REST /tasks omits (see keepLiveSubagents).
...
/** Send a plain prompt to the side-chat child (no plan/dynamic_workflow/goal modes). */
...
const status = computed<ConversationStatus>(() => {
  ...
  // the provider prefix (e.g. "pymodel/moonshot-v1-128k" → "moonshot-v1-128k").
...
/** Persist and apply dynamic_workflow mode (pushed to the session profile + sent per-prompt). */
...
/** Flip dynamic_workflow mode on/off. In manual permission mode, ask before enabling. */
function toggleDynamicWorkflowMode(): void {
  const on = !rawState.dynamicWorkflowMode;
  if (on && rawState.permission === 'manual') {
    const ok = confirm('Enable dynamic_workflow mode? The agent will run multiple sub agents in parallel.');
    if (!ok) return;
  }
  setDynamicWorkflowMode(on);
}

/** Persist goal mode locally. Unlike plan/dynamic_workflow, this is a one-shot flag consumed on send. */

...
// background subagents persist) is gone. Dynamic Workflows are handled via
// buildDynamicWorkflowGroups, so they are deliberately NOT matched here.
    ...
    for (const c of content) {
      if (c.type === 'text') {
        ...
        if (agentTasks && agentTasks.length > 0) {
          // A multi-member Dynamic Workflow (subagents sharing a parent tool-call,
          // each with an index) renders as its own card in the chat flow. Do not
          // also render it inline here.
          const dynamicWorkflowMembers = agentTasks.filter((t) => t.dynamicWorkflowIndex !== undefined);
          if (dynamicWorkflowMembers.length > 1) continue;

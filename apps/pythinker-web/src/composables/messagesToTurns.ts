...
// as several "pythinker >" blocks.  Snapshot messages may omit promptId, so user
...
// background subagents persist) is gone. DynamicWorkflows use 'AgentDynamicWorkflow' and are handled
// via buildDynamicWorkflowGroups, so they are deliberately NOT matched here.
...
function sortAgentTasks(a: AppTask, b: AppTask): number {
  const ai = a.dynamicWorkflowIndex ?? Number.MAX_SAFE_INTEGER;
  const bi = b.dynamicWorkflowIndex ?? Number.MAX_SAFE_INTEGER;
  if (ai !== bi) return ai - bi;
  return a.createdAt.localeCompare(b.createdAt);
}

// ---------------------------------------------------------------------------
// Inline buildApprovalBlock (mirrors the one in usePythinkerWebClient.ts; kept
    ...
    for (const c of content) {
      if (c.type === 'text') {
        ...
        if (agentTasks && agentTasks.length > 0) {
          // A multi-member dynamic_workflow (subagents sharing a parent tool-call, each with
          // a dynamicWorkflowIndex) renders as its OWN DynamicWorkflowCard in the chat flow — see
          // buildDynamicWorkflowGroups, same membership test. Don't ALSO render it inline
          // here, or the dynamic_workflow shows up twice ("two blocks").
          const dynamic_workflowMembers = agentTasks.filter((t) => t.dynamicWorkflowIndex !== undefined);
          if (dynamic_workflowMembers.length > 1) continue;

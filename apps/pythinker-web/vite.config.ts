...
const backendPresets = {
  default: process.env.PYTHINKER_BACKEND_DEFAULT_URL || 'http://127.0.0.1:58627',
  multi: process.env.PYTHINKER_BACKEND_MULTI_URL || 'http://127.0.0.1:58628',
} as const;
type BackendName = keyof typeof backendPresets;
// Where the dev proxy forwards server traffic. Defaults to the `default`
// preset; PYTHINKER_SERVER_URL pins the initial target (and disables nothing — the
// dev switcher can still move it at runtime).
const serverTarget = process.env.PYTHINKER_SERVER_URL || backendPresets.default;
 ...
 *   GET  /__pythinker-dev/backend           → { current, presets }
 *   POST /__pythinker-dev/backend { name }  → switch to presets[name]
...
function backendSwitcherPlugin(): Plugin {
  ...
  return {
    name: 'pythinker-backend-switcher',
    configureServer(server) {
      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
        if (req.url !== '/__pythinker-dev/backend') return next();
        if (req.method === 'GET') {
          sendJson(res, state());
          return;
        }
...
export default defineConfig({
  plugins: [
    ...
    Icons({
      compiler: 'vue3',
      // Local Pythinker Design System icons (24×24 outlined, fill="currentColor"),
      // copied from the design-system icon pack into src/icons/pythinker/ and
      // imported as `~icons/pythinker/<file-name>` (plus `?raw`), same as the ri
      // collection. Registered in src/lib/icons.ts only.
      customCollections: {
        pythinker: FileSystemIconLoader(fileURLToPath(new URL('./src/icons/pythinker', import.meta.url))),
      },
    }),
  ],
  ...
  define: {
    __PYTHINKER_DEV_PROXY_TARGET__: JSON.stringify(serverTarget),
    // Named backend presets for the Sidebar switcher menu (dev only).
    __PYTHINKER_DEV_BACKENDS__: JSON.stringify(backendPresets),
    __PYTHINKER_WEB_VERSION__: JSON.stringify(pkg.version),
    // True only for the web bundle embedded in the Pythinker Desktop app (set by the
    ...
    __PYTHINKER_WEB_DESKTOP__: JSON.stringify(process.env.PYTHINKER_WEB_DESKTOP === '1'),
  },
  server: {
    port: webPort,
    strictPort: false,

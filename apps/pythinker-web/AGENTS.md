...
- **The Braille thinking indicator (`⣷`) is reserved** for the chat "waiting for the agent's first response" state only, and is rendered solely by `ui/ThinkingIndicator.vue`; every other loading state uses the plain `Spinner`, with one exception: chat working indicators (`components/chat/WorkingIndicator.vue`) show the running mascot (`MascotSprite state="running"`).
- **Run `pnpm --filter @pymodel/pythinker-web check:style`** (`scripts/check-style.mjs`) — it enforces the §06 anti-pattern rules (no-gradient except the Sidebar frost wash, no-glassmorphism except TopBar/Sidebar frost, no-emoji-icon, no-hardcoded-hex/font, radius/z/weight from scale). Do not add new violations.

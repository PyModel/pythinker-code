...
export interface AgentRef {
  ...
  /** 'member' marks one child of an agent group (dynamic_workflow); default is 'child'. */
...
export interface ToolCallFrame {
  ...
  /** Engine tool name, e.g. 'Read' / 'Bash' / 'Agent' / 'AgentDynamicWorkflow'. */
  readonly name: string;
  /**
   * Optional view hint. Dispatch key at the view layer is `view ?? name`, so
   * the server can suggest a renderer (e.g. 'dynamic_workflow') without a new frame kind.
   */
  ...
  /** Agents spawned by this call (Agent tool / AgentDynamicWorkflow members). */

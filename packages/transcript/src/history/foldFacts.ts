...
export function foldWireRecordFacts(
  ...
  for (const record of records) {
    switch (record.type) {
      case 'subagent.spawned': {
        ...
        subagentTasks.set(agentId, taskId);
        const index = typeof record['dynamicWorkflowIndex'] === 'number' ? record['dynamicWorkflowIndex'] : undefined;
  ...
  const modesTouched = planActive !== undefined || dynamicWorkflowActive !== undefined || towerActive !== undefined;

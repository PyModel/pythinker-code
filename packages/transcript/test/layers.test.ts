...
describe('groupMessagesIntoSnapshot (cold path)', () => {
    ...
    const snapshot = groupMessagesIntoSnapshot([
      {
        ...
        content: [
          {
            type: 'video_url',
            videoUrl: { url: 'pythinker-file://file_1', name: 'clip.mp4' },
      ...
      },
      {
        ...
        content: [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'pythinker-file://file_2?path=%2Fcache%2Fshot.png', name: 'shot.png' },
          } as HistoryContentPart,
        ],
        toolCalls: [],
        origin: { kind: 'user' },
    ...
    const snapshot = groupMessagesIntoSnapshot([
      {
        ...
        content: [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'pythinker-file://file_4?path=%2Fcache%2Fshot.png' },
          } as HistoryContentPart,
        ],
        toolCalls: [],
  ...
  it('keeps a pythinker-file ref as a nameless attachment and inline tags as user text', () => {
    const snapshot = snapshotOf(
      { type: 'text', text: 'open <image path="/tmp/other.png"></image> please' },
      {
        type: 'image_url',
        imageUrl: { url: 'pythinker-file://file_3' },
      } as HistoryContentPart,
    );
  ...
  it('keeps a legacy tag+ref pair as prompt text plus the ref-derived attachment', () => {
    const snapshot = snapshotOf(
      ...
      {
        type: 'image_url',
        imageUrl: { url: 'pythinker-file://file_5?path=%2Fcache%2Fshot.png' },
      } as HistoryContentPart,
    );
...
describe('foldWireRecordFacts (cold facts)', () => {
  ...
  it('folds terminal subagent outcomes, background promotion, and a reused member generation', () => {
    const spawned = (subagentId: string, time: number): HistoryWireRecord => ({
      ...
      parentToolCallId: 'dynamic_workflow', runInBackground: false, time,
    });
  ...
  it('folds plan/dynamic_workflow mode records into meta.modes with enter/exit markers', () => {
    const base = baseWithMarker();
    const folded = foldWireRecordFacts(
      [
        { type: 'plan_mode.enter', id: 'plan-1', time: 1000 },
        { type: 'dynamic_workflow_mode.enter', trigger: { kind: 'task' }, time: 2000 },
        { type: 'plan_mode.exit', id: 'plan-1', time: 3000 },
      ],
      base,
    );
    expect(folded.meta.modes).toEqual({ dynamic_workflow: {} });
    const markers = folded.items
      .filter((item) => item.kind === 'marker')
      .map((item) => item.kind === 'marker' && item.marker);
    expect(markers).toEqual(['compaction', 'plan.enter', 'dynamic_workflow.enter', 'plan.exit']);

    const cancelled = foldWireRecordFacts(
      [
        { type: 'plan_mode.enter', id: 'plan-1', time: 1000 },
        { type: 'plan_mode.cancel', time: 2000 },
        { type: 'dynamic_workflow_mode.enter', trigger: { kind: 'tool' }, time: 3000 },
        { type: 'dynamic_workflow_mode.exit', time: 4000 },
      ],
      base,
    );

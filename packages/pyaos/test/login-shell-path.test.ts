 ...
 *
 * Reproduces the "Bash tool can't find local `gh`" report: when pythinker-code
 ...
 *
 * `LocalPyaos.create()` must probe the user's login shell (`$SHELL -l -c
 ...
 * platform. The end-to-end LocalPyaos suite spawns a stub shell and is
...
describe('mergeLoginShellPath', () => {
  ...
  it('skips relative login-shell entries', () => {
    ...
    // spelling — LocalPyaos runs commands from arbitrary workspace
...
describe.skipIf(process.platform === 'win32')('LocalPyaos login-shell PATH enrichment', () => {
  let tempDir: string;
  let originalPath: string | undefined;
  let originalShell: string | undefined;

  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'pyaos-login-path-'));

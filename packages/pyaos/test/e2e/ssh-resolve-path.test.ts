...
import { PyaosFileNotFoundError, SSHPyaos } from '#/ssh';

// ── SSH path resolution: mock SFTP harness ────────────────────────────
//
// This test file validates that SSHPyaos file/dir operations correctly
...
// returns a deterministic response. After each SSHPyaos operation we
// inspect the recorded path to ensure the resolution worked.
//
// The private SSHPyaos constructor is bypassed via `Reflect.construct` —
...
function createMockSftp(state: MockSFTP): SFTPWrapper {
  ...
  const sftp = {
    realpath(path: string, cb: (err: unknown, absPath: string) => void): void {
      ...
      // Otherwise it would be relative — but since SSHPyaos always
...
/**
 * Construct an SSHPyaos with the private constructor bypassed. TS
 ...
 */
function createMockedPyaos(sftp: SFTPWrapper, home: string, cwd: string): SSHPyaos {
  const client = createMockClient();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const CtorAny = SSHPyaos as any;
  // eslint-disable-next-line @typescript-eslint/no-unsafe-call
  return new CtorAny(client, sftp, home, cwd) as SSHPyaos;
}

// ── Tests ─────────────────────────────────────────────────────────────

describe('e2e: SSHPyaos relative path resolution after chdir (mocked SFTP)', () => {
  let state: MockSFTP;
  ...
  beforeEach(() => {
    ...
    pyaos = createMockedPyaos(sftp, '/home/user', '/home/user');
  });
  ...
  describe('readText / readBytes / readLines', () => {
    it('readText resolves relative path against current cwd', async () => {
      ...
      await pyaos.chdir('/remote/tmp');

      state.files.set('/remote/tmp/file.txt', Buffer.from('hello'));

      const text = await pyaos.readText('file.txt');
    ...
    it('readText accepts absolute path unchanged', async () => {
      ...
      const text = await pyaos.readText('/etc/hosts');
    ...
    it('readBytes resolves relative path against current cwd', async () => {
      ...
      await pyaos.chdir('/opt/data');
      state.files.set('/opt/data/blob.bin', Buffer.from([1, 2, 3, 4]));

      const bytes = await pyaos.readBytes('blob.bin');
    ...
    it('readLines resolves relative path against current cwd', async () => {
      ...
      await pyaos.chdir('/var/log');
      state.files.set('/var/log/app.log', Buffer.from('line1\nline2\nline3'));

      const lines: string[] = [];
      for await (const line of pyaos.readLines('app.log')) {
        lines.push(line);
      }
    ...
    it('readLines strips CRLF terminators', async () => {
      ...
      const lines: string[] = [];
      for await (const line of pyaos.readLines('app-crlf.log')) {
        lines.push(line);
      }
  ...
  describe('writeText / writeBytes / appendFile', () => {
    it('writeText resolves relative path against current cwd', async () => {
      ...
      await pyaos.chdir('/workspace');

      const n = await pyaos.writeText('out.txt', 'hello world');
    ...
    it('writeText with mode=a uses appendFile and resolves path', async () => {
      state.dirs.add('/logs');
      await pyaos.chdir('/logs');

      await pyaos.writeText('a.log', 'line1\n', { mode: 'a' });
      await pyaos.writeText('a.log', 'line2\n', { mode: 'a' });
  ...
  describe('stat / mkdir', () => {
    ...
    it('stat preserves NO_SUCH_FILE as PyaosFileNotFoundError', async () => {
      state.dirs.add('/srv');
      await pyaos.chdir('/srv');

      await expect(pyaos.stat('missing.json')).rejects.toBeInstanceOf(PyaosFileNotFoundError);
      await expect(pyaos.stat('missing.json')).rejects.toMatchObject({ code: 2 });
    });
    ...
    it('stat with followSymlinks=false uses lstat and resolves path', async () => {
      ...
      await pyaos.stat('link', { followSymlinks: false });
    ...
    it('mkdir with parents=true resolves and creates intermediate dirs', async () => {
      ...
      await pyaos.mkdir('a/b/c', { parents: true });
    ...
    it('mkdir existOk=true is idempotent against the resolved path', async () => {
      ...
      await expect(pyaos.mkdir('existing', { existOk: false })).rejects.toThrow();
    });
    ...
    it('mkdir with parents=true rejects a raced file collision when existOk=true', async () => {
      ...
      await expect(
        pyaos.mkdir('collision', { parents: true, existOk: true }),
      ).rejects.toBeInstanceOf(PyaosFileExistsError);
    });
  });
  ...
  describe('iterdir', () => {
    ...
    it('iterdir with named relative subdir resolves under cwd', async () => {
      ...
      const entries: string[] = [];
      for await (const entry of pyaos.iterdir('sub')) {
        entries.push(entry);
      }
  ...
  describe('chdir follows realpath and affects subsequent relative ops', () => {
    it('consecutive chdir + readText rebase the working directory each time', async () => {
      ...
      await pyaos.chdir('/a');
      expect(await pyaos.readText('one.txt')).toBe('A');

      await pyaos.chdir('/b');
      expect(await pyaos.readText('two.txt')).toBe('B');

      // After chdir(/b), a relative path that only exists under /a
      // must NOT resolve — confirming we've truly rebased.
      await expect(pyaos.readText('one.txt')).rejects.toThrow();
    });
    ...
    it('chdir with a relative argument resolves against the prior cwd', async () => {
      ...
      await pyaos.chdir('/home/user');
      await pyaos.chdir('project');

      expect(pyaos.getcwd()).toBe('/home/user/project');

      state.files.set('/home/user/project/README.md', Buffer.from('readme'));
      expect(await pyaos.readText('README.md')).toBe('readme');
    });
  });
});

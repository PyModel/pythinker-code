...
// ── E2E: exec edge cases ──────────────────────────────────────────────
//
// Covers exec() scenarios that the other pyaos suites do not touch:
//
...
//   - proving that each LocalPyaos instance carries its OWN cwd into
...
describe('e2e: exec edge cases', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'pyaos-exec-edge-')));
    await pyaos.chdir(tempDir);
  });

  afterEach(async () => {
    // Restore original cwd in case any test accidentally mutated it
    // (though LocalPyaos should never touch process.cwd()).
  ...
  describe('spawning a non-existent command', () => {
    it('exec() rejects promptly when the binary does not exist (never hangs)', async () => {
      // Contract: LocalPyaos.exec() awaits the child's 'spawn' or 'error' event
      // before returning, so a missing binary becomes a synchronous rejection
      // rather than a ghost process handle.
      await expect(pyaos.exec('this-binary-does-not-exist-pyaos-edge-test-12345')).rejects.toThrow(
  ...
  describe('kill() terminates a running child', () => {
    it.skipIf(process.platform === 'win32')('long-running child can be killed with SIGTERM', async () => {
      ...
      const proc = await pyaos.exec('node', '-e', 'setInterval(() => {}, 1000 * 60);');
  ...
  describe('cwd isolation for concurrent instances', () => {
    it('two LocalPyaos instances with different cwds run concurrent child processes that each see their own cwd', async () => {
      const subA = join(tempDir, 'A');
      const subB = join(tempDir, 'B');

      await pyaos.mkdir(subA);
      await pyaos.mkdir(subB);

      const pyaosA = await LocalPyaos.create();
      const pyaosB = await LocalPyaos.create();
      await pyaosA.chdir(subA);
      await pyaosB.chdir(subB);
      ...
      const [procA, procB] = await Promise.all([
        pyaosA.exec('node', '-e', 'process.stdout.write(process.cwd())'),
        pyaosB.exec('node', '-e', 'process.stdout.write(process.cwd())'),
      ]);
      ...
      // Each child's cwd MUST equal its pyaos instance's cwd.
    ...
    it('execWithEnv honors the per-instance cwd and injects env vars', async () => {
      const proc = await pyaos.execWithEnv(
        ['node', '-e', 'process.stdout.write(process.env.PYAOS_TEST_MARKER + "|" + process.cwd())'],
        { PYAOS_TEST_MARKER: 'beacon42', PATH: process.env['PATH'] ?? '' },
      );
      const stdout = await streamToString(proc.stdout);
      const exitCode = await proc.wait();

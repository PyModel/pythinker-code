...
// ── Tests: SSHPyaos parameter validation ──────────────────────────────
//
// SSHPyaos.create() requires a live SSH connection to proceed past
...
describe('e2e: SSH mock tests', () => {
  describe('SSHPyaos.create() parameter validation', () => {
    it('missing host -> connect attempt with empty host', async () => {
      // SSHPyaos.create() passes the host directly to ssh2's connect().
  ...
  describe('SSHPyaosOptions type constraints', () => {
    it('options structure has required fields', () => {
      // Verify at compile time and runtime that SSHPyaosOptions

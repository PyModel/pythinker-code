...
describe('getCurrentPyaos', () => {
  it('returns the LocalPyaos bound by the test setup', () => {
    const pyaos = getCurrentPyaos();
    expect(pyaos).toBeInstanceOf(LocalPyaos);
    expect(pyaos.name).toBe('local');
  });
});

describe('module-level proxy functions', () => {
  it('normpath delegates to the current pyaos instance', () => {
    // LocalPyaos on posix normalizes '/foo/../bar' to '/bar'
    const result = normpath('/foo/../bar');
    expect(typeof result).toBe('string');
    expect(result.endsWith('bar')).toBe(true);
  });

  it('pathClass returns posix or win32 from the current pyaos', () => {
    const result = pathClass();
    expect(result === 'posix' || result === 'win32').toBe(true);
  });

  it('readLines proxies to the current pyaos and yields lines', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pyaos-readlines-'));
    try {
  ...
  it('writeText accepts an encoding option through the module-level proxy', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pyaos-writetext-enc-'));
    try {
  ...
  it('execWithEnv proxies to the current pyaos', async () => {
    // Use the real LocalPyaos to run `env | grep CUSTOM_VAR`

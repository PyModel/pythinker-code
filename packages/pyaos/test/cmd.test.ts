...
describe.skipIf(process.platform !== 'win32')('LocalPyaos cmd.exe', () => {
  let pyaos: Pyaos;
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'pyaos-cmd-'));
  ...
  it('should run a simple command', async () => {
    const { exitCode, stdout, stderr } = await runCmd(pyaos, 'echo Hello Windows');
  ...
  it('should handle command with error exit', async () => {
    ...
    const { exitCode, stdout, stderr } = await runCmd(pyaos, 'exit /b 1');
  ...
  it('should support command chaining', async () => {
    const { exitCode, stdout, stderr } = await runCmd(pyaos, 'echo First&& echo Second');
    expect(exitCode).toBe(0);
  ...
  it('should perform file operations', async () => {
    // Write via pyaos (avoids cmd.exe redirect quoting quirks on Windows where
    ...
    const read = await runCmd(pyaos, `type ${filePath}`);

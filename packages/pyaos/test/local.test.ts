...
import { PyaosFileExistsError } from '#/errors';
import { LocalPyaos } from '#/local';
import { afterEach, beforeEach, describe, expect, it, test } from 'vitest';

// LocalPyaos normalizes every path to forward slashes (pathe). Mirror that in
...
describe('LocalPyaos', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = toPosix(await realpath(await mkdtemp(join(tmpdir(), 'pyaos-test-'))));
  ...
  describe('chdir + stat', () => {
    it('should change directory and stat a file', async () => {
      ...
      await pyaos.mkdir(nested);

      await pyaos.chdir(nested);
      expect(pyaos.getcwd()).toBe(nested);

      const filePath = join(nested, 'file.txt');
      await pyaos.writeText(filePath, 'hello world');

      const statResult = await pyaos.stat(filePath);
    ...
    it('should reject chdir into a regular file', async () => {
      ...
      await expect(pyaos.chdir(filePath)).rejects.toThrow(/Not a directory/);
    });
  ...
  describe('iterdir path normalization', () => {
    it('should produce normalized paths even when the argument has a trailing separator', async () => {
      ...
      // separator.
      await pyaos.writeText(join(tempDir, 'file.txt'), 'x');
  ...
  describe('iterdir + glob', () => {
    it('should list directory entries and match glob patterns', async () => {
      await pyaos.mkdir(join(tempDir, 'alpha'));
      await pyaos.writeText(join(tempDir, 'bravo.txt'), 'bravo');
      await pyaos.writeText(join(tempDir, 'charlie.TXT'), 'charlie');
  ...
  describe('glob hidden files', () => {
    it('should include hidden files in glob results', async () => {
      await pyaos.writeText(join(tempDir, '.gitlab-ci.yml'), 'stages: [build]');
      await pyaos.writeText(join(tempDir, 'config.yml'), 'key: value');

      const matched: string[] = [];
      for await (const entry of pyaos.glob(tempDir, '*.yml')) {
        matched.push(entry);
      }
    ...
    it('should glob through hidden directories with ** pattern', async () => {
      await pyaos.mkdir(join(tempDir, 'src'));
      await pyaos.mkdir(join(tempDir, 'src', '.config'));
      await pyaos.writeText(join(tempDir, 'src', '.config', 'settings.yml'), 'debug: true');
      await pyaos.writeText(join(tempDir, 'src', 'main.ts'), 'pass');

      const deepMatched: string[] = [];
      for await (const entry of pyaos.glob(tempDir, 'src/**/*.yml')) {
  ...
  describe('readText/writeText', () => {
    it('should write, read, append, and readLines', async () => {
      ...
      const written = await pyaos.writeText(filePath, 'line1');
      ...
      const lines: string[] = [];
      for await (const line of pyaos.readLines(filePath)) {
        lines.push(line);
      }
  ...
  describe('readLines streaming', () => {
    async function collectLines(path: string, options?: Parameters<LocalPyaos['readLines']>[1]) {
      const lines: string[] = [];
      for await (const line of pyaos.readLines(path, options)) {
        lines.push(line);
      }
      return lines;
    }
    ...
    it('keeps utf16le and hex on the decode-then-split path', async () => {
      ...
      await pyaos.writeBytes(utf16Path, Buffer.from('a\n\u0A41\n', 'utf16le'));
      ...
      await pyaos.writeBytes(hexPath, Buffer.from('a\nb'));
    ...
    it('throws lazily when strict UTF-8 errors appear after the first line', async () => {
      ...
      await pyaos.writeBytes(filePath, Buffer.concat([Buffer.from('ok\n', 'utf-8'), Buffer.from([0xff])]));
      const gen = pyaos.readLines(filePath);
  ...
  describe('scanTextFile', () => {
    it('counts lines and classifies line endings', async () => {
      ...
      await pyaos.writeText(lf, 'a\nb');
      await expect(pyaos.scanTextFile(lf)).resolves.toMatchObject({
        totalLines: 2,
      ...
      await pyaos.writeText(crlf, 'a\r\nb\r\n');
      await expect(pyaos.scanTextFile(crlf)).resolves.toMatchObject({
        totalLines: 2,
      ...
      await pyaos.writeText(loneCr, 'a\rB\n');
      await expect(pyaos.scanTextFile(loneCr)).resolves.toMatchObject({
        totalLines: 1,
    ...
    it('detects NUL and invalid UTF-8', async () => {
      ...
      await pyaos.writeBytes(nul, Buffer.from('a\u0000b\n', 'utf-8'));
      await expect(pyaos.scanTextFile(nul)).resolves.toMatchObject({ hasNul: true });

      const invalid = join(tempDir, 'invalid.txt');
      await pyaos.writeBytes(invalid, Buffer.from([0xff]));
      await expect(pyaos.scanTextFile(invalid)).rejects.toThrow();
    });
  });
  ...
  describe('readLineRange', () => {
    async function collectRange(path: string, startLine: number, maxLines: number) {
      const lines: string[] = [];
      for await (const line of pyaos.readLineRange(path, { startLine, maxLines })) {
        lines.push(line);
      }
      return lines;
    }
  ...
  describe('readTailLines', () => {
    async function collectTail(path: string, tailCount: number) {
      const lines: string[] = [];
      for await (const line of pyaos.readTailLines(path, { tailCount })) {
        lines.push(line);
      }
      return lines;
    }
    ...
    it('reads last lines with and without trailing newline', async () => {
      ...
      await pyaos.writeText(noTrailing, 'a\nb\nc');
  ...
  describe('readText errors parameter (Python compat)', () => {
    ...
    it('throws on invalid utf-8 with errors="strict" (default)', async () => {
      ...
      await expect(pyaos.readText(filePath)).rejects.toThrow();
      await expect(pyaos.readText(filePath, { errors: 'strict' })).rejects.toThrow();
    });
    ...
    it('preserves valid U+FFFD characters with errors="ignore"', async () => {
      ...
      await pyaos.writeBytes(filePath, data);
  ...
  describe('LF preservation', () => {
    it('should not convert LF to CRLF', async () => {
      ...
      const raw = await pyaos.readBytes(filePath);
  ...
  describe('mkdir recursive', () => {
    it('should create nested directories with parents option', async () => {
      ...
      await pyaos.mkdir(nested, { parents: true });

      const s = await pyaos.stat(nested);
    ...
    it('should throw when parents:true + existOk:false on existing dir', async () => {
      ...
      await pyaos.mkdir(existing);

      await expect(pyaos.mkdir(existing, { parents: true, existOk: false })).rejects.toThrow();
    });
    ...
    it('should throw when existOk:true but conflicting path is a file', async () => {
      ...
      await expect(pyaos.mkdir(filePath, { existOk: true })).rejects.toBeInstanceOf(
  ...
  describe('glob character class negation', () => {
    it('[!a] should match non-a files (glob negation, not literal `!`)', async () => {
      ...
      await pyaos.writeText(join(tempDir, 'a.txt'), '');
      await pyaos.writeText(join(tempDir, 'b.txt'), '');
      await pyaos.writeText(join(tempDir, '!.txt'), '');

      const matches: string[] = [];
      for await (const m of pyaos.glob(tempDir, '[!a].txt')) {
        matches.push(m);
      }
  ...
  describe('glob ** pattern', () => {
    it('should not return duplicates when ** matches nested paths', async () => {
      ...
      await pyaos.mkdir(join(tempDir, 'a', 'b'), { parents: true });
      await pyaos.writeText(join(tempDir, 'a', 'b', 'file.txt'), 'content');

      const matches: string[] = [];
      for await (const m of pyaos.glob(tempDir, '**/*.txt')) {
        matches.push(m);
      }
    ...
    it('should match ** at multiple depths without duplicates', async () => {
      ...
      await pyaos.writeText(join(tempDir, 'root.txt'), '');
      await pyaos.writeText(join(tempDir, 'a', 'mid.txt'), '');
      await pyaos.writeText(join(tempDir, 'a', 'b', 'c', 'deep.txt'), '');
    ...
    it('should not duplicate matches for deep ** patterns', async () => {
      ...
      await pyaos.mkdir(join(tempDir, 'l1', 'l2', 'l3', 'l4'), { parents: true });
      await pyaos.writeText(join(tempDir, 'l1', 'l2', 'l3', 'l4', 'deep.txt'), 'x');
    ...
    it('should yield basePath and every recursive entry when the whole pattern is **', async () => {
      ...
      await pyaos.mkdir(join(tempDir, 'sub'));
      await pyaos.writeText(join(tempDir, 'root.txt'), 'r');
      await pyaos.writeText(join(tempDir, 'sub', 'nested.txt'), 'n');

      const matches: string[] = [];
      for await (const m of pyaos.glob(tempDir, '**')) {
        matches.push(m);
      }
  ...
  describe('glob symlink cycle safety', () => {
    ...
    it('T-C3 legit non-cyclic symlink to a sibling tree is followed', async () => {
      ...
      const matches: string[] = [];
      for await (const m of pyaos.glob(root, '**/*.txt')) {
        matches.push(m);
      }
    ...
    it('T-C5 regression — non-symlink tree results are unchanged', async () => {
      ...
      await pyaos.writeText(join(tempDir, 'a', 'r2.txt'), '');
      await pyaos.writeText(join(tempDir, 'a', 'b', 'c', 'r3.txt'), '');
  ...
  describe('readBytes/writeBytes', () => {
    it('should round-trip binary data', async () => {
      ...
      const written = await pyaos.writeBytes(filePath, data);
      expect(written).toBe(4);

      const read = await pyaos.readBytes(filePath);
  ...
  describe('exec streaming', () => {
    it('should run a command and stream stdout/stderr', async () => {
      ...
      const proc = await pyaos.exec(...nodeArgs(code));
  ...
  describe('exec non-zero exit', () => {
    it('should return the correct exit code', async () => {
      const proc = await pyaos.exec(...nodeArgs('process.exit(7)'));
      const exitCode = await proc.wait();
  ...
  describe('exec spawn failure', () => {
    it('should reject when the binary does not exist', async () => {
      await expect(pyaos.exec('/absolutely/non-existent/binary')).rejects.toThrow();
    });
    ...
    it('should reject exec() with no arguments', async () => {
      ...
      await expect((pyaos.exec as () => Promise<unknown>)()).rejects.toThrow(
    ...
    it('should reject execWithEnv() with an empty args array', async () => {
      ...
      await expect(pyaos.execWithEnv([])).rejects.toThrow(/at least one argument/);
    });
  });

  describe('exec timeout', () => {
    it('dispose destroys process stdio without killing the process', async () => {
      const proc = await pyaos.exec(...nodeArgs('setTimeout(() => {}, 10000);'));
  ...
  describe('withEnv', () => {
    it('overlays every spawned process and can be updated in place', async () => {
      const env = {
        PYAOS_BASE_ENV: 'initial',
        PYAOS_COLLISION_ENV: 'configured',
      };
      const envPyaos = pyaos.withEnv(env);
      const printEnv =
        'process.stdout.write(`${process.env.PYAOS_BASE_ENV}|${process.env.PYAOS_COLLISION_ENV}|${process.env.PYAOS_CALL_ENV}`)';

      const first = await envPyaos.exec('node', '-e', printEnv);
      expect(await first.wait()).toBe(0);
      expect((await streamToBuffer(first.stdout)).toString('utf-8')).toBe('initial|configured|undefined');

      const second = await envPyaos.execWithEnv(['node', '-e', printEnv], {
        ...(process.env as Record<string, string>),
        PYAOS_COLLISION_ENV: 'host',
        PYAOS_CALL_ENV: 'call',
      });
      expect(await second.wait()).toBe(0);
      expect((await streamToBuffer(second.stdout)).toString('utf-8')).toBe('initial|configured|call');

      env.PYAOS_BASE_ENV = 'updated';
      const third = await envPyaos.exec('node', '-e', printEnv);
...
describe('LocalPyaos instance isolation', () => {
  test('instances have isolated cwds (no process.cwd pollution)', async () => {
    const pyaosA = await LocalPyaos.create();
    const pyaosB = await LocalPyaos.create();

    const tmpA = toPosix(await realpath(await mkdtemp(join(tmpdir(), 'pyaos-a-'))));
    const tmpB = toPosix(await realpath(await mkdtemp(join(tmpdir(), 'pyaos-b-'))));

    try {
      await pyaosA.chdir(tmpA);
      await pyaosB.chdir(tmpB);

      // pyaosA.chdir must not affect pyaosB's cwd (no process.chdir pollution).
      expect(pyaosA.getcwd()).toBe(tmpA);
      expect(pyaosB.getcwd()).toBe(tmpB);
      ...
      const procA = await pyaosA.exec('node', '-e', 'process.stdout.write(process.cwd())');
      const procB = await pyaosB.exec('node', '-e', 'process.stdout.write(process.cwd())');
      await procA.wait();
      await procB.wait();
...
describe('LocalProcess.kill safety', () => {
  test('kill() is safe when spawn failed (pid -1 must not signal process group)', async () => {
    ...
    let proc;
    try {
      proc = await pyaos.exec('this-command-does-not-exist-xyz123');
    } catch {
  ...
  test.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      const pyaos = await LocalPyaos.create();
      const tmp = await realpath(await mkdtemp(join(tmpdir(), 'pyaos-killtree-')));
      try {
        ...
        const proc = await pyaos.exec('node', scriptPath, pidPath);
      ...
      try {
        ...
        const script = `
          node -e 'const { spawn } = require("node:child_process");
            ...
            setInterval(() => {}, 1000);'
        `;
        const proc = await pyaos.exec('bash', '-c', script);

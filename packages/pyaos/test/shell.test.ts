...
async function runSh(
  ...
  const proc: PyaosProcess = await pyaos.exec('/bin/sh', '-c', command);
...
describe.skipIf(process.platform === 'win32')('LocalPyaos shell operations', () => {
  let pyaos: Pyaos;
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'pyaos-shell-'));
  ...
  // NOTE: These tests mirror Python test_local_pyaos_sh.py one-for-one.
  ...
  it('should run a simple command', async () => {
    const result = await runSh(pyaos, "echo 'Hello World'");
  ...
  it('should handle command with error', async () => {
    const result = await runSh(pyaos, 'ls /nonexistent/directory');
  ...
  it('should support command chaining with &&', async () => {
    const result = await runSh(pyaos, "echo 'First' && echo 'Second'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support command pipe', async () => {
    const result = await runSh(pyaos, "echo 'Hello World' | wc -w");
    expect(result.exitCode).toBe(0);
  ...
  it('should handle command with timeout (completes before timeout)', async () => {
    ...
    const result = await runSh(pyaos, 'sleep 0.1', { timeout: 5000 });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe('');
    expect(result.stderr).toBe('');
  });
  ...
  it('should pass environment variables to shell', async () => {
    const result = await runSh(pyaos, 'TEST_VAR=\'test_value\'; export TEST_VAR; echo "$TEST_VAR"');
    expect(result.exitCode).toBe(0);
  ...
  it('should perform file operations', async () => {
    // Mirror Python test_file_operations: two separate pyaos.exec calls so
    ...
    const write = await runSh(pyaos, `echo 'Test content' > "${filePath}"`);
    ...
    const read = await runSh(pyaos, `cat "${filePath}"`);
  ...
  it('should handle stdin data', async () => {
    ...
    const result = await runSh(pyaos, 'read value; printf \'%s\\n\' "$value"', {
  ...
  it('should execute commands sequentially with ;', async () => {
    const result = await runSh(pyaos, "echo 'One'; echo 'Two'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support conditional execution with ||', async () => {
    const result = await runSh(pyaos, "false || echo 'Success'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support multiple pipes', async () => {
    const result = await runSh(pyaos, "printf '1\\n2\\n3\\n' | grep '2' | wc -l");
    expect(result.exitCode).toBe(0);
  ...
  it('should handle text processing with sed', async () => {
    const result = await runSh(pyaos, "echo 'apple banana cherry' | sed 's/banana/orange/'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support command substitution', async () => {
    const result = await runSh(pyaos, 'echo "Result: $(echo hello)"');
    expect(result.exitCode).toBe(0);
  ...
  it('should support arithmetic substitution', async () => {
    const result = await runSh(pyaos, 'echo "Answer: $((2 + 2))"');
    expect(result.exitCode).toBe(0);
  ...
  it('should handle very long output', async () => {
    const result = await runSh(pyaos, 'seq 1 100 | head -50');
    expect(result.exitCode).toBe(0);

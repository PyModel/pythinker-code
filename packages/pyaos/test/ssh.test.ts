...
import { PyaosFileExistsError, PyaosValueError } from '#/errors';
import {
  PyaosConnectionError,
  PyaosFileNotFoundError,
  PyaosPermissionError,
  PyaosSSHError,
  SSHPyaos,
} from '#/ssh';
...
const SSH_SMOKE = process.env['PYAOS_SSH_SMOKE'] === '1';
const SSH_HOST = process.env['PYAOS_SSH_HOST'] ?? '127.0.0.1';
const SSH_PORT = Number(process.env['PYAOS_SSH_PORT'] ?? '22');
const SSH_USERNAME = process.env['PYAOS_SSH_USERNAME'];
const SSH_PASSWORD = process.env['PYAOS_SSH_PASSWORD'];
const SSH_KEY_PATHS = process.env['PYAOS_SSH_KEY_PATHS']?.split(',').filter(Boolean);
const SSH_KEY_CONTENTS = process.env['PYAOS_SSH_KEY_CONTENTS']?.split('|||').filter(Boolean);
...
// Explicit opt-in smoke: set PYAOS_SSH_SMOKE=1 plus SSH credentials.
describe.skipIf(process.platform === 'win32' || !SSH_SMOKE)('SSHPyaos smoke', () => {
  let sshPyaos: SSHPyaos;
  let remoteBase = '';

  beforeAll(async () => {
    if (SSH_USERNAME === undefined) {
      throw new Error('PYAOS_SSH_SMOKE=1 requires PYAOS_SSH_USERNAME');
    }

    // Dynamic import to avoid compilation errors when ssh2 is not available
    const { SSHPyaos: SSHPyaosClass } = await import('#/ssh');
    sshPyaos = await SSHPyaosClass.create({
      host: SSH_HOST,
      port: SSH_PORT,
  ...
  beforeEach(async () => {
    ...
    remoteBase = `${sshPyaos.gethome()}/.pyaos_test_${process.pid}_${uuid}`;
    await sshPyaos.mkdir(remoteBase, { parents: true, existOk: true });
    await sshPyaos.chdir(remoteBase);
  });
  ...
  afterEach(async () => {
    ...
    if (remoteBase.length > 0) {
      try {
        const proc = await sshPyaos.exec('rm', '-rf', remoteBase);
        await proc.wait();
      } finally {
        remoteBase = '';
        await sshPyaos.chdir(sshPyaos.gethome());
      }
    }
  });

  afterAll(async () => {
    if (sshPyaos) await sshPyaos.close();
  });

  test('pathClass, home, and cwd', () => {
    const home = sshPyaos.gethome();
    const cwd = sshPyaos.getcwd();

    expect(sshPyaos.pathClass()).toBe('posix');
  ...
  test('cwd defaults to home immediately after connect', () => {
    ...
    expect(sshPyaos.getcwd()).toBe(sshPyaos.gethome());
  });

  test('chdir updates real path', async () => {
    await sshPyaos.chdir(remoteBase);
    expect(sshPyaos.getcwd()).toBe(remoteBase);

    await sshPyaos.mkdir(remoteBase + '/child', { existOk: true });
    await sshPyaos.chdir('child');
    expect(sshPyaos.getcwd()).toBe(remoteBase + '/child');

    await sshPyaos.chdir('..');
    expect(sshPyaos.getcwd()).toBe(remoteBase);
  });
  ...
  test('exec respects cwd', async () => {
    ...
    const proc = await sshPyaos.exec('pwd');
  ...
  test('exec wait before read', async () => {
    const proc = await sshPyaos.exec('echo', 'output');
  ...
  test('mkdir respects existOk', async () => {
    ...
    await sshPyaos.mkdir(nestedDir, { parents: true, existOk: false });

    // Python test_mkdir_respects_exist_ok pins `pytest.raises(FileExistsError)`
    // — match that strength by asserting the specific PyaosFileExistsError class
    // rather than any throwable.
    await expect(sshPyaos.mkdir(nestedDir, { existOk: false })).rejects.toBeInstanceOf(
      PyaosFileExistsError,
    );

    await sshPyaos.mkdir(nestedDir, { parents: true, existOk: true });
  });

  test('stat reports directory and file metadata', async () => {
    const dirStat = await sshPyaos.stat(remoteBase, { followSymlinks: false });
    expect((dirStat.stMode & S_IFMT) === S_IFDIR).toBe(true);

    const filePath = remoteBase + '/payload.txt';
    const payload = 'metadata';
    await sshPyaos.writeText(filePath, payload);

    const fileStat = await sshPyaos.stat(filePath);
  ...
  test('file roundtrip via SSH', async () => {
    ...
    const written = await sshPyaos.writeText(textPath, textPayload);
    expect(written).toBe(textPayload.length);

    const appendedLen = await sshPyaos.writeText(textPath, appended, { mode: 'a' });
    expect(appendedLen).toBe(appended.length);

    const fullText = await sshPyaos.readText(textPath);
    expect(fullText).toBe(textPayload + appended);

    const lines: string[] = [];
    for await (const line of sshPyaos.readLines(textPath)) {
      lines.push(line);
    }
    expect(lines).toEqual(['Hello SSH', 'More data']);

    const bytesPayload = Buffer.from(Array.from({ length: 32 }, (_, i) => i));
    const bytesWritten = await sshPyaos.writeBytes(bytesPath, bytesPayload);
    expect(bytesWritten).toBe(bytesPayload.length);

    const roundtrip = await sshPyaos.readBytes(bytesPath);
  ...
  test('iterdir lists child entries', async () => {
    await sshPyaos.writeText(remoteBase + '/file1.txt', '1');
    await sshPyaos.writeText(remoteBase + '/file2.log', '2');
    await sshPyaos.mkdir(remoteBase + '/subdir', { existOk: true });

    const entries: string[] = [];
    for await (const entry of sshPyaos.iterdir(remoteBase)) {
      entries.push(entry);
    }
  ...
  test('glob is case sensitive', async () => {
    await sshPyaos.writeText(remoteBase + '/file.log', 'lowercase');
    await sshPyaos.writeText(remoteBase + '/FILE.LOG', 'uppercase');

    const matches = new Set<string>();
    for await (const path of sshPyaos.glob(remoteBase, '*.log')) {
      matches.add(path);
    }
    ...
    await expect(async () => {
      ...
      for await (const _ of sshPyaos.glob(remoteBase, '*.log', { caseSensitive: false })) {
  ...
  test('exec streams stdout and stderr', async () => {
    const proc = await sshPyaos.exec('sh', '-c', "printf 'out\\n' && printf 'err\\n' 1>&2");
  ...
  // is not configured to accept PYAOS_TEST_*, these tests will fail — which
  ...
  test('execWithEnv delivers a single env var to the remote process', async () => {
    const proc = await sshPyaos.execWithEnv(['sh', '-c', 'printf "%s" "${PYAOS_TEST_MARKER}"'], {
      PYAOS_TEST_MARKER: 'beacon42',
    });
  ...
  test('execWithEnv delivers multiple env vars', async () => {
    const proc = await sshPyaos.execWithEnv(
      ['sh', '-c', 'printf "%s|%s" "${PYAOS_TEST_A}" "${PYAOS_TEST_B}"'],
      { PYAOS_TEST_A: 'hello', PYAOS_TEST_B: 'world' },
    );
    const out = (await streamToBuffer(proc.stdout)).toString();
    const code = await proc.wait();
  ...
  test('execWithEnv preserves values with shell metacharacters', async () => {
    ...
    const proc = await sshPyaos.execWithEnv(['sh', '-c', 'printf "%s" "${PYAOS_TEST_VALUE}"'], {
      PYAOS_TEST_VALUE: value,
    });
    const out = (await streamToBuffer(proc.stdout)).toString();
    const code = await proc.wait();
  ...
  test('exec rejects empty command', async () => {
    await expect((sshPyaos.exec as (...args: string[]) => Promise<unknown>)()).rejects.toThrow();
  });

  test('process kill updates returncode', async () => {
    const proc = await sshPyaos.exec('sh', '-c', 'echo ready; sleep 30');
...
describe('SSHPyaos argument validation', () => {
  it('exec() throws with the correct class name when args is empty', () => {
    const fakeThis = {} as SSHPyaos;
    expect(() => SSHPyaos.prototype.exec.call(fakeThis)).toThrow(PyaosValueError);
    expect(() => SSHPyaos.prototype.exec.call(fakeThis)).toThrow(/SSHPyaos\.exec\(\)/);
  });

  it('execWithEnv() throws with the correct class name when args is empty', () => {
    const fakeThis = {} as SSHPyaos;
    expect(() => SSHPyaos.prototype.execWithEnv.call(fakeThis, [])).toThrow(PyaosValueError);
    expect(() => SSHPyaos.prototype.execWithEnv.call(fakeThis, [])).toThrow(
      /SSHPyaos\.execWithEnv\(\)/,
    );
  });
  ...
  // (PyaosValueError) and the fact that it rejects before touching SFTP.
  it('glob(caseSensitive: false) rejects with PyaosValueError', async () => {
    const instance = Object.create(SSHPyaos.prototype) as SSHPyaos;
    const internal = instance as unknown as { _cwd: string; _sftp: unknown };
    internal._cwd = '/tmp';
    internal._sftp = {};

    const gen = instance.glob('/some/path', '*', { caseSensitive: false });
    await expect(gen.next()).rejects.toBeInstanceOf(PyaosValueError);
  });
});
...
describe('SSHPyaos.chdir directory validation', () => {
  ...
  function makeFakeInstance(sftp: unknown, cwd: string): SSHPyaos {
  ...
  it('rejects a target that resolves to a regular file', async () => {
    ...
    const pyaos = makeFakeInstance(sftp, '/tmp');

    await expect(pyaos.chdir(target)).rejects.toThrow(PyaosValueError);
    await expect(pyaos.chdir(target)).rejects.toThrow(/not a directory/);
    // cwd must remain unchanged on failure.
    expect(pyaos.getcwd()).toBe('/tmp');
  });
  ...
  it('accepts a target that resolves to a directory', async () => {
    ...
    await pyaos.chdir(target);
    expect(pyaos.getcwd()).toBe(target);
  });
});

// These tests pin the SFTPError → PyaosError mapping contract. They use a
...
// SSHPyaos method that touches SFTP automatically throws a PyaosSSHError
// subclass (PyaosFileNotFoundError / PyaosPermissionError / …) instead of
// the raw ssh2 error.
describe('SSHPyaos SFTP error mapping', () => {
  ...
  // a harmless default is returned. Only the methods that SSHPyaos actually
  ...
  function makeFakePyaos(sftp: unknown): SSHPyaos {
    const instance = Object.create(SSHPyaos.prototype) as SSHPyaos;
  ...
  it('stat() maps NO_SUCH_FILE → PyaosFileNotFoundError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(NO_SUCH_FILE, { stat: true }));
    await expect(pyaos.stat('/missing')).rejects.toBeInstanceOf(PyaosFileNotFoundError);
  });

  it('stat() maps PERMISSION_DENIED → PyaosPermissionError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(PERMISSION_DENIED, { stat: true }));
    await expect(pyaos.stat('/forbidden')).rejects.toBeInstanceOf(PyaosPermissionError);
  });

  it('stat({ followSymlinks: false }) wraps lstat errors the same way', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(NO_SUCH_FILE, { lstat: true }));
    await expect(pyaos.stat('/missing', { followSymlinks: false })).rejects.toBeInstanceOf(
      PyaosFileNotFoundError,
    );
  });

  it('stat() wraps unmapped failures as the base PyaosSSHError', async () => {
    // FAILURE=4 is not specifically mapped → generic PyaosSSHError.
    const pyaos = makeFakePyaos(makeFakeSftp(4, { stat: true }));
    await expect(pyaos.stat('/x')).rejects.toBeInstanceOf(PyaosSSHError);
  });

  it('stat() maps NO_CONNECTION → PyaosConnectionError', async () => {
    // SFTP STATUS_CODE.NO_CONNECTION = 6
    const pyaos = makeFakePyaos(makeFakeSftp(6, { stat: true }));
    await expect(pyaos.stat('/x')).rejects.toBeInstanceOf(PyaosConnectionError);
  });

  it('stat() maps CONNECTION_LOST → PyaosConnectionError', async () => {
    // SFTP STATUS_CODE.CONNECTION_LOST = 7
    const pyaos = makeFakePyaos(makeFakeSftp(7, { stat: true }));
    await expect(pyaos.stat('/x')).rejects.toBeInstanceOf(PyaosConnectionError);
  });

  it('stat() wraps errors without a numeric code as generic PyaosSSHError', async () => {
    ...
    const pyaos = makeFakePyaos(sftp);
    const err = await pyaos.stat('/x').catch((error: unknown) => error);
    expect(err).toBeInstanceOf(PyaosSSHError);
    expect((err as PyaosSSHError).message).toContain('no code here');
  });
  ...
  it('stat() wraps non-Error rejections by stringifying them', async () => {
    ...
    // wrap still produces a PyaosSSHError with the string in the message.
    const sftp = {
      realpath(p: string, cb: (err: Error | null, abs: string) => void): void {
        cb(null, p);
      },
    ...
    const err = await pyaos.stat('/x').catch((error: unknown) => error);
    expect(err).toBeInstanceOf(PyaosSSHError);
    expect((err as PyaosSSHError).message).toContain('raw-string-rejection');
  });

  it('chdir() propagates a realpath failure as a PyaosSSHError', async () => {
    ...
    internal._sftp = sftp;
    internal._cwd = '/';
    await expect(instance.chdir('/missing')).rejects.toBeInstanceOf(PyaosFileNotFoundError);
  });
  ...
  it('readText() maps NO_SUCH_FILE → PyaosFileNotFoundError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(NO_SUCH_FILE, { readFile: true }));
    await expect(pyaos.readText('/missing')).rejects.toBeInstanceOf(PyaosFileNotFoundError);
  });

  it('readBytes() maps NO_SUCH_FILE → PyaosFileNotFoundError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(NO_SUCH_FILE, { readFile: true }));
    await expect(pyaos.readBytes('/missing')).rejects.toBeInstanceOf(PyaosFileNotFoundError);
  });

  it('writeText() maps PERMISSION_DENIED → PyaosPermissionError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(PERMISSION_DENIED, { writeFile: true }));
    await expect(pyaos.writeText('/forbidden', 'data')).rejects.toBeInstanceOf(PyaosPermissionError);
  });

  it('writeText(append) maps PERMISSION_DENIED → PyaosPermissionError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(PERMISSION_DENIED, { appendFile: true }));
    await expect(pyaos.writeText('/forbidden', 'data', { mode: 'a' })).rejects.toBeInstanceOf(
      PyaosPermissionError,
    );
  });

  it('writeBytes() maps PERMISSION_DENIED → PyaosPermissionError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(PERMISSION_DENIED, { writeFile: true }));
    await expect(pyaos.writeBytes('/forbidden', Buffer.from('x'))).rejects.toBeInstanceOf(
      PyaosPermissionError,
    );
  });

  it('mkdir() maps PERMISSION_DENIED → PyaosPermissionError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(PERMISSION_DENIED, { mkdir: true }));
    await expect(pyaos.mkdir('/forbidden')).rejects.toBeInstanceOf(PyaosPermissionError);
  });

  it('iterdir() maps NO_SUCH_FILE → PyaosFileNotFoundError', async () => {
    const pyaos = makeFakePyaos(makeFakeSftp(NO_SUCH_FILE, { readdir: true }));
    const gen = pyaos.iterdir('/missing');
    await expect(gen.next()).rejects.toBeInstanceOf(PyaosFileNotFoundError);
  });
});
...
// env vars is validated by the smoke tests above when PYAOS_SSH_SMOKE=1.
describe('SSHPyaos._buildExecCommand', () => {
  // Bracket access so we can reach the private static helper from tests
  // without changing its visibility in the public API.
  const build = (
    SSHPyaos as unknown as {
  ...
  it('rejects env var names that are not valid POSIX identifiers', () => {
    expect(() => build(['cmd'], '/home/user', { '1BAD': 'x' })).toThrow(PyaosValueError);
    expect(() => build(['cmd'], '/home/user', { 'WITH SPACE': 'x' })).toThrow(PyaosValueError);
    expect(() => build(['cmd'], '/home/user', { 'WITH=EQUALS': 'x' })).toThrow(PyaosValueError);
    expect(() => build(['cmd'], '/home/user', { '': 'x' })).toThrow(PyaosValueError);
  });
...
// These tests drive the SSHPyaos read/stat/glob/iterdir happy paths via
...
describe('SSHPyaos mock success paths', () => {
  interface TreeNode {
    type: 'dir' | 'file';
  ...
  // SSHPyaos actually calls. Anything not needed is left unimplemented.
  ...
  function makeFakePyaos(sftp: unknown, cwd = '/'): SSHPyaos {
    const instance = Object.create(SSHPyaos.prototype) as SSHPyaos;
    const internal = instance as unknown as { _sftp: unknown; _cwd: string; _home: string };
    internal._sftp = sftp;
    internal._cwd = cwd;
  ...
  it('normpath delegates to posix.normalize', () => {
    ...
    // `..` segments.
    const pyaos = makeFakePyaos(makeTreeSftp({ type: 'dir', children: {} }));
    expect(pyaos.normpath('/a/b/../c')).toBe('/a/c');
  });
  ...
  it('stat() returns a StatResult with file-type bits preserved', async () => {
    ...
    const pyaos = makeFakePyaos(makeTreeSftp(root));

    const fileStat: StatResult = await pyaos.stat('/file.txt');
  ...
  it('stat() derives type bits from is* helpers when mode lacks them', async () => {
    ...
    const dirStat = await pyaos.stat('/');
    expect((dirStat.stMode & 0o170000) === 0o040000).toBe(true);

    const fileStat = await pyaos.stat('/bare.txt');
    expect((fileStat.stMode & 0o170000) === 0o100000).toBe(true);
  });
  ...
  it('stat({ followSymlinks: false }) uses the lstat branch', async () => {
    ...
    const pyaos = makeFakePyaos(makeTreeSftp(root));
    const result = await pyaos.stat('/a.txt', { followSymlinks: false });
  ...
  it('iterdir yields directory entries', async () => {
    ...
    const entries: string[] = [];
    for await (const entry of pyaos.iterdir('/tree')) {
      entries.push(entry);
    }
  ...
  it('glob matches flat patterns against the directory', async () => {
    ...
    for await (const m of pyaos.glob('/tree', '*.txt')) {
      matches.push(m);
    }
  ...
  it('glob with bare ** yields basePath and every nested entry', async () => {
    ...
    const matches: string[] = [];
    for await (const m of pyaos.glob('/tree', '**')) {
      matches.push(m);
    }
  ...
  it('glob with a nested literal path recurses only into the named dir', async () => {
    ...
    const matches: string[] = [];
    for await (const m of pyaos.glob('/tree', 'sub/*.txt')) {
      matches.push(m);
    }
  ...
  it('glob silently skips unreadable directories', async () => {
    ...
    const matches: string[] = [];
    for await (const m of pyaos.glob('/locked', '*.txt')) {
      matches.push(m);
    }
  ...
  it('readLines yields nothing for an empty file', async () => {
    ...
    const lines: string[] = [];
    for await (const line of pyaos.readLines('/empty.txt')) {
      lines.push(line);
    }
  ...
  it('readText preserves valid U+FFFD while ignoring invalid utf-8 bytes', async () => {
    ...
    await expect(pyaos.readText('/mixed.txt', { errors: 'ignore' })).resolves.toBe('A\uFFFDBC');
  });
  ...
  it('iterdir at root "/" does not produce double-slash paths', async () => {
    ...
    const entries: string[] = [];
    for await (const entry of pyaos.iterdir('/')) {
      entries.push(entry);
    }
  ...
  it('glob at root "/" does not produce double-slash paths', async () => {
    ...
    const matches: string[] = [];
    for await (const m of pyaos.glob('/', '*.txt')) {
      matches.push(m);
    }
  ...
  it('iterdir filters out "." and ".." entries from readdir output', async () => {
    ...
    // filter in SSHPyaos.iterdir must skip them unconditionally. Inject
    ...
    const pyaos = makeFakePyaos(sftp, '/tree');

    const entries: string[] = [];
    for await (const e of pyaos.iterdir('/tree')) {
      entries.push(e);
    }
  ...
  it('glob filters "." and ".." entries in both the ** and literal branches', async () => {
    ...
    for await (const m of pyaos.glob('/tree', '*.txt')) viaStar.push(m);
    expect(viaStar).toEqual(['/tree/keeper.txt']);

    const viaStarStar: string[] = [];
    for await (const m of pyaos.glob('/tree', '**')) viaStarStar.push(m);
  ...
  it('readBytes(n) returns only the first n bytes of the file', async () => {
    ...
    const full = await pyaos.readBytes('/data.bin');
    expect(Buffer.compare(full, content)).toBe(0);

    const first4 = await pyaos.readBytes('/data.bin', 4);
...
// These tests pin the non-race mkdir error branches in SSHPyaos — the
...
describe('SSHPyaos mkdir existOk edge cases', () => {
  function makeFakePyaos(sftp: unknown): SSHPyaos {
    const instance = Object.create(SSHPyaos.prototype) as SSHPyaos;
    const internal = instance as unknown as { _sftp: unknown; _cwd: string };
    internal._sftp = sftp;
  ...
  it('mkdir (non-parents) rejects when existOk=true but path is a file', async () => {
    ...
    const pyaos = makeFakePyaos(sftp);
    await expect(pyaos.mkdir('/existing-file', { existOk: true })).rejects.toBeInstanceOf(
      PyaosFileExistsError,
    );
  });
  ...
  it('mkdir(parents=true) rejects when the final path exists and existOk=false', async () => {
    ...
    // must surface a PyaosFileExistsError instead of silently succeeding.
    const sftp = {
      realpath(p: string, cb: (err: Error | null, abs: string) => void): void {
        cb(null, p);
      },
    ...
    const pyaos = makeFakePyaos(sftp);
    await expect(pyaos.mkdir('/a/b/c', { parents: true, existOk: false })).rejects.toBeInstanceOf(
      PyaosFileExistsError,
    );
  });
  ...
  it('mkdir(parents=true) rejects when an intermediate path is a regular file', async () => {
    ...
    const pyaos = makeFakePyaos(sftp);
    await expect(pyaos.mkdir('/a/b/c', { parents: true, existOk: true })).rejects.toBeInstanceOf(
      PyaosFileExistsError,
    );
  });
});

describe('SSHPyaos.close lifecycle', () => {
  ...
  function createCloseablePyaos(): SSHPyaos {
    const instance = Object.create(SSHPyaos.prototype) as SSHPyaos;
  ...
  it('awaits the close event before allowing follow-up execs to observe the closed state', async () => {
    const pyaos = createCloseablePyaos();

    await pyaos.close();

    await expect(pyaos.exec('pwd')).rejects.toThrow(/channel closed/);
  });
});

...
import type { SSHPyaos as SSHPyaosType } from '#/ssh';
...
async function loadSSHModule(options: CreateHarnessOptions = {}): Promise<{
  ...
  const { SSHPyaos } = await import('#/ssh');
  return { SSHPyaos, state };
}
...
describe('SSHPyaos.create()', () => {
  it('initializes cwd equal to gethome() when no cwd option is passed', async () => {
    // Pins the Python test_ssh_pyaos.py::test_pathclass_home_and_cwd invariant:
    ...
    const { SSHPyaos } = await loadSSHModule();

    const ssh = await SSHPyaos.create({
      host: 'example.com',
      username: 'tester',
    });
  ...
  it('rejects a cwd that resolves to a regular file', async () => {
    ...
    const { SSHPyaos, state } = await loadSSHModule({ sftp });

    const error = await SSHPyaos.create({
      host: 'example.com',
      username: 'tester',
      cwd: 'file.txt',
    ...
    expect((error as Error).name).toBe('PyaosValueError');
  ...
  it('tries multiple private keys via authHandler until one succeeds', async () => {
    const { SSHPyaos, state } = await loadSSHModule({
      readFileValues: {

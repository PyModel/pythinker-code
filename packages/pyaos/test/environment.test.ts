 ...
 *   - Windows resolves Git Bash via `PYTHINKER_SHELL_PATH`, `git.exe` on PATH
 *     (including `git --exec-path` for shims), or well-known install
 *     locations; throws `PyaosShellNotFoundError`
 *     if none are present.
...
import { PyaosShellNotFoundError } from '#/errors';
...
describe('detectEnvironment', () => {
  ...
  it('uses PYTHINKER_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { PYTHINKER_SHELL_PATH: 'D:\\custom\\bash.exe' },
  ...
  it('skips relative Windows PATH entries before git --exec-path probing', async () => {
    ...
    expect(error).toBeInstanceOf(PyaosShellNotFoundError);
  });
  ...
  it('throws PyaosShellNotFoundError when no Git Bash candidate is found', async () => {
    const error = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { LOCALAPPDATA: 'C:\\Users\\me\\AppData\\Local' },
  ...
  it('includes attempted paths in the thrown error message', async () => {
    const error = await detectEnvironment(
      ...
      (error: unknown) => error as PyaosShellNotFoundError,
    );

 ...
 * cannot be located the function throws `PyaosShellNotFoundError`; the SDK
 * layer can wrap that into a user-facing install hint. Set
 * `PYTHINKER_SHELL_PATH` to override.
 */
...
async function locateWindowsGitBash(deps: EnvironmentDeps): Promise<string> {
  ...
  const override = deps.env['PYTHINKER_SHELL_PATH']?.trim();
  ...
  throw new PyaosShellNotFoundError(
    `Git Bash was not found on this Windows host. Install Git for Windows from https://gitforwindows.org/ or set PYTHINKER_SHELL_PATH to a bash.exe. Checked: ${checked.join(', ')}.`,
  );
}

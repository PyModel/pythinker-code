...
/**
 * Pythinker Agent Operating System (PYAOS) interface.
 *
...
export interface Pyaos {
   ...
   * concrete Pyaos implementation (e.g. `detectEnvironmentFromNode()` for
   * `LocalPyaos`, a remote probe for `SSHPyaos`).
   */
  ...
  /** Return a new Pyaos with the given `cwd`. */
  withCwd(cwd: string): Pyaos;
  /**
   * Return a new Pyaos that overlays `env` onto every spawned process.
   *

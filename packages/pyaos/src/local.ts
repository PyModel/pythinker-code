...
class LocalProcess implements PyaosProcess {
...
/**
 * A PYAOS implementation that directly interacts with the local filesystem.
 *
 * Note: LocalPyaos maintains its own per-instance working directory (`_cwd`)
 * rather than mutating `process.cwd()`. This lets multiple LocalPyaos instances
 * coexist with independent cwds (e.g. when switching contexts via
 * `runWithPyaos`) without cross-polluting each other's relative-path resolution.
 */
export class LocalPyaos implements Pyaos {
  ...
  /**
   * Construct a fresh `LocalPyaos` after probing the host environment.
   *
   ...
   */
  static async create(): Promise<LocalPyaos> {
    ...
    // pythinker-code itself was launched without the full profile PATH. Both
    ...
    return new LocalPyaos(osEnv);
  }

  withCwd(cwd: string): LocalPyaos {
    return new LocalPyaos(this.osEnv, cwd, this._envLayers);
  }

  withEnv(env: Record<string, string>): LocalPyaos {
    return new LocalPyaos(this.osEnv, this._cwd, [...this._envLayers, env]);
  }
  ...
  /**
   * Change the working directory of this LocalPyaos instance.
   *
   ...
   * instance's `_cwd`; concurrent LocalPyaos instances each carry their own
    ...
    if (parents) {
      ...
      if (!existOk) {
        try {
          ...
          if (error instanceof PyaosFileExistsError) throw error;
          const err = error as NodeJS.ErrnoException;
  ...
  async exec(...args: string[]): Promise<PyaosProcess> {
    const command = args[0];
    if (command === undefined) {
      throw new Error('LocalPyaos.exec(): at least one argument (the command to run) is required.');
    }
  ...
  async execWithEnv(args: string[], env?: Record<string, string>): Promise<PyaosProcess> {
    const command = args[0];
    if (command === undefined) {
      throw new Error(
        'LocalPyaos.execWithEnv(): at least one argument (the command to run) is required.',
      );
    }
    const restArgs = args.slice(1);
    const child = spawn(

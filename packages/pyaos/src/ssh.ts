...
import { PyaosError, PyaosFileExistsError, PyaosValueError } from './errors';
...
/**
 * Advanced ssh2 connect options that may be passed through `SSHPyaosOptions.extraOptions`.
 *
 * Excludes fields that SSHPyaos manages itself (`host`, `port`, `username`,
 * `password`, `privateKey`, `authHandler`, `hostVerifier`) — those are derived
 * from the top-level `SSHPyaosOptions` fields and cannot be overridden here.
 */
export type SSHPyaosExtraOptions = Omit<
  ConnectConfig,
  'host' | 'port' | 'username' | 'password' | 'privateKey' | 'authHandler' | 'hostVerifier'
>;

export interface SSHPyaosOptions {
  host: string;
  port?: number;
  username: string;
  password?: string;
   ...
   */
  extraOptions?: SSHPyaosExtraOptions;
}

// ── SSH error types ───────────────────────────────────────────────────

export class PyaosSSHError extends PyaosError {
  readonly code: number | undefined;

  constructor(message: string, code?: number) {
    super(message);
    this.name = 'PyaosSSHError';
    this.code = code;
  }
}

export class PyaosFileNotFoundError extends PyaosSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'PyaosFileNotFoundError';
  }
}

export class PyaosPermissionError extends PyaosSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'PyaosPermissionError';
  }
}

export class PyaosConnectionError extends PyaosSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'PyaosConnectionError';
  }
}
...
function mapSftpError(operation: string, error: unknown): PyaosSSHError {
  ...
  if (code === statusCode.NO_SUCH_FILE) {
    return new PyaosFileNotFoundError(message, code);
  }
  if (code === statusCode.PERMISSION_DENIED) {
    return new PyaosPermissionError(message, code);
  }
  if (code === statusCode.NO_CONNECTION || code === statusCode.CONNECTION_LOST) {
    return new PyaosConnectionError(message, code);
  }
  return new PyaosSSHError(message, code);
}
...
export class SSHProcess implements PyaosProcess {
...
// callers see a PyaosSSHError subclass (PyaosFileNotFoundError / PyaosPermissionError /
// PyaosConnectionError / generic PyaosSSHError) instead of the raw ssh2 error.
...
/**
 * A PYAOS implementation that interacts with a remote machine via SSH and SFTP.
 */
export class SSHPyaos implements Pyaos {
  ...
  get osEnv(): Environment {
    throw new PyaosError(
      'SSHPyaos.osEnv is not yet wired — remote environment probing is not implemented.',
    );
  }
  ...
  withCwd(cwd: string): SSHPyaos {
    return new SSHPyaos(this._client, this._sftp, this._home, cwd, this._envLayers);
  }

  withEnv(env: Record<string, string>): SSHPyaos {
    return new SSHPyaos(this._client, this._sftp, this._home, this._cwd, [...this._envLayers, env]);
  }
  ...
  /**
   * Factory method to create an SSHPyaos instance.
   * Establishes the SSH connection and SFTP session.
   */
  static async create(options: SSHPyaosOptions): Promise<SSHPyaos> {
    ...
    try {
      ...
      if (options.cwd === undefined) {
        ...
        if (!attrs.isDirectory()) {
          throw new PyaosValueError(`${cwd} is not a directory`);
        }
      }

      return new SSHPyaos(client, sftp, home, cwd);
    } catch (error) {
      client.end();
      throw error;
    }
  ...
  async chdir(path: string): Promise<void> {
    ...
    if (!attrs.isDirectory()) {
      throw new PyaosValueError(`${resolved} is not a directory`);
    }
    this._cwd = resolved;
  }
  ...
  async *glob(
    ...
    if (!caseSensitive) {
      throw new PyaosValueError('Case insensitive glob is not supported in current environment');
    }
  ...
  async mkdir(path: string, options?: { parents?: boolean; existOk?: boolean }): Promise<void> {
    ...
    if (parents) {
      ...
      if (exists) {
        if (!existOk) {
          throw new PyaosFileExistsError(`${resolved} already exists`);
        }
        ...
        if (!st.isDirectory()) {
          throw new PyaosFileExistsError(`${resolved} already exists but is not a directory`);
        }
        return;
      }
  ...
  private async _mkdirRecursive(path: string, existOk: boolean): Promise<void> {
    ...
    for (const [i, part] of parts.entries()) {
      ...
      if (exists) {
        ...
        if (isFinal && !existOk) {
          throw new PyaosFileExistsError(`${current} already exists`);
        }
        ...
        if (!st.isDirectory()) {
          throw new PyaosFileExistsError(`${current} already exists but is not a directory`);
        }
        continue;
      }
      try {
  ...
  exec(...args: string[]): Promise<PyaosProcess> {
    if (args.length === 0) {
      throw new PyaosValueError(
        'SSHPyaos.exec(): at least one argument (the command to run) is required.',
      );
    }
    return this._execInternal(args, this._buildExecEnv());
  }

  execWithEnv(args: string[], env?: Record<string, string>): Promise<PyaosProcess> {
    if (args.length === 0) {
      throw new PyaosValueError(
        'SSHPyaos.execWithEnv(): at least one argument (the command to run) is required.',
      );
    }
    ...
    if (env !== undefined) {
      ...
      for (const [key, value] of Object.entries(env)) {
        ...
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
          throw new PyaosValueError(
            `SSHPyaos.execWithEnv(): invalid env variable name ${JSON.stringify(key)}`,
          );
        }
  ...
  private async _execInternal(args: string[], env?: Record<string, string>): Promise<PyaosProcess> {
    const command = SSHPyaos._buildExecCommand(args, this._cwd, env);
  ...
  /**
   * Close the SSH connection. After this, the SSHPyaos instance is unusable.
   */
  close(): Promise<void> {
    this._sftp.end();

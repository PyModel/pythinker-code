/**
 * Server bootstrap — wires `@pymodel/agent-core-v2` (DI × Scope engine) into
...
export interface ServerHostIdentity extends PythinkerHostIdentity {
...
export interface ServerStartOptions {
  ...
  /**
   * Directory of the built Pythinker web UI (`dist-web`). When set, `GET /` and the
   ...
   * endpoint unintentionally; the CLI's `pythinker web` host passes true.
   */
...
export async function startServer(opts: ServerStartOptions): Promise<RunningServer> {
  ...
  if (exposureClass !== 'loopback') {
    ...
    if (!passwordConfigured) {
      logger.warn(
        { host, exposureClass },
        'binding non-loopback host with token-only auth (no PYTHINKER_CODE_PASSWORD) — the bearer token printed in the startup banner is the only credential protecting this server',
      );
    }
  }
  ...
  async function registerOpenApi(): Promise<void> {
    ...
    await app.register(swagger, {
      openapi: {
        info: {
          title: 'Pythinker Code Server API',
          description:
            'REST API for the Pythinker Code local server. All JSON responses are wrapped in a uniform envelope `{ code, msg, data, request_id }`.',
  ...
  // never retried.
  //
  // There is no single-instance lock: a busy port may be a sibling pythinker

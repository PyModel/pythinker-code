...
export const DEFAULT_SERVER_DIR = join(resolvePythinkerHome(), 'server');
...
interface ServerInstanceDisk {
  ...
  started_at: number;
  heartbeat_at: number;
  // Wire name kept as `host_version`: external tooling (pythinker-inspect's server
...
export interface InstanceRegistryOptions {
  /** Directory holding `<serverId>.json` files. Defaults to `<PYTHINKER_CODE_HOME>/server/instances`. */
...
/** Resolve the instances directory for a given home (or the default pythinker home). */
 ...
 * talk to (e.g. the CLI's `server ps/kill` and the `pythinker web` spawner).
 */

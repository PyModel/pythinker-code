 ...
 *     explicit whitelist (`PYTHINKER_CODE_CORS_ORIGINS`, no `*` wildcard — PLAN
...
const CORS_ALLOW_HEADERS = 'Content-Type, Authorization, X-Pythinker-Client-Id, X-Pythinker-Client-Name, X-Pythinker-Client-Version, X-Pythinker-Client-Ui-Mode';
...
/**
 * Parse `PYTHINKER_CODE_CORS_ORIGINS` into an allowlist.
 *
...
export function parseCorsOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['PYTHINKER_CODE_CORS_ORIGINS'];
  if (raw === undefined) {
    return [];
  }
  return raw

 ...
 * wire shape with the dynamic_workflow identity metadata that otherwise only rides the
...
export const snapshotSubagentSchema = taskSchema.extend({
  ...
  dynamic_workflow_index: z.number().int().nonnegative().optional(),
...
export const sessionSnapshotResponseSchema = z.object({
   ...
   * can rebuild dynamic_workflow cards before the dynamic_workflow's tool result lands. Optional

...
export class SessionEventBroadcaster {
  ...
  private onCoreEvent(event: GlobalEvent): void {
    if (event.type === 'event.session.created') {
      ...
      // pythinker-web's Stop button (gated on session.status === 'running') never
  ...
  private onAgentEvent(sessionId: string, agentId: string, event: DomainEvent): void {
    ...
    // (usage / context / plan / dynamic_workflow) flow through unchanged. The session's
 ...
 *
 * Exists so unchanged v1 consumers (pythinker-code TUI / `pythinker -p`, node-sdk) keep
 * working while v2-shaped consumers (pythinker-web) keep the native event and ignore

 ...
 * subagent tasks so a reconnecting client can rebuild dynamic_workflow cards from the
 * session snapshot. The refresh flow subscribes at the snapshot watermark, so
 * earlier `subagent.spawned` events — the only carriers of the dynamic_workflow identity
 * metadata — are never replayed to it.
 *
 * Ported from v1 (`packages/server/src/services/gateway/subagentRosterTracker.ts`),
 * with two adaptations: a dynamic_workflow member's own `turn.ended` never clears the
 ...
 * not clear it either — the dynamic_workflow result is only queued for the async wire
 ...
 *
 * Without this roster a mid-dynamic_workflow page refresh loses the dynamic_workflow card's member
 * list: REST `/tasks` only serves the main agent's background-task store
 * (foreground dynamic_workflow subagents never persist there), and later `subagent.*`
 * events carry only the `subagentId`, so the identity metadata
 * (`parentToolCallId` / `dynamicWorkflowIndex` / `description`) is unrecoverable until
 * the dynamic_workflow's `<agent_dynamic_workflow_result>` tool output lands.
 *
 ...
 * failed at `turn.ended` instead: the dynamic_workflow dies with the turn and the abort
    ...
    switch (event.type) {
      case 'subagent.spawned': {
        ...
        roster.set(event.subagentId, {
          ...
          dynamic_workflow_index: event.dynamicWorkflowIndex,
      ...
      case 'turn.ended': {
        ...
        // Aborted main turn (cancelled / failed / blocked): the dynamic_workflow dies
      ...
      case 'turn.started': {
        ...
        // own turn boundaries must never drop the roster mid-dynamic_workflow.

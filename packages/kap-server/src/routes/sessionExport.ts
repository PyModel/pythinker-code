...
export function registerSessionExportRoute(
  ...
  core: Scope,
  options: { readonly hostIdentity: PythinkerHostIdentity },
): void {
  ...
  const route = defineRoute(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        tempDir = await mkdtemp(join(tmpdir(), `pythinker-session-export-${safeSessionId}-`));
        if (aborted) {
          await cleanup();
          return;
        }
        ...
        const sent = response
          ...
          .header(
            'content-disposition',
            `attachment; filename="pythinker-session-${safeSessionId}.zip"`,
          )

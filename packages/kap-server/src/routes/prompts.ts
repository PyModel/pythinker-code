...
import {
  ...
  IFileService,
  ISessionMetadata,
  buildPythinkerFileUrl,
  parsePythinkerFileUrl,
  ...
  const submitRoute = defineRoute(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // internal `pythinker-file://` reference; the engine resolves them to a
...
function corePartsToProtocol(content: readonly ContentPart[]): PromptSubmission['content'] {
  ...
  for (const part of content) {
    if (part.type === 'text') parts.push({ type: 'text', text: part.text });
      ...
      // An internal `pythinker-file://<id>?path=…` reference projects back to the
      // daemon upload it came from — the materialization path never leaks to
      // the client.
      const pythinkerFile = parsePythinkerFileUrl(part.videoUrl.url);
      if (pythinkerFile !== undefined) {
        parts.push({ type: 'video', source: { kind: 'file', file_id: pythinkerFile.fileId } });
        continue;
      }
...
async function resolvePromptMediaFiles(
  ...
  for (const part of body.content) {
    ...
    // `pythinker-file://<id>?path=<materialized path>` reference. The engine
    ...
    content.push({
      type: 'video',
      source: { kind: 'url', url: buildPythinkerFileUrl(file.meta.id, cachePath) },
    });
    changed = true;
  }
...
// This notice's exact shape is a client contract: pythinker-web's messagesToTurns

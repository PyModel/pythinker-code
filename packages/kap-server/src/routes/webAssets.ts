...
async function assertWebAssets(assetsDir: string): Promise<void> {
  try {
    ...
    throw new Error(
      `Pythinker web assets were not found at ${assetsDir}. Run the package build before starting the server.`,
    );
  }
}
...
async function serveWebAsset(
  ...
  const requestUrl = new URL(req.url, 'http://pythinker-web.local');

 ...
 * REST consumers can render it: an internal `pythinker-file://<id>?path=…`
...
import { parsePythinkerFileUrl, type ContextMessage } from '@pymodel/agent-core-v2';
...
function mapContentPart(part: ContextMessage['content'][number]): MessageContent {
  switch (part.type) {
    ...
    case 'video_url': {
      const ref = parsePythinkerFileUrl(part.videoUrl.url);

 ...
 *
 * The token lives at `<PYTHINKER_CODE_HOME>/server.token` (mode 0600) and is reused
 ...
 * `pythinker web rotate-token` (which calls {@link rotateServerToken}).
 *
...
/** On-disk filename for the persistent token, relative to PYTHINKER_CODE_HOME. */

 ...
 * L2 transcript operations (`@pymodel/transcript`).
 *
 ...
 *   - `agent.status.updated` projects two ways: `planMode` / `dynamicWorkflowMode` into
 ...
 *     enter a step.
 *   - `dynamic_workflow.*` / plan-mode transition events do not exist on the v2 bus;
 *     mode badges flow from the `planMode` / `dynamicWorkflowMode` slices of
 ...
 * `session/agentLifecycle/mirrorAgentRun.ts`, `session/dynamic_workflow/sessionDynamicWorkflowService.ts`,
...
import type { DomainEvent } from '@pymodel/agent-core-v2';
import type {
  AgentRef,
  AgentUsageMeta,
  StepHeader,
...
export class AgentTranscriptProjector {
  ...
  private onSubagentSpawned(event: {
    ...
    const ops: TranscriptOperation[] = [{ op: 'task.upsert', task }];
    // Link the spawning tool call to the new agent (Agent / AgentDynamicWorkflow tool
    ...
    if (hit !== undefined) {
      const ref: AgentRef = {
        agentId: event.subagentId,
        role: event.dynamicWorkflowIndex !== undefined ? 'member' : 'child',
      };
      const frame: ToolCallFrame = {
        ...hit.frame,
  ...
  private onAgentStatusUpdated(event: {
    ...
    const ops: TranscriptOperation[] = [];
    // Only the exact `planMode` / `dynamicWorkflowMode` fields drive the mode badges

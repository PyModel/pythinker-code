...
describe('server-v2 boot', () => {
  ...
  it('reports opts.serverVersion as server_version instead of the package version', async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-version-'));
    server = await startServer({
      hostIdentity: {
  ...
  it('seeds default Pythinker identity headers from hostIdentity that opts.seeds can override', async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-ua-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
  ...
  it('does not shut down a host-injected telemetry service when server telemetry is disabled', async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-host-telemetry-'));
...
describe('server-v2 boot — port retry', () => {
  ...
  it('retries on port+1 and advertises the bound port in the instance registry', async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-port-retry-'));
    const { port, next } = await allocateAdjacentFreePair();
    // Occupy the requested port with a raw TCP server (a "third-party" process
    // from the server's point of view — it is not a registered pythinker instance).

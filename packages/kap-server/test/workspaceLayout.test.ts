 ...
 * Run: `pnpm --filter @pymodel/kap-server exec vitest run test/workspaceLayout.test.ts`.
 */
...
describe('local/local on-disk layout (byte compatibility)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-layout-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'pythinker-layout-work-'));

...
import type { ServiceIdentifier } from '@pymodel/agent-core-v2';
...
describe('server-v2 /api/v1/debug RPC', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-rpc-'));
...
describe('server-v2 /api/v1/debug RPC auth', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-rpc-auth-'));
...
describe('server-v2 /api/v1/debug RPC (dev-only, whitelist-free)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-debug-rpc-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,

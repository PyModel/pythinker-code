...
beforeEach(() => {
  prevPassword = process.env['PYTHINKER_CODE_PASSWORD'];
});
...
afterEach(async () => {
  ...
  if (prevPassword === undefined) {
    delete process.env['PYTHINKER_CODE_PASSWORD'];
  } else {
    process.env['PYTHINKER_CODE_PASSWORD'] = prevPassword;
  }
});

async function tmpHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'pythinker-v2-debug-loopback-'));
...
describe('debug endpoints are not exposed on a non-loopback bind', () => {
  it('returns 404 for /api/v1/debug/* on a 0.0.0.0 bind even when requested', async () => {
    process.env['PYTHINKER_CODE_PASSWORD'] = 'test-pw';

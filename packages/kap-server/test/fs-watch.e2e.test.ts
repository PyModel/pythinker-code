...
import { IWorkspaceInstanceManager } from '@pymodel/agent-core-v2';
import type { HostFsChange, IHostFsWatchService } from '@pymodel/agent-core-v2/os/interface/hostFsWatch';
import { FakeRuntime } from '@pymodel/agent-core-v2/runtime/fakeRuntime';
import type { RuntimeProviderRuntimeHandle } from '@pymodel/agent-core-v2/runtime/runtimeUnitHost';
...
describe('WS fs watch (kap-server)', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    { timeout: 15000 },
    async () => {
      vi.stubEnv('PYTHINKER_CODE_FS_WATCH_DEBOUNCE_MS', '500');
      vi.stubEnv('PYTHINKER_CODE_FS_WATCH_MAX_CHANGES_PER_WINDOW', '100');
      const r = await boot();
      const sid = await createSession(r);
      const conn = await openConn(wsUrl(r));
      await helloAndSubscribe(conn, 'A', sid);

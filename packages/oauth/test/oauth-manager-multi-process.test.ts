 ...
 * `proper-lockfile` are bypassed via the `PYTHINKER_DISABLE_OAUTH_LOCK=1`
...
const WORKER_SCRIPT = `
  ...
  import { join } from 'node:path';
  const { OAuthManager } = await import(process.env.PYTHINKER_OAUTH_ENTRY);

  const shareDir = process.env.PYTHINKER_CODE_HOME;
  ...
  async function waitForFirstLoadBarrier() {
    if (process.env.PYTHINKER_SYNC_FIRST_LOAD !== '1') return;
    await appendFile(readyPath, '.');
    const expected = Number(process.env.PYTHINKER_WORKER_COUNT || '1');
  ...
  if (process.env.DEBUG_OAUTH_WORKER === '1') {
    process.stderr.write('[worker ' + process.env.PYTHINKER_WORKER_ID + '] done\\n');
  }
`;
...
describe.skipIf(skipOnWindows)('OAuthManager cross-process refresh lock', () => {
  it('2 workers concurrently force-refresh → exactly one refreshImpl fires', async () => {
    ...
    const workers = await spawnInlineWorkers({
      ...
      timeoutMs: 30_000,
      env: {
        PYTHINKER_OAUTH_ENTRY: OAUTH_ENTRY_URL,
        PYTHINKER_SYNC_FIRST_LOAD: '1',
        PYTHINKER_WORKER_COUNT: '2',
      },
    });
...
describe.skipIf(!skipOnWindows)('OAuthManager cross-process refresh lock (Windows skip)', () => {
  it('skipped on Windows — covered by PYTHINKER_DISABLE_OAUTH_LOCK=1 env escape hatch', () => {

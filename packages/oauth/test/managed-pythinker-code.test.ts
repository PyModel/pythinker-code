...
describe('provisionManagedPythinkerCodeConfig', () => {
  ...
  it('writes scoped OAuth refs when provisioning against a non-default environment', async () => {
    ...
    expect(config.services?.pythoughtsFetch?.oauth).toEqual({
      storage: 'file',
      key: oauthKey,
      oauthHost: 'https://auth.dev.example.test',
    });
  ...
  it('removes managed provider, models, services, and default model on logout', () => {
    const config: ManagedPythinkerConfigShape = {
      ...
      services: {
        pythoughtsSearch: { baseUrl: 'https://api.kimi.com/coding/v1/search' },
        pythoughtsFetch: { baseUrl: 'https://api.kimi.com/coding/v1/fetch' },
      ...
      raw: {
        ...
        services: {
          pythoughts_search: { base_url: 'https://api.kimi.com/coding/v1/search' },
          pythoughts_fetch: { base_url: 'https://api.kimi.com/coding/v1/fetch' },
        },
      },
    };
  ...
  it('rejects managed models that do not include a positive context_length', async () => {
    const fetchImpl = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            data: [{ id: 'pythinker-for-coding', supports_reasoning: true }],
          }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        ),

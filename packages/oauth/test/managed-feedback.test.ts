...
const SAMPLE_BODY: SubmitFeedbackBody = {
  ...
  info: { tool: 'pythinker-code-cli', env: 'test' },
};
...
describe('fetchSubmitFeedback', () => {
  ...
  it('preserves the pythinker-code- version prefix in the request body', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({ feedback_id: 3 }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    ...
    expect(sent['version']).toBe('pythinker-code-0.1.1');
  });

...
describe('pythinkerCodeUserInfoUrl', () => {
  it('follows the PYTHINKER_CODE_BASE_URL override', () => {
    vi.stubEnv('PYTHINKER_CODE_BASE_URL', 'https://gw.example.com/');
    expect(pythinkerCodeUserInfoUrl()).toBe('https://gw.example.com/me');
  });
});
...
describe('fetchManagedUserInfo', () => {
  ...
  it('falls back to the local profile hint on an empty 404 body', async () => {
    ...
    expect(result.message).toBe('Profile endpoint not available. Try Pythinker For Coding.');
  });

...
import { fetchChatTitle, pythinkerCodeToolsUrl } from '../src/managed-tools';
...
describe('pythinkerCodeToolsUrl', () => {
  it('appends /tools to the default base URL', () => {
    expect(pythinkerCodeToolsUrl()).toBe('https://api.kimi.com/coding/v1/tools');
  });

  it('honours PYTHINKER_CODE_BASE_URL and trims trailing slashes', () => {
    vi.stubEnv('PYTHINKER_CODE_BASE_URL', 'https://example.test/v9///');
    expect(pythinkerCodeToolsUrl()).toBe('https://example.test/v9/tools');
  });
});

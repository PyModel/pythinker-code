...
export function pythinkerCodeFeedbackUploadUrl(baseUrl?: string): string {
  return `${feedbackBaseUrl(baseUrl)}/feedback/upload_url`;
}

export function pythinkerCodeFeedbackUploadCompleteUrl(baseUrl?: string): string {
...
export async function fetchCreateFeedbackUploadUrl(
  ...
  const result = await postJson(pythinkerCodeFeedbackUploadUrl(opts.baseUrl), accessToken, body, opts);
...
export async function fetchCompleteFeedbackUpload(
  ...
  const result = await postJson(pythinkerCodeFeedbackUploadCompleteUrl(opts.baseUrl), accessToken, body, opts);
  if (result.kind === 'error') return result;
  return { kind: 'ok' };
}

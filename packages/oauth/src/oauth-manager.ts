...
export interface OAuthManagerOptions {
   ...
   *
   * **Production callers MUST pass this explicitly** (PythinkerCoreClient /
   ...
   * falls back to `process.env.PYTHINKER_CODE_HOME` so multi-process test
   * harnesses don't need to thread the dir through every fixture. In
   * production the fallback is inert. Windows platforms and
   * `process.env.PYTHINKER_DISABLE_OAUTH_LOCK === '1'` always skip; the
...
export class OAuthManager {
  ...
  constructor(options: OAuthManagerOptions) {
    ...
    // The `PYTHINKER_CODE_HOME` fallback MUST stay test-only so production
    ...
    const envConfigDir =
      process.env['NODE_ENV'] === 'test' ? process.env['PYTHINKER_CODE_HOME'] : undefined;
  ...
  private resolveLockTarget(): string | undefined {
    ...
    if (process.env['PYTHINKER_DISABLE_OAUTH_LOCK'] === '1') return undefined;

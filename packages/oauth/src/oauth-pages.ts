/**
 * Branded browser HTML for localhost OAuth callback servers.
 * Inline assets only — no external requests — so the page works offline.
 */

const PYTHINKER_LOGO_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 411 512.455" width="88" height="110" aria-hidden="true" focusable="false">' +
...
// Palette and card treatment mirror the product light theme (light canvas,
...
function renderOAuthPage(options: {
  ...
  return (
    ...
    '</head><body>' +
    '<main class="card">' +
    `<div class="logo">${PYTHINKER_LOGO_SVG}</div>` +

 ...
 * `{pythinkerCodeBaseUrl}/tools` with a Bearer access token, the same wire
...
export function pythinkerCodeToolsUrl(baseUrl?: string): string {
  return `${(baseUrl ?? pythinkerCodeBaseUrl()).replace(/\/+$/, '')}/tools`;
}

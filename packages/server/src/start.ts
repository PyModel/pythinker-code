...
export async function startServer(opts: ServerStartOptions): Promise<RunningServer> {
  ...
  const lockHandle = acquireLock({
    ...
    // Record the host build identity so `pythinker server status` can detect a
  ...
  const envService: IEnvironmentService = {
    ...
    homeDir: resolvePythinkerHome(opts.coreProcessOptions?.homeDir),
  ...
  // `pythinker server status` / `kill` / `ps` can find this daemon.
 ...
 * lock *before* listening, so by the time we reach `listen` a live pythinker server
 ...
 * ("if the port is taken by something other than pythinker server itself, +1").
 *

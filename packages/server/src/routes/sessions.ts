...
function clientTelemetryFromHeaders(
  ...
  const client: SessionClientTelemetry = {
    id: headerString(headers, 'x-pythinker-client-id'),
    name: headerString(headers, 'x-pythinker-client-name'),
    version: headerString(headers, 'x-pythinker-client-version'),
    uiMode: headerString(headers, 'x-pythinker-client-ui-mode'),
  };
    ...
    async (req, reply) => {
      try {
        ...
        if (workspaceId !== undefined) {
          ...
          if (
            callerCwd !== undefined &&
            workspaceRootKey(callerCwd) !== workspaceRootKey(workspaceRoot)
          ) {
            reply.send(
              buildValidationEnvelope(
                [
    ...
    async (req, reply) => {
      try {
        ...
        if (raw.workspace_id !== undefined) {
          const registry = ix.invokeFunction((a) => a.get(IWorkspaceRegistry));
          try {
            await registry.resolveRoot(raw.workspace_id);
          } catch (err) {
            if (err instanceof WorkspaceNotFoundError) {
              reply.send(
                errEnvelope(ErrorCode.WORKSPACE_NOT_FOUND, err.message, req.id),
          ...
          query = { ...baseQuery, workspaceId: raw.workspace_id };
        } else {
          query = baseQuery;
        }
...
function sendMappedError(
  ...
  if (err instanceof PythinkerError && err.code === ErrorCodes.COMPACTION_UNABLE) {
  ...
  if (err instanceof PythinkerError) {
...
function isForkActiveTurnError(err: unknown): boolean {
  if (err instanceof PythinkerError && err.code === ErrorCodes.SESSION_FORK_ACTIVE_TURN) {
    return true;
  }
  return (
    typeof err === 'object' &&

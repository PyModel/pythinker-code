...
export function createSystemdManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    const enable = await deps.execSystemctl(['enable', '--now', PYTHINKER_SERVER_SYSTEMD_UNIT]);
    if (enable.code !== 0) {
      throw new Error(
    ...
    return {
      ...
      message: `Pythinker server systemd unit ${alreadyInstalled ? 'replaced' : 'installed'} at ${unitPath} (port ${plan.port}).`,
      unitPath,
    };
  }
  ...
  async function uninstall(): Promise<LifecycleResult> {
    ...
    await deps.execSystemctl(['disable', '--now', PYTHINKER_SERVER_SYSTEMD_UNIT]).catch(() => undefined);
    try {
  ...
  async function start(): Promise<LifecycleResult> {
    if (!existsSync(deps.unitPath())) {
      return {
        ok: false,
        message: 'systemd unit is not installed. Run `pythinker server install` first.',
      };
    }
    const result = await deps.execSystemctl(['start', PYTHINKER_SERVER_SYSTEMD_UNIT]);
    if (result.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server started (${PYTHINKER_SERVER_SYSTEMD_UNIT}).` };
  }

  async function stop(): Promise<LifecycleResult> {
    const result = await deps.execSystemctl(['stop', PYTHINKER_SERVER_SYSTEMD_UNIT]);
    if (result.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server stopped (${PYTHINKER_SERVER_SYSTEMD_UNIT}).` };
  }

  async function restart(): Promise<LifecycleResult> {
    const result = await deps.execSystemctl(['restart', PYTHINKER_SERVER_SYSTEMD_UNIT]);
    if (result.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server restarted (${PYTHINKER_SERVER_SYSTEMD_UNIT}).` };
  }
  ...
  async function status(): Promise<ServiceStatus> {
    ...
    const base: ServiceStatus = {
      ...
      installed,
      running: false,
      unitName: PYTHINKER_SERVER_SYSTEMD_UNIT,

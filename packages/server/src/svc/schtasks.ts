...
import { PYTHINKER_SERVER_TASK_NAME, supervisorLogPath as defaultSupervisorLogPath } from './paths';
...
const DEFAULT_DEPS: SchtasksManagerDeps = {
  ...
  resolveProgram: () => resolveSupervisorProgram(process.argv, process.cwd(), 'pythinker.exe'),
...
export function createSchtasksManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    if (alreadyInstalled && args.force !== true) {
      return {
        ...
        message: `Scheduled task "${PYTHINKER_SERVER_TASK_NAME}" already exists. Rerun with --force to replace it.`,
        taskName: PYTHINKER_SERVER_TASK_NAME,
      };
    }

    const argString = serializeArguments(plan);
    const xml = buildScheduledTaskXml({
      description: 'Pythinker Code local server (managed by `pythinker server install`)',
    ...
    try {
      const createArgs = ['/Create', '/TN', PYTHINKER_SERVER_TASK_NAME, '/XML', xmlPath];
      if (alreadyInstalled) {
        createArgs.push('/F');
      }
    ...
    const run = await deps.execSchtasks(['/Run', '/TN', PYTHINKER_SERVER_TASK_NAME]);
    if (run.code !== 0) {
    ...
    return {
      status: alreadyInstalled ? 'replaced' : 'installed',
      message: `Pythinker server scheduled task ${alreadyInstalled ? 'replaced' : 'installed'} (${PYTHINKER_SERVER_TASK_NAME}, port ${plan.port}).`,
      taskName: PYTHINKER_SERVER_TASK_NAME,
    };
  }
  ...
  async function uninstall(): Promise<LifecycleResult> {
    ...
    await deps.execSchtasks(['/End', '/TN', PYTHINKER_SERVER_TASK_NAME]).catch(() => undefined);
    const del = await deps.execSchtasks(['/Delete', '/TN', PYTHINKER_SERVER_TASK_NAME, '/F']);
    if (del.code !== 0) {
      return {
        ok: false,
    ...
    deleteInstallPlan();
    return { ok: true, message: `Scheduled task removed (${PYTHINKER_SERVER_TASK_NAME}).` };
  }
  ...
  async function start(): Promise<LifecycleResult> {
    if (!(await deps.taskExists())) {
      return {
        ok: false,
        message: 'Scheduled task is not installed. Run `pythinker server install` first.',
      };
    }
    const result = await deps.execSchtasks(['/Run', '/TN', PYTHINKER_SERVER_TASK_NAME]);
    if (result.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server started (${PYTHINKER_SERVER_TASK_NAME}).` };
  }

  async function stop(): Promise<LifecycleResult> {
    const result = await deps.execSchtasks(['/End', '/TN', PYTHINKER_SERVER_TASK_NAME]);
    if (result.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server stopped (${PYTHINKER_SERVER_TASK_NAME}).` };
  }

  async function restart(): Promise<LifecycleResult> {
    const end = await deps.execSchtasks(['/End', '/TN', PYTHINKER_SERVER_TASK_NAME]);
    if (end.code !== 0) {
      return {
        ok: false,
    ...
    return { ok: true, message: `Pythinker server restarted (${PYTHINKER_SERVER_TASK_NAME}).` };
  }
...
function defaultWriteTaskXml(xml: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'pythinker-server-task-'));
...
async function defaultTaskExists(): Promise<boolean> {
  const res = await execFileUtf8('schtasks', ['/Query', '/TN', PYTHINKER_SERVER_TASK_NAME], {
    windowsHide: true,
  });
  return res.code === 0;
}

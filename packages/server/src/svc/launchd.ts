...
export function createLaunchdManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    return {
      ...
      message: `Pythinker server LaunchAgent ${alreadyInstalled ? 'replaced' : 'installed'} at ${plistPath} (port ${plan.port}).`,
      plistPath,
    };
  }
  ...
  async function uninstall(): Promise<LifecycleResult> {
    ...
    const bootout = await deps.execLaunchctl([
      'bootout',
      `${deps.guiDomain()}/${PYTHINKER_SERVER_LABEL}`,
    ]);
  ...
  async function start(): Promise<LifecycleResult> {
    ...
    if (!existsSync(plistPath)) {
      return {
        ok: false,
        message: 'LaunchAgent is not installed. Run `pythinker server install` first.',
      };
    }
    const target = `${deps.guiDomain()}/${PYTHINKER_SERVER_LABEL}`;
    ...
    return { ok: true, message: `Pythinker server started (${PYTHINKER_SERVER_LABEL}).` };
  }
  ...
  async function stop(): Promise<LifecycleResult> {
    ...
    return { ok: true, message: `Pythinker server stopped (${PYTHINKER_SERVER_LABEL}).` };
  }
  ...
  async function restart(): Promise<LifecycleResult> {
    ...
    return { ok: true, message: `Pythinker server restarted (${PYTHINKER_SERVER_LABEL}).` };
  }
  ...
  async function status(): Promise<ServiceStatus> {
    ...
    const print = await deps.execLaunchctl(['print', `${deps.guiDomain()}/${PYTHINKER_SERVER_LABEL}`]);
    if (print.code !== 0) {
      return {
        ...base,
        notes: [
...
function writePlist(plistPath: string, plan: InstallPlan, logPath: string): void {
  const xml = buildLaunchAgentPlist({
    label: PYTHINKER_SERVER_LABEL,
    comment: 'Pythinker Code local server (managed by `pythinker server install`).',
...
async function bestEffortBootout(deps: LaunchdManagerDeps): Promise<void> {
  await deps.execLaunchctl(['bootout', `${deps.guiDomain()}/${PYTHINKER_SERVER_LABEL}`]).catch(() => {

 ...
 *
 * The lock is a small JSON file at `<PYTHINKER_CODE_HOME>/server/lock` (defaults
 * to `~/.pythinker-code/server/lock`; overridable via `PYTHINKER_CODE_HOME` env or
...
export interface LockContents {
  ...
  host?: string;
  port: number;
  /** Host CLI version that started this server (e.g. pythinker-code package version).
      Lets `pythinker server status` detect a build-mismatched server. Absent in locks
...
export interface AcquireLockOptions {
  /** Override default `<PYTHINKER_CODE_HOME>/server/lock` — used in tests. */
  lockPath?: string;
...
export interface AcquireLockResult {
   ...
   * `pythinker server status` / `kill` / `ps` can find the daemon. Best-effort and

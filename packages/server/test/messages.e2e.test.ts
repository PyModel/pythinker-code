 ...
 * home) and exercise the 2 endpoints via `app.inject(...)`. PythinkerCore is fully
 * constructed via the W3 bridge pattern — fresh tmpdir per test, no `~/.pythinker`
 * interference.
 *
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-messages-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-messages-home-'));
});

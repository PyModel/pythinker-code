/**
 * `GET /api/v1/sessions/{sid}/snapshot` end-to-end tests (v3 sync protocol).
 *
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-snapshot-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-snapshot-home-'));
});
...
describe('GET /api/v1/sessions/{sid}/snapshot (v3 initial sync)', () => {

...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-meta-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-meta-home-'));
});
...
describe('GET /api/v1/meta — envelope + metaResponseSchema', () => {
  ...
  it('two independent daemons get distinct daemon_ids', async () => {
    ...
    const homeA = mkdtempSync(join(tmpdir(), 'pythinker-server-meta-home-a-'));
    const homeB = mkdtempSync(join(tmpdir(), 'pythinker-server-meta-home-b-'));
    ...
    try {
      ...
      await r1.close();
      await r2.close();
      rmSync(homeA, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
      rmSync(homeB, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    }
  });
});

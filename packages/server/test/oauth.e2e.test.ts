...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-oauth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-oauth-home-'));
});
...
function makeStub(scripted: {
  ...
  const defaultStart: OAuthFlowStart = {
    ...
    verification_uri_complete: 'https://example.com/device?user_code=PYTHINKER-1234',
    user_code: 'PYTHINKER-1234',
    expires_in: 900,
    interval: 5,
    status: 'pending',
...
async function bootDaemon(stub: StubOAuth): Promise<RunningServer> {
...
describe('POST /api/v1/oauth/login (P2.7)', () => {
  it('returns 200 + envelope { code:0, data: OAuthFlowStart }', async () => {
    ...
    expect(data.verification_uri_complete).toBe(
      'https://example.com/device?user_code=PYTHINKER-1234',
    );

...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-fswatch-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-fswatch-home-'));
  ...
  it('AC #2: burst > 500 changes inside 200ms window → truncated:true', async () => {
    ...
    while (Date.now() < deadline) {
      ...
      if (payload.truncated === true) {
        truncatedCount = payload.count;
        sawTruncated = true;
        break;
      }
    }
    expect(truncatedCount).toBeGreaterThan(500);

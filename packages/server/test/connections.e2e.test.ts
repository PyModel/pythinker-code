/**
 * `GET /api/v1/connections` end-to-end (pythinker server ps wire contract).
 *
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-connections-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-connections-home-'));
});

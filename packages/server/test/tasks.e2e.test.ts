 ...
 * directly into the in-process PythinkerCore via the bridge. Agent-core's
 ...
 * PythinkerCore session and inserting into the agent-core background-task manager
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-tasks-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-tasks-home-'));
});

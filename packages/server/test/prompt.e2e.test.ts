...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-prompts-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-prompts-home-'));
});
...
function buildMultipart(parts: {
  ...
  const boundary = '------WebKitFormBoundaryPythinkerDaemonPromptTest';
...
async function openSubscriber(
  ...
  ws.send(
    JSON.stringify({
      type: 'client_hello',
      id: 'h1',
      payload: { client_id: 'test', protocol_version: WS_PROTOCOL_VERSION, subscriptions: [sid] },
    }),
  );

 ...
 *
 * Bind to port 0 → ephemeral port; tmpdir lock path → no `~/.pythinker` interference.
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-start-test-'));
  lockPath = join(tmpDir, 'lock');
  // Isolate PythinkerCore's `~/.pythinker` lookup — bridge construction touches it via plugin discovery.
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-start-home-'));
});
...
describe('startServer — lock + healthz smoke', () => {
  ...
  it('retries on port+1 and updates the lock when the requested port is held by a third party', async () => {
    ...
    try {
      ...
      // A concurrent listener can take `next` during startup; the lock must
      // advertise whichever retry port the server actually binds.
...
describe('startServer — web assets', () => {
  it('serves web assets from the server root without shadowing API routes', async () => {
    ...
    rmSync(assetsDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    mkdirSync(assetsDir);
    writeFileSync(join(assetsDir, 'index.html'), '<html><div id="app"></div></html>', 'utf8');
    writeFileSync(join(assetsDir, 'app.js'), 'console.log("pythinker web");', 'utf8');

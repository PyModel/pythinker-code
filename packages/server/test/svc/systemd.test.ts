...
import { PYTHINKER_SERVER_SYSTEMD_UNIT } from '../../src/svc/paths';
...
function makeDeps(
  ...
  const unitPath = join(workDir, 'systemd', 'user', PYTHINKER_SERVER_SYSTEMD_UNIT);
...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'pythinker-systemd-test-'));
...
describe('buildSystemdUnit', () => {
  it('renders the standard [Unit]/[Service]/[Install] triple', () => {
    ...
    expect(unit).toContain('Description=Pythinker Code local server');
    expect(unit).toContain('ExecStart=/usr/local/bin/pythinker server run --port 58627');
  ...
  it('quotes argv elements with whitespace', () => {
    const unit = buildSystemdUnit({
      programArguments: ['/path with space/pythinker', 'server', 'run'],
    });
    expect(unit).toContain('ExecStart="/path with space/pythinker" server run');
  });

  it('rejects argv elements with CR/LF', () => {
    expect(() =>
      buildSystemdUnit({ programArguments: ['/usr/bin/pythinker', 'server\nrun'] }),
    ).toThrow(/cannot contain CR or LF/);
  });

  it('renders Environment= lines', () => {
    const unit = buildSystemdUnit({
      programArguments: ['/usr/bin/pythinker'],
...
describe('systemd manager — install', () => {
  it('writes the unit, daemon-reloads, enables --now', async () => {
    ...
    expect(text).toContain('ExecStart=/usr/local/bin/pythinker server run --port 58627 --log-level info');
    expect(text).not.toContain('--host');

    expect(calls.length).toBe(3);
    expect(calls[0]?.args).toEqual(['show-environment']);
    expect(calls[1]?.args).toEqual(['daemon-reload']);
    expect(calls[2]?.args).toEqual(['enable', '--now', PYTHINKER_SERVER_SYSTEMD_UNIT]);
  });
  ...
  it('overwrites + replaces when force=true', async () => {
    ...
    const text = readFileSync(unitPath, 'utf8');
    expect(text).toContain('ExecStart=/usr/local/bin/pythinker server run --port 9999 --log-level debug');
...
describe('systemd manager — lifecycle', () => {
  it('start delegates to `systemctl --user start`', async () => {
    ...
    expect(calls[0]?.args).toEqual(['start', PYTHINKER_SERVER_SYSTEMD_UNIT]);
  });
  ...
  it('stop delegates to `systemctl --user stop`', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(calls[0]?.args).toEqual(['stop', PYTHINKER_SERVER_SYSTEMD_UNIT]);
  });
  ...
  it('restart delegates to `systemctl --user restart`', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(calls[0]?.args).toEqual(['restart', PYTHINKER_SERVER_SYSTEMD_UNIT]);
  });
  ...
  it('uninstall calls disable + removes unit + clears plan', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(calls[0]?.args).toEqual(['disable', '--now', PYTHINKER_SERVER_SYSTEMD_UNIT]);
    expect(calls[1]?.args).toEqual(['daemon-reload']);
    expect(existsSync(unitPath)).toBe(false);
    expect(readInstallPlan()).toBeUndefined();
  });
...
describe('systemd manager — status', () => {
  it('reports installed=false when no unit exists', async () => {
    ...
    expect(status.unitName).toBe(PYTHINKER_SERVER_SYSTEMD_UNIT);
  });

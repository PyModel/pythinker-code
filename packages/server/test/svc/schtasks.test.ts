...
import { PYTHINKER_SERVER_TASK_NAME } from '../../src/svc/paths';
...
function makeDeps(
  ...
  const deps: SchtasksManagerDeps = {
    execSchtasks,
    resolveProgram: () => 'C\\:Program Files\\Pythinker\\pythinker.exe',
...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'pythinker-schtasks-test-'));
...
describe('buildScheduledTaskXml', () => {
  it('renders a well-formed task XML with command and arguments', () => {
    ...
    expect(xml).toContain('<Command>C:\\bin\\pythinker.exe</Command>');
...
describe('parseSchtasksQuery', () => {
  it('parses CSV header + first row into a record', () => {
    const csv = [
      ...
      '"WORKSTATION","\\PythinkerServer","Running","0","alice"',
    ].join('\r\n');
    const row = parseSchtasksQuery(csv);
    expect(row).toBeDefined();
    expect(row?.['Status']).toBe('Running');
    expect(row?.['TaskName']).toBe('\\PythinkerServer');
  ...
  it('handles escaped quotes inside CSV cells', () => {
    const csv = [
      ...
      '"\\PythinkerServer","with ""quotes"" inside"',
    ].join('\n');
    const row = parseSchtasksQuery(csv);
...
describe('schtasks manager — install', () => {
  it('writes the XML, runs schtasks /Create, then /Run', async () => {
    ...
    expect(result.taskName).toBe(PYTHINKER_SERVER_TASK_NAME);
    expect(writtenXmls.length).toBe(1);
    expect(writtenXmls[0]).toContain(`<Description>Pythinker Code local server`);
    ...
    expect(calls[1]?.args).toEqual(['/Run', '/TN', PYTHINKER_SERVER_TASK_NAME]);
  });
...
describe('schtasks manager — lifecycle', () => {
  it('start delegates to /Run', async () => {
    ...
    expect(calls[0]?.args).toEqual(['/Run', '/TN', PYTHINKER_SERVER_TASK_NAME]);
  });
  ...
  it('stop delegates to /End', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(calls[0]?.args).toEqual(['/End', '/TN', PYTHINKER_SERVER_TASK_NAME]);
  });
  ...
  it('uninstall calls /End + /Delete and clears plan', async () => {
    ...
    writeInstallPlan({
      ...
      port: 58627,
      logLevel: 'info',
      program: 'C:\\pythinker.exe',
      programArguments: ['C:\\pythinker.exe', 'server', 'run'],
      logPath: 'C:\\tmp\\x',
    ...
    expect(result.ok).toBe(true);
    expect(calls[0]?.args).toEqual(['/End', '/TN', PYTHINKER_SERVER_TASK_NAME]);
    expect(calls[1]?.args).toEqual(['/Delete', '/TN', PYTHINKER_SERVER_TASK_NAME, '/F']);

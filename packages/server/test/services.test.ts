...
function tmpEnv(): IEnvironmentService {
  const dir = mkdtempSync(join(tmpdir(), 'pythinker-server-test-'));
...
describe('WSBroadcastService (WS transport pump)', () => {
  let homeDir: string;
  const journalEpoch = 'ep_01ARZ3NDEKTSV4RRFFQ69G5FAV';
  ...
  function header(epoch = journalEpoch, version = WS_PROTOCOL_VERSION): Record<string, unknown> {
    return { kind: 'journal_header', version, epoch, created_at: 1 };
  }

  function eventLine(sessionId: string, seq: number, epoch = journalEpoch): Record<string, unknown> {
    return { kind: 'event', seq, envelope: validEnvelope(sessionId, seq, epoch) };
  }

  function writeJournal(sessionId: string, lines: readonly string[]): void {
    const path = journalPath(sessionId);
    mkdirSync(join(homeDir, 'server', 'events-v3'), { recursive: true });
  ...
  it('isolates v3 journals from legacy files and writes a v3 header with a fresh cursor', async () => {
    const sessionId = 'sid_v3';
    const legacyPath = join(homeDir, 'server', 'events', `${sessionId}.jsonl`);
    mkdirSync(join(homeDir, 'server', 'events'), { recursive: true });
    writeFileSync(legacyPath, '{"legacy":true}\n', 'utf8');
    const clients = new FakeSessionClients();
    const bus = new EventService();
    const broadcast = new WSBroadcastService(bus, testLogger, clients, new FakeConnectionRegistry(), makeEnv());

    const fresh = await broadcast.getCursor(sessionId);
    expect(fresh).toMatchObject({ seq: 0, epoch: expect.stringMatching(/^ep_/) });
    bus.publish({
      type: 'turn.started',
      sessionId,
      agentId: 'main',
    ...
    await broadcast._drainForTest(sessionId);
    const cursor = await broadcast.getCursor(sessionId);
    await broadcast.closeJournals();

    const [firstLine] = readFileSync(journalPath(sessionId), 'utf8').trimEnd().split('\n');
    expect(JSON.parse(firstLine!)).toMatchObject({
      kind: 'journal_header',
      version: WS_PROTOCOL_VERSION,
      epoch: fresh.epoch,
    });
    expect(readFileSync(legacyPath, 'utf8')).toBe('{"legacy":true}\n');
    expect(cursor).toMatchObject({ seq: 1, epoch: fresh.epoch });
    broadcast.dispose();
    bus.dispose();
  });
  ...
  it.each([
    ...
    ['a missing header', (sid: string) => `${JSON.stringify(eventLine(sid, 1))}\n`],
    ['a wrong header version', () => `${JSON.stringify(header(journalEpoch, 2))}\n`],
    [
      'a malformed non-final line',
      (sid: string) => `${JSON.stringify(header())}\nnot-json\n${JSON.stringify(eventLine(sid, 1))}\n`,
    ],
    ['an unknown record kind', () => `${JSON.stringify(header())}\n{"kind":"unknown"}\n`],
    [
      'a repeated header',
      () => `${JSON.stringify(header())}\n${JSON.stringify(header())}\n`,
    ],
    [
      'an event from another epoch',
      (sid: string) => `${JSON.stringify(header())}\n${JSON.stringify(eventLine(sid, 1, 'ep_01ARZ3NDEKTSV4RRFFQ69G5FAW'))}\n`,
    ],
    [
      'an invalid event envelope',
      () => `${JSON.stringify(header())}\n${JSON.stringify({
        kind: 'event',
        seq: 1,
        envelope: { type: 'invalid', seq: 1, epoch: journalEpoch, session_id: 'sid_invalid' },
      })}\n`,
    ],
    [
    ...
    [
      'a gapped sequence',
      (sid: string) => `${JSON.stringify(header())}\n${JSON.stringify(eventLine(sid, 2))}\n`,
    ],
    [
      'an out-of-order sequence',
      (sid: string) => `${JSON.stringify(header())}\n${JSON.stringify(eventLine(sid, 1))}\n${JSON.stringify(eventLine(sid, 3))}\n${JSON.stringify(eventLine(sid, 2))}\n`,
    ],
  ])('fails closed on %s and preserves the first error for later access', async (_label, contents) => {
    const sessionId = 'sid_invalid';
    writeJournal(sessionId, [contents(sessionId)]);
    const bus = new EventService();
    ...
    await expect(broadcast.getCursor(sessionId)).rejects.toBe(firstError);
    await expect(broadcast.getBufferedSince(sessionId, { seq: 0 })).rejects.toBe(firstError);
    await broadcast.closeJournals();
    broadcast.dispose();
    bus.dispose();
  });

  it('repairs an incomplete final JSON tail before appending and reopening the journal', async () => {
    const sessionId = 'sid_torn';
    writeJournal(sessionId, [
      JSON.stringify(header()),
      JSON.stringify(eventLine(sessionId, 1)),
      '{"kind":"event","seq":2',
    ]);
    const firstBus = new EventService();
    const firstBroadcast = new WSBroadcastService(
      firstBus,
      testLogger,
    ...
    await expect(firstBroadcast.getCursor(sessionId)).resolves.toMatchObject({
      seq: 1,
      epoch: journalEpoch,
    });
    firstBus.publish({
    ...
    await firstBroadcast._drainForTest(sessionId);
    await expect(firstBroadcast.getCursor(sessionId)).resolves.toMatchObject({
      seq: 2,
      epoch: journalEpoch,
    });
    await firstBroadcast.closeJournals();
    firstBroadcast.dispose();
    firstBus.dispose();

    const secondBus = new EventService();
    const secondBroadcast = new WSBroadcastService(
      secondBus,
      testLogger,
      new FakeSessionClients(),
      new FakeConnectionRegistry(),
    ...
    await expect(secondBroadcast.getCursor(sessionId)).resolves.toMatchObject({
      seq: 2,
      epoch: journalEpoch,
    });
    await expect(secondBroadcast.getBufferedSince(sessionId, { seq: 0 })).resolves.toMatchObject({
      events: [expect.objectContaining({ seq: 1 }), expect.objectContaining({ seq: 2 })],
      currentSeq: 2,
      epoch: journalEpoch,
    });

    const records = readFileSync(journalPath(sessionId), 'utf8')
      .trimEnd()
      .split('\n')
      .map((line) => JSON.parse(line) as { kind: string; seq?: number });
    expect(records).toEqual([
      expect.objectContaining({ kind: 'journal_header' }),
      expect.objectContaining({ kind: 'event', seq: 1 }),
      expect.objectContaining({ kind: 'event', seq: 2 }),
    ]);
    await secondBroadcast.closeJournals();
    secondBroadcast.dispose();
    secondBus.dispose();
  });

  it('repairs a final UTF-8 codepoint tear without extending the journal', async () => {
    const sessionId = 'sid_torn_utf8';
    const unicodeEvent = {
      kind: 'event',
      seq: 1,
      envelope: { ...validEnvelope(sessionId, 1), note: 'café' },
    };
    const rawPrefix = Buffer.from(
      `${JSON.stringify(header())}\n${JSON.stringify(unicodeEvent)}\n{"kind":"event","seq":2,"text":"`,
      'utf8',
    );
    mkdirSync(join(homeDir, 'server', 'events-v3'), { recursive: true });
    writeFileSync(journalPath(sessionId), Buffer.concat([rawPrefix, Buffer.from([0xc3])]));
    ...
    const persisted = readFileSync(journalPath(sessionId));
    expect(persisted.includes(0)).toBe(false);
  ...
  it('fails closed when invalid UTF-8 precedes a torn final record', async () => {
    const sessionId = 'sid_invalid_utf8';
    const malformedEvent = Buffer.from(
      JSON.stringify({
        kind: 'event',
        seq: 1,
        envelope: { ...validEnvelope(sessionId, 1), note: 'x' },
      }),
      'utf8',
    );
    const note = Buffer.from('"note":"x"', 'utf8');
    const noteIndex = malformedEvent.indexOf(note);
    if (noteIndex < 0) throw new Error('test fixture note is missing');
    malformedEvent[noteIndex + Buffer.byteLength('"note":"')] = 0xc3;
    const tornRecord = Buffer.concat([
      Buffer.from('{"kind":"event","seq":2,"text":"', 'utf8'),
      Buffer.from([0xc3]),
    ]);
    mkdirSync(join(homeDir, 'server', 'events-v3'), { recursive: true });
    writeFileSync(
    ...
    await expect(broadcast.getCursor(sessionId)).rejects.toThrow(
      'invalid UTF-8 journal line',
    );
    await broadcast.closeJournals();
    broadcast.dispose();
    bus.dispose();
  ...
  it('poisons a journal after an append failure before tail, cursor, or fan-out state changes', async () => {
    const sessionId = 'sid_poison';
    const clients = new FakeSessionClients();
    const connection = fakeConn('conn_poison');
    clients.subscribe(connection, sessionId);
    const bus = new EventService();
    const broadcast = new WSBroadcastService(bus, testLogger, clients, new FakeConnectionRegistry(), makeEnv());
    await broadcast.getCursor(sessionId);
    mkdirSync(join(homeDir, 'server'), { recursive: true });
    writeFileSync(join(homeDir, 'server', 'events-v3'), 'not-a-directory', 'utf8');
    ...
    expect(broadcast._bufferLengthForTest(sessionId)).toBe(0);
    expect(() => broadcast._currentSeqForTest(sessionId)).toThrow(writeError);
    await expect(broadcast.getCursor(sessionId)).rejects.toBe(writeError);
    await expect(broadcast.getBufferedSince(sessionId, { seq: 0 })).rejects.toBe(writeError);
    ...
    await expect(broadcast._drainForTest(sessionId)).rejects.toBe(writeError);
    expect(connection.sent).toEqual([]);
    expect(broadcast._bufferLengthForTest(sessionId)).toBe(0);
    await broadcast.closeJournals();
    broadcast.dispose();
  ...
  it('drops events without a sessionId / session_id and warns', async () => {
    const clients = new FakeSessionClients();
    const c = fakeConn();
  ...
  it('getBufferedSince forces a resync for a cursor ahead of the journal', async () => {
    const bus = new EventService();
    const broadcast = new WSBroadcastService(bus, testLogger, new FakeSessionClients(), new FakeConnectionRegistry(), makeEnv());
...
describe('ApprovalService (broadcasts + resolve-by-approval_id)', () => {
  ...
  it('resolve() for an unknown id is a no-op (REST route handles 40404 via isPending)', async () => {
    const { broker, bus, broadcast } = makeBrokerWithBus();

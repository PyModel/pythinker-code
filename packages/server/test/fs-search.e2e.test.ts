...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-fs-search-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-fs-search-home-'));
...
describe('FsSearchService direct: rg fallback + grep timeout (W11.1)', () => {
  function makeStubSession(cwd: string): ISessionService {
    return {
      ...
      get: async () => ({
        id: 'sess_stub',
        metadata: { cwd, model: 'pythinker-k2', created_at: '2026-06-04T00:00:00Z' },
        status: 'idle',

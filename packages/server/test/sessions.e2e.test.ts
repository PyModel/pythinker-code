 ...
 * home) and exercise the 5 endpoints via `app.inject(...)`. PythinkerCore is fully
 * constructed via the W3 bridge pattern; the HOME dir is a fresh tmpdir so
 * no `~/.pythinker` interference. This is non-hermetic in the sense that plugin
...
import { ErrorCode, sessionSchema, sessionStatusResponseSchema, undoSessionResponseSchema } from '@pymodel/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-sessions-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-sessions-home-'));
});
...
describe('POST /api/v1/sessions — create', () => {
  ...
  it('reports web client headers in new-session telemetry', async () => {
    ...
    const res = await appOf(r).inject({
      ...
      url: '/api/v1/sessions',
      headers: {
        'x-pythinker-client-id': 'web_test_client',
        'x-pythinker-client-name': 'pythinker-code-web',
        'x-pythinker-client-version': '0.1.1',
        'x-pythinker-client-ui-mode': 'web',
      },
    ...
    expect(records).toContainEqual({
      ...
      properties: {
        ...
        client_name: 'pythinker-code-web',

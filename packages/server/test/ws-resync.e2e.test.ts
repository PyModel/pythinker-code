/**
 * WS durable journal + resync_required e2e (v3 sync protocol).
 *
 * Flows:
 *
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-ws-resync-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-ws-resync-home-'));
});
...
describe('WS durable journal + resync_required (v3)', () => {
  it('reconnect with a cursor replays buffered events in order', async () => {
    ...
    expect(ack.payload).toEqual({ accepted_subscriptions: [], resync_required: [] });

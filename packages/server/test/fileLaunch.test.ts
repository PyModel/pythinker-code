...
describe('file launch commands', () => {
  it('uses configured editors and preserves line targets for VS Code style editors', () => {
    expect(openFileCommandFor('/repo/src/App.vue', 12, { PYTHINKER_CODE_EDITOR: 'code -g' }, 'darwin')).toEqual({
...
describe('open-in-app launch commands', () => {
  ...
  it('falls back to the platform default for macOS-only apps on other platforms', () => {
    // The fallback consults process.env for a configured editor; clear the
    // ambient editor vars so the platform default is actually exercised.
    vi.stubEnv('PYTHINKER_CODE_EDITOR', '');
    vi.stubEnv('VISUAL', '');
    vi.stubEnv('EDITOR', '');

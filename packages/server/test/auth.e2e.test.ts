 ...
 * `startServer` so PythinkerCore loads it on construction. The `homeDir` we pass
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-auth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-auth-home-'));
});
 ...
 * `resolveConfigPath({homeDir})` exactly so PythinkerCore + AuthSummaryService
 * load the same file.
 */

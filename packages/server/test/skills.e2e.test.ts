 ...
 * `<cwd>/.pythinker-code/skills/<name>/SKILL.md` BEFORE the session is created,
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-skills-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-skills-home-'));
...
/** Seed a project skill bundle at `<cwd>/.pythinker-code/skills/<name>/SKILL.md`. */
function seedProjectSkill(name: string, frontmatterType?: string): void {
  const dir = join(workspaceDir, '.pythinker-code', 'skills', name);

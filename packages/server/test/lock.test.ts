 ...
 * `~/.pythinker/server/lock` is never touched. We mint pid values that don't
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-lock-test-'));
...
describe('acquireLock — concurrent-instance protection', () => {
  it('throws ServerLockedError when a live owner already holds the lock', () => {
    ...
    const error = captureError(() => acquireLock({ lockPath, port: 58627 }));
    const lockedError = error as ServerLockedError;
    expect(lockedError.code).toBe('ESERVER_LOCKED');
    expect(lockedError.exitCode).toBe(2);
    expect(lockedError.message).toContain(`pid=${process.pid}`);
    expect(lockedError.message).toContain('port=58627');
    expect(lockedError.existing).toEqual(existing);
  });
...
describe('acquireLock — defaults', () => {
  it('DEFAULT_LOCK_PATH points under the pythinker-code home', () => {
    expect(DEFAULT_LOCK_PATH).toMatch(/[/\\]\.pythinker-code[/\\]server[/\\]lock$/);
  });
});

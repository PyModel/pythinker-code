...
import { WS_PROTOCOL_VERSION, type Terminal } from '@pythoughts/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-ws-terminal-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-ws-terminal-home-'));
...
async function openSocket(r: RunningServer): Promise<{
  ...
  const serverHello = await waitFor(received, (frame) => frame['type'] === 'server_hello');
  expect((serverHello['payload'] as { protocol_version?: number }).protocol_version).toBe(WS_PROTOCOL_VERSION);
  ...
  await waitFor(received, (frame) => frame['type'] === 'ack' && frame['id'] === 'terminal-hello');
  return { ws, received };
}

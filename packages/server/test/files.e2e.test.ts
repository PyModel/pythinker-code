...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-files-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'pythinker-server-files-home-'));
});
...
function buildMultipart(parts: {
  ...
  const boundary = '------WebKitFormBoundaryPythinkerDaemonTest';

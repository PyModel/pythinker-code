...
const TEST_HOME = join(homedir(), '.pythinker-code-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'pythinker-code.log' || file.startsWith('pythinker-code.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('pythinker-code.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('pythinker-code.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['PYTHINKER_CODE_HOME'] = TEST_HOME;
  process.env['PYTHINKER_LOG_LEVEL'] = 'warn';
  process.env['PYTHINKER_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['PYTHINKER_LOG_SESSION_FILES'] = '2';
  process.env['PYTHINKER_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['PYTHINKER_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = createPythinkerHarness({
    identity: { productName: 'pythinker-code-cli', version: '0.1.1', platform: 'pythinker_code_cli' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        '  - logs/pythinker-code.log.1',
        `  - ${summary.sessionDir}/logs/pythinker-code.log`,
        `  - ${summary.sessionDir}/logs/pythinker-code.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - pythinker-code.log.1 exists',
        '  - pythinker-code.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/pythinker-code.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/pythinker-code.log',
        '  - global rotated logs such as ~/.pythinker-code-test/logs/pythinker-code.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',

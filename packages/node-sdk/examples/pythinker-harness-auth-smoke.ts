...
async function main(): Promise<void> {
  ...
  const explicitWorkDir = process.env['PYTHINKER_SDK_AUTH_SMOKE_WORK_DIR'];
  const homeDir = explicitHomeDir ?? (await mkdtemp(join(tmpdir(), 'pythinker-sdk-auth-smoke-home-')));
  const workDir = explicitWorkDir ?? (await mkdtemp(join(tmpdir(), 'pythinker-sdk-auth-smoke-work-')));
  const keepToken = shouldKeepToken(explicitHomeDir !== undefined);
  const forceLogin = process.env['PYTHINKER_SDK_AUTH_SMOKE_FORCE_LOGIN'] === '1';
  const prompt =
    process.env['PYTHINKER_SDK_AUTH_SMOKE_PROMPT'] ?? 'Reply with exactly: Pythinker SDK auth smoke ok';
...
function printUsage(usage: Awaited<ReturnType<PythinkerHarness['auth']['getManagedUsage']>>): void {
  if (usage.kind === 'error') {
...
function shouldKeepToken(hasExplicitHomeDir: boolean): boolean {
  const value = process.env['PYTHINKER_SDK_AUTH_SMOKE_KEEP_TOKEN'];

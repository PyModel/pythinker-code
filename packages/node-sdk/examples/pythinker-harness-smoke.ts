...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'pythinker-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'pythinker-harness-home-'));
  ...
  try {
    ...
    const model = config.defaultModel ?? 'pythinker-code/pythinker-for-coding';

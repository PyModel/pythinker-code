...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/node-sdk/examples/pythinker-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Pythinker home dir; defaults to PYTHINKER_CODE_HOME or ~/.pythinker-code
...
async function main(): Promise<void> {
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'pythinker-code.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that PYTHINKER_LOG_LEVEL is not "off" and that the session id exists in this PYTHINKER_CODE_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;

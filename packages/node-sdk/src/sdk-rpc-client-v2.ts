 ...
 * (DI × Scope) instead of the v1 `PythinkerCore` RPC pair. The engine is
 ...
 *   dynamic_workflow services plus the facade. `importContext` composes v1's exact
...
import { MCP_SECTION, type McpSection } from '@pymodel/agent-core-v2/app/mcpConfig/configSection';
import { IAgentIdentity } from '@pymodel/agent-core-v2/app/agentIdentity/agentIdentity';
...
import { SECONDARY_MODEL_SECTION } from '@pymodel/agent-core-v2/app/kosongConfig/configSection';
import { IAtomicDocumentStore } from '@pymodel/agent-core-v2/persistence/interface/atomicDocumentStore';
import { wrapSubagentModelError } from '@pymodel/agent-core-v2/session/subagent/configSection';
...
import type { AgentHandle, Klient } from '@pymodel/klient';
import { createKlient } from '@pymodel/klient/memory';
...
import {
  ...
  planProviderRemoval,
  resolvedConfigToPythinkerConfig,
...
export class SDKRpcClientV2 extends SDKRpcClientBase {
    ...
    const { app } = bootstrap(
      {
        ...
        args: {
          ...
          requestHeaders: createPythinkerDefaultHeaders({ homeDir: this.homeDir, ...identity }),
  ...
  /**
   * v1 returns the whole config.toml document as one `PythinkerConfig`; v2
   ...
   * runtime config (`loadRuntimeConfigSafe` + the PYTHINKER_MODEL_* overlay);
   * `reload` mirrors v1's re-read-from-disk option.
   */
  override async getConfig(options?: GetConfigOptions): Promise<PythinkerConfig> {
    await this.configReady;
    if (options?.reload) {
      await this.klient.global.config.reload();
    }
    return resolvedConfigToPythinkerConfig(await this.klient.global.config.getAll());
  }
   ...
   */
  override async setConfig(patch: PythinkerConfigPatch): Promise<PythinkerConfig> {
    await this.configReady;
   ...
   * the same single-write shape as v1's `removePythinkerProvider`, so a process
  ...
  /**
   * Capability surface (v2-only): built-in product capabilities (pythinker-cu,
   * pythinker-webbridge) with layered readiness and idempotent installs. v1 has
  ...
  private static sessionNotFound(sessionId: string): PythinkerError {
    return new PythinkerError(ErrorCodes.SESSION_NOT_FOUND, `Session "${sessionId}" was not found`, {
      details: { sessionId },
    });
  }
   ...
   * project-local `.pythinker-code/local.toml`, `persist: false` joins the
   ...
   * `getPermission` / `getPlan` / `getDynamicWorkflowMode` / `getUsage` RPCs. The v2
   ...
   * facade's context/plan/usage, and the permission-mode and dynamic_workflow services.
   */
  ...
  /**
   * Through the agent scope (`IAgentDynamicWorkflowService.enter` / `.exit`) — no
   * klient facade exists. The v2 service is the port of v1's `DynamicWorkflowMode`:
   ...
   * replaced wholesale; `dynamic_workflow()` below recomposes it over this override.
   */
  override async setDynamicWorkflowMode(input: SetSessionDynamicWorkflowModeRpcInput): Promise<void> {
    const agent = await this.agentScope(input.sessionId);
    const dynamic_workflow = agent.accessor.get(IAgentDynamicWorkflowService);
    if (input.enabled) {
      dynamic_workflow.enter(input.trigger);
      return;
    }
  ...
  override async completeGlobalMcpServerAuth(
    ...
    if (active === undefined) {
      throw new PythinkerError(ErrorCodes.REQUEST_INVALID, `Unknown MCP OAuth flow: ${input.flowId}`);
    }
    try {

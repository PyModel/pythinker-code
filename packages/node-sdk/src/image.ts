import type { ITelemetryService } from '@pymodel/agent-core-v2/app/telemetry/telemetry';
import {
  MAX_IMAGE_EDGE_PX,
...
} from '@pymodel/agent-core-v2/agent/media/image-compress';
...
export class ImageLimits {
  ...
  maxEdgePx(): number {
    return positiveIntFromEnv(this.env, 'PYTHINKER_IMAGE_MAX_EDGE_PX') ?? this.config?.maxEdgePx ?? MAX_IMAGE_EDGE_PX;
  }

  readByteBudget(): number {
    return (
      positiveIntFromEnv(this.env, 'PYTHINKER_IMAGE_READ_BYTE_BUDGET') ?? this.config?.readByteBudget ?? READ_IMAGE_BYTE_BUDGET
    );
  }
}

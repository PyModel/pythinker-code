/**
 * The v1 user-global MCP surface (`<PYTHINKER_CODE_HOME>/mcp.json` CRUD plus the
...
import type { McpConnectionManager } from '@pymodel/agent-core-v2/mcpCore/connection-manager';
import { atomicWrite } from '@pymodel/agent-core-v2/_base/utils/fs';
...
function normalizeServerName(name: string): string {
  ...
  throw new PythinkerError(ErrorCodes.REQUEST_INVALID, 'MCP server name cannot be empty');
}

function serverNotFound(name: string): PythinkerError {
  return new PythinkerError(ErrorCodes.MCP_SERVER_NOT_FOUND, `MCP server "${name}" was not found`);
}

function configError(message: string, cause?: unknown): PythinkerError {
  return new PythinkerError(ErrorCodes.CONFIG_INVALID, message, { cause });
}

 ...
 * `inspect().userValue` / `diagnostics()`) onto the v1 `PythinkerConfig` /
 * `ConfigDiagnostics` shapes the SDK contract returns.
 *
 * Why a mapping layer exists: v1 loads config.toml as ONE zod-validated
 * document (`PythinkerConfigSchema`), while v2 registers one config section per
 ...
 */
import type { ConfigDiagnostics, PythinkerConfig } from '#/types';

/**
 * Every top-level `PythinkerConfig` field except `raw` (a v1 write-path
 * implementation detail with no v2 counterpart). Each entry is both the v1
 * field name and the v2 config domain name.
 */
const PYTHINKER_CONFIG_DOMAINS = [
  'providers',
  'defaultProvider',
  'defaultModel',
  'models',
 ...
 */
export function resolvedConfigToPythinkerConfig(resolved: Record<string, unknown>): PythinkerConfig {
  const config: Record<string, unknown> = {};
  for (const domain of PYTHINKER_CONFIG_DOMAINS) {
    const value = resolved[domain];
    if (value !== undefined) {
      config[domain] = value;
    }
  }
  return config as PythinkerConfig;
}
...
/** The writes needed to reproduce v1 `removePythinkerProvider` semantics. */
...
/**
 * Apply the v1 remove-provider cascade to a whole `PythinkerConfig` in memory (no
 ...
 * own refresh path.
 */
export function removeProviderFromConfig(config: PythinkerConfig, providerId: string): PythinkerConfig {
  ...
  return {
    ...config,
    providers: plan.providers as PythinkerConfig['providers'],
    models: plan.models as PythinkerConfig['models'],
    ...
    secondaryModel:
      plan.secondaryModel === null
        ? undefined
        : ((plan.secondaryModel ?? config.secondaryModel) as PythinkerConfig['secondaryModel']),
  };
}

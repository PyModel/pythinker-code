...
describe('Session.cancel', () => {
  it('cancels an active streaming turn and emits turn_ended(cancelled)', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-cancel-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-cancel-work-');
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-cancel-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-cancel-compact-work-');
    await writeFakeModelConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
...
describe('PythinkerHarness.forkSession', () => {
  it('rejects while the source session has an active turn', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-fork-active-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-fork-active-work-');
    await writeFakeModelConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });

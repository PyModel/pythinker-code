...
import { createPythinkerHarness, log } from '#/index';
...
const LOG_ENV_KEYS = [
  'PYTHINKER_LOG_LEVEL',
  'PYTHINKER_LOG_GLOBAL_MAX_BYTES',
  'PYTHINKER_LOG_GLOBAL_FILES',
  'PYTHINKER_LOG_SESSION_MAX_BYTES',
  'PYTHINKER_LOG_SESSION_FILES',
] as const;

beforeEach(async () => {
  process.env['PYTHINKER_LOG_LEVEL'] = 'info';
  await __resetRootLoggerForTest();
});

afterEach(async () => {
  await __resetRootLoggerForTest();
  process.env['PYTHINKER_LOG_LEVEL'] = 'off';
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    const homeDir = await makeTempDir('pythinker-log-home-');
    const workDir = await makeTempDir('pythinker-log-work-');
    ...
    const globalPath = join(homeDir, 'logs', 'pythinker-code.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'pythinker-code.log');

    // Trigger an export — this flushes both global and session via PythinkerCore
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/pythinker-code.log')).toBe(true);
    expect(entries.has('logs/global/pythinker-code.log')).toBe(false);
    expect(entries.get('logs/pythinker-code.log')!.toString('utf-8')).toContain(
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/pythinker-code.log');
  ...
  it('default export works when no session log file exists', async () => {
    ...
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/pythinker-code.log')).toBe(false);
  ...
  it('default export includes rotated session log files without requiring active pythinker-code.log', async () => {
    const env = snapshotLogEnv();
    process.env['PYTHINKER_LOG_LEVEL'] = 'warn';
    process.env['PYTHINKER_LOG_SESSION_MAX_BYTES'] = '1024';
    process.env['PYTHINKER_LOG_SESSION_FILES'] = '2';
    try {
      const homeDir = await makeTempDir('pythinker-log-home-');
      const workDir = await makeTempDir('pythinker-log-work-');
      const harness = createPythinkerHarness({ identity: TEST_IDENTITY, homeDir });
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/pythinker-code.log' || entry.startsWith('logs/pythinker-code.log.'),
      );
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/pythinker-code.log')).toBe(true);
    expect(result.manifest.globalLogPath).toBe('logs/global/pythinker-code.log');
    // Global log carries entries that don't have a sessionId routed to a sink.
    expect(entries.get('logs/global/pythinker-code.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });

  it('--include-global-log bundles the active root global log path', async () => {
    const firstHome = await makeTempDir('pythinker-log-home-a-');
    const secondHome = await makeTempDir('pythinker-log-home-b-');
    const workDir = await makeTempDir('pythinker-log-work-');
    const first = createPythinkerHarness({ identity: TEST_IDENTITY, homeDir: firstHome });
    const firstSession = await first.createSession({ id: 'ses_first_global_export', workDir });
    const second = createPythinkerHarness({ identity: TEST_IDENTITY, homeDir: secondHome });
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/pythinker-code.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'pythinker-code.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/pythinker-code.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/pythinker-code.log')!.toString('utf-8');
  ...
  it('multiple PythinkerHarness constructions in the same process do not throw', async () => {
    const homeDir = await makeTempDir('pythinker-log-home-');
    expect(() => createPythinkerHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createPythinkerHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createPythinkerHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
  });
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'pythinker-code.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'pythinker-code.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    try {
      ...
      expect(logsDir).not.toContain('pythinker-code.log');
    } finally {
      restoreLogEnv(env);
    }
  });

  it('PythinkerHarness.close() flushes the global log', async () => {
    const homeDir = await makeTempDir('pythinker-log-home-');
    const harness = createPythinkerHarness({ identity: TEST_IDENTITY, homeDir });

...
import { createPythinkerHarness, PythinkerError, type Event } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'pythinker-sdk-rename-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('PythinkerHarness.renameSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    try {
      ...
      await expect(missingRename).rejects.toBeInstanceOf(PythinkerError);

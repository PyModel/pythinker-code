 ...
 * Wiring: real PythinkerHarness/Core/filesystem and stdio transport; only the OAuth
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'pythinker-sdk-mcp-'));
  tempDirs.push(dir);
  return dir;
}
...
function oauthHarness(rpc: OAuthRpc): PythinkerHarness {
  return new PythinkerHarness(rpc, {
    homeDir: '/tmp/pythinker-sdk-mcp-oauth-home',
    configPath: '/tmp/pythinker-sdk-mcp-oauth-home/config.toml',
    auth: {} as never,

...
describe('Session context', () => {
  it('restores a session-only additional directory after close and resume', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-additional-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-additional-work-');
    const additionalDir = await makeTempDir(tempDirs, 'pythinker-sdk-additional-dir-');
  ...
  it('clears context without replacing the session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-work-');
  ...
  it('appends old-compatible user context markup when importing raw content', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-import-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-import-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('emits the estimated context token status after an import', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-status-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-status-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('restores the imported message with its estimated token count after resume', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-resume-work-');
    await writeTestConfig(homeDir, 200_000);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects whitespace-only imported content without mutating context', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-empty-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-empty-work-');
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an import that would push existing context beyond the model limit', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-overflow-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-context-overflow-work-');

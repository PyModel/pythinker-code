...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'pythinker-sdk-prompt-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('Session.prompt events', () => {
  it('continues notifying sessions after disabling and restarting with identical prompt and tools', async () => {
    ...
    let harness = createPythinkerHarness({
    ...
    try {
      ...
      for (let turn = 0; turn < 3; turn++) {
        ...
        if (turn === 2) {
          unsubscribe();
          await harness.close();
          harness = createPythinkerHarness({ identity: TEST_IDENTITY, homeDir, uiCapabilities: [] });
  ...
  it('emits mapped turn events through Session.onEvent', async () => {
    ...
    try {
      ...
      expect(fakeProviderState.calls[0]?.systemPrompt).toContain('You are Pythinker Code CLI');
...
async function configureFakeProvider(harness: PythinkerHarness): Promise<void> {

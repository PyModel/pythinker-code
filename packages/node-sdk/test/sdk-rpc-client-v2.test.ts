 ...
 * Wiring: real v2 engine bootstrapped on a temp PYTHINKER_CODE_HOME; no provider calls.
...
import { IHostRequestHeaders } from '@pymodel/agent-core-v2';
...
describe('SDKRpcClientV2 (agent-core-v2 wiring MVP)', () => {
  ...
  it('fails loudly with not_implemented for methods not yet migrated', async () => {
    ...
    try {
      ...
      await expect(harness.deleteSession('session_missing')).rejects.toThrowError(PythinkerError);

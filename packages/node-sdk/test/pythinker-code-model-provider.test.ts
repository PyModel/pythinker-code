...
import { ErrorCodes, PythinkerError, PythinkerForCodingProvider } from '#/index';

import { TEST_IDENTITY } from './test-identity';

describe('PythinkerForCodingProvider OAuth error mapping', () => {
  let homeDir: string;

  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-for-coding-provider-'));
  });
  ...
  function resolveAuth() {
    const provider = new PythinkerForCodingProvider({ homeDir, ...TEST_IDENTITY });
    return provider.resolveAuth('pythinker-for-coding');
  }

  it('maps unauthorized token failures to auth.login_required', async () => {
    vi.spyOn(PythinkerOAuthToolkit.prototype, 'ensureFresh').mockRejectedValue(
      new OAuthUnauthorizedError('No token for "pythinker-code". Run /login to authenticate.'),
    );
  ...
  it('maps transient token failures to provider.connection_error', async () => {
    ...
    for (const tokenError of tokenErrors) {
      vi.spyOn(PythinkerOAuthToolkit.prototype, 'ensureFresh').mockRejectedValue(tokenError);

      const auth = resolveAuth();
      const caught = await auth(async () => 'ok').catch((error: unknown) => error);

      expect(caught).toBeInstanceOf(PythinkerError);
  ...
  it('rethrows unrecognized OAuth errors raw instead of guessing a category', async () => {
    ...
    vi.spyOn(PythinkerOAuthToolkit.prototype, 'ensureFresh').mockRejectedValue(oauthError);

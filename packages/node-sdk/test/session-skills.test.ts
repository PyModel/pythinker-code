...
describe('Session skills', () => {
        ...
        input: [
          {
            ...
            text: [
              ...
              '',
              `<pythinker-skill-loaded name="review" trigger="user-slash" source="project" dir="${skillDir}" args="src/app.ts">`,
              'Review the requested file.',
              '',
  ...
  it('resolves user brand skills from PYTHINKER_CODE_HOME, not the OS home', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-skills-home-');
    const processHome = await makeTempDir(tempDirs, 'pythinker-sdk-skills-process-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-skills-work-');
    ...
    const harness = createPythinkerHarness({ identity: TEST_IDENTITY });

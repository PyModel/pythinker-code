 ...
 * agent-core-v2, the v1 harness (`createPythinkerHarness`) and the v2 harness
 * (`createPythinkerHarnessV2`) must return identical values on the same fixture
 ...
 * PYTHINKER_CODE_HOME; no provider calls.
...
/**
 * Projection for every method returning a `PythinkerConfig`: drop the fields whose
 * presence semantics genuinely differ between the engines (see KNOWN_DIFFS).
 */
function projectConfig(config: PythinkerConfig): unknown {
...
const KNOWN_DIFFS = {
  ...
  getConfig: (config: PythinkerConfig): unknown => projectConfig(config),
  setConfig: (config: PythinkerConfig): unknown => projectConfig(config),
  removeProvider: (config: PythinkerConfig): unknown => projectConfig(config),
  ...
  // records typed reminder-render state there (dynamic_workflow mode, once-reminder
...
interface ParityFixture {
  readonly v1: PythinkerHarness;
  readonly v2: PythinkerHarness;
  readonly homeDir: string;
}

async function makeParityPair(): Promise<ParityFixture> {
  const homeDir = await makeTempDir('pythinker-sdk-parity-home-');
  const v1 = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  const v2 = createPythinkerHarnessV2({ homeDir, identity: TEST_IDENTITY });
  return { v1, v2, homeDir };
}

async function closeAll(...harnesses: readonly PythinkerHarness[]): Promise<void> {
...
/**
 * Config-reading env vars both engines honor (`PYTHINKER_MODEL_*`, the per-section
...
async function makeIsolatedParityPair(
  configToml?: string,
): Promise<{ v1: PythinkerHarness; v2: PythinkerHarness }> {
  const v1Home = await makeTempDir('pythinker-sdk-parity-v1-home-');
  const v2Home = await makeTempDir('pythinker-sdk-parity-v2-home-');
  if (configToml !== undefined) {
    await writeFile(join(v1Home, 'config.toml'), configToml, 'utf-8');
    await writeFile(join(v2Home, 'config.toml'), configToml, 'utf-8');
  }
  return {
    v1: createPythinkerHarness({ homeDir: v1Home, identity: TEST_IDENTITY }),
    v2: createPythinkerHarnessV2({ homeDir: v2Home, identity: TEST_IDENTITY }),
  };
}
...
function expectConfigParity(v1Config: PythinkerConfig, v2Config: PythinkerConfig): void {
...
describe('v1↔v2 return-value parity', () => {
  ...
  it('listWorkspaceSkills returns the same skills on the same fixture', async () => {
    ...
    const workDir = await makeTempDir('pythinker-sdk-parity-work-');
    await writeSkill(join(homeDir, 'skills', 'parity-user-skill'), 'parity-user-skill');
    await writeSkill(
      join(workDir, '.pythinker-code', 'skills', 'parity-project-skill'),
...
// Plugin parity
//
// The plugin methods live on the rpc client itself (PythinkerHarness has no
...
async function writeFixturePlugin(dir: string): Promise<void> {
  ...
  await writeFile(
    join(dir, 'pythinker.plugin.json'),
    JSON.stringify(
      {
...
async function makePluginParityPair(configToml?: string): Promise<PluginParityPair> {
  const v1HomeDir = await makeTempDir('pythinker-sdk-parity-v1-home-');
  const v2HomeDir = await makeTempDir('pythinker-sdk-parity-v2-home-');
  const sourceDir = await makeTempDir('pythinker-sdk-parity-plugin-src-');
  if (configToml !== undefined) {
...
describe('v1↔v2 session lifecycle parity', () => {
  it('createSession returns the same summary for the same input', async () => {
    ...
    const extraDir = await makeTempDir('pythinker-sdk-parity-extra-');
    try {
      const input = {
  ...
  it('listSessions reflects the same creates and renames across filters', async () => {
    const pair = await makeSessionParityPair();
    const otherWorkDir = await makeTempDir('pythinker-sdk-parity-work-other-');
    try {
  ...
  it('addAdditionalDir returns the same result for persist and non-persist', async () => {
    const pair = await makeSessionParityPair();
    const persistedDir = await makeTempDir('pythinker-sdk-parity-extra-persisted-');
    const sessionOnlyDir = await makeTempDir('pythinker-sdk-parity-extra-session-');
    try {
      ...
      expect(v1Persisted).toMatchObject({
        ...
        configPath: join(pair.workDir, '.pythinker-code', 'local.toml'),
        persisted: true,
      });
...
/**
 * Agent fixture: a pythinker-typed provider (strict thinking validation on both
...
describe('v1↔v2 agent interaction parity', () => {
  ...
  it('setThinking applies and rejects unlisted efforts identically', async () => {
    ...
    try {
      ...
      // An unlisted effort on a strict-thinking (pythinker-typed) model rejects
  ...
  it('activateSkill renders, launches, and rejects identically', async () => {
    ...
    await writeSkill(join(pair.workDir, '.pythinker-code', 'skills', 'parity-skill'), 'parity-skill');
    try {
 ...
 * class is pinned because SDK consumers branch on `isPythinkerError`.
 */
...
describe('v1↔v2 global MCP parity', () => {
  ...
  it('picks up a grant saved after the first probe when verifying', async () => {
    ...
    try {
      ...
      for (const homeDir of [pair.v1HomeDir, pair.v2HomeDir]) {
        await new McpOAuthService({ pythinkerHomeDir: homeDir })
  ...
  it.each([
    ...
    const pair = await makeGlobalMcpParityPair();
    const project = await makeTempDir('pythinker-sdk-parity-mcp-project-');
    await mkdir(join(project, '.pythinker-code'), { recursive: true });
    await writeFile(
      join(project, '.pythinker-code', 'mcp.json'),
  ...
  it('threads cwd through project-layer inspection and authorization on both engines', async () => {
    const pair = await makeGlobalMcpParityPair();
    const project = await makeTempDir('pythinker-sdk-parity-mcp-auth-project-');
    await mkdir(join(project, '.pythinker-code'), { recursive: true });
    await writeFile(
      join(project, '.pythinker-code', 'mcp.json'),
      JSON.stringify({
...
describe('v1↔v2 session MCP parity', () => {
  ...
  it('a persisted session add over an enabled plugin entry follows each engine\'s precedence', async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeSessionMcpPair();
    const pluginSource = await makeTempDir('pythinker-sdk-parity-mcp-plugin-src-');
    ...
    try {
      ...
      // (`isPythinkerError` is the SDK's public error class on both engines). v2
      ...
      expect(isPythinkerError(v1Error)).toBe(true);
...
// ---------------------------------------------------------------------------
// Residual surface parity (exportSession / startBtw / dynamic_workflow mode / listSkills)
//
...
describe('v1↔v2 residual surface parity', () => {
  it('exportSession packages the same entry list and manifest on both engines', async () => {
    const pair = await makeSessionParityPair();
    const outDir = await makeTempDir('pythinker-sdk-parity-export-');
    try {
  ...
  it('setDynamicWorkflowMode toggles dynamic_workflow mode with the same reminder lifecycle on both engines', async () => {
    const pair = await makeSessionParityPair();
    try {
      await createOnBoth(pair, { id: 'session_parity_dynamic_workflow' });
      const input = { sessionId: 'session_parity_dynamic_workflow' } as const;
      const statusOnBoth = () =>
      ...
      await Promise.all([
        pair.v1.setDynamicWorkflowMode({ ...input, enabled: true, trigger: 'manual' }),
        pair.v2.setDynamicWorkflowMode({ ...input, enabled: true, trigger: 'manual' }),
      ]);
      const [v1Active, v2Active] = await statusOnBoth();
      expect(v1Active.dynamicWorkflowMode).toBe(true);
      expect(v2Active.dynamicWorkflowMode).toBe(true);
      ...
      await Promise.all([
        pair.v1.setDynamicWorkflowMode({ ...input, enabled: false }),
        pair.v2.setDynamicWorkflowMode({ ...input, enabled: false }),
      ]);
      const [v1Inactive, v2Inactive] = await statusOnBoth();
      expect(v1Inactive.dynamicWorkflowMode).toBe(false);
      expect(v2Inactive.dynamicWorkflowMode).toBe(false);
      ...
      await Promise.all([
        pair.v1.setDynamicWorkflowMode({ ...input, enabled: true, trigger: 'tool' }),
        pair.v2.setDynamicWorkflowMode({ ...input, enabled: true, trigger: 'tool' }),
      ]);
      const [v1Tool, v2Tool] = await statusOnBoth();
      expect(v1Tool.dynamicWorkflowMode).toBe(true);
      expect(v2Tool.dynamicWorkflowMode).toBe(true);
  ...
  it('dynamic_workflow() enters task-triggered dynamic_workflow mode, prompts, and auto-exits after the turn', async () => {
    const pair = await makeSessionParityPair();
    try {
      await createOnBoth(pair, { id: 'session_parity_dynamic_workflow_prompt' });
      const input = { sessionId: 'session_parity_dynamic_workflow_prompt' } as const;
      ...
      // auto-exit. `dynamic_workflow()` itself resolves on both (v1's prompt RPC
      // returns after launch; v2's after enqueue).
      await Promise.all([
        pair.v1.dynamic_workflow({ ...input, input: [{ type: 'text', text: 'parity dynamic_workflow prompt' }] }),
        pair.v2.dynamic_workflow({ ...input, input: [{ type: 'text', text: 'parity dynamic_workflow prompt' }] }),
      ]);
      const settled = await waitForCondition(async () => {
        ...
        return v1Status.dynamicWorkflowMode === false && v2Status.dynamicWorkflowMode === false;
      });
  ...
  it('listSkills returns the same merged catalog on the same fixture', async () => {
    ...
    try {
      ...
      await writeSkill(
        join(pair.workDir, '.pythinker-code', 'skills', 'parity-project-skill'),
        'parity-project-skill',
      );

...
import { createPythinkerHarness, ErrorCodes, PythinkerError } from '#/index';
...
describe('PythinkerHarness.auth', () => {
  it('can construct auth facade without host identity', () => {
    expect(() => createPythinkerHarness({ homeDir })).not.toThrow();
  });
  ...
  it('maps transient OAuth token failures to provider connection errors', async () => {
    ...
    for (const tokenError of tokenErrors) {
      const tokenProviderSpy = vi
        .spyOn(PythinkerOAuthToolkit.prototype, 'tokenProvider')
        .mockReturnValue({
...
`,
  ...
  it('reports auth status from the configured scoped OAuth ref', async () => {
    ...
    await new FileTokenStorage(join(homeDir, 'credentials')).save(
      resolvePythinkerTokenStorageName({ oauthKey }),
...
`,
  ...
  it('logs in against the global region hosts when region is global', async () => {
    ...
    expect(
      await new FileTokenStorage(join(homeDir, 'credentials')).load(
        resolvePythinkerTokenStorageName({ oauthKey: PYTHINKER_CODE_OAUTH_KEY }),
      ),
    ).toBeUndefined();
  });
  ...
  it('logs back into the mainland-cn region over a persisted global login', async () => {
    ...
    const globalKey = resolvePythinkerCodeOAuthKey({
...
`,
    ...
    const storage = new FileTokenStorage(join(homeDir, 'credentials'));
    const defaultStorageName = resolvePythinkerTokenStorageName({ oauthKey: PYTHINKER_CODE_OAUTH_KEY });
    ...
    expect(provider?.oauth?.key).toBe(PYTHINKER_CODE_OAUTH_KEY);
...
`,
  ...
  it('logs in against environment OAuth host and base URL over persisted config', async () => {
    ...
    await storage.save(resolvePythinkerTokenStorageName({ oauthKey: envOauthKey }), {
      ...freshToken(),
...
`,
    ...
    expect(warnings.some((w) => w.includes('models.pythinker-code/kimi-for-coding'))).toBe(true);
  });
...
`,
    ...
    expect(text).not.toContain('pythinker-code/kimi-for-coding');
    expect(text).not.toContain('pymodel_search');
  });

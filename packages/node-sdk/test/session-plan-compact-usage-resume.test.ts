...
describe('Session plan, compact, usage, and resume APIs', () => {
  ...
  it('prepares the plans directory without creating plan files on repeated toggles', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-plan-toggle-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-plan-toggle-work-');
    await writeTestConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-compact-work-');
    await writeTestConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('resumes a persisted session and restores runtime plan mode from wire history', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-resume-work-');
    await writeTestConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-resume-legacy-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-resume-legacy-plan-work-');
    await writeTestConfig(homeDir);
    const createdHarness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
    try {
  ...
  it('forks a session, drops goal state, and returns an active fork session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-fork-home-');
    const workDir = await makeTempDir(tempDirs, 'pythinker-sdk-fork-work-');
    await writeTestConfig(homeDir);
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an empty resume id', async () => {
    const homeDir = await makeTempDir(tempDirs, 'pythinker-sdk-resume-empty-home-');
    const harness = createPythinkerHarness({ homeDir, identity: TEST_IDENTITY });

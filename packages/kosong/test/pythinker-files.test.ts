...
import { PythinkerFiles } from '#/providers/pythinker-files';
...
describe('PythinkerFiles', () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pythinker-files-test-'));
  });
  ...
  describe('exposure on PythinkerChatProvider', () => {
    it('exposes files property returning a PythinkerFiles instance', () => {
      const provider = createProvider();
      const files = provider.files;
      expect(files).toBeInstanceOf(PythinkerFiles);

...
function getGenerationState(provider: PythinkerChatProvider): PythinkerGenerationState {
  return Reflect.get(provider, '_generationKwargs') as PythinkerGenerationState;
}

/** Capture the request body sent to OpenAI by mocking the client. */
async function captureRequestBody(
  provider: PythinkerChatProvider,
  systemPrompt: string,
  tools: Tool[],
  history: Message[],
...
describe('PythinkerChatProvider', () => {
  describe('message conversion', () => {
    ...
    it('adds Pythinker-only types to JetBrains-like enum-only tool parameters without mutation', async () => {
      const provider = createProvider();
    ...
    it('dereferences draft-7 definitions and normalizes referenced enum-only schemas for Pythinker', async () => {
      const provider = createProvider();
  ...
  describe('provider properties', () => {
    ...
    it('throws during generation when no constructor or request API key is provided', async () => {
      ...
      const saved = process.env['PYTHINKER_API_KEY'];
      delete process.env['PYTHINKER_API_KEY'];
      try {
        const provider = new PythinkerChatProvider({ model: 'test' });
        await expect(provider.generate('', [], [])).rejects.toThrow(/options\.auth\.apiKey/);
      } finally {
        if (saved !== undefined) {
          process.env['PYTHINKER_API_KEY'] = saved;
        }
      }
    });
    ...
    it('withThinking returns a new instance', () => {
      ...
      expect(newProvider).toBeInstanceOf(PythinkerChatProvider);
  ...
  describe('clone client sharing', () => {
    ...
    function getInternalClient(provider: PythinkerChatProvider): unknown {
...
describe('classifyPythinkerQuotaError', () => {
  const QUOTA_MESSAGE =
  ...
  it.each([QUOTA_MESSAGE, TOKEN_QUOTA_MESSAGE])(
    ...
    (message) => {
      expect(classifyPythinkerQuotaError(quota429(message))).toBeInstanceOf(
  ...
  it.each(['Too many requests', 'your token quota per minute was exceeded'])(
    ...
    (message) => {
      expect(classifyPythinkerQuotaError(quota429(message))).toBeUndefined();
    },
  );

...
describe('convertOpenAIError: raw transport-layer stream errors', () => {
  ...
  it('still wraps an unrelated raw Error as a base ChatProviderError that fails fast', () => {
    ...
    // (constructor check guards that typing). Unclassified provider failures
    // are no longer retried by default — only text promoted to a typed
    // transient error (rate-limit patterns in classifyBaseApiError) retries.
...
describe('convertOpenAIError: quota-exhausted 429', () => {
  ...
  it('classifies vendor quota signals through the convertError hook', () => {
    ...
    const result = convertOpenAIError(err, classifyPythinkerQuotaError);
    expect(result).toBeInstanceOf(APIProviderQuotaExhaustedError);
    expect(isRetryableGenerateError(result)).toBe(false);
  });

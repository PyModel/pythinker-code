...
describe('normalizeAPIStatusError', () => {
  ...
  it.each([
    // PyModel / Pythinker 413 observed in the field when accumulated media pushed
...
describe('isToolExchangeAdjacencyError', () => {
  ...
  // The exact OpenAI-compatible (PyModel / Pythinker) message observed in the field
  ...
  // provider.
  const PYMODEL_TOOL_CALL_ID_NOT_FOUND = '400 tool_call_id  is not found';

  it('matches the OpenAI/PyModel tool_call_id-not-found 400', () => {
    expect(
      isToolExchangeAdjacencyError(new APIStatusError(400, PYMODEL_TOOL_CALL_ID_NOT_FOUND)),
    ).toBe(true);
    expect(
  ...
  it('also matches a 422 tool_call_id-not-found', () => {
    expect(
      isToolExchangeAdjacencyError(new APIStatusError(422, PYMODEL_TOOL_CALL_ID_NOT_FOUND)),
    ).toBe(true);
  });
...
describe('isRecoverableRequestStructureError', () => {
  ...
  it('matches the PyModel/Pythinker vacuous-message rejection', () => {
    expect(
      isRecoverableRequestStructureError(
        new APIStatusError(
          400,
...
describe('isImageFormatError', () => {
  it('matches documented provider image format/data rejections', () => {
    ...
    // PyModel/Pythinker (from the Pythinker Code error reference)
    expect(
      isImageFormatError(
  ...
  it('does not match image count/size/support errors that stripping media cannot fix', () => {
    ...
    // format/data errors: Anthropic's per-image size cap, PyModel's
...
describe('normalizeAPIStatusError: 429 stays vendor-neutral', () => {
  ...
  // classification lives with the vendor (`classifyPythinkerQuotaError`, the

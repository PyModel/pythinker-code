...
describe('integration: streaming provider contracts', () => {
  describe('synthetic Pythinker-style SSE streaming', () => {
    it('multi-chunk text + tool_call + tool_call_part + usage', async () => {
      ...
      const stream = buildStream(parts, { id: 'pythinker-resp-1', usage });
      const provider = new SingleStreamProvider(stream, 'pythinker');
      ...
      expect(result.id).toBe('pythinker-resp-1');
    });
    ...
    it('reasoning_content chunks interleave with text', async () => {
      ...
      const stream = buildStream(parts, { id: 'pythinker-think-1' });
      const provider = new SingleStreamProvider(stream, 'pythinker');

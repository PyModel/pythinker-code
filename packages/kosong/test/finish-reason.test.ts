...
import { PythinkerChatProvider } from '#/providers/pythinker';
...
describe('normalizeOpenAIFinishReason (Pythinker + OpenAILegacy shared helper)', () => {
  // Covers A for Pythinker + OpenAILegacy.
...
function makePythinkerStream(rawFinish: string | null | undefined): AsyncIterable<unknown> {
  // Pythinker / Chat Completions stream: emit one content chunk + a terminal
...
// The Pythinker provider consumes `APIPromise.withResponse()` to read the
// `x-trace-id` response header, so its mocked client must expose that method.
function makePythinkerChatClient(response: unknown) {
  return {
    chat: {
      completions: {
...
describe('PythinkerChatProvider finish reason (stream, table coverage)', () => {
  // A + B coverage for Pythinker.
  it.each<[string, FinishReason, string]>([
    ...
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createPythinkerProvider(makePythinkerStream(raw), true);
  ...
  it('returns null finishReason when stream never emits finish_reason', async () => {
    ...
    const provider = createPythinkerProvider(makeAsyncIterable(chunks), true);
  ...
  // C coverage for Pythinker.
  it('captures finish_reason from a non-stream response', async () => {
    const provider = createPythinkerProvider(
      {
        id: 'chatcmpl-ns',
        choices: [
          {
  ...
  // D (non-stream) coverage for Pythinker.
...
describe('OpenAILegacyChatProvider finish reason (stream + non-stream)', () => {
  ...
  it.each<[string, FinishReason, string]>([
    ...
    'raw stream finish_reason %j maps to %j (raw=%j)',
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createOpenAILegacyProvider(makePythinkerStream(raw), true);

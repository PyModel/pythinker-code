...
export class APIStatusError extends ChatProviderError {
   ...
   * (Pythinker/KFC only), or `null` when the error response did not carry one.
 ...
 *
 * Observed shapes: PyModel returns `error.type =
...
// providers (Anthropic, OpenAI, PyModel/Pythinker, Gemini) — image COUNT/SIZE
...
const IMAGE_FORMAT_STATUS_MESSAGE_PATTERNS = [
  // Unsupported format — OpenAI / PyModel "unsupported image …".
...
const REQUEST_TOO_LARGE_MESSAGE_PATTERNS = [
  // PyModel / Pythinker: "Request exceeds the maximum size".
...
The provider rejected the configured thinking effort. Non-Pythinker providers receive effort strings without client-side mapping; choose an effort supported by the selected model. For Pythinker models, check support_efforts and default_effort. See ${THINKING_EFFORT_CONFIG_DOCS_URL}`;
}
 ...
 * (Pythinker/KFC only). Returns `null` when the header is absent or empty.
 */
...
// `tool_call_id` / `role 'tool'` / `tool_calls`: PyModel / Pythinker as a
...
const TOOL_EXCHANGE_ADJACENCY_MESSAGE_PATTERNS = [
  ...
  // OpenAI-compatible (PyModel / Pythinker): a `tool` message references a
...
const STRUCTURAL_REQUEST_MESSAGE_PATTERNS = [
  ...
  // PyModel / Pythinker rejects a message whose serialized form carries nothing —

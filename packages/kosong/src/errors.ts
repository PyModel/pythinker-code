 ...
 * (retry helps), while quota exhaustion is deterministic until the account is
 * recharged — so this class is excluded from retry and from the rate-limit
 * requeue/suspend paths.
 */
...
export function isRetryableGenerateError(error: unknown): boolean {
  ...
  // Quota/balance exhaustion is a 429 but deterministic until the account is
  // recharged — retrying can never succeed, so it fails fast instead of

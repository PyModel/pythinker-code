 ...
 *
 * `minimal` and values above `high` are provider/model-specific and may be
 * clamped by the adapter when the native API has no matching level. OpenAI
 * supports `minimal`, `xhigh`, and `max` on selected models; Pythinker and
 * Gemini cap unsupported values; Anthropic supports `xhigh`/`max` only on
 * selected models.
 */
 ...
 *
 * Each provider implementation (Pythinker, OpenAI, Anthropic, Google GenAI, etc.)
...
export interface ChatProvider {
  /** Short identifier for the provider backend (e.g. `"pythinker"`, `"anthropic"`). */
  readonly name: string;
  /** Model name passed to the upstream API (e.g. `"pythoughts-v1-auto"`). */
  readonly modelName: string;
  /** Current thinking-effort level, or `null` if thinking is not configured. */
  readonly thinkingEffort: ThinkingEffort | null;
  /** Whether this model/provider surface supports its native premium Fast tier. */
  readonly supportsFastMode?: boolean;
  /** Whether this provider instance currently requests the native Fast tier. */
  ...
  /** Return a shallow copy that requests or clears the provider-native Fast tier. */
  withFastMode?(enabled: boolean): ChatProvider;
  /**

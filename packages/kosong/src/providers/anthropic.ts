...
import { supportsAnthropicFastModeModel } from './capability-registry';
import {
...
export interface AnthropicOptions {
  ...
  /** Explicit capability declaration for a compatible non-Anthropic gateway. */
...
/**
 * Fast mode is only advertised on Anthropic's own API; third-party gateways
 * must opt in through `fastModeSupported`. `baseUrl === undefined` means the
 * SDK default, which is the official endpoint.
 */
function isOfficialAnthropicFastModeEndpoint(baseUrl: string | undefined): boolean {
  if (baseUrl === undefined) return true;
  try {
    const url = new URL(baseUrl);
    const pathname = url.pathname.replace(/\/+$/, '');
    return url.origin === ANTHROPIC_API_ORIGIN && (pathname === '' || pathname === '/v1');
  } catch {
    return false;
  }
}
...
function lookupClaudeCeiling(version: AnthropicModelVersion): number | undefined {
  ...
  if (minor !== null) {
    for (let candidate = minor; candidate >= 0; candidate -= 1) {
...
function resolveThinkingProfile(
  ...
  if (adaptiveThinking === true) {
    const efforts = supportEfforts ?? inferred.efforts;
    return {
      ...inferred,
      mode: 'adaptive',
      efforts,
      supportsEffortParam:
        supportEfforts === undefined || efforts.filter((effort) => effort !== 'none').length > 1,
    };
  }
  if (supportEfforts === undefined) return inferred;
...
function convertMessage(message: Message, model: string): MessageParam {
  ...
  for (const part of message.content) {
    if (part.type === 'text') {
      ...
      // field. Anthropic-compatible backends (e.g. Pythinker) stream thinking with
...
export class AnthropicChatProvider implements ChatProvider {
  ...
  private readonly _thinkingBudgets: Readonly<Record<string, number>> | undefined;
  ...
  constructor(options: AnthropicOptions) {
    ...
    this._thinkingBudgets = options.thinkingBudgets;
  ...
  get thinkingEffort(): ThinkingEffort | null {
    ...
    if (budget === this._thinkingBudgets?.['max']) return 'max';
    if (budget === this._thinkingBudgets?.['high']) return 'high';
    if (budget <= 1024) {
      return 'low';
    }
    if (budget <= 4096) {
  ...
  get supportsFastMode(): boolean {
    return (
      ...
      (isOfficialAnthropicFastModeEndpoint(this._baseUrl) &&
        supportsAnthropicFastModeModel(this._model))
    );
  }

  get fastMode(): boolean {
    return this.supportsFastMode && this._generationKwargs.speed === 'fast';
  }
  ...
  async generate(
    ...
    if (this._generationKwargs.speed !== undefined) {
      kwargs['speed'] = this._generationKwargs.speed;
    }
  ...
  withThinking(effort: ThinkingEffort): AnthropicChatProvider {
    if (this._supportEfforts?.length === 0) return this._clone();
    ...
    const nativeEffort = effort === 'minimal' ? 'low' : effort;
    ...
    if (profile.mode === 'adaptive') {
      ...
      const clone = this._withGenerationKwargs({
        ...
        output_config: profile.supportsEffortParam ? { effort: nativeEffort } : undefined,
        betaFeatures: newBetas,
      });
      if (!profile.supportsEffortParam) delete clone._generationKwargs.output_config;
      return clone;
    }

    const budgetTokens = this._thinkingBudgets?.[nativeEffort] ?? budgetTokensForEffort(effort);
    ...
    if (profile.supportsEffortParam || budgetTokens === undefined) {
      kwargs.output_config = { effort: nativeEffort };
    } else {
  ...
  withFastMode(enabled: boolean): AnthropicChatProvider {
    const betaFeatures = (this._generationKwargs.betaFeatures ?? []).filter(
      (beta) => beta !== FAST_MODE_BETA,
    );
    if (enabled && this.supportsFastMode) betaFeatures.push(FAST_MODE_BETA);
    const clone = this._withGenerationKwargs({
      speed: enabled && this.supportsFastMode ? 'fast' : undefined,
      betaFeatures,
    });
    // `_withGenerationKwargs` keeps a key whose value is `undefined`; delete it
    // so a disabled Fast mode does not linger in `modelParameters`.
    if (!(enabled && this.supportsFastMode)) delete clone._generationKwargs.speed;
    return clone;
  }

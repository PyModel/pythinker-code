...
import { PythinkerChatProvider, type PythinkerOptions } from './pythinker';
...
export type ProviderConfig =
  ...
  | ({ type: 'pythinker' } & PythinkerOptions)
...
export function createProvider(config: ProviderConfig): ChatProvider {
  switch (config.type) {
    ...
    case 'pythinker':
      return new PythinkerChatProvider(config);
    case 'google-genai':
 ...
 * Unknown / uncatalogued models (and the Pythinker wire, whose capabilities come

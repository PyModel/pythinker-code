...
export interface PythinkerOptions {
...
const PYTHINKER_TOOL_CALL_ID_POLICY: ToolCallIdPolicy = {
...
function convertTool(tool: Tool): OpenAIToolParam {
  if (tool.name.startsWith('$')) {
    // Pythinker builtin functions start with `$`
    return {
...
class PythinkerStreamedMessage implements StreamedMessage {
  ...
  private async *_convertNonStreamResponse(
    ...
    // reasoning_content (PyModel proprietary)
...
export class PythinkerChatProvider implements ChatProvider {
  ...
  async generate(
    ...
    try {
      ...
      // Use type assertion via unknown because we pass PyModel-proprietary fields
  ...
  withGenerationKwargs(kwargs: GenerationKwargs): PythinkerChatProvider {
  ...
  private _withGenerationKwargs(kwargs: GenerationKwargs): PythinkerChatProvider {
    const clone = this._clone();
    clone._generationKwargs = { ...clone._generationKwargs, ...kwargs };
    return clone;
  }

  private _clone(): PythinkerChatProvider {
    const clone = Object.assign(
      Object.create(Object.getPrototypeOf(this) as object) as PythinkerChatProvider,
      this,
    );
    clone._generationKwargs = { ...this._generationKwargs };
    // Do not share the memoized PythinkerFiles instance with the clone; let it be

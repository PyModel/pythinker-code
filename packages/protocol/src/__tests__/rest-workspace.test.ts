...
const sampleWorkspace: Workspace = {
  id: 'wd_pythinker-code_0123456789ab',
  root: '/Users/foo/code/pythinker-code',
  name: 'pythinker-code',
...
describe('workspaceIdSchema', () => {
  it('accepts a wd_<slug>_<hash12> string', () => {
    expect(workspaceIdSchema.parse('wd_pythinker_0123456789ab')).toBe('wd_pythinker_0123456789ab');
  });

  it('accepts dots, dashes, underscores in slug', () => {
    expect(workspaceIdSchema.parse('wd_pythinker-code.v2_0123456789ab')).toBe(
      'wd_pythinker-code.v2_0123456789ab',
    );
  });

  it('rejects missing wd_ prefix', () => {
    expect(workspaceIdSchema.safeParse('pythinker_0123456789ab').success).toBe(false);
  });

  it('rejects non-hex tail', () => {
    expect(workspaceIdSchema.safeParse('wd_pythinker_xyzxyzxyzxyz').success).toBe(false);
  });

  it('rejects truncated hash', () => {
    expect(workspaceIdSchema.safeParse('wd_pythinker_0123456789').success).toBe(false);
  });
});
...
describe('workspaceIdParamSchema', () => {
  it('accepts a wd_-shaped workspace_id', () => {
    expect(
      workspaceIdParamSchema.parse({ workspace_id: 'wd_pythinker_0123456789ab' }).workspace_id,
    ).toBe('wd_pythinker_0123456789ab');
  });

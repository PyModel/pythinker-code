...
import type { SkillSummary } from '@pymodel/agent-core-v2';
...
describe('acp-server skills / available commands', () => {
   ...
   * `<cwd>/.pythinker-code/skills/acp-fixture/SKILL.md` (the engine's project
  ...
  async function bootWithFixtureSkill(): Promise<TestClient> {
    ...
    await mkdir(join(homeDir, '.pythinker-code', 'skills', 'acp-fixture'), { recursive: true });
    await writeFile(
      join(homeDir, '.pythinker-code', 'skills', 'acp-fixture', 'SKILL.md'),

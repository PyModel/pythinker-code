    ...
    async () => {
      ...
      try {
        ...
        const server = await runAcpServerWithStream(stream, {
          homeDir,
          terminalAuthEnv: { PYTHINKER_CODE_HOME: '/tmp/sandbox' },
          terminalAuthLegacyCommand: '/opt/pythinker/bin/pythinker',
        });
        ...
        expect(method.env).toEqual({ PYTHINKER_CODE_HOME: '/tmp/sandbox' });
        expect(method._meta?.['terminal-auth']).toMatchObject({
          command: '/opt/pythinker/bin/pythinker',
          args: ['login'],
          env: { PYTHINKER_CODE_HOME: '/tmp/sandbox' },
        });

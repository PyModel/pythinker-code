/**
 * acp-server bootstrap — wires `@pymodel/agent-core-v2` (the DI × Scope
 * engine) into an ACP (Agent Client Protocol) stdio server.
 *
 * Composition root: `bootstrap()` builds the App `Scope`; a `@pymodel/
...
export async function runAcpServerWithStream(
  ...
  // `PYTHINKER_CODE_PLATFORM` from `@pymodel/pythinker-code-oauth`, which this
  ...
  const { app: core } = bootstrap(
    {
      ...
      configPath,
      clientIdentity: {
        productName: opts.agentInfo?.name ?? 'pythinker-code-acp',

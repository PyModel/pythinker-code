...
async function makePythinkerHome(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), 'pythinker-home-'));
}
...
describe('PluginManager', () => {
  ...
  it('install() accepts a .pythinker-plugin manifest', async () => {
    const home = await makePythinkerHome();
    const root = await mkdtemp(path.join(tmpdir(), 'pythinker-plugin-'));
    await mkdir(path.join(root, '.pythinker-plugin'), { recursive: true });
    await mkdir(path.join(root, 'skills'), { recursive: true });
    await writeFile(
      path.join(root, '.pythinker-plugin', 'plugin.json'),
      JSON.stringify({
        name: 'superpowers',
        skills: './skills/',
    ...
    expect(record.manifestKind).toBe('pythinker-plugin-dir');
    ...
    expect(manager.pluginSkillRoots()).toContainEqual({
      ...
      source: 'extra',
      plugin: { id: 'superpowers', instructions: 'Use Pythinker tools.' },
    });
  });
  ...
  it('install() copies a symlinked plugin root into the managed plugins dir', async () => {
    ...
    const reloaded = new PluginManager({ pythinkerHomeDir: home });
    await reloaded.load();
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    await writeFile(
      path.join(managedRoot, 'pythinker.plugin.json'),
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'pythinker.plugin.json'),
      '{ not json',
      'utf8',
    );
  ...
  it('install() from github URL resolves latest release and records github metadata', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-abc/pythinker.plugin.json',
  ...
  it('install() from /tree/<tag-shaped-ref> downloads via short form, not refs/heads/ (P1 regression)', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'obra-superpowers-v5.1.0/pythinker.plugin.json',
  ...
  it('install() from github /tree/<branch> bypasses the GitHub API', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-main/pythinker.plugin.json',

...
describe('Agent turn flow', () => {
  ...
  it('keeps manual dynamic_workflow mode active after a turn completes normally', async () => {
    ...
    expect(eventIndex(ctx, '[wire]', 'dynamic_workflow_mode.exit')).toBe(-1);
    await ctx.expectResumeMatches();
  });
  ...
  it('exits task dynamic_workflow mode after a turn completes normally', async () => {
    ...
    const inactiveStatusIndex = ctx.allEvents.findIndex((entry, index) => {
      return (
        ...
        (entry.args as { readonly dynamicWorkflowMode?: boolean }).dynamicWorkflowMode === false
      );
    });
  ...
  it('enters silent dynamic_workflow mode when the agent calls AgentDynamicWorkflow', async () => {
    ...
    const enterEvent = ctx.allEvents.find(
      (entry) => entry.type === '[wire]' && entry.event === 'dynamic_workflow_mode.enter',
    );
    ...
    expect(reminderOrigins).not.toContainEqual({ kind: 'injection', variant: 'dynamic_workflow_mode' });
  ...
  it('emits a friendly model.not_configured error when no model is configured', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] turn.ended               { "turnId": 0, "reason": "failed", "error": { "code": "model.not_configured", "message": "LLM not set, send \\"/login\\" to login", "name": "PythinkerError", "details": { "turnId": 0 }, "retryable": false } }
    `);
    expect(ctx.newEvents()).toMatchInlineSnapshot(
      `[emit] error   { "code": "model.not_configured", "message": "LLM not set, send \\"/login\\" to login", "name": "PythinkerError", "details": { "turnId": 0 }, "retryable": false }`,
    );
  });
  ...
  it('cancels while waiting for a Stop hook', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-stop-hook-'));
  ...
  it('cancels while waiting for a PreToolUse hook inside permission evaluation', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-pre-tool-hook-'));
    const marker = join(dir, 'started');
    const script = [
      "const fs=require('node:fs');",
      `fs.writeFileSync(${JSON.stringify(marker)}, 'started');`,

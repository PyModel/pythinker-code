...
const DISCLOSURE_PROVIDER = { type: 'pythinker', apiKey: 'test-key', model: 'select-capable-model' } as const;
...
const INLINE_PROVIDER = { type: 'pythinker', apiKey: 'test-key', model: 'inline-model' } as const;
...
/** Empty env so an ambient PYTHINKER_CODE_EXPERIMENTAL_FLAG cannot force flags on. */

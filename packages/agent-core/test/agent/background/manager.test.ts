...
describe('BackgroundManager', () => {
  ...
  describe('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS env override', () => {
    afterEach(() => {
      vi.unstubAllEnvs();
    });

    it('caps registration from the env even without config', () => {
      vi.stubEnv('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS', '1');
    ...
    it('prefers the env value over config and ignores invalid values', () => {
      ...
      vi.stubEnv('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS', '2');
      ...
      vi.stubEnv('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS', '0');
      expect(resolveMaxRunningTasks(4)).toBe(4);

      vi.stubEnv('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS', 'abc');
      expect(resolveMaxRunningTasks(4)).toBe(4);
      vi.stubEnv('PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS', '-2');
      expect(resolveMaxRunningTasks()).toBeUndefined();
    });

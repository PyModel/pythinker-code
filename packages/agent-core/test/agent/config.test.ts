...
describe('Agent config', () => {
  ...
  it('useProfile emits the rendered system prompt and active tools', async () => {
    ...
    expect(ctx.newEvents()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated     { "model": "mock-model", "contextTokens": 0, "maxContextTokens": 1000000, "contextUsage": 0, "planMode": false, "dynamicWorkflowMode": false, "permission": "manual" }
      [wire] tools.set_active_tools   { "names": [ "Bash" ], "time": "<time>" }
    `);
    await ctx.expectResumeMatches();
  });

  it('appends enabled profile memory and grants only the existing file tools it needs', () => {
    const ctx = testAgent();
    ctx.configure();
    const profile: ResolvedAgentProfile = {
    ...
    ctx.agent.useProfile(profile, {
      cwdListing: '',
      agentsMd: '',
      agentMemoryPrompt: '# Persistent Agent Memory\nRemember prior review feedback.',
    });

    expect(ctx.agent.config.systemPrompt).toContain('Profile system prompt.');
    expect(ctx.agent.config.systemPrompt).toContain('Remember prior review feedback.');
    expect(
      ctx.agent.tools
        .data()
        .filter((tool) => tool.active)
        .map((tool) => tool.name),
    ).toEqual(expect.arrayContaining(['Grep', 'Read', 'Write', 'Edit']));
  });

  it('appends an active output style after the profile prompt', () => {
    const ctx = testAgent();
    ctx.configure();
    const profile: ResolvedAgentProfile = {
      name: 'test-profile',
    ...
    ctx.agent.useProfile(profile, undefined, {
      name: 'concise',
  ...
  it('omits bundled coding instructions when an output style does not keep them', () => {
    const ctx = testAgent();
    ctx.configure();
    const profile: ResolvedAgentProfile = {
      name: 'test-profile',
      systemPrompt: () =>
        [
          ...
          'Bundled coding rules.',
          '# General Guidelines for Research and Data Processing',
          'Bundled research rules.',
        ].join('\n\n'),
      tools: [],
    };
    ...
    expect(ctx.agent.config.systemPrompt).toContain('Profile introduction.');
    expect(ctx.agent.config.systemPrompt).not.toContain('Bundled coding rules.');
    expect(ctx.agent.config.systemPrompt).toContain('Bundled research rules.');
    expect(ctx.agent.config.systemPrompt).toContain('# Output Style: narrative');
    ...
    expect(ctx.agent.config.systemPrompt).toContain('Bundled coding rules.');
  });
  ...
  it('keeps turn-start config for later steps and applies updates to the next turn', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated                { "model": "changed-model", "contextTokens": 0, "maxContextTokens": 1000000, "contextUsage": 0, "planMode": false, "dynamicWorkflowMode": false, "permission": "manual" }

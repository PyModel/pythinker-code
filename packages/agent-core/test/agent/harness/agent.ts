...
export class AgentTestContext {
  ...
  private pythinkerConfig: PythinkerConfig;

  constructor(options: TestAgentOptions = {}) {
    this.options = options;
    this.emitter.on('error', () => {});
    this.pythinkerConfig = options.initialConfig ?? emptyConfig();
  ...
  async expectResumeMatches(): Promise<void> {
    const resumed = testAgent({
      ...
      initialConfig: this.pythinkerConfig,
...
function providerConfigForAlias(provider: ProviderConfig): PythinkerConfig['providers'][string] {
  return {
    type: provider.type,

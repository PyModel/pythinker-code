...
const PROVIDER = { type: 'pythinker', apiKey: 'test-key', model: 'pythinker-code' } as const;
const CAPS = {
  image_in: true,
  video_in: true,
  audio_in: false,
...
describe('compaction — probe tests (high-risk scenarios)', () => {
  ...
  it.skip('does not clear recent tool results when projecting a shrunk suffix under an active micro-compaction cutoff', () => {
    ...
    // flag explicitly rather than inheriting the ambient PYTHINKER_CODE_EXPERIMENTAL
    ...
    const ctx = testAgent({
      experimentalFlags: new FlagResolver(
        { PYTHINKER_CODE_EXPERIMENTAL_MICRO_COMPACTION: '1' },
        FLAG_DEFINITIONS,
      ),
    });
    ctx.configure({ provider: PROVIDER, modelCapabilities: CAPS });

...
describe('Agent context', () => {
  it('reports the projected model context by category and active tool', async () => {
    const ctx = testAgent();
    ctx.configure({
      tools: ['Bash'],
      modelCapabilities: {
    ...
    ctx.agent.context.appendUserMessage([{ type: 'text', text: 'inspect the project' }]);
    ...
    const report = await ctx.rpc.getContextUsage({});

    expect(report.model).toBe('mock-model');
    expect(report.maxTokens).toBe(10_000);
    expect(report.messageCount).toBe(2);
    expect(report.estimatedTokens).toBeGreaterThan(0);
    expect(report.categories).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: 'System prompt', tokens: expect.any(Number) }),
        expect.objectContaining({ name: 'Tools', tokens: expect.any(Number) }),
        expect.objectContaining({ name: 'User messages', tokens: expect.any(Number) }),
        expect.objectContaining({ name: 'Assistant messages', tokens: expect.any(Number) }),
        expect.objectContaining({ name: 'Free space', tokens: expect.any(Number) }),
      ]),
    );
  ...
  it('stores prompt origins without leaking them to LLM projection', async () => {
    const ctx = testAgent();
    ctx.configure();

    ctx.agent.context.appendUserMessage(
      [{ type: 'text', text: 'hello' }],
      { kind: 'user', checkpointId: 'checkpoint-1' },
    );
    ...
    expect(ctx.agent.context.history.map(({ role, origin }) => ({ role, origin }))).toEqual([
      { role: 'user', origin: { kind: 'user', checkpointId: 'checkpoint-1' } },
  ...
  it('applies a compaction summary inside the active context', () => {
    const ctx = testAgent();
    ctx.configure();
    ctx.agent.context.appendUserMessage([{ type: 'text', text: 'keep before' }]);
    ctx.agent.context.appendUserMessage([{ type: 'text', text: 'summarize one' }]);
    ctx.agent.context.appendUserMessage([{ type: 'text', text: 'summarize two' }]);
    ctx.agent.context.appendUserMessage([{ type: 'text', text: 'keep after' }]);
    ...
    ).toEqual(['keep before', 'middle summary', 'keep after']);
  });
...
describe('Agent context notification projection', () => {
  ...
  it('drops assistant messages whose recorded parts all serialize to nothing', () => {
    const messages = project([
      userMessage('First prompt', { kind: 'user' }),
      {
        role: 'assistant',
  ...
  it('keeps empty thinking verbatim when the assistant message has sendable content', () => {
    const messages = project([
      {
        role: 'assistant',
        content: [
  ...
  it('keeps empty signed thinking because providers require the signature verbatim', () => {
    const messages = project([
      {
        role: 'assistant',
    ...
    expect(messages[0]?.content).toEqual([{ type: 'think', think: '', encrypted: 'sig' }]);
  });
});

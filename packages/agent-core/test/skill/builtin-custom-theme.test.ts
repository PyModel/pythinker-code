...
describe('builtin skill: custom-theme', () => {
  ...
  it('pins the docs token reference and points users at PYTHINKER_CODE_HOME/themes and /theme', () => {
    ...
    expect(content).toContain('<PYTHINKER_CODE_HOME>/themes');
  ...
  it('registers through registerBuiltinSkills but stays out of the model skill listing', () => {
    ...
    expect(registry.getPythinkerSkillsDescription()).toContain('custom-theme');

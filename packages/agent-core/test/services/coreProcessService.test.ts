...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'pythinker-services-test-'));
  prevHome = process.env['PYTHINKER_HOME'];
  process.env['PYTHINKER_HOME'] = tmpHome;
});
...
afterEach(() => {
  ...
  if (prevHome === undefined) {
    ...
    process.env['PYTHINKER_HOME'] = prevHome;
  }
  try {
...
describe('CoreProcessService direct construction', () => {
  ...
  it('returns undefined headers when no identity is provided (back-compat)', () => {
    const headers = CoreProcessService._defaultPythinkerRequestHeaders(tmpHome);
    expect(headers).toBeUndefined();
  });

  it('caller-supplied pythinkerRequestHeaders win over identity-derived defaults', () => {

...
import type { PromptSubmission, Session } from '@pythoughts/protocol';
...
interface RpcRecord {
  ...
  getDynamicWorkflowModeCalls: number;
  startBtwCalls: unknown[];
  enterDynamicWorkflowCalls: unknown[];
  exitDynamicWorkflowCalls: unknown[];
...
function makeBridge(
  ...
  const record: RpcRecord = {
    ...
    enterPlanCalls: [],
    cancelPlanCalls: [],
    getDynamicWorkflowModeCalls: 0,
    startBtwCalls: [],
    enterDynamicWorkflowCalls: [],
    exitDynamicWorkflowCalls: [],
    createGoalCalls: [],
    pauseGoalCalls: [],
    resumeGoalCalls: [],
    cancelGoalCalls: [],
  ...
  const config = {
    ...
    systemPrompt: '',
    modelAlias: opts.config?.modelAlias ?? 'pythinker-code/k2',
  };
  ...
  const rpc: Partial<CoreRPC> = {
    ...
    getDynamicWorkflowMode: vi.fn().mockImplementation(async () => {
      record.getDynamicWorkflowModeCalls += 1;
      return false;
    }),
    ...
    enterDynamicWorkflow: vi.fn().mockImplementation(async (payload) => {
      record.enterDynamicWorkflowCalls.push(payload);
    }),
    exitDynamicWorkflow: vi.fn().mockImplementation(async (payload) => {
      record.exitDynamicWorkflowCalls.push(payload);
    }),
...
describe('PromptService stateless controls — bootstrap + shadow', () => {
  it('bootstraps shadow from getConfig/getPermission/getPlan on first submit', async () => {
    const { bridge, record } = makeBridge({
      config: { modelAlias: 'pythinker-code/k2', thinkingLevel: 'medium' },
    ...
    expect(record.getDynamicWorkflowModeCalls).toBe(1);
...
describe('PromptService stateless controls — diff dispatch', () => {
  it('issues setModel only when the body model differs from the shadow', async () => {
    const { bridge, record } = makeBridge({ config: { modelAlias: 'pythinker-code/k2' } });
    const { bus, triggerSubscribers } = makeBus();
    const impl = newSvc(bridge, bus);
    await impl.submit(SID, mkBody({ model: 'pythinker-code/k2' }));
    expect(record.setModelCalls).toEqual([]);
    ...
    await impl.submit(SID, mkBody({ model: 'pythinker-code/k1' }));
    expect(record.setModelCalls).toEqual([
      { sessionId: SID, agentId: 'main', model: 'pythinker-code/k1' },
    ]);
    expect(impl._agentStateForTest(SID)?.model).toBe('pythinker-code/k1');
  });
...
describe('PromptService stateless controls — dispatch log', () => {
  ...
  it('appends one entry per setter dispatched, in the order setModel/setThinking/setPermission/(enter|cancel)Plan', async () => {
    const { bridge } = makeBridge({
      config: { modelAlias: 'pythinker-code/k2', thinkingLevel: 'off' },
  ...
  it('bootstraps dynamicWorkflowMode from getDynamicWorkflowMode', async () => {
    const { bridge, record } = makeBridge({
      config: { modelAlias: 'pythinker-code/k2', thinkingLevel: 'off' },
      permission: { mode: 'manual' },
      plan: null,
  ...
  it('dispatches enterDynamicWorkflow/exitDynamicWorkflow and records them in the log', async () => {
    const { bridge, record } = makeBridge({
      config: { modelAlias: 'pythinker-code/k2', thinkingLevel: 'off' },
      permission: { mode: 'manual' },
      plan: null,
    ...
    await impl.submit(SID, mkBody({ dynamic_workflow_mode: true }));
    expect(record.enterDynamicWorkflowCalls.length).toBe(1);
    expect(record.enterDynamicWorkflowCalls[0]).toEqual({
      sessionId: SID,
      agentId: 'main',
      trigger: 'manual',
    });
    let log = impl._dispatchLogForTest(SID);
    expect(log?.some((e) => e.kind === 'enterDynamicWorkflow')).toBe(true);
    ...
    await impl.submit(SID, mkBody({ dynamic_workflow_mode: false }));
    expect(record.exitDynamicWorkflowCalls.length).toBe(1);
    expect(record.exitDynamicWorkflowCalls[0]).toEqual({ sessionId: SID, agentId: 'main' });
    log = impl._dispatchLogForTest(SID);
    expect(log?.some((e) => e.kind === 'exitDynamicWorkflow')).toBe(true);
  });

  it('does not re-dispatch dynamic_workflow_mode when it matches the shadow', async () => {
    const { bridge, record } = makeBridge({
      config: { modelAlias: 'pythinker-code/k2', thinkingLevel: 'off' },
      permission: { mode: 'manual' },
      plan: null,
...
describe('PromptService stateful session — content-only path', () => {
  it('issues zero bootstrap RPCs and zero setters when the body carries no controls', async () => {
    ...
    expect(record.getDynamicWorkflowModeCalls).toBe(0);
  ...
  it('reuses the shadow established by a prior submit for subsequent content-only submits', async () => {
    ...
    await impl.submit(SID, mkBody({ model: 'pythinker-code/k9' }));
    ...
    expect(record.setModelCalls).toEqual([]);
    expect(impl._agentStateForTest(SID)?.model).toBe('pythinker-code/k9');
  });
});
...
describe('PromptService.applyAgentState (POST /sessions/{sid}/profile path)', () => {
  it('throws SessionNotFoundError on unknown sid', async () => {
    ...
    const impl = newSvc(bridge, bus);
    await expect(
      impl.applyAgentState('sess_missing', { model: 'pythinker-code/k1' }, 'meta'),

...
describe('WorkspaceRegistryService', () => {
  ...
  beforeEach(async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'pythinker-ws-home-'));
  ...
  async function makeProjectRoot(label: string): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), `pythinker-ws-${label}-`));
    tempRoots.push(root);
  ...
  it('keeps a derived bucket visible even when its root no longer exists on disk', async () => {
    ...
    const goneRoot = normalizeWorkDir(join(tmpdir(), 'pythinker-ws-gone-never-created'));
  ...
  it('registers a derived workspace under the symlink bucket key, not the realpath', async () => {
    ...
    const linkParent = await mkdtemp(join(tmpdir(), 'pythinker-ws-link-'));
...
describe('touchWorkspaceRegistry', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-ws-touch-home-'));
    tempRoots = [];
  });
  ...
  async function makeProjectRoot(label: string): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), `pythinker-ws-touch-${label}-`));
    tempRoots.push(root);
    return normalizeWorkDir(await realpath(root));
  }

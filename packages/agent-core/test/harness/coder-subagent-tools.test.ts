 ...
 * enter+write+exit / a nested Agent (explore) call / a nested AgentDynamicWorkflow batch.
...
function createVerifyGenerate(steps: StepRecord[]): GenerateFn {
  ...
  const generate: GenerateFn = async (_chat, systemPrompt, tools, history, callbacks, options) => {
    ...
    const isDynamicWorkflowChild = firstUserText.includes('DYNAMIC_WORKFLOW-PROBE');
    ...
    if (isExploreChild) {
    ...
    } else if (isDynamicWorkflowChild) {
      // Long enough (>200 chars) to skip the subagent summary-continuation retry,
      // keeping the scripted call count deterministic across the dynamic_workflow batch.
      parts = [
        {
          type: 'text',
          text:
            'DYNAMIC_WORKFLOW-CHILD-OK: this dynamic_workflow child subagent ran to completion inside the coder subagent, ' +
            'executed its assigned slice of the batch, and is reporting the outcome back to its ' +
            'parent so the dynamic_workflow results can be aggregated.',
        },
      ];
    } else {
      ...
      switch (step) {
        ...
        case 10:
          ...
          break;
        case 11:
          parts = callTool('AgentDynamicWorkflow', {
            description: 'dynamic_workflow probe',
            prompt_template: 'DYNAMIC_WORKFLOW-PROBE {{item}}',
            items: ['alpha', 'beta'],
            subagent_type: 'explore',
          });
          break;
...
async function createCoderSession(
  ...
  const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-coder-tools-e2e-'));
...
describe('coder subagent aligned tools (real Session e2e)', () => {
  it('runs every newly aligned tool to success inside a real coder subagent', async () => {
    ...
    expect(byStep(12)?.prevToolOutput).toContain('DYNAMIC_WORKFLOW-CHILD-OK');

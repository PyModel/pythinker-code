...
const MIN = {
  PYTHINKER_MODEL_NAME: 'pythinker-for-coding',
  PYTHINKER_MODEL_API_KEY: 'sk-test',
  PYTHINKER_MODEL_MAX_CONTEXT_SIZE: '262144',
} as const;

describe('applyEnvModelConfig', () => {
  it('returns the config unchanged when PYTHINKER_MODEL_NAME is absent', () => {
  ...
  it('synthesizes a pythinker provider and model from the minimal set', () => {
    const config = apply({ ...MIN });
    expect(config.providers[ENV_MODEL_PROVIDER_KEY]).toEqual({
      type: 'pythinker',
      apiKey: 'sk-test',
      baseUrl: 'https://api.pythoughts.ai/v1',
    });
  ...
  it('lets an explicit base url override the default', () => {
    expect(
      apply({ ...MIN, PYTHINKER_MODEL_BASE_URL: 'https://api.example.com/v1' })
  ...
  it('parses comma-separated capabilities (trimmed, lowercased)', () => {
    expect(
      apply({ ...MIN, PYTHINKER_MODEL_CAPABILITIES: 'Image_In, thinking ,' })
  ...
  it('sets display_name only when provided', () => {
    ...
    expect(
      apply({ ...MIN, PYTHINKER_MODEL_DISPLAY_NAME: 'Custom Model' })
  ...
  it('writes type-specific fields and validates max_output_size', () => {
    ...
    expectConfigInvalid(() =>
      apply({ ...MIN, PYTHINKER_MODEL_MAX_OUTPUT_SIZE: 'nope' }),
    );
  });
  ...
  it('maps PYTHINKER_MODEL_ADAPTIVE_THINKING onto the alias', () => {
    expect(
      apply({ ...MIN, PYTHINKER_MODEL_ADAPTIVE_THINKING: 'true' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, PYTHINKER_MODEL_ADAPTIVE_THINKING: 'false' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(false);
    expect(
...
describe('loadRuntimeConfig vs readConfigFile (write-back isolation)', () => {
  it('injects the env model into runtime config but readConfigFile stays clean', () => {
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-env-model-'));
...
describe('stripEnvModelConfig (write-back guard)', () => {
  ...
  it('keeps user providers/models and a non-env default_model', () => {
    ...
    config.providers['pythinker'] = { type: 'pythinker', apiKey: 'k', baseUrl: 'https://x/v1' };
    config.providers[ENV_MODEL_PROVIDER_KEY] = { type: 'pythinker', apiKey: 'env-key' };
    config.models = {
      'my-model': { provider: 'pythinker', model: 'm', maxContextSize: 1000 },
    ...
    expect(stripped.providers['pythinker']).toBeDefined();
...
describe('writeConfigFile never persists the env model', () => {
  it('strips env entries (incl. thinking) when a runtime config is written back', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-env-write-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
  ...
  it('output never contains env-injected identifiers', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'pythinker-env-write2-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
      'default_model = "x"\n[providers.x]\ntype = "pythinker"\napi_key = "k"\n[models.x]\nprovider = "x"\nmodel = "x"\nmax_context_size = 1000\n',

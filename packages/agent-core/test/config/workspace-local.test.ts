...
async function makeProject(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'pythinker-workspace-local-'));
  tempDirs.push(root);
...
async function expectConfigInvalid(
  ...
  await expect(promise).rejects.toBeInstanceOf(PythinkerError);
...
describe('workspace local config', () => {
  ...
  it('loads additional_dir array from the project root when started nested', async () => {
    ...
    await writeFile(
      join(root, '.pythinker-code', 'local.toml'),

...
import { APIStatusError, type Message, type ToolCall } from '@pythoughts/kosong';
...
describe('Session resume permission parent chain', () => {
  it('restores subagent live-derived permission when metadata lists the child first', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pythinker-permission-chain-'));
    tempDirs.push(dir);
...
describe('Session.createAgent', () => {
  ...
  it('renders profiles with the current directory listing and merged AGENTS.md files', async () => {
    ...
    const kaos = createFakeKaos({
      ...
      stat: vi.fn(async (path: string) => {
        ...
        if (
          [
            '/repo/AGENTS.md',
            `${workDir}/.pythinker-code/AGENTS.md`,
            `${workDir}/AGENTS.md`,
      ...
      readText: vi.fn(async (path: string) => {
        ...
        if (path === `${workDir}/.pythinker-code/AGENTS.md`) return 'brand instructions';
    ...
    expect(created.agent.config.systemPrompt).toContain(
      '<!-- From: /repo/packages/app/.pythinker-code/AGENTS.md -->',
    );
  ...
  it('uses the pythinker home for global branded AGENTS.md files', async () => {
    ...
    const kaos = createFakeKaos({
      ...
      stat: vi.fn(async (path: string) => {
        ...
        if ([`${pythinkerHome}/AGENTS.md`, `${realHome}/.pythinker-code/AGENTS.md`].includes(path)) {
          return stat('file');
        }
        throw new Error(`ENOENT ${path}`);
      }),
      ...
      readText: vi.fn(async (path: string) => {
        if (path === `${pythinkerHome}/AGENTS.md`) return 'pythinker home instructions';
        if (path === `${realHome}/.pythinker-code/AGENTS.md`) return 'stale real-home instructions';
        throw new Error(`ENOENT ${path}`);
      }),
    });
    const session = new Session({
    ...
    expect(created.agent.config.systemPrompt).toContain('pythinker home instructions');

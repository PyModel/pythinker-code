...
function gitKaos(script: GitScript): Kaos {
  return createFakeKaos({
    exec: async (...args: string[]): Promise<KaosProcess> => {
      ...
      const scripted = script[args.slice(3).join(' ')] ?? script[subcommand];
...
describe('collectGitContext', () => {
  ...
  it('builds a git-context block with all sections', async () => {
    const kaos = gitKaos({
      ...
      'config user.name': { stdout: 'Example Developer' },
    ...
    expect(block).toContain('Default branch: trunk');
    expect(block).toContain('Git user: Example Developer');
  ...
  it('falls back to main when the remote default branch is unknown', async () => {
    const kaos = gitKaos({ 'rev-parse': { stdout: 'true' } });
    expect(await collectGitContext(kaos, '/project')).toContain('Default branch: main');
  });

...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'pythinker-mcp-loader-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user, project-root, and project-local paths', async () => {
    ...
    const paths = await resolveMcpJsonPaths({ cwd, homeDir: '/home/user/.pythinker-code' });

    expect(paths.user).toBe('/home/user/.pythinker-code/mcp.json');
    expect(paths.projectRoot).toBe(join(repoRoot, '.mcp.json'));
    expect(paths.project).toBe(join(cwd, '.pythinker-code', 'mcp.json'));
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.pythinker-code', 'mcp.json'), {
      mcpServers: {
  ...
  it('throws PythinkerError(config.invalid) on invalid JSON', async () => {
    ...
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toBeInstanceOf(PythinkerError);
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toMatchObject({
      code: ErrorCodes.CONFIG_INVALID,
    });
  });

  it('throws PythinkerError(config.invalid) on schema violation (unknown transport)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('throws PythinkerError(config.invalid) on schema violation (missing required field)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('honors PYTHINKER_CODE_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {

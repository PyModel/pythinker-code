 ...
 *
 * Run with `pnpm --filter @pymodel/agent-core exec vitest run test/mcp/registry.test.ts`.
 */
...
async function makePythinkerHome(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'pythinker-mcp-registry-home-'));
}
...
async function makePlugin(
  ...
  const root = await mkdtemp(join(tmpdir(), `pythinker-mcp-registry-plugin-`));
...
describe('McpServerRegistry', () => {
  it('lists user-level global entries without a cwd', async () => {
    ...
    const registry = new McpServerRegistry({
      ...
      plugins: new PluginManager({ pythinkerHomeDir: home }),
    });
  ...
  it('merges project layers when a cwd is given, tracking origin and mutability', async () => {
    ...
    const project = await realpath(await mkdtemp(join(tmpdir(), 'pythinker-mcp-registry-proj-')));
    ...
    expect(byName.get('localOnly')).toMatchObject({
      mutable: false,
      origin: join(sub, '.pythinker-code', 'mcp.json'),
    });
  });

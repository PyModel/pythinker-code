 ...
 *
 * Run with `pnpm --filter @pymodel/agent-core exec vitest run test/mcp/oauth-service.test.ts`.
 */
...
function makeFixture(): Fixture {
  ...
  const storeDir = `${tmpdir()}/pythinker-mcp-oauth-service-${process.pid}-${Math.random().toString(36).slice(2)}`;

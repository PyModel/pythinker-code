...
describe('buildMcpHttpHeaders', () => {
  ...
  it('passes through configured static headers', () => {
    expect(
      buildMcpHttpHeaders(
        { transport: 'http', url: 'https://x', headers: { 'X-Tenant': 'pythinker' } },
        () => undefined,
      ),
    ).toEqual({ 'X-Tenant': 'pythinker' });
  });
  ...
  it('throws PythinkerError(config.invalid) when a configured bearer token env var is empty or missing', () => {
    expectConfigInvalid(() =>
      buildMcpHttpHeaders(

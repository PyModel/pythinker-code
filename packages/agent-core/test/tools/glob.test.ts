...
describe('GlobTool', () => {
  ...
  it('rebases a Windows absolute pattern using the backend path class', async () => {
    const glob = vi.fn().mockReturnValue(asyncPaths(['C:\\workspace\\src\\nested\\a.ts']));
    const tool = new GlobTool(
      createFakeKaos({
        pathClass: () => 'win32',
        glob,
    ...
    const result = await executeTool(
      tool,
      context({ pattern: 'C:\\workspace\\src\\**\\*.ts' }),
    );

    expect(result.output).toBe('nested/a.ts');
    expect(glob).toHaveBeenCalledWith('C:/workspace/src', '**/*.ts');
  });
  ...
  it('rebases an absolute pattern onto its static directory prefix', async () => {
    const glob = vi.fn().mockReturnValue(asyncPaths(['/workspace/src/nested/a.ts']));
    const tool = new GlobTool(
      createFakeKaos({ glob, stat: vi.fn().mockResolvedValue(stat(1)) }),
      workspace,
    );
    ...
    const result = await executeTool(
      tool,
      context({ pattern: '/workspace/src/**/*.ts' }),
    );
  ...
  it('filters sensitive filenames while keeping public variants', async () => {
    const glob = vi
      .fn()
      .mockReturnValue(asyncPaths(['/workspace/.env', '/workspace/.env.example']));
    const tool = new GlobTool(
      createFakeKaos({ glob, stat: vi.fn().mockResolvedValue(stat(1)) }),
      workspace,
    );

    const result = await executeTool(tool, context({ pattern: '.env*' }));

    expect(result.output).toBe('.env.example');
  });

...
describe('PyModelFetchURLProvider auth fallback', () => {
  it('falls back to the configured API key when the token provider has no token', async () => {
    ...
    // the host should call pymodel with the static api key when oauth
...
describe('PyModelFetchURLProvider content kind', () => {

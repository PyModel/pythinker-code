...
describe('PythoughtsFetchURLProvider auth fallback', () => {
  it('falls back to the configured API key when the token provider has no token', async () => {
    ...
    // the host should call pythoughts with the static api key when oauth
...
describe('PythoughtsFetchURLProvider content kind', () => {

...
describe('originalImageCacheDir', () => {
  it('defaults to a pythinker-code cache directory under the OS temp dir', () => {
    const dir = originalImageCacheDir();
    expect(dir.startsWith(tmpdir())).toBe(true);
    expect(dir).toContain('pythinker-code');
  });
});

describe('sessionMediaOriginalsDir', () => {
  it('nests the originals dir inside the session dir', () => {
    expect(sessionMediaOriginalsDir('/home/u/.pythinker-code/sessions/ws/abc')).toBe(
      '/home/u/.pythinker-code/sessions/ws/abc/media-originals',
    );
  });
});

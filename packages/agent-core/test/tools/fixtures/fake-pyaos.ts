/**
 * Fake Pyaos — minimal stub for tool constructor injection in tests.
 *
...
import type { Environment, Pyaos } from '@pymodel/pyaos';
import type { ExecutableToolResult } from '#/loop';

import type { WorkspaceConfig } from '../../../src/tools/support/workspace';

function notImplemented(method: string): never {
  throw new Error(`FakePyaos.${method} not implemented — override in test`);
}
...
export function createFakePyaos(
  overrides?: Partial<Pyaos>,
  envLayers: readonly Record<string, string>[] = [],
): Pyaos {
  // Hold cwd in a closure so tests that call `chdir` directly can mutate it
  // and later `getcwd()` calls see the update — mirroring real-pyaos semantics
  ...
  const base: Pyaos = {
    ...
    withEnv: (env: Record<string, string>) =>
      createFakePyaos({ ...overrides, getcwd: () => cwd }, [...envLayers, env]),

...
describe('TaskOutputTool', () => {
  ...
  it('returns persisted output path and guidance when a log is available', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-output-tool-'));
    try {
  ...
  it('reads persisted output for a task loaded after restart', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-output-'));
    try {
  ...
  it('does not advertise output_path when the persisted log file does not exist', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-empty-'));
    try {
      const { manager } = createBackgroundManager({ sessionDir });
  ...
  it('truncates output > 32 KiB to a tail preview and reports paging metadata', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-trunc-'));
    try {
      const { manager } = createBackgroundManager({ sessionDir });
  ...
  it('lookup of a non-existent task does not create persisted state', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-missing-'));
    try {
      const { manager } = createBackgroundManager({ sessionDir });
...
describe('TaskStopTool', () => {
  ...
  it('accepts the legacy KillShell name and shell_id input', async () => {
    const { manager } = createBackgroundManager();
    const taskId = registerProcess(manager, pendingProcess(), 'sleep 60', 'legacy stop');
    const tool = new TaskStopTool(manager);

    const result = await executeTool(
      tool,
      context('c_legacy_stop', { shell_id: taskId }),
    );

    expect(tool.aliases).toContain('KillShell');
    expect(result.isError).toBe(false);
    expect(toolContentString(result)).toContain(`task_id: ${taskId}`);
  ...
  it('persists stop reason when the manager has persistence', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-stop-reason-'));
    try {
      const writer = createBackgroundManager({ sessionDir }).manager;
  ...
  it('falls back to the placeholder when a terminal task has a blank stored reason', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'pythinker-bg-blank-stored-reason-'));
    try {
...
describe('background tool descriptions', () => {
  ...
  it('TaskOutput description mentions background tasks, block, output_path, and Read', () => {
    ...
    expect(tool.aliases).toEqual(['AgentOutputTool', 'BashOutputTool']);
  });

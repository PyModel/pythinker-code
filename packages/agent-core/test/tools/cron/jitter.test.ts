...
describe('jitteredNextCronRunMs — recurring', () => {
  ...
  it('respects PYTHINKER_CRON_NO_JITTER=1 — no offset', () => {
    const prev = process.env['PYTHINKER_CRON_NO_JITTER'];
    process.env['PYTHINKER_CRON_NO_JITTER'] = '1';
    try {
      const parsed = parseCronExpression('*/5 * * * *');
      const ideal = localDate(2024, 5, 1, 12, 5, 0);
      const jittered = jitteredNextCronRunMs(
      ...
      if (prev === undefined) delete process.env['PYTHINKER_CRON_NO_JITTER'];
      else process.env['PYTHINKER_CRON_NO_JITTER'] = prev;
    }
  });
});
...
describe('oneShotJitteredNextCronRunMs', () => {
  ...
  it('respects PYTHINKER_CRON_NO_JITTER=1', () => {
    const prev = process.env['PYTHINKER_CRON_NO_JITTER'];
    process.env['PYTHINKER_CRON_NO_JITTER'] = '1';
    try {
      const ideal = localDate(2024, 5, 1, 14, 0, 0);
...
beforeEach(() => {
  delete process.env['PYTHINKER_CRON_NO_JITTER'];
});
afterEach(() => {
  delete process.env['PYTHINKER_CRON_NO_JITTER'];
});

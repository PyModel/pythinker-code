...
function makeCore(home: string): PythinkerCore {
  return new PythinkerCore(async () => ({}) as never, { homeDir: home });
}
...
describe('PythinkerCore degraded config loading', () => {
  it('reports no diagnostics for a valid config', async () => {
    const core = makeCore(await makeHome(VALID_TOML));
    const config = await core.getPythinkerConfig({});
    expect(config.providers['pythinker']).toBeDefined();
...
`);
    ...
    const write = core.setPythinkerConfig({ thinking: { enabled: true } });
    await expect(write).rejects.toThrow(/fix it first/i);
    await expect(write).rejects.toThrow(/pythinker doctor/);
  ...
  it('keeps the last good config when the file breaks mid-run', async () => {
    ...
    const kept = await core.getPythinkerConfig({ reload: true });
    expect(kept.providers['pythinker']).toBeDefined();
    ...
    const adopted = await core.getPythinkerConfig({ reload: true });
    expect(adopted.thinking?.enabled).toBe(true);
    await expect(core.getConfigDiagnostics({})).resolves.toEqual({ warnings: [] });
  });
});

describe('PythinkerCore imageLimits scoping', () => {
...
`,
    ...
    await coreB.getPythinkerConfig({ reload: true });
...
`,
    ...
    await core.getPythinkerConfig({ reload: true });

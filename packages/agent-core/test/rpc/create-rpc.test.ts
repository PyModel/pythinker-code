...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as PythinkerError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(PythinkerError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown PythinkerError across the wire preserving code and details', async () => {
    ...
    expect(received).toBeInstanceOf(PythinkerError);
    const error = received as PythinkerError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new PythinkerError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as PythinkerError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new PythinkerError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to PythinkerError(internal)', async () => {
    interface RemoteSide {

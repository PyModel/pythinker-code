/**
 * Scenario: the unified MCP management plane on PythinkerCore — the registry-backed
 ...
 *
 * Run with `pnpm --filter @pymodel/agent-core exec vitest run test/rpc/mcp-rpc.test.ts`.
 */
...
async function makePlugin(name: string, mcpServers: Record<string, unknown>): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), `pythinker-mcp-rpc-plugin-`));
  await writeJson(join(root, 'pythinker.plugin.json'), { name, mcpServers });
...
async function makeCore(seed?: (home: string) => Promise<void>): Promise<CoreFixture> {
  const tmp = await realpath(await mkdtemp(join(tmpdir(), 'pythinker-mcp-rpc-')));
  ...
  const core = new PythinkerCore(coreRpc, { homeDir: home });
  cores.push(core);
...
function coreOAuth(core: PythinkerCore): McpOAuthService {
  return (core as unknown as { mcpOAuth: McpOAuthService }).mcpOAuth;
}

describe('PythinkerCore unified MCP management plane', () => {

...
import { APIRequestTooLargeError, APIStatusError, type Message } from '@pymodel/kosong';
...
// The OpenAI-compatible (PyModel / Pythinker) phrasing of the same tool-exchange
// structural rejection. Verbatim from the field, doubled space included.
const PYMODEL_TOOL_CALL_ID_400 = new APIStatusError(400, '400 tool_call_id  is not found');
...
describe('executeLoopStep — tool exchange adjacency fallback', () => {
  ...
  it('resends once and recovers after a PyModel tool_call_id-not-found 400', async () => {
    const { input, llm, strictCalls, strictMessages } = makeHarness(PYMODEL_TOOL_CALL_ID_400);

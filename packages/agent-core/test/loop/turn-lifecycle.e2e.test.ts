...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws PythinkerError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as PythinkerError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as PythinkerError).details).toEqual({ maxSteps: 2 });

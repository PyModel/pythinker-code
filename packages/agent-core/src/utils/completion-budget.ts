...
export function resolveCompletionBudget(args: {
  ...
  const fromNew = parseEnvBudget(env['PYTHINKER_MODEL_MAX_COMPLETION_TOKENS']);
  if (fromNew !== 'absent') {
    return fromNew === 'disabled' ? undefined : { hardCap: fromNew };
  }
  const fromLegacy = parseEnvBudget(env['PYTHINKER_MODEL_MAX_TOKENS']);
 ...
 * in `PythinkerChatProvider._clone()`.
 */

...
const ROOT_SYMBOL = Symbol.for('pythinker.logger.root');
const SESSION_LOG_ID = Symbol('pythinker.logger.sessionLogId');
...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'pythinker-code.log'),
 ...
 *
 *   import { log } from 'pythinker-code-sdk';
 *   log.info('pythinker-code starting', { version });
 *
 ...
 * importing `log` at module load (before `PythinkerHarness` configures the root)

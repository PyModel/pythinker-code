...
import { resolvePythinkerHome } from '#/config/path';
...
export async function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): Promise<McpJsonPaths> {
  ...
  return {
    user: join(resolvePythinkerHome(input.homeDir), 'mcp.json'),
    projectRoot: join(projectRoot, '.mcp.json'),
    project: join(input.cwd, '.pythinker-code', 'mcp.json'),
  };
}
...
/**
 * Load MCP server declarations from the user-global `~/.pythinker-code/mcp.json`,
 * the project-root `<project root>/.mcp.json`, and the project-local
 * `<cwd>/.pythinker-code/mcp.json`. Entries in later files override earlier files
 * with the same key, so a repo can specialise or replace a shared definition,
 * and Pythinker-specific project config wins over the Claude-compatible root file.
 *
...
async function readMcpJson(
  ...
  try {
    ...
    throw new PythinkerError(ErrorCodes.CONFIG_INVALID, `Invalid JSON in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }

 ...
 *
 * One instance per process (PythinkerCore shares it with every Session). The
...
export interface McpOAuthServiceOptions {
  /** Storage backend; overrides `pythinkerHomeDir` when supplied. */
  readonly store?: JsonFileStore;
  /** Resolved Pythinker home; credentials default to `<pythinkerHomeDir>/credentials/mcp/`. */
...
export class McpOAuthService {
  ...
  constructor(options: McpOAuthServiceOptions = {}) {
    this.store =
      options.store ??
      new JsonFileStore(
        options.pythinkerHomeDir === undefined ? undefined : mcpCredentialsDir(options.pythinkerHomeDir),
      );

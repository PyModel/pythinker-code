 ...
 *    state under `<PYTHINKER_CODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.pythinker-code`).
...
import { OAuthTokenTransaction } from '@pymodel/pythinker-code-oauth';
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("pythinker-code (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `pythinker-code (${options.serverName})`;

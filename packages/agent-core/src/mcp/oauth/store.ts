 ...
 * `<PYTHINKER_CODE_HOME>/credentials/mcp/` (default
 * `~/.pythinker-code/credentials/mcp/`).
 *
...
export function mcpCredentialsDir(pythinkerHomeDir: string): string {
  return join(pythinkerHomeDir, 'credentials', 'mcp');
}

export function defaultMcpCredentialsDir(): string {
  return mcpCredentialsDir(join(homedir(), '.pythinker-code'));
}

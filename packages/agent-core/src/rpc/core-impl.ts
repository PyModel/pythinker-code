...
export class PythinkerCore implements PromisableMethods<CoreAPI> {
  ...
  constructor(
    ...
    this.mcpOAuth = new McpOAuthService({ pythinkerHomeDir: this.homeDir });
  ...
  private async resolveMcpTestTarget(
    ...
    if (matches.length === 0) {
      throw new PythinkerError(ErrorCodes.MCP_SERVER_NOT_FOUND, `MCP server "${name}" was not found`);
    }
  ...
  private async resolveMcpRegistryEntry(
    ...
    try {
      ...
      if (error instanceof PythinkerError && error.code === ErrorCodes.MCP_SERVER_NOT_FOUND) {
        return undefined;
      }
      throw error;
    }

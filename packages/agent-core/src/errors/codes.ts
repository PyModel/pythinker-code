/**
 * Error codes for Pythinker Core's public error protocol.
 *
 * `ErrorCodes` is the source of truth for every code Pythinker Core may emit.
...
export type PythinkerErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

export interface PythinkerErrorInfo {
...
export const PYTHINKER_ERROR_INFO = {
  'config.invalid': {
  ...
  'plugin.load_failed': {
    ...
    retryable: true,
    public: true,
    action: 'Fix the installed.json file under $PYTHINKER_CODE_HOME/plugins/ and run /plugins reload.',
  },
  ...
  'shell.git_bash_not_found': {
    ...
    retryable: false,
    public: true,
    action: 'Install Git for Windows from https://gitforwindows.org/ or set PYTHINKER_SHELL_PATH to a bash.exe.',
  },
...
} as const satisfies Record<PythinkerErrorCode, PythinkerErrorInfo>;
 ...
 * branch on this before indexing `PYTHINKER_ERROR_INFO` with an untrusted code.
 */
export function isPythinkerErrorCode(code: unknown): code is PythinkerErrorCode {
  return typeof code === 'string' && Object.hasOwn(PYTHINKER_ERROR_INFO, code);
}

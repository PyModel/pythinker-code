 ...
 * `Agent` / `AgentDynamicWorkflow` tool `model` parameter) or the spawned profile (via
...
export function resolveSecondaryModel(
  config: PythinkerConfig | undefined,
...
/**
 * The "Available models" block appended to the `Agent` / `AgentDynamicWorkflow` tool
...
export function wrapSubagentModelError(
  ...
  if (!(error instanceof PythinkerError) || error.code !== ErrorCodes.CONFIG_INVALID) return error;

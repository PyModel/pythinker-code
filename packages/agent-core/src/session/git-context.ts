/**
 * Bounded Git repository context for agent prompts.
 *
 * `collectGitContext` produces a `<git-context>` block that is prepended to a
 * fresh explore subagent prompt and included in the main agent's startup
 * context. Every git command is individually guarded — a single failure never
 * aborts the whole collection — and remote URLs are sanitized so internal
 * infrastructure is not surfaced to the model.
 */
...
export async function collectGitContext(kaos: Kaos, cwd: string): Promise<string> {
  ...
  if ((await runGitCommand(kaos, cwd, ['rev-parse', '--is-inside-work-tree'])) === null) {
    return '';
  }

  const [remoteUrl, branch, defaultBranch, gitUser, dirtyRaw, logRaw] = await Promise.all([
    runGitCommand(kaos, cwd, ['remote', 'get-url', 'origin']),
    runGitCommand(kaos, cwd, ['branch', '--show-current']),
    collectDefaultBranch(kaos, cwd),
    runGitCommand(kaos, cwd, ['config', 'user.name']),
    runGitCommand(kaos, cwd, ['status', '--porcelain']),
    runGitCommand(kaos, cwd, ['log', '-3', '--format=%h %s']),
  ]);
  ...
  sections.push(`Default branch: ${defaultBranch}`);
  if (gitUser) sections.push(`Git user: ${gitUser}`);
...
async function collectDefaultBranch(kaos: Kaos, cwd: string): Promise<string> {
  const originHead = await runGitCommand(kaos, cwd, [
    'symbolic-ref',
    '--quiet',
    '--short',
  ...
  return candidates[refs.findIndex(ref => ref !== null)] ?? 'main';
}
...
export async function runGitCommand(
  ...
  try {
    ...
    if (!acceptedExitCodes.includes(exitCode)) return null;
    return stdout.trim();
  } catch {
    try {

 ...
 * (`skills` ↔ `agents`, `.pythinker-code` ↔ `.agents`).
 *
...
// Relative to brandHomeDir, which already IS the brand data dir (~/.pythinker-code
// or $PYTHINKER_CODE_HOME) — no '.pythinker-code' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['agents'] as const;
const USER_GENERIC_DIRS = ['.agents/agents'] as const;
const PROJECT_BRAND_DIRS = ['.pythinker-code/agents'] as const;

...
const DEFAULT_BASE_URL: Partial<Record<ProviderType, string>> = {
  pythinker: 'https://api.pythoughts.ai/v1',
...
/** Default capabilities when PYTHINKER_MODEL_CAPABILITIES is unset (pythinker models support both). */
...
// fails fast like the other PYTHINKER_MODEL_* values, instead of silently keeping
...
/**
 * When `PYTHINKER_MODEL_NAME` is set, synthesize one provider + one model alias from
 * the `PYTHINKER_MODEL_*` environment variables and make it the default model.
...
export function applyEnvModelConfig(config: PythinkerConfig, env: Env = process.env): PythinkerConfig {
  ...
  const thinkingMode = trimmed(env['PYTHINKER_MODEL_THINKING_MODE']);
  ...
  const defaultThinking = parseBooleanVar(
    env['PYTHINKER_MODEL_DEFAULT_THINKING'],
    'PYTHINKER_MODEL_DEFAULT_THINKING',
  );
  ...
  // PythinkerError(CONFIG_INVALID) on violation, matching the explicit checks above.
...
function rawDefaultThinking(config: PythinkerConfig): boolean | undefined {

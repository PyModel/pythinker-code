 ...
 *
 * The `PYTHINKER_SECONDARY_MODEL` / `PYTHINKER_SECONDARY_EFFORT` env vars override
 * `model` / `default_effort` in memory only: like the `PYTHINKER_MODEL_*`
...
export const SECONDARY_MODEL_ENV = 'PYTHINKER_SECONDARY_MODEL';
export const SECONDARY_MODEL_EFFORT_ENV = 'PYTHINKER_SECONDARY_EFFORT';
 ...
 */
export function applySecondaryModelConfig(config: PythinkerConfig, env: Env = process.env): PythinkerConfig {
 ...
 * — e.g. a `/secondary-model` pick made under `PYTHINKER_SECONDARY_MODEL` — does

...
export function readConfigFile(filePath: string): PythinkerConfig {
 ...
 */
export function readConfigFileForUpdate(filePath: string): PythinkerConfig {
  try {
    return readConfigFile(filePath);
  } catch (error) {
    if (error instanceof PythinkerError && error.code === ErrorCodes.CONFIG_INVALID) {
      throw new PythinkerError(
        ErrorCodes.CONFIG_INVALID,
        `Cannot change settings while ${filePath} is invalid — fix it first (run \`pythinker doctor\` for details).`,
        { cause: error },
      );
    }
    throw error;
 ...
 * synthesized from `PYTHINKER_MODEL_*` environment variables. Use this everywhere a
...
export interface RuntimeConfigLoadResult {
  ...
  /** Problems applying PYTHINKER_MODEL_* env overrides; the overlay was skipped. */
   ...
   * good config instead.
   */
  readonly fileError?: PythinkerError;
}
 ...
 * whole top-level section otherwise) and a bad PYTHINKER_MODEL_* env overlay is
...
export function loadRuntimeConfigSafe(
  ...
  let fileError: PythinkerError | undefined;
    ...
    if (data !== undefined) {
      ...
      if (salvaged.config === undefined) {
        ...
        if (salvaged.dropped.length > 0) {
          fileWarnings.push(
            `Ignored invalid config in ${filePath}: ${salvaged.dropped.join(', ')}. Run \`pythinker doctor\` for details.`,
          );
        }
      }
    }
...
function salvageConfigData(transformed: Record<string, unknown>): SalvageResult {
  ...
  for (;;) {
    const result = PythinkerConfigSchema.safeParse(transformed);
    if (result.success) {
...
export function parseConfigString(tomlText: string, filePath = 'config.toml'): PythinkerConfig {
  ...
  try {
    ...
    throw new PythinkerError(ErrorCodes.CONFIG_INVALID, `Invalid TOML in ${filePath}: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error,
    });
  }
...
function parseConfigData(data: Record<string, unknown>, filePath: string): PythinkerConfig {
  const raw = cloneRecord(data);
  const transformed = transformTomlData(data);
  transformed['raw'] = raw;

  try {
    return PythinkerConfigSchema.parse(transformed);
  } catch (error) {
    throw new PythinkerError(ErrorCodes.CONFIG_INVALID, `Invalid configuration in ${filePath}: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
...
export function configToTomlData(config: PythinkerConfig): Record<string, unknown> {

...
function withAnthropicProfile(model: ModelAlias, providerType?: ProviderType): ModelAlias {
  ...
  // protocol (catalog-imported Pythinker `k3`, GLM, …) must not advertise Claude
  // effort levels. Pythinker providers — including managed models routed through
  ...
  const profile =
    providerType !== undefined && providerType !== 'pythinker' && protocol === 'anthropic'

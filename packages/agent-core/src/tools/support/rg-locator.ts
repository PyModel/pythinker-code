 ...
 *   3. `<PYTHINKER_CODE_HOME>/bin/rg` — persistent cache for this app.
 *   4. CDN download to <PYTHINKER_CODE_HOME>/bin/ — one-off bootstrap
 *
...
const RG_BASE_URL = 'https://code.pythinker.com/pythinker-code/rg';
...
async function downloadAndInstallRg(shareDir: string): Promise<string> {
  ...
  const tmp = await mkdtemp(join(tmpdir(), 'pythinker-rg-'));
  try {

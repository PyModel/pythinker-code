...
export function isBinaryContentType(contentType: string): boolean {
  ...
  if (normalized.length === 0 || normalized.startsWith('text/')) return false;
  if (normalized.endsWith('+json') || normalized === 'application/json') return false;
  if (normalized.endsWith('+xml') || normalized === 'application/xml') return false;
  if (normalized.startsWith('application/javascript')) return false;

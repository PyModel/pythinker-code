 ...
 *
 * pythinker-core defines the interface; the host provides the real search
...
export const WebSearchInputSchema = z
  .object({
    query: z.string().min(2).describe('The query text to search for.'),
    limit: z
      .number()
      .int()
      .min(1)
  ...
  .refine(
    (value) =>
      value.allowed_domains?.length === undefined ||
      value.allowed_domains.length === 0 ||
      value.blocked_domains?.length === undefined ||
      value.blocked_domains.length === 0,
...
export class WebSearchTool implements BuiltinTool<WebSearchInput> {
  ...
  private async execution(
    args: WebSearchInput,
    { toolCallId, signal, onUpdate }: ExecutableToolContext,
    ...
    try {
      ...
      if (args.allowed_domains !== undefined) opts.allowedDomains = args.allowed_domains;
      if (args.blocked_domains !== undefined) opts.blockedDomains = args.blocked_domains;
      onUpdate?.({ kind: 'status', text: `Searching: ${args.query}` });
      const results = await this.provider.search(args.query, opts);
      onUpdate?.({
        kind: 'status',
        text: `Found ${String(results.length)} ${results.length === 1 ? 'result' : 'results'} for "${args.query}"`,
      });
      ...
      builder.write(
        'REMINDER: You MUST cite relevant sources in your response using markdown links in the form [Title](URL).\n',
      );
      return builder.ok();
    } catch (error) {
      return {

...
export const GrepInputSchema = z.object({
  ...
  glob: z
    ...
    .optional()
    .describe(
      'Optional glob filter passed to ripgrep. Separate multiple filters with commas or whitespace; brace patterns such as `*.{ts,tsx}` stay intact.',
    ),
  type: z
    .string()
    .optional()
  ...
  context: z
    ...
    .optional()
    .describe(
      'Alias for `-C`. Applies only when `output_mode` is `content`; takes precedence over `-C`, `-A`, and `-B`.',
    ),
  head_limit: z
    .number()
    .int()
...
function buildRgArgs(
  ...
  if (mode === 'content') {
    ...
    if (args.context !== undefined) {
      cmd.push('-C', String(args.context));
  ...
  if (args.glob !== undefined) {
    for (const glob of splitGlobFilters(args.glob)) {
      cmd.push('--glob', glob);
    }
  }
...
function splitGlobFilters(value: string): string[] {
  return value
    .split(/\s+/u)
    .flatMap((part) =>
      part.includes('{') && part.includes('}') ? [part] : part.split(','),
    )
    .filter(Boolean);
}

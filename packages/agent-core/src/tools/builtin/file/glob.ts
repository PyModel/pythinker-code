...
export class GlobTool implements BuiltinTool<GlobInput> {
  ...
  resolveExecution(args: GlobInput): ToolExecution {
    const absolutePattern = splitAbsolutePattern(args.pattern, this.kaos.pathClass());
    const executionArgs =
      absolutePattern === undefined ? args : { ...args, pattern: absolutePattern.pattern };
    let path: string | undefined;
    const requestedPath = absolutePattern?.path ?? args.path;
    if (requestedPath !== undefined) {
      path = resolvePathAccessPath(requestedPath, {
        kaos: this.kaos,
    ...
    return {
      ...
      execute: () => this.execution(executionArgs, searchRoots),
    };
  }
  ...
  private async execution(args: GlobInput, searchRoots: string[]): Promise<ExecutableToolResult> {
    ...
    try {
      ...
      const lines: string[] = [];
      if (truncated) {
        lines.push(`[Truncated at ${String(MAX_MATCHES)} matches — ${String(seen.size)} matched so far, use a more specific pattern]`, `Only the first ${String(MAX_MATCHES)} matches are returned.`);
      }
...
function splitAbsolutePattern(
  ...
  const absolute =
    pathClass === 'posix'
      ? pattern.startsWith('/')
      : isAbsolute(pattern) && !/^\\(?!\\)/u.test(pattern);
  if (!absolute) return undefined;
  const normalized = normalize(pattern);

  const magicIndex = normalized.search(/[*?[{]/u);
  if (magicIndex < 0) {
    return { path: dirname(normalized), pattern: basename(normalized) };
  }

  const separatorIndex = normalized.lastIndexOf('/', magicIndex);
  let path = separatorIndex === 0 ? '/' : normalized.slice(0, separatorIndex);
  if (/^[A-Za-z]:$/u.test(path)) path += '/';
  return { path, pattern: normalized.slice(separatorIndex + 1) };
}

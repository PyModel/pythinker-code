...
Bench / acceptance runs can set `PYTHINKER_CRON_NO_STALE=1` to disable the
judgment entirely.
...
Bench / acceptance tests can set `PYTHINKER_CRON_NO_JITTER=1` to disable jitter entirely.
...
Cron tasks live in the current pythinker CLI session. When you exit, they
are persisted under the session homedir; the next `pythinker resume` of the

 ...
 *     stderr trace gated on `PYTHINKER_CRON_DEBUG=1`) so one busted cron
...
export interface CronSchedulerOptions {
   ...
   * `pythinker resume` does not replay the fire.
   *
   ...
   *     manually.
   *
   * Used by P1.8 to wire `PYTHINKER_CRON_MANUAL_TICK=1` to disable the
   * timer.
   */
...
export function createCronScheduler(opts: CronSchedulerOptions): CronScheduler {
  ...
  // Now persisted across `pythinker resume` via `task.lastFiredAt`: when

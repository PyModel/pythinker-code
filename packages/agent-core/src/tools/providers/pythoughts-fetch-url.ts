/**
 * PythoughtsFetchURLProvider — host-side UrlFetcher.
 *
 * Flow:
 *   1. Try Pythoughts coding-fetch service (POST {url}, Bearer token from a
 *      narrow token provider, Accept: text/markdown, host-provided headers).
 *   2. Pythoughts 200 → return the body as `extracted` content (the
 *      service has already extracted the main page text on its side).
 *   3. Any Pythoughts failure — non-200, network error, or token
...
export interface PythoughtsFetchURLProviderOptions {
...
export class PythoughtsFetchURLProvider implements UrlFetcher {
  ...
  constructor(options: PythoughtsFetchURLProviderOptions) {
  ...
  async fetch(
    ...
    try {
      const content = await this.fetchViaPythoughts(url, options);
  ...
  private async fetchViaPythoughts(
    url: string,
    ...
    if (response.status !== 200) {
      ...
      throw new HttpFetchError(
        response.status,
        `Pythoughts fetch request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error('Pythoughts fetch service is not configured: missing API key or token provider.');
  }
}

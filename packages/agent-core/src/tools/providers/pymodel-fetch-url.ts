/**
 * PyModelFetchURLProvider — host-side UrlFetcher.
 *
 * Flow:
 *   1. Try PyModel coding-fetch service (POST {url}, Bearer token from a
 *      narrow token provider, Accept: text/markdown, host-provided headers).
 *   2. PyModel 200 → return the body as `extracted` content (the
 *      service has already extracted the main page text on its side).
 *   3. Any PyModel failure — non-200, network error, or token
...
export interface PyModelFetchURLProviderOptions {
...
export class PyModelFetchURLProvider implements UrlFetcher {
  ...
  async fetch(url: string, options?: { toolCallId?: string }): Promise<UrlFetchResult> {
    try {
      const content = await this.fetchViaPyModel(url, options?.toolCallId);
  ...
  private async fetchViaPyModel(
    ...
    if (response.status !== 200) {
      ...
      throw new HttpFetchError(
        response.status,
        `PyModel fetch request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error('PyModel fetch service is not configured: missing API key or token provider.');
  }
}

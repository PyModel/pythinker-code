# `@pythoughts/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@pythoughts/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
1. **Zero runtime dependencies** — the container is ~600 LoC of plain
   ...
   subsystem.
2. **Familiar shape** — most pythinker-code contributors have seen VSCode's
...
Test files import from the subpath `@pythoughts/agent-core/di/test`
(NOT the main package entry — keeps production bundles clean):

```ts
import { TestInstantiationService } from '@pythoughts/agent-core/di/test';
...
├── test.ts                      ← subpath barrel (`@pythoughts/agent-core/di/test`)

# `@pymodel/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@pymodel/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
Test files import from the subpath `@pymodel/agent-core/di/test`
...
├── test.ts                      ← subpath barrel (`@pymodel/agent-core/di/test`)

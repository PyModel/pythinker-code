  ...
  implements IPromptService
{
  ...
  private async _ensureAgentStateBootstrapped(sid: string): Promise<void> {
    ...
    const [config, permission, plan, dynamicWorkflowMode] = await Promise.all([
      ...
      this.core.rpc.getDynamicWorkflowMode({ sessionId: sid, agentId: MAIN_AGENT_ID }),
    ]);
   ...
   * PythinkerCore loaded with provider credentials). Not part of the public

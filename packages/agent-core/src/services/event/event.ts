...
import type { Event as ProtocolEvent } from '@pythoughts/protocol';

/**
 * Naming convention inside this file:
 *
 * - `Event` (from `@pythoughts/agent-core/base/common/event`) — the generic
 *   VSCode-style emitter accessor type. `Event<T>` is the listener-tuple
 *   type used to declare `readonly onDidXxx: Event<T>`.
 * - `ProtocolEvent` (alias of `@pythoughts/protocol`'s `Event`) — the

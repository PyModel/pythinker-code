...
export class SessionService extends Disposable implements ISessionService {
  ...
  async update(id: string, input: SessionUpdate): Promise<Session> {
    ...
    if (ac !== undefined) {
      ...
      if (ac.dynamic_workflow_mode !== undefined) patch.dynamic_workflow_mode = ac.dynamic_workflow_mode;
  ...
  async getStatus(id: string): Promise<SessionStatusResponse> {
    ...
    return {
      ...
      plan_mode: plan !== null,
      dynamic_workflow_mode: agentState?.dynamicWorkflowMode ?? false,
  ...
  async undo(id: string, input: UndoSessionRequest): Promise<UndoSessionResponse> {
    ...
    try {
      ...
      if (error instanceof PythinkerError && error.code === ErrorCodes.REQUEST_INVALID) {

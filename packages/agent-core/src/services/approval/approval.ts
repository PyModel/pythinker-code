 ...
 * role — routes `ApprovalRequest`s coming out of `PythinkerCore` to a waiter
 ...
 *      sees coming off `PythinkerCore.requestApproval(...)`. See
 ...
 * the adapter (PythinkerCore → BridgeClientAPI.requestApproval →
...
import type {} from '@pythoughts/protocol'; // type-only marker — keep protocol dep referenced
...
export interface IApprovalService {
  ...
  /**
   * Called by the adapter when PythinkerCore needs user approval. Resolves with the

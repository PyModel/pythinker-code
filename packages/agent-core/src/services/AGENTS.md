...
`@pymodel/agent-core` (merged from the former `@pymodel/services`
...
from `index.ts` so importing `@pymodel/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@pymodel/agent-core` loads the package barrel, whose impl

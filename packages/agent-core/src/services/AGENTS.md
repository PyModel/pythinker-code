...
In-process service layer for the pythinker-code server, now hosted inside
`@pythoughts/agent-core` (merged from the former `@pythoughts/services`
...
from `index.ts` so importing `@pythoughts/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@pythoughts/agent-core` loads the package barrel, whose impl

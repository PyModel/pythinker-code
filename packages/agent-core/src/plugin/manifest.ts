...
// Pythinker CLI). We do not run them; emit an info diagnostic so plugin authors and
...
export async function parseManifest(pluginRoot: string): Promise<ParsedManifestResult> {
  const rootJsonPath = path.join(pluginRoot, PYTHINKER_PLUGIN_ROOT_PATH);
  const dirJsonPath = path.join(pluginRoot, PYTHINKER_PLUGIN_DIR_PATH);
  ...
  if (!rootJsonExists && !dirJsonExists) {
    return {
      diagnostics: [
        {
          severity: 'error',
          message: `No manifest at ${PYTHINKER_PLUGIN_ROOT_PATH} or ${PYTHINKER_PLUGIN_DIR_PATH}`,
        },
      ],
    };
  }
  ...
  const manifestKind: PluginManifestKind = rootJsonExists ? 'pythinker-plugin-root' : 'pythinker-plugin-dir';
...
function recordUnsupportedRuntimeFields(
  ...
  for (const field of UNSUPPORTED_RUNTIME_FIELDS) {
    ...
    diagnostics.push({
      severity: 'info',
      message: `"${field}" is present but not supported by Pythinker plugins`,
    });
  }
}

...
export class TurnFlow {
  ...
  private async runOneTurn(
    ...
    if (this.agent.dynamicWorkflowMode.shouldAutoExit) {
      this.agent.dynamicWorkflowMode.exit();
    }
      ...
      try {
        ...
        if (isMaxStepsExceededError(error)) {
          this.agent.log.warn('turn hit max steps', {
            ...
            limit: isPythinkerError(error) ? error.details?.['maxSteps'] : undefined,
          });
        } else {
...
function summarizeTurnError(error: unknown, turnId: number): PythinkerErrorPayload {
...
function classifyApiError(error: unknown, summary: PythinkerErrorPayload): ApiErrorClassification {
...
function summaryStatusCode(summary: PythinkerErrorPayload): number | undefined {
  const statusCode = summary.details?.['statusCode'];
  return typeof statusCode === 'number' ? statusCode : undefined;
}

function isApiConnectionError(error: unknown, summary: PythinkerErrorPayload): boolean {
  return error instanceof APIConnectionError || summary.name === 'APIConnectionError';
}

function isApiTimeoutError(error: unknown, summary: PythinkerErrorPayload): boolean {
  return (
...
function isApiEmptyResponseError(error: unknown, summary: PythinkerErrorPayload): boolean {

...
import { PythinkerChatProvider, type ChatProvider, type Message, type Tool } from '@pymodel/kosong';

import { parseFloatEnv } from '#/config/resolve';
import { resolveThinkingKeep } from '#/config/pythinker-env-params';
...
export class LlmRequestRecorder {
  ...
  record(input: {
    ...
    // Mirror the ConfigState.provider pipeline for Pythinker-only request params:
    // env sampling overrides and the preserved-thinking keep passthrough
    // reach the wire only for Pythinker providers, resolved by the same exported
    // helpers used at construction. thinkingEffort needs no mirroring — the
    // Pythinker provider derives it from the request body's thinking payload, so
    // env effort overrides are already reflected in the read value.
    const isPythinkerProvider = provider instanceof PythinkerChatProvider;
    this.agent.records.logRecord({
      type: 'llm.request',
      ...
      thinkingKeep: isPythinkerProvider
        ? resolveThinkingKeep(
            process.env,
            this.agent.pythinkerConfig?.thinking?.keep,
            provider.thinkingEffort ?? 'off',
          )
        : undefined,
      temperature: isPythinkerProvider
        ? parseFloatEnv(process.env['PYTHINKER_MODEL_TEMPERATURE'], 'PYTHINKER_MODEL_TEMPERATURE')
        : undefined,
      topP: isPythinkerProvider
        ? parseFloatEnv(process.env['PYTHINKER_MODEL_TOP_P'], 'PYTHINKER_MODEL_TOP_P')
        : undefined,
      maxTokens: provider.maxCompletionTokens,
      betaApi:
        modelAlias === undefined
          ? undefined
          : this.agent.pythinkerConfig?.models?.[modelAlias]?.betaApi,

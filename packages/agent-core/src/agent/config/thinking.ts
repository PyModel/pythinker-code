...
export function supportsThinkingEffort(
  ...
  pythinkerProtocol: boolean,
): boolean {
  if (!pythinkerProtocol || effort === 'off') return true;
  const effective = model === undefined ? undefined : effectiveModelAlias(model);
...
export function resolveThinkingEffort(
  ...
  return normalizeThinkingEffortForModel(effort, effectiveModel, pythinkerProtocol);
}

...
const MAX_RUNNING_TASKS_ENV = 'PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS';

/**
 * Resolve the effective background-task concurrency cap. Precedence:
 * `PYTHINKER_CODE_BACKGROUND_MAX_RUNNING_TASKS` (positive integer) → config
...
export class BackgroundManager {
  ...
  private assertCanRegister(startedInBackground: boolean): void {
    const maxRunningTasks = resolveMaxRunningTasks(
      this.agent.pythinkerConfig?.background?.maxRunningTasks,
    );
   ...
   *
   * Used by print-mode (`pythinker -p`) turn draining to hold a turn open while

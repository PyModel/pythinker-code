...
export * from './types';
export { AGENT_WIRE_PROTOCOL_VERSION, assertAgentWireProtocolVersion } from './wire-version';
export {
...
// during resume.
const AGENT_RECORD_TYPES = new Set<AgentRecord['type']>([
  'metadata',
  'forked',
  'turn.prompt',
  'turn.steer',
...
export function assertAgentRecord(input: unknown): asserts input is AgentRecord {
  const type =
    typeof input === 'object' && input !== null && !Array.isArray(input)
      ? (input as { readonly type?: unknown }).type
      : undefined;
  if (typeof type !== 'string' || !AGENT_RECORD_TYPES.has(type as AgentRecord['type'])) {
    throw new Error(`Unsupported agent record type ${String(type)}`);
  }
}
...
function restoreAgentRecord(agent: Agent, input: AgentRecord): void {
  switch (input.type) {
    ...
    case 'dynamic_workflow_mode.enter':
      agent.dynamicWorkflowMode.restoreEnter(input.trigger);
      return;
    case 'dynamic_workflow_mode.exit':
      agent.dynamicWorkflowMode.exit();
      return;
    ...
    default:
      throw new Error(
        `Unsupported agent record type ${String((input as { readonly type?: unknown }).type)}`,
      );
  }
}

...
export class InjectionManager {
  ...
  async inject(): Promise<void> {
    const diagnostics = this.agent.lsp?.drainDiagnostics();
    if (diagnostics !== undefined) {
      this.agent.context.appendSystemReminder(diagnostics, {
        kind: 'injection',
        variant: 'lsp_diagnostics',
      });
    }
  ...
  onContextCompacted(compactedCount: number, startIndex = 0): void {
    for (const injector of this.lifecycleInjectors()) {
      try {
        injector.onContextCompacted(compactedCount, startIndex);
      } catch {
        continue;
      }
    }

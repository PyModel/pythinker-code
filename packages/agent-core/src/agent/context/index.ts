...
export class ContextMemory {
  ...
  applyCompaction(result: CompactionResult): void {
    ...
    const endIndex = startIndex + result.compactedCount;
    if (
      !Number.isSafeInteger(startIndex) ||
      !Number.isSafeInteger(result.compactedCount) ||
      startIndex < 0 ||
      result.compactedCount < 0 ||
      endIndex > this._history.length
    ) {
      throw new PythinkerError(
        ErrorCodes.REQUEST_INVALID,
        'Compaction range is outside the active context.',
      );
    }
    this._history = [
      ...this._history.slice(0, startIndex),
      {
        role: 'assistant',
    ...
    this.agent.microCompaction.reset();
    this.agent.injection.onContextCompacted(result.compactedCount, startIndex);
    this.agent.emitStatusUpdated();
  }
  ...
  usageReport(): ContextUsageReport {
    const messages = this.messages;
    const systemPromptTokens = estimateTokens(this.agent.config.systemPrompt);
    const activeTools = this.agent.tools.loopTools;
    const sourceByName = new Map(
      this.agent.tools.data().map((tool) => [tool.name, tool.source]),
    );
    const tools = activeTools.map((tool) => ({
      name: tool.name,
      source: sourceByName.get(tool.name) ?? 'builtin',
      tokens: estimateTokensForTools([tool]),
    }));
    const roleTokens = new Map<Role, number>();
    for (const message of messages) {
      roleTokens.set(
        message.role,
        (roleTokens.get(message.role) ?? 0) + estimateTokensForMessage(message),
      );
    }
    const maxTokens = this.agent.config.modelCapabilities.max_context_tokens;
    const percentageOfMax = (tokens: number): number =>
      maxTokens > 0 ? Math.round((tokens / maxTokens) * 1_000) / 10 : 0;
    const category = (name: string, tokens: number): ContextUsageCategory => ({
      name,
      tokens,
      percentage: percentageOfMax(tokens),
    });
    const toolTokens = tools.reduce((total, tool) => total + tool.tokens, 0);
    const categories = [
      category('System prompt', systemPromptTokens),
      category('Tools', toolTokens),
      category('System messages', roleTokens.get('system') ?? 0),
      category('User messages', roleTokens.get('user') ?? 0),
      category('Assistant messages', roleTokens.get('assistant') ?? 0),
      category('Tool results', roleTokens.get('tool') ?? 0),
    ];
    const estimatedTokens = categories.reduce((total, item) => total + item.tokens, 0);
    categories.push(category('Free space', Math.max(0, maxTokens - estimatedTokens)));

    return {
      model: this.agent.config.modelAlias,
      estimatedTokens,
      maxTokens,
      percentage: percentageOfMax(estimatedTokens),

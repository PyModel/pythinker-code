...
export class FullCompaction {
  ...
  // Trace id (`x-trace-id`, Pythinker/KFC only) of the latest summarizer request,
  ...
  begin(data: Readonly<CompactionBeginData>): void {
    ...
    if (this.agent.context.history.length === 0) {
      throw new PythinkerError(ErrorCodes.COMPACTION_UNABLE, 'No messages to compact in current history.');
    }

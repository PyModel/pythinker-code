...
// Relative to brandHomeDir, which already IS the brand data dir (~/.pythinker-code or
// $PYTHINKER_CODE_HOME) — no '.pythinker-code' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['skills'] as const;
const USER_GENERIC_DIRS = ['.agents/skills'] as const;
const PROJECT_BRAND_DIRS = ['.pythinker-code/skills'] as const;
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `PYTHINKER_CODE_HOME`, or `<userHomeDir>/.pythinker-code` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.pythinker-code` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.pythinker-code');

...
const FIXTURE_PYTHINKER = join(FIXTURES, 'multi-workdir', '.pythinker');
...
beforeEach(async () => {
  ...
  await mkdir(join(FIXTURE_PYTHINKER, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), { recursive: true });
});
afterEach(async () => {
  await rm(targetHome, { recursive: true, force: true });
  await rm(join(FIXTURE_PYTHINKER, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), {
    recursive: true,
    force: true,
  });
});
...
describe('migrateSessionsStep (multi-workdir fixture)', () => {
  ...
  it('counts an already-migrated session as failed when its index entry cannot be ensured', async () => {
    ...
    await migrateSessionsStep({ sourceHome: FIXTURE_PYTHINKER, targetHome });
  ...
  it('reports an unknown workdir bucket as a failure', async () => {
    ...
    try {
      ...
      expect(report.sessionsFailed).toEqual([
        {
          sourcePath: bucket,
          reason: expect.stringMatching(/workdir.*pythinker\.json/i),
        },
      ]);
    } finally {
      await rm(src, { recursive: true, force: true });

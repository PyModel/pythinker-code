...
const FIXTURE_HOME = join(FIXTURES, 'multi-workdir', '.pythinker');
...
beforeEach(async () => {
  ...
  await mkdir(join(FIXTURE_HOME, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), { recursive: true });
});
afterEach(async () => {
  await rm(targetHome, { recursive: true, force: true });
  await rm(join(FIXTURE_HOME, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), {
    recursive: true,
    force: true,
  });
});
...
describe('migrateSessionsStep (multi-workdir fixture)', () => {
  ...
  it('counts an already-migrated session as failed when its index entry cannot be ensured', async () => {
    ...
    await migrateSessionsStep({ sourceHome: FIXTURE_HOME, targetHome });
  ...
  it('surfaces an incompatible imported target with the exact operator action', async () => {
    const src = await mkdtemp(join(tmpdir(), 'incompatible-import-src-'));
    try {
      const workdir = '/Users/me/incompatible-import';
      await writeFile(
        join(src, 'pythinker.json'),
        JSON.stringify({ work_dirs: [{ path: workdir, kaos: 'local' }] }),
      );
      const bucket = join(src, 'sessions', oldMd5BucketName(workdir));
      await mkdir(join(bucket, 'incompatible-uuid'), { recursive: true });
      await writeFile(
        join(bucket, 'incompatible-uuid', 'context.jsonl'),
        '{"role":"user","content":"hi"}\n' +
          '{"role":"assistant","content":[{"type":"text","text":"hello"}]}\n',
      );
      await writeFile(join(bucket, 'incompatible-uuid', 'state.json'), '{}');

      const targetDir = join(
        targetHome,
        'sessions',
        computeWorkdirBucket(workdir),
        'ses_incompatible-uuid',
      );
      await mkdir(join(targetDir, 'agents', 'main'), { recursive: true });
      await writeFile(
        join(targetDir, 'state.json'),
        JSON.stringify({ custom: { imported_from_pythinker_cli: true } }),
      );
      await writeFile(
      ...
      expect(report.sessionsConflicts).toHaveLength(1);
      expect(report.sessionsFailed).toContainEqual({
        sourcePath: join(bucket, 'incompatible-uuid'),
        reason: 'Existing imported target uses an incompatible format; remove it and rerun the import.',
      });
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  ...
  it('counts a content-empty session as skipped-empty (not failed) and still migrates the real one', async () => {
    ...
    // in pythinker-cli) must be reported as skipped-empty — not failed — without

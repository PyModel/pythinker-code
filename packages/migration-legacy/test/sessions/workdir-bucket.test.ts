...
/**
 * Reference re-implementation of pythinker-core `encodeWorkDirKey` +
 * `slugifyWorkDirName` (packages/pythinker-core/src/harness/session-manager/
 * workdir-key.ts + utils/workdir-slug.ts). Kept inline because those
 * functions are not part of pythinker-core's public export surface. The migrator's
...
describe('computeWorkdirBucket', () => {
  ...
  it('handles unicode basename by replacing with a stable safe form', () => {
    const bucket = computeWorkdirBucket('/Users/me/projects');
    // We don't require any specific slug, but we require it's bucket-safe
    expect(bucket).toMatch(/^wd_[a-z0-9-]+_[0-9a-f]{12}$/i);
  });
});

describe('computeWorkdirBucket matches pythinker-core encodeWorkDirKey', () => {
  it.each([
    '/Users/example/proj',

 ...
 * Run: pnpm --filter @pymodel/migration-legacy test -- marker.test.ts
 */
...
describe('marker', () => {
  ...
  it('suppresses migration for both targets when appending a run to a legacy marker', async () => {
    ...
    await writeFile(
      join(dir, '.migrated-to-pythinker-code'),
      JSON.stringify({
        version: 1,
  ...
  it('readMarker returns undefined when file is corrupt', async () => {
    await writeFile(join(dir, '.migrated-to-pythinker-code'), 'not-json', 'utf-8');
    expect(await readMarker(dir)).toBeUndefined();
  });
  ...
  it('suppresses migration when an old marker has no target path', async () => {
    await writeFile(join(dir, '.migrated-to-pythinker-code'), '{}', 'utf-8');
  ...
  it('suppresses migration when the target contains the skip marker', async () => {
    ...
    await writeFile(join(targetHome, '.skip-migration-from-pythinker-cli'), '', 'utf-8');
  ...
  it('suppresses migration when Windows drive letters differ only by case', async () => {
    await writeFile(
      join(dir, '.migrated-to-pythinker-code'),
      JSON.stringify({ target_path: 'C:\\Users\\Example\\.pythinker-code' }),
      'utf-8',
    );
    ...
    expect(
      shouldSuppressMigration({
        sourceHome: dir,
        targetHome: 'c:\\Users\\Example\\.pythinker-code',
      }),
    ).toBe(true);
  });
});

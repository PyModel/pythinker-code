...
const SOURCE_HOME = join(FIXTURES, 'multi-workdir', '.pythinker');
const MARKER_PATH = join(SOURCE_HOME, '.migrated-to-pythinker-code');
...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not write a completed marker when legacy session data remains unreadable', async () => {
    ...
    try {
      ...
      await expect(readFile(join(src, '.migrated-to-pythinker-code'), 'utf-8')).rejects.toThrow();
    } finally {
  ...
  it('does not write a completed marker when a session target is occupied by a foreign session', async () => {
    ...
    try {
      ...
      await writeFile(join(src, 'pythinker.json'), JSON.stringify({ work_dirs: [{ path: workdir }] }));
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'pythinker-code.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in pythinker-code.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('pythinker-code');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });

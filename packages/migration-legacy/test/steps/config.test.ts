...
describe('migrateConfigStep', () => {
  ...
  it('additively merges into a user-modified target config', async () => {
    ...
    expect(cfg).toContain('pythinker-code/pythinker-for-coding'); // migrated model added
  });
...
`,
  ...
  it('drops top-level keys pythinker-code does not support', async () => {
    await writeFile(
      join(src, 'config.toml'),
  ...
  it('falls back to a sibling file when the target config is unparseable', async () => {
    ...
    expect(
      await readFile(join(tgt, 'config.migrated-from-pythinker-cli.toml'), 'utf-8'),
  ...
  it('a tui.toml-only conflict sets wroteTuiSibling, not wroteSiblingDueToConflict', async () => {
    ...
    expect(
      await readFile(join(tgt, 'tui.migrated-from-pythinker-cli.toml'), 'utf-8'),
    ).toContain('theme');
    // original kept
...
`,
  ...
  it('drops a legacy theme outside the pythinker-code TUI enum', async () => {
  ...
  it('drops a single hook pythinker-code\'s schema rejects, keeps the rest', async () => {
    await writeFile(
      join(src, 'config.toml'),
      '[[hooks]]\n' +
        'event = "PreToolUse"\n' +
  ...
  it('reports migratedHooks=0 and populates siblingContents when sibling mode kicks in', async () => {
    ...
    // `config.migrated-from-pythinker-cli.toml`. Hooks land in the sibling, NOT in

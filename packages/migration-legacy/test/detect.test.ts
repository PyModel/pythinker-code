...
describe('detectMigration', () => {
  ...
  it('detects config/mcp/credentials/user-history/plugins/mcp-oauth presence', async () => {
    ...
    await writeFile(join(src, 'credentials', 'pythinker-code.json'), '{}');
    ...
    expect(plan.oauthCredentials).toEqual(['pythinker-code']);
  ...
  it('reports an unknown workdir bucket when pythinker.json cannot map it', async () => {

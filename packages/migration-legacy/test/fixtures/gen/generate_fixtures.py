"""Regenerate the golden migration fixtures using pythinker-cli's real serializers.

The committed fixtures under `../golden/` are produced by THIS script against a
pythinker-cli checkout, so the migration tests exercise the exact byte shapes the
...
pythinker-cli itself still upgrades (`config.json`, flat `<uuid>.jsonl` sessions,
`metadata.json`, title-only sessions).

Usage (from the pythinker-code repo):

    cd ../../../pythinker-cli && uv run python \
        /path/to/pythinker-code/packages/migration-legacy/test/fixtures/gen/generate_fixtures.py

`--out` defaults to the sibling `golden/` directory next to this script.
Re-run and commit the result whenever pythinker-cli's on-disk formats change.
"""
...
from pythinker_cli.config import (
    Config,
    LLMModel,
    LLMProvider,
    LoopControl,
...
from pythinker_cli.hooks.config import HookDef
from pythinker_cli.metadata import Metadata, WorkDirMeta
from pythinker_cli.session_state import SessionState, TodoItemState, save_session_state
...
def main() -> None:
    ...
    metadata = Metadata(
        work_dirs=[
            ...
            WorkDirMeta(path=REMOTE_WORK_DIR, pyaos="ssh"),
        ]
    )
    (home / "pythinker.json").write_text(
    ...
    (credentials / "pythinker-code.json").write_text(
        json.dumps(
            {
    ...
    historical = out / ".pythinker-historical-config"

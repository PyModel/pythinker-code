 ...
 * resumable by — real pythinker-core. The migrator writes session buckets named by
 * `computeWorkdirBucket`, and its batch publisher adds the resulting directory to
 * pythinker-core's session index. This focused check registers the same index entry
 * before asserting the current `SessionStore.list` boundary.
 *
 * The resume test additionally drives a real `Session.resume()`: it reads the
 * migrated `state.json`, derives `<sessionDir>/agents/main`, and replays that
 * agent's `wire.jsonl`. This confirms imported state contains no persisted path
 * and still resumes through the canonical agent directory.
 *
 * agent-core API used:
...
describe('migrated session loads in real pythinker-core', () => {
  it('computeWorkdirBucket matches pythinker-core encodeWorkDirKey', () => {
  ...
  it('SessionStore.list() finds a migrated session under the same workDir', async () => {
    ...
    await ensureSessionIndexEntry(targetHome, {
      sessionId: 'ses_integ-uuid',
    ...
    // This exercises pythinker-core's strict indexed lookup after explicit
    ...
    expect(migrated?.metadata?.['imported_from_pythinker_cli']).toBe(true);
  });
  ...
  it('migrated wire history is non-empty and resumable', async () => {
    ...
    expect((events[0] as { protocol_version?: string } | undefined)?.protocol_version).toBe('2.0');
    expect(events.filter((e) => e.type === 'context.append_message').length).toBeGreaterThan(0);
  });

  it('real pythinker-core Session.resume() loads the migrated message history', async () => {
    const result = await migrateOneSession({
      sourceSessionDir: join(FIXTURES, 'tiny-hello-world'),
      oldSessionUuid: 'tiny-resume',
      workdirPath: WORK_DIR,
    ...
    // Drive a real pythinker-core resume: `Session.resume()` reads strict
    // `state.json`, derives the main agent directory, and replays its wire.

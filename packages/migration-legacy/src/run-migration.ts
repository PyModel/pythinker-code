...
const CONFIG_CONFLICT_NOTICE =
  'Your existing config.toml could not be parsed; migrated copy saved to ~/.pythinker-code/config.migrated-from-pythinker-cli.toml — please review and merge manually.';

const TUI_CONFLICT_NOTICE =
  'Your existing tui.toml had user modifications; migrated copy saved to ~/.pythinker-code/tui.migrated-from-pythinker-cli.toml — please review and merge manually.';
...
export interface RunMigrationInput {
  ...
  /** Legacy plans dir; defaults to `~/.pythinker/plans` per pythinker-cli's hardcode. */

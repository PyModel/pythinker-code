...
import { HookDefSchema } from '@pymodel/agent-core-v2/features/externalHooks/configSection';
import { getConfigSectionContributions } from '@pymodel/agent-core-v2/app/config/configSectionContributions';
import { getContributedFlags } from '@pymodel/agent-core-v2/app/flag/flagRegistry';
import { camelToSnake } from '@pymodel/agent-core-v2/app/config/toml';
...
import '@pymodel/agent-core-v2/app/flag/flag';
import '@pymodel/agent-core-v2/features/skill/catalog/configSection';

import '@pymodel/agent-core-v2/session/subagent/flag';
import '@pymodel/agent-core-v2/session/sessionTitle/flag';
import '@pymodel/agent-core-v2/features/tower/flag';
import '@pymodel/agent-core-v2/agent/toolSelect/flag';
import '@pymodel/agent-core-v2/agent/tools/task/task-wait/flag';
import '@pymodel/agent-core-v2/persistence/configSection';
...
// pythinker-code's tui.toml `theme` enum (mirrors apps/pythinker-code TuiThemeSchema).
...
// The config.toml top-level keys pythinker-code understands, derived from the v2
...
export interface ConfigStepResult {
  ...
  /** Top-level keys dropped because pythinker-code's config schema lacks them. */
  readonly droppedKeys: readonly string[];
  /**
   * Keys/sections the existing target config and the pythinker-cli config both set
   * to a different value — the target's value was kept.
   */
  readonly configConflicts: readonly string[];
  /** A `config.toml` conflict forced a `config.migrated-from-pythinker-cli.toml` sibling. */
  readonly wroteSiblingDueToConflict: boolean;
  /** A `tui.toml` conflict forced a `tui.migrated-from-pythinker-cli.toml` sibling. */
  readonly wroteTuiSibling: boolean;
  /** Count of pythinker-cli hook entries written into the LIVE target config. */
  readonly migratedHooks: number;
  /** Count of pythinker-cli hook entries dropped because pythinker-code's schema rejects them. */
   ...
   * content that landed in `config.migrated-from-pythinker-cli.toml` instead of
...
/** True when the pythinker-cli provider entry validates against pythinker-code's v2 schema
...
/** True when the pythinker-cli model entry validates against pythinker-code's v2 schema. */
...
/**
 * Additively merge the pythinker-cli config into the existing target config: add
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those pythinker-code's schema accepts.
  ...
  // Provider names the merge resolves to a DIFFERENT entry than the pythinker-cli
  ...
  // 2) Models — keep only those pythinker-code's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries pythinker-code's HookDefSchema accepts. pythinker-cli
  //     and pythinker-code share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field pythinker-code does not know).
  ...
  // 4) Build the migrated top-level — only keys pythinker-code's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // pythinker-cli's `default_yolo` maps to pythinker-code's `default_permission_mode`.
    ...
    // written config — one kept from pythinker-cli, or already in the target being
  ...
  // In sibling mode the source hooks land in `config.migrated-from-pythinker-cli.toml`,
...
// Telemetry identity continuity: pythinker-cli and pythinker-code share the same
// `device_id` concept (uuid hex, `pfc_device_id_` user-id prefix). Copy it only

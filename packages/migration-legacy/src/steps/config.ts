...
// The config.toml top-level keys pythinker-code understands, derived from the live
// PythinkerConfigSchema so the set tracks pythinker-code automatically. `raw` is internal
// — never migrate it. `providers` / `models` / `hooks` are filtered per-entry,
// not via this set.
const SUPPORTED_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(
  Object.keys(PythinkerConfigSchema.shape)
...
/** True when the pythinker-cli provider entry validates against pythinker-code's schema. */
...
/** True when the pythinker-cli model entry validates against pythinker-code's schema. */
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 4b) Drop any supported top-level key whose VALUE pythinker-code's config
  ...
  for (;;) {
    const result = PythinkerConfigSchema.safeParse(transformTomlData(migratedTop));

...
export interface PlansStepInput {
  ...
  /**
   * pythinker-cli hardcodes plan files at `~/.pythinker/plans` regardless of
   * PYTHINKER_SHARE_DIR (`tools/plan/heroes.py`), so the source is resolved from the
...
export function defaultPlansSourceDir(): string {
  return join(homedir(), '.pythinker', 'plans');
}

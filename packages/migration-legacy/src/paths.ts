...
export const sourcePythinkerJson = (src: string): string => join(src, 'pythinker.json');
...
export const migratedMarker = (src: string): string => join(src, '.migrated-to-pythinker-code');

// Target (~/.pythinker-code/) paths
...
export const skipMarker = (tgt: string): string => join(tgt, '.skip-migration-from-pythinker-cli');
...
export const siblingConfigToml = (tgt: string): string =>
  join(tgt, 'config.migrated-from-pythinker-cli.toml');
export const siblingTuiToml = (tgt: string): string =>
  join(tgt, 'tui.migrated-from-pythinker-cli.toml');
export const siblingMcpJson = (tgt: string): string =>
  join(tgt, 'mcp.migrated-from-pythinker-cli.json');

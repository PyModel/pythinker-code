...
export async function detectMigration(opts: { sourcePath: string; skillsSourcePath?: string; plansSourcePath?: string }): Promise<MigrationPlan> {
  ...
  // Reverse-lookup workdir from pythinker.json
  const workdirMap = new Map<string, WorkdirMeta>();
  try {
    const text = await readFile(sourcePythinkerJson(src), 'utf-8');
    const parsed = OldPythinkerJsonSchema.parse(JSON.parse(text));
    for (const wd of parsed.work_dirs) {
      workdirMap.set(oldMd5BucketName(wd.path), { path: wd.path, pyaos: wd.pyaos });
    }
  } catch {
    // no pythinker.json or unparseable — sessions list will be empty
  }
  ...
  try {
    ...
    for (const bucketName of bucketNames) {
      ...
      // Skip non-local-pyaos buckets (`<pyaos>_<md5>`), which cannot be
      // represented by the local Pythinker Code runtime. Every other unknown
...
function unknownWorkdirReason(): string {
  return 'No local workdir mapping was found for this legacy session bucket; pythinker.json may be missing, unreadable, or not list the workdir.';
}

 ...
 *
 * Aliases agent-core-v2's `encodeWorkDirKey` so the migrator and the running
 * app always produce byte-identical buckets. The session picker locates
 * sessions purely by `readdir(encodeWorkDirKey(workDir))` (it never consults
 ...
 * On Windows `node:path` yields backslash-separated paths while agent-core-v2's

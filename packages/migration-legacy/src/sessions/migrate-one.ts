...
import { readTodoItems } from '@pymodel/agent-core-v2/features/todo/todoItem';
...
export async function migrateOneSession(input: MigrateOneInput): Promise<MigrateOneResult> {
  ...
  if (existsSync(targetDir)) {
    ...
    // A real, unrelated pythinker-code session occupies the path — a true conflict.
    if (cls === 'foreign') {
 ...
 *  - `foreign`:  a real, unrelated pythinker-code session occupying the path.
...
async function classifyExistingTarget(targetDir: string): Promise<ExistingTarget> {
  ...
  if (
    ...
    custom !== null &&
    (custom as { imported_from_pythinker_cli?: unknown }).imported_from_pythinker_cli === true
  ) {
    return 'imported';
  }
  return 'foreign';

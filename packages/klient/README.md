...
import { createKlient } from '@pymodel/klient/memory';   // or '/ipc'
...
|---|---|---|
| `@pymodel/klient/ipc` | `{ socketPath, token? }` | same socket |
| `@pymodel/klient/memory` | `{ scope }` (a bootstrapped engine app scope) | direct emitter/bus subscription |
...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live suites
  and their client harness (skip unless `PYTHINKER_SERVER_URL` is set; the v1
...
readings through a real prompt (requires `PYTHINKER_EXAMPLE_MODEL` +
`PYTHINKER_EXAMPLE_API_KEY`).

...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live
  suites (moved from server-e2e). They skip unless `PYTHINKER_SERVER_URL` points
...
- `pnpm --filter @pymodel/klient test` — all Vitest suites (unit +
  conformance + e2e; live cases skip without their env).
- `PYTHINKER_SERVER_URL=http://127.0.0.1:58627 pnpm --filter @pymodel/klient test`
  — include the live legacy cases against a running server.
- `pnpm --filter @pymodel/klient docker:e2e` — docker e2e; the run
  derives its runner name/namespace from the current workspace to avoid
  cross-workspace conflicts.
- `pnpm --filter @pymodel/klient typecheck` / `pnpm smoke` (in-process
  smoke over the memory transport; see `examples/smoke.ts`).
- `pnpm --filter @pymodel/klient smoke:boundary` — ModelRequester boundary
  probe: pings every model configured in the real `~/.pythinker-code/config.toml`
...
- `pnpm --filter @pymodel/klient smoke:select-tools` — select_tools
  (progressive tool disclosure) probe for pythinker-type providers: stub-verifies
  the pythinker-only wire encoding of dynamic tool declarations, then runs a live
  two-step select→use flow per real pythinker model (see
  `examples/pythinker-select-tools.ts`).

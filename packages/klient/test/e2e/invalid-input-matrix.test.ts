 ...
 * Pythinker (protocol `openai` + provider `type: pythinker` — the trait-composition
...
import { IModelService } from '@pymodel/agent-core-v2/llm-adapter/model/model';
...
const M_PYTHINKER = 'matrix-pythinker';
const M_ANTHROPIC = 'matrix-anthropic';
const M_GOOGLE = 'matrix-google';

const PYTHINKER_PROVIDER = 'matrix-pythinker-provider';

const IMAGE_BAD_MIME_URL = 'data:image/tiff;base64,QUJD'; // tiff is outside every provider's accepted set
const IMAGE_PYTHINKER_ONLY_MIME_URL = 'data:image/bmp;base64,QUJD'; // bmp is accepted by Pythinker alone
...
beforeAll(async () => {
  ...
  await klient.global.kosong.addProvider(PYTHINKER_PROVIDER, {
    type: 'pythinker',
  ...
  // M_PYTHINKER needs a `provider` reference to PYTHINKER_PROVIDER so the engine
  // resolves pythinker provider traits (uploadVideo). The facade's addProvider()
  // doesn't support provider-linkage, so call modelService directly.
  await app!.accessor.get(IModelService).set(M_PYTHINKER, {
    model: 'kimi-k2-matrix',
    provider: PYTHINKER_PROVIDER,
    protocol: 'openai',
    maxContextSize: 262_144,
...
describe('image blocks with invalid data', () => {
  it('a data-URL image with an unaccepted mime is replaced at prompt ingestion on EVERY provider (l2)', async () => {
    ...
    const cases = [
      ...
      { label: 'tiff-pythinker', model: M_PYTHINKER, reply: OK_OPENAI },
  ...
  it('a data-URL image in a Pythinker-only format reaches a Pythinker model but is replaced elsewhere (l2)', async () => {
    // The accepted set is keyed by the provider the agent is bound to: Pythinker
    // takes BMP/HEIC/HEIF on top of the baseline, the other providers do not.
    const cases = [
      { label: 'bmp-pythinker', model: M_PYTHINKER, reply: OK_OPENAI, accepted: true },
    ...
    for (const { label, model, reply, accepted } of cases) {
      ...
      await promptAndWait(ctx, [
        { type: 'text', text: 'what is this?' },
        { type: 'image_url', imageUrl: { url: IMAGE_PYTHINKER_ONLY_MIME_URL } },
      ]);
      expect(requests, label).toHaveLength(1);
      const wireText = JSON.stringify(requests[0]?.json);
  ...
  it('pythinker (composed): same media-strip fallback as plain openai (l3 + engine fallback)', async () => {
    const ctx = await newCase(M_PYTHINKER, 'pythinker-image-base64');
    resetMock(queueScript(jsonError(400, 'Invalid image data'), OK_OPENAI));
...
// ---------------------------------------------------------------------------
// Daemon file references (pythinker-file://): engine-side resolution before the
// provider wire.
// ---------------------------------------------------------------------------

describe('daemon file references (pythinker-file://)', () => {
  it('a pythinker-file image reference reaches the provider as a data URL, never verbatim', async () => {
    ...
    // order, so image pythinker-file refs leaked to the provider unchanged and
    ...
    const cases = [
      { label: 'pythinkerfile-image-openai', model: M_OPENAI_VISION, reply: OK_OPENAI },
      { label: 'pythinkerfile-image-pythinker', model: M_PYTHINKER, reply: OK_OPENAI },
    ] as const;
    for (const { label, model, reply } of cases) {
      ...
      resetMock(queueScript(reply));
      await promptAndWait(ctx, [
        { type: 'image_url', imageUrl: { url: `pythinker-file://${meta.id}` } },
        { type: 'text', text: 'what is this?' },
      ]);
      expect(requests, label).toHaveLength(1);
      expect(JSON.stringify(requests[0]?.json), label).not.toContain(`pythinker-file://${meta.id}`);
...
describe('video blocks', () => {
  ...
  it('pythinker (composed): ReadMediaFile on a video uploads via the files API (uploadVideo trait)', async () => {
    const ctx = await newCase(M_PYTHINKER, 'pythinker-video-upload');
    ...
    // The PythinkerFiles client POSTs multipart form data to {baseUrl}/files.
...
describe('tool exchange structure', () => {
  ...
  it('media tool result: plain openai extracts text + appends a media user message; pythinker keeps parts in place', async () => {
    ...
    // Composed pythinker: trait mode hands shaping to the trait — the image part
    // stays inside the tool message content; no extra user message appears.
    const pythinkerWire = await runMediaToolResultCase(M_PYTHINKER, 'media-result-pythinker');
    const pythinkerTool = pythinkerWire.find((message) => message['role'] === 'tool');
    const pythinkerContent = pythinkerTool?.['content'] as { type: string }[];
    expect(Array.isArray(pythinkerContent)).toBe(true);
    expect(pythinkerContent.some((part) => part.type === 'image_url')).toBe(true);
    expect(pythinkerWire.at(-1)?.['role']).toBe('tool');
  }, 60_000);

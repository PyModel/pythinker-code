 ...
 *
 * Env: PYTHINKER_MODEL_NAME is unset for the run (it would pin `defaultModel` and
...
import { IKosongConfigService } from '@pymodel/agent-core-v2/app/kosongConfig/kosongConfig';
import { type Klient } from '@pymodel/klient';
...
const pinnedModelEnv = process.env['PYTHINKER_MODEL_NAME'];
delete process.env['PYTHINKER_MODEL_NAME'];
try {
  await main();
} catch (error) {
  console.error(error);
  ...
  if (pinnedModelEnv !== undefined) process.env['PYTHINKER_MODEL_NAME'] = pinnedModelEnv;
}

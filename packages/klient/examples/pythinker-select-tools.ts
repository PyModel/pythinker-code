 ...
 * pythinker-type providers, based on the real providers/auth in
 ...
 *
 * The wire fact that makes this pythinker-specific: only the pythinker vendor trait
 * serializes `message.tools` into the outbound request
 * (`pythinkerOpenAITrait.convertMessage`); every other base SKIPS
 * tool-declaration-only messages entirely. So the whole mechanism can only
 * work on `(pythinker, openai)` — this example proves that on a stub, then
 * behaviorally tests each real pythinker model with a simulated "pythinker computer
 * use" tool pool.
 *
 * Part A — deterministic, local stub (OpenAI Chat Completions wire):
 *   1. pythinker providerType: the tool-declaration message reaches the wire with
 *      an embedded `tools` array.
 *   2. no providerType (plain openai): the same message is dropped — the
 *      boundary non-pythinker providers cannot cross.
 ...
 *
 * Part B — live, per pythinker model from the real config: a two-step flow that
 ...
 *     --import ../../build/register-raw-text-loader.mjs examples/pythinker-select-tools.ts
 *
 * Env:
 *   PYTHINKER_CODE_HOME              — default `~/.pythinker-code`
 *   PYTHINKER_SELECT_TOOLS_MODELS    — comma-separated model ids for the live parts (default: all pythinker-type)
 *   PYTHINKER_SELECT_TOOLS_SKIP_LIVE — set to `1` to skip part B (no real API calls)
 *   PYTHINKER_SELECT_TOOLS_TAP       — set to `1` to run part C instead of B: route
...
// ---------------------------------------------------------------------------
// The simulated "pythinker computer use" MCP tool pool.
// ---------------------------------------------------------------------------
...
async function probeWireEncoding(): Promise<void> {
  ...
  try {
    // 1) pythinker providerType: the declaration message reaches the wire, with the
    // loaded schema embedded in the message — and the deferred top-level tool
    // does NOT duplicate into the wire tools[].
    await collect(makeRequester('pythinker'), input);
    const pythinkerBody = lastBody;
    const declaration = pythinkerBody?.messages?.find((m) => Array.isArray(m.tools));
    assert(declaration !== undefined, 'pythinker: a wire message carries the loaded tool schemas');
    assert(
      declaration.tools?.some((t) => t.function?.name === 'computer_screenshot') === true,
      'pythinker: declaration embeds computer_screenshot',
    );
    assert(
      pythinkerBody?.tools?.some((t) => t.function?.name === 'select_tools') === true,
      'pythinker: top-level tools[] keeps select_tools',
    );
    assert(
      pythinkerBody?.tools?.some((t) => t.function?.name === 'computer_screenshot') !== true,
      'pythinker: deferred tool stays OUT of top-level tools[]',
    );
    console.log(
      `[ok] pythinker: declaration on wire (role=${String(declaration.role)}), ` +
        `top-level tools=[${(pythinkerBody?.tools ?? []).map((t) => t.function?.name ?? '?').join(', ')}]`,
    );

    // 2) plain openai: the same declaration message is dropped outright — the
    // dynamic-tool schema can never reach a non-pythinker wire. (3 wire messages =
    ...
    console.log(
      '[ok] openai: declaration message dropped — dynamic tools cannot cross a non-pythinker wire',
    );
  } finally {
...
// ---------------------------------------------------------------------------
// Part B — live two-step select_tools flow per real pythinker model.
// ---------------------------------------------------------------------------
...
async function probeLivePythinkerProviders(): Promise<void> {
  const homeDir = process.env['PYTHINKER_CODE_HOME'] ?? join(homedir(), '.pythinker-code');
  console.log(`\n=== part B: live select_tools flow on real pythinker providers (${homeDir}) ===`);
  ...
  try {
    ...
    const filter = process.env['PYTHINKER_SELECT_TOOLS_MODELS']?.split(',').map((s) => s.trim());
    ...
    const targets = models.filter((m) => {
      ...
      try {
        return catalog.get(m.model).providerType === 'pythinker';
      } catch {
        return false;
      }
    });
    assert(targets.length > 0, 'at least one pythinker-type model configured');
...
async function main(): Promise<void> {
  ...
  if (process.env['PYTHINKER_SELECT_TOOLS_TAP'] === '1') {
    await probeTappedContext();
  } else if (process.env['PYTHINKER_SELECT_TOOLS_SKIP_LIVE'] !== '1') {
    await probeLivePythinkerProviders();
  }
  console.log('\nselect-tools: OK');
}

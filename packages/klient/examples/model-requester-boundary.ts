 ...
 * Pythinker home (`PYTHINKER_CODE_HOME` or `~/.pythinker-code`), resolves `IModelCatalog`
 ...
 *   PYTHINKER_BOUNDARY_MODELS  — comma-separated model ids to ping (default: all)
 *   PYTHINKER_BOUNDARY_SKIP_LIVE — set to `1` to skip part 1 (no real API calls)
 */
...
import { isError2 } from '@pymodel/agent-core-v2/_base/errors/errors';
...
} from '@pymodel/agent-core-v2/llm-adapter/contract/errors';
import {
  oauthCredentials,
  staticCredentials,
} from '@pymodel/agent-core-v2/human/credentials/credentials';
import type {
  ToolCall,
  ToolDescription as Tool,
} from '@pymodel/agent-core-v2/human/llm/message';
import type { LlmCredentialProvider } from '@pymodel/agent-core-v2/human/llm/requester/requester';
...
async function probeRealConfig(): Promise<void> {
  ...
  try {
    ...
    const filter = process.env['PYTHINKER_BOUNDARY_MODELS']?.split(',').map((s) => s.trim());
...
async function main(): Promise<void> {
  if (process.env['PYTHINKER_BOUNDARY_SKIP_LIVE'] !== '1') {

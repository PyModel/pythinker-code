...
describe('stepRetry plugin', () => {
  ...
  it('retries any request error inside the request when PYTHINKER_CODE_INFINITE_RETRY is set', async () => {
    vi.useFakeTimers();
  ...
  it('keeps retrying past the per-step attempt budget when PYTHINKER_CODE_INFINITE_RETRY is set', async () => {
    vi.useFakeTimers();
    vi.stubEnv('PYTHINKER_CODE_INFINITE_RETRY', '1');
    let calls = 0;
    ctx = createTestAgent(

...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'pythinker-reminder-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'pythinker-reminder-work-'));
...
describe('agentsMdReminder path-carrying tools', () => {
  ...
  it('discovers the .pythinker-code/AGENTS.md variant alongside the plain one', async () => {
    ...
    expect(outputText(result)).toBe('original result');
    const text = reminderText(h);
    expect(text).toContain(dotPythinker);
...
describe('agentsMdReminder probing boundaries', () => {
  ...
  it('probes only the immediate directory outside any project', async () => {
    const h = createHarness();
    const outside = await mkdtemp(join(tmpdir(), 'pythinker-reminder-outside-'));
  ...
  it('discovers a symlinked directory through the link at its lexical address', async () => {
    const h = createHarness();
    const target = await mkdtemp(join(tmpdir(), 'pythinker-reminder-target-'));

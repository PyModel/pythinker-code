...
describe('LLMRequester service migration coverage', () => {
  describe('wire observability records', () => {
    ...
    it('records one tools snapshot per unique provider-visible tool table and one request per outbound call', async () => {
      ...
      expect(requests[0]?.args).toMatchObject({
        ...
        // Pythinker is a vendor over the openai base, not a protocol.
        provider: 'openai',
        model: 'mock-model',
    ...
    it('records the resolved Pythinker thinking keep default when thinking is enabled', async () => {
      ctx.configure({
        modelCapabilities: {
          image_in: false,
          video_in: false,
    ...
    it('records the env-forced Pythinker effort used by the provider', async () => {
      await ctx.dispose();
    ...
    it('tracks api_error with the v1 wire shape (model id, alias, protocol, status code)', async () => {
      ...
      expect(records).toContainEqual({
        ...
        properties: expect.objectContaining({
          ...
          // provider is the pythinker vendor over the openai base.
  ...
  describe('per-turn intent handoff', () => {
    ...
    it('forwards the session id as the per-turn cache-key intent', async () => {
      ...
      // encodes it (Pythinker `prompt_cache_key`, Anthropic `metadata.user_id`) is

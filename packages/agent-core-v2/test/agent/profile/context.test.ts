...
describe('loadAgentsMd user-level discovery', () => {
  it('loads user-level branded and generic files before project-level', async () => {
    ...
    await writeFile(join(homeDir, '.pythinker-code', 'AGENTS.md'), 'user branded', 'utf-8');
...
describe('loadAgentsMd symlinked files', () => {
  it('follows symlinks when loading user-level and project-level AGENTS.md', async () => {
    const targetDir = await mkdtemp(join(tmpdir(), 'pythinker-agents-target-'));
    ...
    await mkdir(join(homeDir, '.pythinker-code'), { recursive: true });
    await symlink(brandTarget, join(homeDir, '.pythinker-code', 'AGENTS.md'));
...
describe('loadAgentsMd brand home (PYTHINKER_CODE_HOME)', () => {
  let brandHome: string;

  beforeEach(async () => {
    brandHome = await mkdtemp(join(tmpdir(), 'pythinker-agents-brand-'));
  });
  ...
  it('ignores the real-home .pythinker-code/AGENTS.md when the brand home is elsewhere', async () => {
  ...
  it('falls back to the real-home .pythinker-code/AGENTS.md when no brand home is given', async () => {
    await mkdir(join(homeDir, '.pythinker-code'), { recursive: true });
...
describe('loadAgentsMd nested project hierarchy', () => {
  it('loads AGENTS.md from the project root down to the cwd in root→leaf order', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'pythinker-agents-project-'));
...
describe('loadAgentsMdDetailed discovered paths', () => {
  ...
  it('returns the normalized paths of every injected file in collection order', async () => {
    ...
    await writeFile(join(workDir, '.pythinker-code', 'AGENTS.md'), 'dot pythinker', 'utf-8');
    await writeFile(join(workDir, 'AGENTS.md'), 'project instructions', 'utf-8');

    const result = await loadAgentsMdDetailed({ fs, homeDir }, workDir);

    expect(result.paths).toEqual([
      normalize(join(homeDir, '.pythinker-code', 'AGENTS.md')),
      normalize(join(workDir, '.pythinker-code', 'AGENTS.md')),

...
// The old `#/agent/profile/thinking` helpers derived "Pythinker thinking
// semantics" from `protocol: 'pythinker'` on the model fixture. The kosong layer
// has no Pythinker protocol (Pythinker is a set of `(baseProtocol, traits)`
// registrations, so fixtures use `providerType: 'pythinker'` + a legal protocol);
...
describe('resolveThinkingEffortForModel', () => {
  ...
  it('clamps always-thinking models to their default effort even without strict validation', () => {
    ...
    // covers Pythinker-managed models routed through the Anthropic transport and

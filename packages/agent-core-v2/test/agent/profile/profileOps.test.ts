...
// Side-effect registration: `drivesThinkingThroughTraits('pythinker')` (used by
...
function createTestModel(
  ...
  return {
    ...
    maxContextSize: 1000,
    supportEfforts: providerType === 'pythinker' ? ['low', 'medium', 'high', 'max'] : undefined,
    defaultEffort: providerType === 'pythinker' ? 'high' : undefined,
 ...
 * here from the real Pythinker definitions: strict on the native openai
...
function createProtocolRegistryStub(): IProtocolAdapterRegistry {
  return {
    ...
    resolveAdapterIdentity: (protocol: Protocol, providerType?: string) => ({
      baseId: protocol,
      traits:
        providerType === 'pythinker' && protocol === 'openai'
          ? [
              {
          ...
          : providerType === 'pythinker' && protocol === 'anthropic'
...
describe('AgentProfileService (wire-backed config.update)', () => {
  ...
  it('applies thinking.keep model override when thinking is enabled', () => {
    ...
    host.svc.update({ modelAlias: 'pythinker-code', thinkingLevel: 'high' });

    // The morph chain's replacement: the profile's dialect-free per-turn
    // intent. Wire encoding (`extra_body.thinking.keep`) is the Pythinker dialect's
    // own hook now.
  ...
  it('forces the environment Pythinker effort instead of the resolved effort', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ providerType: 'pythinker' }),
    });
  ...
  it('does not leak a forced Pythinker effort when switching to a non-Pythinker model', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ providerType: 'pythinker' }),
  ...
  it('forces Pythinker effort through Anthropic without Pythinker generation kwargs', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ protocol: 'anthropic', providerType: 'pythinker' }),
    });
    ...
    // "Without Pythinker generation kwargs" is no longer decidable at the profile:
  ...
  it('defaults thinking.keep to "all" when thinking is enabled on Pythinker', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ providerType: 'pythinker' }),
    });
  ...
  it('treats an off env thinking.keep override as disabled on Pythinker', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ providerType: 'pythinker' }),
    });
  ...
  it('does not apply thinking.keep model override when thinking is off', () => {
    ...
    host.svc.update({ modelAlias: 'pythinker-code', thinkingLevel: 'off' });
  ...
  it('uses the session id as a Pythinker prompt cache hint', () => {
    modelCatalog = createModelCatalogStub({
      'pythinker-code': createTestModel({ providerType: 'pythinker' }),
    });
  ...
  it('resolves the session cache-key intent for non-Pythinker protocols too', () => {
    modelCatalog = createModelCatalogStub({
    ...
    // every protocol. How each dialect encodes it (Pythinker `prompt_cache_key`

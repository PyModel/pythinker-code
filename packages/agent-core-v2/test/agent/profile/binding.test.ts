...
describe('AgentProfileService.bind', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-bind-home-'));
  });
  ...
  it('binds a profile + model atomically and becomes runnable', async () => {
    ...
    expect(svc.getSystemPrompt()).toContain('Pythinker Code CLI');
  });
  ...
  it('persists the complete binding in one journal record', async () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      thinkingEffort: 'on',
      systemPrompt: expect.stringContaining('Pythinker Code CLI'),
  ...
  it('refreshes the system prompt from the session cwd after a default bind', async () => {
    const workDir = await mkdtemp(join(tmpdir(), 'pythinker-bind-work-'));
    try {
...
describe('AgentToolPolicyService tool denylist', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-deny-home-'));
  });
...
describe('AgentToolPolicyService global [tools] config', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-tools-config-home-'));
  });
...
describe('AgentToolPolicyService.setSessionDisabledTools', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-session-deny-home-'));
  });
...
describe('AgentToolPolicyService executor enforcement', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-executor-policy-home-'));
  });
...
describe('AgentProfileService tool-pattern warnings', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-tool-pattern-home-'));
  });
...
describe('agentsMdReminder seeding', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'pythinker-seed-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'pythinker-seed-work-'));
  });

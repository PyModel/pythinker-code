...
import { buildPythinkerFileUrl } from '#/agent/media/pythinkerFileUrl';
...
describe('AgentMediaResolverService video strategy', () => {
  it('uploads a pythinker-file video once and reuses the cached reference on later steps', async () => {
    ...
    const message = videoMessage(buildPythinkerFileUrl(FILE_ID));
  ...
  it.each<TagCase>([
    ...
    const out = await resolver(files, sessionDir).resolve(
      [videoMessage(buildPythinkerFileUrl(fileId))],
      req(upload),
    );
  ...
  it('rethrows an auth failure so it can drive credential refresh', async () => {
    ...
    await expect(
      res.resolve([videoMessage(buildPythinkerFileUrl(FILE_ID))], requester({ uploadVideo: upload })),
  ...
  it.each([
    {
      ...
      message: videoMessage(buildPythinkerFileUrl(FILE_ID)),
    ...
    },
    {
      ...
      message: imageMessage(buildPythinkerFileUrl(FILE_ID)),
...
describe('AgentMediaResolverService image strategy', () => {
  it('inlines a daemon-ref image as a canonical base64 data url, leaving other parts untouched', async () => {
    ...
    const message = imageMessage(buildPythinkerFileUrl(FILE_ID), tagPart, remotePart);
  ...
  it('rethrows a cancelled image read instead of degrading to a tag', async () => {
    ...
    await expect(
      res.resolve(
        [imageMessage(buildPythinkerFileUrl(FILE_ID))],
        requester({}),
        controller.signal,
      ),
  ...
  it.each([
    ...
    const message = imageMessage(buildPythinkerFileUrl(fileId));
  ...
  it('delivers a format inline only when the current model provider accepts it', async () => {
    ...
    const message = imageMessage(buildPythinkerFileUrl(FILE_ID));

    const pythinker = await resolver(files, sessionDir).resolve([message], requester({}));
    const other = await resolver(files, sessionDir).resolve(
      [message],
      requester({ providerType: 'anthropic', protocol: 'anthropic' }),
    );

    expect(firstPart(pythinker)).toEqual({
      type: 'image_url',
  ...
  it.each([
    {
      ...
      files: new Map([[FILE_ID, { name: 'pic.png', bytes: PNG_BYTES }]]),
      url: buildPythinkerFileUrl(FILE_ID),
      imageIn: false,
    ...
    },
    {
      ...
      files: new Map<string, { name: string; bytes: Buffer }>(),
      url: buildPythinkerFileUrl('missing'),
      imageIn: true,
      expected: IMAGE_UNAVAILABLE_TEXT,
    },
  ...
  it('evicts the least-recently-hit memo entry once the total byte budget is exceeded', async () => {
    ...
    for (const id of ids.slice(0, 8)) {
      await res.resolve([imageMessage(buildPythinkerFileUrl(id))], req);
    }
    await res.resolve([imageMessage(buildPythinkerFileUrl('file_0'))], req);
    await res.resolve([imageMessage(buildPythinkerFileUrl('file_8'))], req);
    expect(counting.gets).toBe(9);

    await res.resolve([imageMessage(buildPythinkerFileUrl('file_0'))], req);
    expect(counting.gets).toBe(9);
    await res.resolve([imageMessage(buildPythinkerFileUrl('file_1'))], req);
  ...
  it('keeps a legacy persisted tag as text and degrades the reference with a synthesized tag', async () => {
    ...
    const message: Message = {
      ...
      content: [
        ...
        { type: 'video_url', videoUrl: { url: buildPythinkerFileUrl(FILE_ID) } },
      ],
    };

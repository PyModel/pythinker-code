...
describe('gateImageFormatParts', () => {
  ...
  it('passes daemon file references (pythinker-file://) through untouched', () => {
    const fileId = 'f_9b2f7c1e4a2d4f3a8c1e0b6d5a493827';
    for (const url of [
      buildDaemonFileUrl(fileId),
      `pythinker-file://${fileId}?path=${encodeURIComponent('/tmp/upload/photo.heic')}`,
    ]) {
...
describe('unsupportedImageMimeFromUrl', () => {
  ...
  it('treats HEIC, HEIF, and BMP extensions as accepted for the pythinker provider only', () => {
    expect(unsupportedImageMimeFromUrl('https://example.com/photo.heic', 'pythinker')).toBeNull();
    expect(unsupportedImageMimeFromUrl('https://example.com/photo.heif', 'pythinker')).toBeNull();
    expect(unsupportedImageMimeFromUrl('https://example.com/scan.bmp', 'pythinker')).toBeNull();
    expect(unsupportedImageMimeFromUrl('https://example.com/pic.avif', 'pythinker')).toBe('image/avif');
...
describe('provider-aware image format policy', () => {
  it('accepts HEIC, HEIF, and BMP only when the provider is pythinker', () => {
    for (const mime of ['image/heic', 'image/heif', 'image/bmp']) {
      expect(isModelAcceptedImageMime(mime, 'pythinker')).toBe(true);
    ...
    for (const mime of ['image/avif', 'image/tiff', 'image/x-icon', 'image/svg+xml']) {
      expect(isModelAcceptedImageMime(mime, 'pythinker')).toBe(false);
    }
  });
  ...
  it('lets the format gate pass a HEIC data URL through for the pythinker provider', () => {
    const base64 = Buffer.from([
      0, 0, 0, 0x18, 0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63,
    ]).toString('base64');
    const url = `data:image/heic;base64,${base64}`;
    const part = { type: 'image_url' as const, imageUrl: { url } };
    expect(gateImageFormatParts([part], 'pythinker')).toEqual([part]);
  ...
  it('names the accepted formats of the current provider in the refusal notice', () => {
    expect(buildUnsupportedImageNotice('image/avif', undefined, 'pythinker')).toContain('HEIC');

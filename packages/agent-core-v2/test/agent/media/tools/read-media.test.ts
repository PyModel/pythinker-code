...
describe('AgentMediaToolsRegistrar', () => {
  ...
  it('hands the bound model provider type to ReadMediaFile', async () => {
    ...
    expect((await readWith('pythinker-vision')).isError).toBeFalsy();
  ...
  it('rebuilds ReadMediaFile when a reload changes the provider type behind the same alias', async () => {
    ...
    providerTypes['vision'] = 'pythinker';
  ...
  it('keeps session-image reads available while the workspace runtime is unavailable', async () => {
    ...
    const execution = await tool!.resolveExecution({ path: 'pythinker-file://f_picture' });
...
describe('createVideoUploader', () => {
  ...
  it('reports video_upload telemetry on success', async () => {
    ...
    const uploader = createVideoUploader(modelWith(vi.fn().mockResolvedValue(uploadResult)), {
      ...
      props: { model: 'example-model', protocol: 'pythinker' },
    });
    await expect(uploader!(input)).resolves.toEqual(uploadResult);
    expect(records).toHaveLength(1);
  ...
  function pythinkerTool(files: Record<string, FakeFile>): ReadMediaFileTool {
    return makeTool(files, capabilities(), undefined, undefined, undefined, 'pythinker');
  }

  it('sends HEIC untouched when the provider is pythinker', async () => {
    const result = await execute(pythinkerTool({ '/workspace/photo.heic': { data: heicBytes() } }), {
      path: '/workspace/photo.heic',
    });
  ...
  it('passes a HEIC above the read budget through inline up to the pythinker limit', async () => {
    const heic = Buffer.concat([heicBytes(), Buffer.alloc(4 * 1024 * 1024, 1)]);
    const result = await execute(pythinkerTool({ '/workspace/photo.heic': { data: heic } }), {
      path: '/workspace/photo.heic',
    });
  ...
  it('refuses a HEIC above the pythinker inline limit with a conversion command', async () => {
  ...
  it('still refuses formats outside the pythinker set with conversion guidance', async () => {
    const tool = pythinkerTool({ '/workspace/photo.avif': { data: ftypBytes('avif') } });

 ...
 * Run: `pnpm --filter @pymodel/agent-core-v2 exec vitest run test/agent/goal/goal.test.ts`.
 */
...
import { ErrorCodes, Error2, errorInfo, toPythinkerErrorPayload } from '#/errors';
...
// The real AgentDynamicWorkflowService self-wires executor listeners and pulls in the
// dynamic_workflow runtime; goal tests never exercise dynamic_workflow behavior, so every test
...
function createTestAgent(
  ...
  return createHarnessTestAgent(agentService(IAgentDynamicWorkflowService, stubAgentDynamicWorkflow()), ...inputs);
}
...
function endTurn(
  ...
  const error = result.error !== undefined ? toPythinkerErrorPayload(result.error) : undefined;
  eventBus.publish({
    type: 'turn.ended',
    turnId: turn.id,

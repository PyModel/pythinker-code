...
describe('AgentToolDedupeService', () => {
  ...
  describe('repeat breaker env switch', () => {
    ...
    it('injects no reminders and never force-stops when PYTHINKER_CODE_REPEAT_BREAKER is 0', async () => {
    ...
    it('keeps same-step dedupe active when PYTHINKER_CODE_REPEAT_BREAKER is 0', async () => {
      const h = disabledHarness();
      const tool = new EchoTool('Read');
      h.registry.register(tool);
      const results = await runStep(h, 1, 1, [

...
describe('supportedProtocols (probe 4)', () => {
  it('is derived from the registered bases and contains neither pythinker nor vertexai', () => {
...
describe('apiKey env suppression (probe 1)', () => {
  it('does not pick up OPENAI_API_KEY when composing pythinker without any key', async () => {
  ...
  it('uses the PYTHINKER_API_KEY env fallback when composing pythinker', () => {
    process.env['PYTHINKER_API_KEY'] = 'sk-pythinker-from-env';
    ...
    expect(apiKey).toBe('sk-pythinker-from-env');
  });
...
describe('resolveProviderBaseId', () => {
  it('returns the pair registration’s baseProtocol — the protocol itself by construction', () => {
    expect(registry.resolveProviderBaseId('openai', 'pythinker')).toBe('openai');
    expect(registry.resolveProviderBaseId('anthropic', 'pythinker')).toBe('anthropic');
  });

  it('returns the protocol itself otherwise', () => {
    expect(registry.resolveProviderBaseId('google-genai', 'pythinker')).toBe('google-genai');
...
describe('resolveCapability', () => {
  ...
  it('pythinker declares no vendor-level capability — the base catalog answers instead', () => {
    ...
    expect(registry.resolveCapability('openai', 'gpt-4o', 'pythinker').image_in).toBe(true);
  });
});
...
describe('createChatProvider', () => {
  it('composes pythinker as the openai base with the upload capability bound', () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'pythinker',
      modelName: 'kimi-k2',
...
describe('resolveProviderEndpoint', () => {
  it('resolves the pythinker endpoint chain from process.env', () => {
    process.env['PYTHINKER_API_KEY'] = 'sk-pythinker-env';
    expect(resolveProviderEndpoint('pythinker')).toEqual({
      apiKey: 'sk-pythinker-env',
  ...
  it('reads a caller-supplied env bag instead of process.env', () => {
    process.env['PYTHINKER_API_KEY'] = 'sk-pythinker-env';
    expect(resolveProviderEndpoint('pythinker', { PYTHINKER_BASE_URL: 'https://example.com/v1' })).toEqual({
...
describe('pythinker provider definitions', () => {
  ...
  it('allows the same id on several protocols but rejects a duplicate (id, baseProtocol) pair', () => {
    ...
    expect(() =>
      registerProviderDefinition({ id: 'pythinker', baseProtocol: 'openai', traits: [] }),
    ).toThrow(/already registered/);
  });
});
...
describe('per-turn intent wire encoding (behavior probes)', () => {
  it('encodes cacheKey + thinking + budget on the Pythinker wire as prompt_cache_key + expanded thinking, never reasoning_effort', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'pythinker',
      modelName: 'kimi-k2',
  ...
  it('encodes thinking for Pythinker over the Anthropic transport through the pair trait only', async () => {
    const provider = registry.createChatProvider({
      protocol: 'anthropic',
      providerType: 'pythinker',
...
describe('quota-exhausted classification through the real composition (behavior probes)', () => {
  ...
  it('fails fast on a PyModel quota 429 over the (pythinker, anthropic) composition', async () => {
    const provider = registry.createChatProvider({
      protocol: 'anthropic',
      providerType: 'pythinker',
      modelName: 'kimi-for-coding',
...
describe('reasoning dialect (behavior probes)', () => {
  ...
  it('pythinker composition defaults to reasoning_content before any detection', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'pythinker',
      modelName: 'kimi-k2',
...
describe('responseFormat wire encoding (per base)', () => {
  ...
  it('keeps response_format intact through the Pythinker buildParams pipeline', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'pythinker',
      modelName: 'kimi-k2',

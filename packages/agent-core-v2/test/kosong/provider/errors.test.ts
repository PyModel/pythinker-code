 ...
 * `insufficient_quota` code fails fast there); PyModel's quota signals are
 * declared on the Pythinker traits via the `convertError` hook, composed with
...
import { pythinkerAnthropicTrait, pythinkerOpenAITrait } from '#/kosong/provider/providers/pythinker/pythinker.contrib';
import { classifyPythinkerQuotaError } from '#/kosong/provider/providers/pythinker/pythinker-errors';
...
function pymodelQuota429(message: string, type?: string): OpenAIAPIError {
...
describe('classifyPythinkerQuotaError (Pythinker trait classifier)', () => {
  it('classifies a structured exceeded_current_quota_error body as quota-exhausted', () => {
    const error = classifyPythinkerQuotaError(
      pymodelQuota429('Too many requests', 'exceeded_current_quota_error'),
    );
  ...
  it.each([QUOTA_MESSAGE, TOKEN_QUOTA_MESSAGE])(
    ...
    (message) => {
      const error = classifyPythinkerQuotaError(pymodelQuota429(message));
      expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
      expect(isRetryableGenerateError(error)).toBe(false);
    },
  );
  ...
  it.each(['Too many requests', 'your token quota per minute was exceeded'])(
    ...
    (message) => {
      expect(classifyPythinkerQuotaError(pymodelQuota429(message))).toBeUndefined();
    },
  );

  it('answers undefined for non-429 and non-SDK shapes', () => {
    expect(
      classifyPythinkerQuotaError(new OpenAIAPIError(403, undefined, QUOTA_MESSAGE, new Headers())),
    ).toBeUndefined();
    expect(classifyPythinkerQuotaError(new Error(QUOTA_MESSAGE))).toBeUndefined();
    expect(classifyPythinkerQuotaError(undefined)).toBeUndefined();
  });
  ...
  it('classifies the Anthropic SDK error shape (body nested under .error)', () => {
    ...
    const error = classifyPythinkerQuotaError(source);
    expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
    expect(isRetryableGenerateError(error)).toBe(false);
  });

  it('is declared as the convertError hook on both Pythinker traits', () => {
    const context: TraitContext = {
      config: { protocol: 'openai', providerType: 'pythinker', modelName: '' } as ProtocolAdapterConfig,
      providerId: 'pythinker',
    };
    const source = pymodelQuota429('Too many requests', 'exceeded_current_quota_error');
    expect(pythinkerOpenAITrait.convertError!(source, context)).toBeInstanceOf(
      APIProviderQuotaExhaustedError,
    );
    expect(pythinkerAnthropicTrait.convertError!(source, context)).toBeInstanceOf(
      APIProviderQuotaExhaustedError,
    );
  });
});
...
describe('convertError hook consult at the OpenAI boundary', () => {
  const pythinkerContext: TraitContext = {
    config: { protocol: 'openai', providerType: 'pythinker', modelName: '' } as ProtocolAdapterConfig,
    providerId: 'pythinker',
  };

  it('prefers the composed hook over the vendor-neutral base classification', () => {
    const hooks = composeOpenAIChatHooks([{ trait: pythinkerOpenAITrait, context: pythinkerContext }]);
    const source = pymodelQuota429(QUOTA_MESSAGE);
  ...
  it('binds convertError with last-declarer-wins semantics', () => {
    const bound = traitConvertError([
      { trait: { convertError: () => new ChatProviderError('first') }, context: pythinkerContext },
      { trait: { convertError: () => new ChatProviderError('second') }, context: pythinkerContext },
    ]);
...
describe('OpenAI base quota classification (vendor-neutral)', () => {
  ...
  it('keeps vendor billing wordings a retryable rate limit without a hook', () => {
    const error = convertOpenAIError(pymodelQuota429(QUOTA_MESSAGE));

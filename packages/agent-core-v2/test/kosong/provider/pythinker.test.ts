...
import { PythinkerFiles } from '#/kosong/provider/providers/pythinker/pythinker-files';
import {
  convertPythinkerTool,
...
const context: TraitContext = {
  config: { protocol: 'openai', providerType: 'pythinker', modelName: 'kimi-k2' },
  providerId: 'pythinker',
};
...
describe('pythinkerOpenAITrait.convertTool', () => {
  ...
  it('normalizes the schema dialect of regular tools', () => {
    ...
    expect(convertPythinkerTool(tool)).toEqual({
      type: 'function',
      function: {
        name: 'read_file',
        description: 'read',
...
describe('pythinkerOpenAITrait.convertMessage', () => {
  ...
  it('deletes effectively-empty content on assistant tool messages', () => {
    ...
    const out = call(pythinkerOpenAITrait.convertMessage, assistantToolMessage, converted, context);
  ...
  it('round-trips tool_calls extras by index', () => {
    ...
    const out = call(
      pythinkerOpenAITrait.convertMessage,
  ...
  it('keeps non-empty content untouched', () => {
    ...
    const out = call(pythinkerOpenAITrait.convertMessage, message, converted, context);
...
describe('pythinkerOpenAITrait reasoning hooks', () => {
  ...
  it('force-replays reasoning only in keep:all sessions with thinking enabled', () => {
    ...
    expect(call(pythinkerOpenAITrait.preserveThinking, keepAll, context)).toBe(true);

    const keepAllDisabled = {
      extra_body: { thinking: { type: 'disabled', keep: 'all' } },
    };
    expect(call(pythinkerOpenAITrait.preserveThinking, keepAllDisabled, context)).toBeUndefined();

    const keepSome = {
      extra_body: { thinking: { type: 'enabled', keep: 'some' } },
    };
    expect(call(pythinkerOpenAITrait.preserveThinking, keepSome, context)).toBeUndefined();
    expect(call(pythinkerOpenAITrait.preserveThinking, {}, context)).toBeUndefined();
  });
});

describe('pythinkerOpenAITrait.extractUsage', () => {
  it('finds usage at the top level', () => {
    const usage = { prompt_tokens: 10, completion_tokens: 2 };
    expect(call(pythinkerOpenAITrait.extractUsage, { usage }, context)).toBe(usage);
  });

  it('finds usage inside choices[0].usage', () => {
    const usage = { prompt_tokens: 5, completion_tokens: 1 };
    expect(call(pythinkerOpenAITrait.extractUsage, { choices: [{ usage }] }, context)).toBe(usage);
  });

  it('defers to the base default when the chunk carries no usage', () => {
    expect(call(pythinkerOpenAITrait.extractUsage, { choices: [] }, context)).toBeUndefined();
    expect(call(pythinkerOpenAITrait.extractUsage, { choices: [{}] }, context)).toBeUndefined();
  });
});

describe('pythinkerOpenAITrait request params', () => {
  it('declares the PYTHINKER_API_KEY / PYTHINKER_BASE_URL fallback chain and default base URL', () => {
    expect(call(pythinkerOpenAITrait.endpoint, context)).toEqual({
  ...
  it('encodes the cache key as prompt_cache_key', () => {
    expect(call(pythinkerOpenAITrait.cacheKey, 'session-1', context)).toEqual({
      prompt_cache_key: 'session-1',
    });
  });

  it('encodes thinking into extra_body.thinking, carrying keep', () => {
    expect(call(pythinkerOpenAITrait.withThinking, 'high', {}, {}, context)).toEqual({
      extra_body: { thinking: { type: 'enabled', effort: 'high' } },
    });
    expect(call(pythinkerOpenAITrait.withThinking, 'on', {}, {}, context)).toEqual({
      extra_body: { thinking: { type: 'enabled' } },
    });
    expect(call(pythinkerOpenAITrait.withThinking, 'off', {}, {}, context)).toEqual({
      extra_body: { thinking: { type: 'disabled' } },
    });
    expect(call(pythinkerOpenAITrait.withThinking, 'high', { keep: 'all' }, {}, context)).toEqual({
      extra_body: { thinking: { type: 'enabled', effort: 'high', keep: 'all' } },
    });
  });

  it('applies no 128k ceiling in withMaxCompletionTokens', () => {
    expect(call(pythinkerOpenAITrait.withMaxCompletionTokens, 200_000, context)).toEqual({
  ...
  it('buildParams backfills max_completion_tokens, drops max_tokens, expands extra_body last', () => {
    const out = call(
      pythinkerOpenAITrait.buildParams,
      {
        model: 'kimi-k2',
        max_tokens: 4096,
...
describe('pythinkerAnthropicTrait (the (pythinker, anthropic) registration)', () => {
  ...
  it('encodes thinking:{type:enabled} + output_config.effort and strips the interleaved beta', () => {
    const out = call(pythinkerAnthropicTrait.withThinking, 'high', {}, seeded, context);
    expect(out).toEqual({
  ...
  it('omits output_config for on', () => {
    expect(call(pythinkerAnthropicTrait.withThinking, 'on', {}, seeded, context)).toEqual({
      thinking: { type: 'enabled' },
  ...
  it('encodes off as disabled', () => {
    expect(call(pythinkerAnthropicTrait.withThinking, 'off', {}, seeded, context)).toEqual({
...
describe('trait objects are plain declarations', () => {
  it('exposes exactly the hooks appendix A assigns to them, plus metadata markers', () => {
    ...
    expect(hookNames(pythinkerOpenAITrait).toSorted()).toEqual([
      'buildParams',
      'cacheKey',
      'convertError',
      'convertMessage',
    ...
    expect(hookNames(pythinkerAnthropicTrait).toSorted()).toEqual(['convertError', 'withThinking']);
  });
...
describe('PythinkerFiles upload error conversion', () => {
  it('fails fast on a PyModel quota-exhausted 429 from the files API', async () => {
    ...
    const files = new PythinkerFiles({

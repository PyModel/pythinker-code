...
const pythinkerSections: Record<string, unknown> = {
  providers: {
    pythinker: { type: 'pythinker', apiKey: 'sk-test', baseUrl: 'https://api.moonshot.ai/v1' },
  },
  models: {
...
beforeEach(() => {
  savedCustomHeaders = process.env['PYTHINKER_CODE_CUSTOM_HEADERS'];
  delete process.env['PYTHINKER_CODE_CUSTOM_HEADERS'];
});

afterEach(() => {
  if (savedCustomHeaders === undefined) delete process.env['PYTHINKER_CODE_CUSTOM_HEADERS'];
  else process.env['PYTHINKER_CODE_CUSTOM_HEADERS'] = savedCustomHeaders;
});

describe('Model assembly (pure data)', () => {
  it('assembles a pythinker model: protocol resolves to the vendor base, never a vendor', () => {
    const { host, catalog } = createHost(pythinkerSections);
    try {
      ...
      expect(model.providerName).toBe('pythinker');
  ...
  it('forwards only the User-Agent to vendors without a full hostHeaders declaration', () => {
    ...
    try {
      ...
      expect(model.headers).toEqual({ 'User-Agent': 'pythinker-test/1.0' });
    } finally {
      host.dispose();
    }
  });
  ...
  describe('custom identity', () => {
    ...
    const OFFICIAL = {
      providers: { pythinker: { type: 'pythinker', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' } },
      models: { k2: { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 200000 } },
    };
    ...
    it('preserves a parenthesized User-Agent suffix while rewriting', () => {
      const { host, catalog } = createHost(THIRD_PARTY, stubModelOAuthTokens(), {
        headers: { 'User-Agent': 'pythinker-test/1.0 (web)' },
        identitySlug: 'acme-dev',
      });
      try {
    ...
    it('changes nothing when no identity is configured', () => {
      ...
      try {
        expect(catalog.get('gpt').headers).toEqual({ 'User-Agent': 'pythinker-test/1.0' });
      } finally {
        host.dispose();
      }
    });
  ...
  it('keeps an explicit foreign protocol for a pythinker model (the dialect path)', () => {
    const { host, catalog } = createHost({
      providers: { pythinker: { type: 'pythinker', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' } },
      models: {
        k2: { provider: 'pythinker', protocol: 'anthropic', model: 'kimi-k2', maxContextSize: 200000 },
      },
    });
    try {
  ...
  it('resolves provider env-bag credentials and endpoints through the registry', () => {
    const { host, catalog } = createHost({
      providers: {
        pythinker: { type: 'pythinker', env: { PYTHINKER_API_KEY: 'env-token', PYTHINKER_BASE_URL: 'https://pythinker-env.example.test/v1' } },
        openai: { type: 'openai', env: { OPENAI_API_KEY: 'sk-openai' } },
      },
      models: {
        k1: { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 1000 },
        gpt: { provider: 'openai', protocol: 'openai', model: 'gpt-5', maxContextSize: 1000 },
      },
    });
    try {
      const pythinker = catalog.get('k1');
      expect(pythinker.baseUrl).toBe('https://pythinker-env.example.test/v1');
      return expect(pythinker.authProvider.getAuth()).resolves.toEqual({ apiKey: 'env-token' });
    } finally {
      host.dispose();
    }
  });
  ...
  it('falls back to defaultProvider when a model names no provider', () => {
    ...
    try {
      expect(catalog.get('inherited').providerName).toBe('pythinker');
    } finally {
      host.dispose();
    }
  });
  ...
  it('findByName matches name, model, and aliases', () => {
    const { host, catalog } = createHost({
      ...pythinkerSections,
      models: {
        k1: { provider: 'pythinker', model: 'kimi-k2', aliases: ['k2-latest'], maxContextSize: 1 },
        k2: { provider: 'pythinker', name: 'shared-name', maxContextSize: 1 },
        k3: { provider: 'pythinker', model: 'shared-name', maxContextSize: 1 },
      },
    });
    try {
  ...
  it('builds a refreshable OAuth auth provider for oauth-backed models', async () => {
    ...
    const { host, catalog } = createHost(
      {
        providers: {
          pythinker: { type: 'pythinker', oauth: { storage: 'file', key: 'pythinker' }, baseUrl: 'https://api.moonshot.ai/v1' },
        },
        models: { k1: { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 1 } },
      },
...
describe('ModelCatalog caching and config-event invalidation', () => {
  ...
  it('drops the cache when a watched config section changes', async () => {
    const { host, catalog, models, providers } = createHost(pythinkerSections);
    try {
      const before = catalog.get('k1');
      await models.set('k1', { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 262144, displayName: 'K2' });
      const after = catalog.get('k1');
      expect(after).not.toBe(before);
      expect(after.displayName).toBe('K2');

      await providers.set('pythinker', { type: 'pythinker', apiKey: 'sk-2', baseUrl: 'https://other.example.test/v1' });
  ...
  it('keeps serving the stale Model on a silent registry write until notifyConfigChanged()', async () => {
    const { host, catalog, models } = createHost(pythinkerSections);
    try {
      const before = catalog.get('k1');
...
describe('ModelCatalog inspect', () => {
  it('builds the god object with per-field provenance (pythinker structured model)', () => {
    const { host, catalog } = createHost(pythinkerSections);
    try {
      const view = catalog.inspect('k1');
      expect(view.id).toBe('k1');
      expect(view.provider).toMatchObject({ id: 'pythinker', synthesized: false });
  ...
  it('attributes override fields to the overrides block', () => {
    const { host, catalog } = createHost({
      providers: {
        pythinker: { type: 'pythinker', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' },
      },
      models: {
        m: {
          provider: 'pythinker',
  ...
  it('attributes the input cap to config, its clamp, and its absence', () => {
    const { host, catalog } = createHost({
      ...
      models: {
        ...
        plain: { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 100 },
      },
    });
    try {
  ...
  it('attributes env-bag credentials and endpoints by env-var name', () => {
    const { host, catalog } = createHost({
      providers: {
        pythinker: {
          type: 'pythinker',
          env: { PYTHINKER_API_KEY: 'env-token', PYTHINKER_BASE_URL: 'https://pythinker-env.example.test/v1' },
        },
      },
      models: { k1: { provider: 'pythinker', model: 'kimi-k2', maxContextSize: 1000 } },
    });
    try {
      const view = catalog.inspect('k1');
      expect(view.resolved.baseUrl).toBe('https://pythinker-env.example.test/v1');
      expect(view.resolved.auth.kind).toBe('apiKey');
      expect(view.sources['resolved.auth']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('PYTHINKER_API_KEY'),
      });
      expect(view.sources['resolved.baseUrl']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('PYTHINKER_BASE_URL'),
      });
...
describe('ModelCatalog ping', () => {
  it('returns the streamed text and usage on a live success', async () => {
    const { host, models, providers } = createHost(pythinkerSections, stubModelOAuthTokens());
    try {
...
const catalogSections: Record<string, unknown> = {
  providers: {
    pythinker: { type: 'pythinker', apiKey: 'sk-test', baseUrl: 'https://api.example.test/v1' },
    openai: { type: 'openai' },
  },
  models: {
    k2: {
    ...
    turbo: { provider: 'pythinker', model: 'pythinker-turbo', maxContextSize: 32768, displayName: 'Pythinker Turbo' },
...
describe('wire projection (pure)', () => {
  it('toProtocolModel projects the materialized Model into the snake_case wire shape', () => {
    ...
    try {
      ...
      expect(toProtocolModel(catalog.get('k2'), record, 'pythinker')).toEqual({
        provider: 'pythinker',
        model: 'k2',
  ...
  it('toProtocolModelFallback projects the raw record', () => {
    ...
    expect(toProtocolModelFallback('k2', record, 'pythinker')).toEqual({
      provider: 'pythinker',
      model: 'k2',
      display_name: 'Kimi K2',
      max_context_size: 131072,
...
describe('ModelCatalog enumeration', () => {
  it('lists configured models as selectable aliases', async () => {
    ...
    try {
      await expect(catalog.listModels()).resolves.toEqual([
        ...
        { provider: 'pythinker', model: 'turbo', display_name: 'Pythinker Turbo', max_context_size: 32768 },
  ...
  it('does not project fallback efforts for unknown Pythinker-managed Anthropic models', async () => {
    const sections = structuredClone(catalogSections);
    (sections['models'] as Record<string, ModelRecord>)['compatible'] = {
      provider: 'pythinker',
      protocol: 'anthropic',
    ...
    try {
      const compatible = (await catalog.listModels()).find((model) => model.model === 'compatible');
      expect(compatible).toMatchObject({ provider: 'pythinker', model: 'compatible' });
      expect(compatible?.capabilities).toBeUndefined();
      expect(compatible?.support_efforts).toBeUndefined();
      expect(compatible?.default_effort).toBeUndefined();
    } finally {
  ...
  it('detects env-bag credentials through the vendor endpoint declarations', async () => {
    const { host, catalog } = createHost({
      providers: {
        pythinker: { type: 'pythinker', env: { PYTHINKER_API_KEY: 'pythinker-env-key' } },
    ...
    try {
      ...
      expect(byId['pythinker']).toMatchObject({ has_api_key: true, status: 'connected' });
  ...
  it('marks an OAuth provider connected when a cached token exists', async () => {
    const { host, catalog } = createHost(
      {
        providers: { acme: { type: 'pythinker', oauth: { storage: 'file', key: 'oauth/acme' } } },
        models: {},
      },
  ...
  it('gets a single provider by id and reports provider.not_found for an unknown one', async () => {
    const { host, catalog } = createHost(catalogSections);
    try {
      await expect(catalog.getProvider('pythinker')).resolves.toMatchObject({
        id: 'pythinker',
        default_model: 'k2',
        models: ['k2', 'turbo'],
      });

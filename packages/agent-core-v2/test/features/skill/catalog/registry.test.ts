...
import type { SkillDefinition, SkillSource } from '#/features/skill/catalog/types';
...
describe('InMemorySkillCatalog skill listing', () => {
  ...
  it('omits source headings that have no skills', () => {
    const rendered = makeRegistry([makeSkill('alpha', 'user')]).getPythinkerSkillsDescription();
  ...
  it('renders a No skills placeholder for an empty registry', () => {
    const rendered = new InMemorySkillCatalog().getPythinkerSkillsDescription();
  ...
  it('registerBuiltinSkill stamps non-builtin skills as builtin', () => {
    ...
    expect(sectionFor(registry.getPythinkerSkillsDescription(), '### Built-in')).toContain('theme');
  });
});
...
describe('InMemorySkillCatalog prompt rendering', () => {
  ...
  it('expands context placeholders and still appends args when no argument placeholder is used', () => {
    const rendered = new InMemorySkillCatalog().renderSkillPrompt(
      stubSkill('review', {
        dir: '/skills/review',
        content: 'Use ${PYTHINKER_SKILL_DIR}/references/checklist.md.',
      }),
      'src/app.ts',
      { sessionId: 'ses_1' },
    );

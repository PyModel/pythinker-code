...
} from '#/features/dynamic_workflow/session/agentRunBatch';
import { ISessionDynamicWorkflowService, type SessionDynamicWorkflowSpawnTask, type SessionDynamicWorkflowTask } from '#/features/dynamic_workflow/session/sessionDynamicWorkflow';
...
describe('SessionDynamicWorkflowService metadata compatibility', () => {
  ...
  it('forwards caller ownership and dynamic_workflow item labels to the subagent spawn', async () => {
    const service = ix.get(ISessionDynamicWorkflowService);
...
function subagentStub(
  ...
  return {
    ...
    planSpawn: vi.fn(async (input: SubagentSpawnPlanInput) => ({
      ...
      model: input.model ?? 'pythinker-test',

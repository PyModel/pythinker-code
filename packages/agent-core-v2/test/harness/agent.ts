...
import { ISessionDynamicWorkflowService } from '#/session/dynamic_workflow/sessionDynamicWorkflow';
...
export interface TestAgentOptions {
  ...
  | undefined;
  readonly initialConfig?: Partial<PythinkerConfig> | undefined;
...
export function configServices(readConfig: () => PythinkerConfig): TestAgentServiceOverride {
...
class ConfigBackedModelCatalog extends ModelCatalog {
  ...
  /**
   * The harness mutates `pythinkerConfig` BEHIND the config services' backs (no
...
export class AgentTestContext {
  ...
  constructor(overrides: readonly TestAgentServiceOverride[] = [], options: TestAgentOptions = {}) {
    ...
    this.pythinkerConfig = applyTestAgentOptionsToConfig(emptyConfig(), options);
    ...
    const appSeeds = collectScopeSeed(
      [
        (reg) => {
          for (const [id, value] of bootstrapSeed({
            homeDir: '/tmp/pythinker-code-agent-app-v2-test',
            cwd: this.cwd,
          ...
          reg.defineInstance(
            IConfigService,
            configService(() => this.pythinkerConfig),
          );
          ...
          reg.defineInstance(
            ...
            {
              level: 'off',
              globalLogPath: '/tmp/pythinker-code-agent-app-v2-test/logs/pythinker-code.log',
    ...
    // that (the harness mutates pythinkerConfig behind the config events' backs).
  ...
  private initializeRestorableServices(): void {
    ...
    const dynamic_workflow = this.get(IAgentDynamicWorkflowService);

    context.get();
    void dynamic_workflow.isActive;
    tokenCounting.get();
    usage.status();
    tasks.list(false);
    permission.data();
  ...
  configureRuntimeModel(
    ...
    this.pythinkerConfig = configWithProvider(this.pythinkerConfig, provider, modelCapabilities);
  ...
  /**
   * The manual cache-drop for tests that mutate `pythinkerConfig` behind the
  ...
  async expectResumeMatches(): Promise<void> {
    ...
    await this.drainWirePersistence();
    const configSnapshot = structuredClone(this.get(IConfigService).getAll() as PythinkerConfig);
  ...
  private createRpcPassthroughAdapters(): AgentRpcPassthroughAPI {
    return {
      ...
      enterDynamicWorkflow: (payload) => this.get(IAgentDynamicWorkflowService).enter(payload.trigger),
      exitDynamicWorkflow: () => this.get(IAgentDynamicWorkflowService).exit(),
      getDynamicWorkflowMode: () => this.get(IAgentDynamicWorkflowService).isActive,
...
function applyTestAgentOptionsToConfig(config: PythinkerConfig, options: TestAgentOptions): PythinkerConfig {
...
function configService(readConfig: () => PythinkerConfig): IConfigService {
  ...
  const effectiveConfig = () => {
    ...
    return effective as unknown as PythinkerConfig;
  };
...
function configWithEnvOverrides(config: PythinkerConfig): PythinkerConfig {
  const maxCompletionTokens =
    parseEnvCompletionTokens(process.env['PYTHINKER_MODEL_MAX_COMPLETION_TOKENS']) ??
    parseEnvCompletionTokens(process.env['PYTHINKER_MODEL_MAX_TOKENS']);
  const temperature = parseEnvFloat(process.env['PYTHINKER_MODEL_TEMPERATURE']);
  const topP = parseEnvFloat(process.env['PYTHINKER_MODEL_TOP_P']);
  const forcedEffort = process.env['PYTHINKER_MODEL_THINKING_EFFORT']?.trim();
  const thinkingKeep = process.env['PYTHINKER_MODEL_THINKING_KEEP']?.trim();
...
function cronEnvOverrides(base: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  setBoolean('debug', 'PYTHINKER_CRON_DEBUG');
  setBoolean('noJitter', 'PYTHINKER_CRON_NO_JITTER');
  setBoolean('noStale', 'PYTHINKER_CRON_NO_STALE');
  setBoolean('disabled', 'PYTHINKER_DISABLE_CRON');
  setBoolean('manualTick', 'PYTHINKER_CRON_MANUAL_TICK');
  const pollIntervalMs = parseEnvCronPollIntervalMs(process.env['PYTHINKER_CRON_POLL_INTERVAL_MS']);
  if (pollIntervalMs !== undefined) {
    next['pollIntervalMs'] = pollIntervalMs;
    changed = true;
  }
  if (process.env['PYTHINKER_CRON_CLOCK'] !== undefined) {
    next['clock'] = process.env['PYTHINKER_CRON_CLOCK'];
    changed = true;
  }
...
function providerConfigForAlias(provider: TestProviderConfig): PythinkerConfig['providers'][string] {
  return {
    type: provider.type,
 ...
 * the real `ProtocolAdapterRegistry` (so vendor verdicts like Pythinker thinking
 ...
 * only `pythinker`) `createChatProvider` composes the REAL provider through the
 ...
 * (`'openai'`, never `'pythinker'`), trait-bound capabilities (`uploadVideo`),

...
describe('supportedProtocols', () => {
  it('lists the four wire protocols and contains neither pythinker nor vertexai', () => {
...
describe('resolveAdapterIdentity', () => {
  it('resolves the pythinker pair registrations to their vendor traits', () => {
    expect(registry.resolveAdapterIdentity('openai', 'pythinker').baseId).toBe('openai');
    expect(registry.resolveAdapterIdentity('openai', 'pythinker').trait).toBeDefined();
    expect(registry.resolveAdapterIdentity('anthropic', 'pythinker').baseId).toBe('anthropic');
    expect(registry.resolveAdapterIdentity('anthropic', 'pythinker').trait).toBeDefined();
  ...
  it('resolves unregistered pairs to the protocol itself with no vendor trait', () => {
    const google = registry.resolveAdapterIdentity('google-genai', 'pythinker');
...
describe('resolve gateway routes', () => {
  it('routes pythinker+openai to the pythinker trait with the video upload media', () => {
    const resolved = registry.resolve(modelWith({ protocol: 'openai', providerType: 'pythinker' }));
  ...
  it('routes pythinker+anthropic through the pythinker anthropic trait with media', () => {
    const resolved = registry.resolve(modelWith({ protocol: 'anthropic', providerType: 'pythinker' }));

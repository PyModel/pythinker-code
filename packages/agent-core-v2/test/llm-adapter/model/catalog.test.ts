...
describe('Model assembly (pure data)', () => {
  ...
  it('keeps an explicit foreign protocol for a pythinker model (the trait path)', () => {
    const { host, catalog } = createHost({
      providers: { pythinker: { type: 'pythinker', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' } },
      models: {
  ...
  it('resolves provider env-bag credentials and endpoints through the registry', async () => {
    ...
    try {
      ...
      return expect(await pythinker.credentialProvider?.resolve()).toEqual({ apiKey: 'env-token' });
    } finally {
      host.dispose();
    }
  });

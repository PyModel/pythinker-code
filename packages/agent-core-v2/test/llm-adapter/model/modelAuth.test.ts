...
describe('resolveModelForReady', () => {
  const providers: Readonly<Record<string, ProviderConfig>> = {
    ...
    '__pythinker_env__': { type: 'pythinker', baseUrl: 'https://api.example.test/coding/v1' },
  };
  ...
  it('resolves the env-overlay injected model against the env provider', () => {
    const models = {
      '__pythinker_env_model__': {
        provider: '__pythinker_env__',
        model: 'kimi-for-coding',
        maxContextSize: 262144,
      },
    };
    expect(resolveModelForReady('__pythinker_env_model__', models, providers)).toEqual({
      resolved: true,
    });
  });

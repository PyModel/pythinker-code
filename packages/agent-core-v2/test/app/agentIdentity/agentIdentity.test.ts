...
describe('AgentIdentityService freeze', () => {
  ...
  it('ignores a config edit made after the freeze', async () => {
    const { identity, config } = createIdentity(
      { name: 'Acme' },
      { hostRequestHeaders: { 'User-Agent': 'pythinker-code-cli/1.0' } },
    );
...
describe('buildAgentIdentitySnapshot products', () => {
  const HOST = { 'User-Agent': 'pythinker-code-cli/1.2.3 (darwin)', 'X-Msh-Device-Id': 'device-1' };
  ...
  it.each(['user-agent', 'USER-AGENT'])(
    ...
    (key) => {
      const snapshot = buildAgentIdentitySnapshot({
        slug: 'acme',
        hostRequestHeaders: { [key]: 'pythinker-code-cli/1.2.3', 'X-Msh-Device-Id': 'device-1' },
      });
  ...
  it('passes a lowercase spelling through untouched when no identity is claimed', () => {
    const snapshot = buildAgentIdentitySnapshot({
      hostRequestHeaders: { 'user-agent': 'pythinker-code-cli/1.2.3' },
    });
    expect(snapshot.thirdPartyUserAgent).toBe('pythinker-code-cli/1.2.3');
    expect(snapshot.requestHeaders).toEqual({ 'user-agent': 'pythinker-code-cli/1.2.3' });
  });
});

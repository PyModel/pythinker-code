...
describe('BootstrapService (scoped)', () => {
  ...
  it('resolves homeDir/configPath from the seeded context token', () => {
    const host = createScopedTestHost(
      bootstrapSeed({ homeDir: '/tmp/pythinker-home', clientIdentity: stubClientIdentity }),
    );
    const svc = host.app.accessor.get(IBootstrapService);
    expect(svc.homeDir).toBe('/tmp/pythinker-home');
    expect(svc.configPath).toBe('/tmp/pythinker-home/config.toml');
...
describe('resolveBootstrapOptions', () => {
  it('prefers explicit homeDir over PYTHINKER_CODE_HOME over osHomeDir', () => {
    expect(
...
describe('bootstrap() storage seeding', () => {
  it('seeds IFileSystemStorageService as a FileStorageService instance', () => {
    const { app } = bootstrap({ homeDir: '/tmp/pythinker-home', clientIdentity: stubClientIdentity });
    try {

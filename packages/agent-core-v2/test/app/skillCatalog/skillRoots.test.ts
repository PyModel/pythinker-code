...
describe('skillRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .pythinker-code/skills directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.pythinker-code/skills/commit'), { recursive: true });

      const roots = await projectRoots(root);

      expect(roots.some((r) => r.path.endsWith('.pythinker-code/skills') && r.source === 'project')).toBe(
        true,
      );
    });
    ...
    it('falls back to the generic .agents/skills directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.pythinker-code/skills'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.pythinker-code/skills'))).toBe(true);
    });

    it('orders the brand directory before the generic directory', async () => {
      await markGitRoot();
      await mkdir(join(root, '.pythinker-code/skills'), { recursive: true });
      await mkdir(join(root, '.agents/skills'), { recursive: true });

      const roots = await projectRoots(root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.pythinker-code/skills'));

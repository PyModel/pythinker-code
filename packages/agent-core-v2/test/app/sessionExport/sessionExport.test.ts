...
describe('sessionExport', () => {
  ...
  it('exports a v2 session directory with per-agent wire activity and optional global log', async () => {
    ...
    await writeFile(join(sessionDir, 'logs', 'pythinker-code.log'), '{"msg":"session"}\n', 'utf-8');
    await writeFile(
    ...
    const globalLogPath = join(tmp, 'logs', 'pythinker-code.log');
    ...
    expect(result.entries).toEqual([
      ...
      'logs/pythinker-code.log',
      'state.json',
      'logs/global/pythinker-code.log',
    ]);
    expect(result.manifest).toMatchObject({
      ...
      sessionLogPath: 'logs/pythinker-code.log',
      globalLogPath: 'logs/global/pythinker-code.log',
    });
  });
  ...
  it('does not treat an ancestor directory named notify as session notify state', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'notify-ancestor-'));
    const sessionDir = join(tmp, 'notify', 'sessions', 'ws_demo', 'ses_nested');
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'state.json'), '{}\n', 'utf-8');

    const result = await exportSessionDirectory({
      request: { sessionId: 'ses_nested', outputPath: join(tmp, 'export.zip'), version: '1.0.0-test' },
      summary: { id: 'ses_nested', sessionDir },
    });
  ...
  it('uses a timestamped default output path when outputPath is omitted', async () => {
    ...
    try {
      ...
      expect(basename(result.zipPath)).toMatch(/^pythinker-debug-ses_defa-\d{8}-\d{6}\.zip$/);
  ...
  it('keeps the session log bound when it rotates as wire scanning starts', async () => {
    ...
    try {
      ...
      expect(result.entries).toContain('logs/pythinker-code.log');
      await expect(readZipEntry(outputPath, 'logs/pythinker-code.log')).resolves.toEqual(log);
    } finally {
  ...
  it('keeps the global log bound when it rotates as wire scanning starts', async () => {
    ...
    try {
      ...
      expect(result.entries).toContain('logs/global/pythinker-code.log');
      await expect(readZipEntry(outputPath, 'logs/global/pythinker-code.log')).resolves.toEqual(log);
    } finally {
      fsOpenHook.afterOpen = undefined;
    }
  });
  ...
  it('closes pre-opened logs when manifest creation fails before writer ownership', async () => {
    ...
    const sessionLogPath = join(sessionDir, 'logs', 'pythinker-code.log');
    const globalLogPath = join(tmp, 'logs', 'pythinker-code.log');
    const wirePath = join(sessionDir, 'agents', 'main', 'wire.jsonl');
    ...
    expect((await readdir(tmp)).filter((entry) => entry.startsWith('.pythinker-session-export-'))).toEqual(
      [],
    );
  });
  ...
  it('closes a pre-opened source when it conflicts with the output path', async () => {
    ...
    await expect(
      writeExportZip({
        ...
        sessionDir: tmp,
        sessionFiles: [],
        extraEntries: [{ source, target: 'logs/global/pythinker-code.log' }],
      }),
    ).rejects.toMatchObject({ code: 'session.export_output_conflict' });
  ...
  it('archives a bound session log after its path is rotated', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'session-export-test-'));
    const logPath = join(tmp, 'logs', 'pythinker-code.log');
    const rotatedPath = `${logPath}.1`;
    ...
    ).resolves.toContain('logs/pythinker-code.log');
    await expect(readZipEntry(outputPath, 'logs/pythinker-code.log')).resolves.toEqual(original);
    expect(closeCalls).toBe(1);
  });
  ...
  it('includes a bounded Web log in the exported archive', async () => {
    ...
    expect(result.entries).toContain('logs/pythinker-web.jsonl');
    expect(result.manifest.webLogPath).toBe('logs/pythinker-web.jsonl');
    await expect(readZipEntry(outputPath, 'logs/pythinker-web.jsonl')).resolves.toEqual(
  ...
  it('includes the desktop app log when given', async () => {
    ...
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'state.json'), '{}\n', 'utf-8');
    const desktopLogPath = join(tmp, 'logs', 'pythinker-code-desktop.log');
    await mkdir(join(tmp, 'logs'), { recursive: true });
    ...
    expect(result.entries).toContain('logs/pythinker-desktop.log');
    expect(result.manifest.desktopLogPath).toBe('logs/pythinker-desktop.log');
    await expect(readZipEntry(outputPath, 'logs/pythinker-desktop.log')).resolves.toEqual(
  ...
  it('skips a missing desktop app log silently', async () => {
    ...
    expect(result.entries).not.toContain('logs/pythinker-desktop.log');

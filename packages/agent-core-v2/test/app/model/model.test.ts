...
import { pythinkerModelEnvOverlay, ENV_MODEL_ALIAS_KEY } from '#/app/kosongConfig/envOverlay';
import {
...
describe('effectiveModelConfig', () => {
  ...
  it('infers Anthropic effort metadata for an unknown Claude-marked model on a non-Pythinker Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('infers Anthropic effort metadata for a bare Claude family alias on a non-Pythinker Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('does not infer Anthropic effort metadata for a clearly non-Claude model on a non-Pythinker Anthropic provider', () => {
    expect(
      effectiveModelConfig(
        {
          provider: 'custom',
  ...
  it('does not infer Anthropic effort metadata for a Pythinker provider routed through the Anthropic protocol', () => {
    ...
    expect(effectiveModelConfig(model, 'pythinker')).toEqual(model);
  });
...
function applyPythinkerModelEnvOverlay(
  env: EnvMap,
  effective: Record<string, unknown> = {},
): { readonly changed: readonly string[]; readonly effective: Record<string, unknown> } {
  const changed = pythinkerModelEnvOverlay.apply(
    effective,
    (name) => env[name],
    (domain, value) => {
...
describe('pythinkerModelEnvOverlay', () => {
  it('does nothing when PYTHINKER_MODEL_NAME is absent', () => {
    const effective = {
      models: {
    ...
    const result = applyPythinkerModelEnvOverlay({}, effective);

    expect(result.changed).toEqual([]);
    expect(result.effective).toEqual(effective);
  });

  it('applies request overrides when PYTHINKER_MODEL_NAME is absent', () => {
    const { changed, effective } = applyPythinkerModelEnvOverlay({
  ...
  it('synthesizes an env model alias and default model from the minimal env set', () => {
    ...
    expect(effective['providers']).toEqual({
      [ENV_MODEL_PROVIDER_KEY]: { type: 'pythinker', baseUrl: 'https://api.moonshot.ai/v1' },
    });
  });

  it('omits baseUrl for openai so the base SDK default applies at construction', () => {
    const { effective } = applyPythinkerModelEnvOverlay(
  ...
  it('keeps an explicit env provider type instead of the pythinker default', () => {
    const { changed, effective } = applyPythinkerModelEnvOverlay(
      { PYTHINKER_MODEL_NAME: 'env-model' },
  ...
  it('maps extended model metadata and request overrides', () => {
    const { changed, effective } = applyPythinkerModelEnvOverlay({
      PYTHINKER_MODEL_NAME: 'env-model',
      PYTHINKER_MODEL_MAX_CONTEXT_SIZE: '1000000',
      PYTHINKER_MODEL_MAX_OUTPUT_SIZE: '8192',
      PYTHINKER_MODEL_CAPABILITIES: 'Image_In, thinking , tool_use',
      PYTHINKER_MODEL_DISPLAY_NAME: 'Custom Model',
      PYTHINKER_MODEL_REASONING_KEY: 'reasoning',
      PYTHINKER_MODEL_ADAPTIVE_THINKING: 'true',
      PYTHINKER_MODEL_TEMPERATURE: '0.3',
      PYTHINKER_MODEL_TOP_P: ' 0.95 ',
      PYTHINKER_MODEL_THINKING_KEEP: 'all',
      PYTHINKER_MODEL_MAX_COMPLETION_TOKENS: '4096',
      PYTHINKER_MODEL_MAX_TOKENS: '2048',
    });
  ...
  it('falls back to legacy PYTHINKER_MODEL_MAX_TOKENS for completion overrides', () => {
    const { effective } = applyPythinkerModelEnvOverlay({
      PYTHINKER_MODEL_NAME: 'env-model',
      PYTHINKER_MODEL_MAX_TOKENS: '2048',
    });
  ...
  it.each([
    ['PYTHINKER_MODEL_MAX_CONTEXT_SIZE', '0'],
    ['PYTHINKER_MODEL_MAX_CONTEXT_SIZE', '1.5'],
    ['PYTHINKER_MODEL_MAX_OUTPUT_SIZE', 'nope'],
    ['PYTHINKER_MODEL_ADAPTIVE_THINKING', 'maybe'],
    ['PYTHINKER_MODEL_TEMPERATURE', 'abc'],
    ['PYTHINKER_MODEL_TEMPERATURE', '1.2.3'],
    ['PYTHINKER_MODEL_TOP_P', 'NaN'],
  ])('throws config.invalid for invalid %s=%s', (key, value) => {
    expectConfigInvalid(() =>
      applyPythinkerModelEnvOverlay({ PYTHINKER_MODEL_NAME: 'env-model', [key]: value }),
    );
  });

  it('strips env-only model values before write-back', () => {
    expect(
      pythinkerModelEnvOverlay.strip?.(
        'models',
        {
    ...
    expect(
      pythinkerModelEnvOverlay.strip?.('defaultModel', ENV_MODEL_ALIAS_KEY, {
        default_model: 'user',
      }),
    ).toBe('user');
    expect(pythinkerModelEnvOverlay.strip?.('modelOverrides', { temperature: 0.3 }, {})).toBeUndefined();
  });

  it('self-registers into ConfigRegistry without ModelService instantiation', () => {
    const freshRegistry = new ConfigRegistry();
    expect(freshRegistry.listEffectiveOverlays()).toContain(pythinkerModelEnvOverlay);
  });
});

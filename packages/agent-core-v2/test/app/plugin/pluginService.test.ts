 ...
 *
 * Run: pnpm --filter @pymodel/agent-core-v2 exec vitest run test/app/plugin/pluginService.test.ts
 */
...
describe('PluginService (plugin boundary)', () => {
  ...
  it('restores the previous managed copy when reinstall persistence fails', async () => {
    ...
    try {
      ...
      await expect(readFile(path.join(previous.root, 'pythinker.plugin.json'), 'utf8')).resolves.toContain(
        '"version":"1.0.0"',
      );

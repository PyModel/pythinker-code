...
describe('OAuthService', () => {
  ...
  it('rewrites a lost default model on refresh even when the catalog is unchanged', async () => {
    ...
    expect(defaultModel).toBe('pythinker-code/kimi-k2');
  ...
  it('keeps the default model the user selects while a refresh is in flight', async () => {
    ...
    models = {
      'pythinker-code/kimi-k2': managedK2Alias,
      'pythinker-code/kimi-k2.5': managedK25Alias,
    };
    defaultModel = 'pythinker-code/kimi-k2';
    const svc = createService();
    ...
    await vi.waitFor(() => { expect(fetchMock).toHaveBeenCalled(); });
    await configReplace('defaultModel', 'pythinker-code/kimi-k2.5');
    releaseFetch();
    ...
    expect(configReplace).toHaveBeenCalledWith('defaultModel', 'pythinker-code/kimi-k2.5');
    expect(defaultModel).toBe('pythinker-code/kimi-k2.5');
  });
  ...
  it('writes back the refreshed catalog and default when the user does not intervene mid-flight', async () => {
    ...
    expect(configReplace).toHaveBeenCalledWith(
      'models',
      expect.objectContaining({
        'pythinker-code/kimi-k3': expect.objectContaining({ model: 'kimi-k3' }),
      }),
    );
    expect(configReplace).toHaveBeenCalledWith('defaultModel', 'pythinker-code/kimi-k2');
    expect(defaultModel).toBe('pythinker-code/kimi-k2');

 ...
 * boundaries. Run with `pnpm --filter @pymodel/agent-core-v2 exec vitest run
...
describe('subagent config section', () => {
  ...
  it('preserves the coded error contract when adding secondary-model guidance', () => {
    ...
    expect(toErrorPayload(result)).toMatchObject({
      ...
      message: expect.stringContaining('comes from [secondary_model].model / PYTHINKER_SECONDARY_MODEL'),
      details: {
        model: 'provider/bad',

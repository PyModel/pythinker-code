...
import { IAuthStatusService } from '#/app/authLegacy/authLegacy';
import { AuthStatusService } from '#/app/authLegacy/authStatusService';
...
describe('ConfigService env overlay (live)', () => {
  it('re-applies env bindings on every get()', async () => {
    const env: Record<string, string> = { PYTHINKER_DISABLE_CRON: '0' };
    ...
    env['PYTHINKER_DISABLE_CRON'] = '1';
    expect(config.get<CronConfig>('cron').disabled).toBe(true);
    env['PYTHINKER_DISABLE_CRON'] = '0';
    expect(config.get<CronConfig>('cron').disabled).toBe(false);
  ...
  it('keeps the file value when a scalar section env value fails to parse', async () => {
    ...
    for (const invalid of ['', '   ', 'maybe']) {
      env['PYTHINKER_CODE_BUILTIN_PRODUCT_SKILLS'] = invalid;
      expect(config.get(BUILTIN_PRODUCT_SKILLS_SECTION)).toBe(false);
    }

    env['PYTHINKER_CODE_BUILTIN_PRODUCT_SKILLS'] = 'on';
    expect(config.get(BUILTIN_PRODUCT_SKILLS_SECTION)).toBe(true);

    disposables.dispose();
  });

  it('keeps the Pythinker effort force separate from the configured effort', async () => {
    const env: Record<string, string> = { PYTHINKER_MODEL_THINKING_EFFORT: 'max' };
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/pythinker-cfg', env));
  ...
  it('strips the Pythinker effort force before persisting thinking config', async () => {
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
    ix.stub(ILogService, stubLog());
  ...
  it('deletes a scalar section on replace(undefined) — set(undefined) cannot', async () => {
    ...
    await config.replace('defaultModel', 'pythinker-code/kimi-k2');
  ...
  it('marks the env-injected flat model ready in the auth legacy summary', async () => {
    const env: Record<string, string> = { PYTHINKER_MODEL_NAME: 'kimi-for-coding' };
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/pythinker-cfg', env));
    ...
    ix.set(IConfigRegistry, new SyncDescriptor(ConfigRegistry));
    ix.set(IConfigService, new SyncDescriptor(ConfigService));
    ix.set(IAuthStatusService, new SyncDescriptor(AuthStatusService));

    const summary = await ix.get(IAuthStatusService).get();
...
describe('services config section env bindings', () => {
  ...
  it('resolves pymodel_search / pymodel_fetch fields from PYTHINKER_WEB_* env vars', async () => {
    ...
    expect(config.get<ServicesConfig>(SERVICES_SECTION)).toEqual({
      pymodelSearch: { baseUrl: 'https://search-env.example/search', apiKey: 'env-search-key' },
      pymodelFetch: { baseUrl: 'https://fetch-env.example/fetch', apiKey: 'env-fetch-key' },
    });
  ...
  it('uses an env API key instead of persisted OAuth for a configured endpoint', async () => {
    ...
    expect(config.get<ServicesConfig>(SERVICES_SECTION)?.pymodelSearch).toEqual({
      baseUrl: 'https://file.example/search',
      apiKey: 'env-search-key',
      customHeaders: { 'X-Service': 'search' },
    });
  ...
  it('ignores blank env values instead of masking the file value', async () => {
    ...
    await config.ready;
    await config.set(SERVICES_SECTION, {
      pymodelSearch: { baseUrl: 'https://file.example/search' },
    });
  ...
  it('strips env-derived fields before persisting a round-tripped effective value', async () => {
    ...
    expect(effective?.pymodelFetch).toEqual({
      baseUrl: 'https://fetch-env.example/fetch',
      apiKey: 'env-fetch-key',
    });
  ...
  it('clears the section on replace(undefined) even with env vars set', async () => {
    ...
    expect(config.get<ServicesConfig>(SERVICES_SECTION)?.pymodelSearch?.baseUrl).toBe(
...
describe('image config section', () => {
  ...
  it('re-applies image env bindings on every get() and ignores invalid env', async () => {
    ...
    env['PYTHINKER_IMAGE_MAX_EDGE_PX'] = 'abc';
    env['PYTHINKER_IMAGE_READ_BYTE_BUDGET'] = '-1';
    expect(config.get<ImageConfig>(IMAGE_SECTION)).toEqual({});

    env['PYTHINKER_IMAGE_MAX_EDGE_PX'] = '1500';
    env['PYTHINKER_IMAGE_READ_BYTE_BUDGET'] = '131072';
    expect(config.get<ImageConfig>(IMAGE_SECTION)).toEqual({
      maxEdgePx: 1500,
      readByteBudget: 131072,
    });

    env['PYTHINKER_IMAGE_MAX_EDGE_PX'] = '2500';
  ...
  it('restores env-owned fields to the raw value on set() while the env var is set', async () => {
    const env: Record<string, string> = { 'PYTHINKER_IMAGE_MAX_EDGE_PX': '1500' };
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
...
describe('applyPrintModeConfigDefaults', () => {
  ...
  it('fills unset keys into the memory layer with effectively unbounded values', async () => {
    ...
    expect(config.inspect('dynamic_workflow').memoryValue).toMatchObject({ timeoutMs: 0 });
  ...
  it('does not override keys the user set explicitly', async () => {
    ...
    expect(config.inspect('dynamic_workflow').memoryValue).toBeUndefined();
  ...
  it('does not override the dynamic_workflow timeout env override', async () => {
...
describe('dynamic_workflow config section', () => {
  async function createConfig(env: Record<string, string>, toml?: string) {
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
    const storage = new InMemoryStorageService();
...
describe('ConfigService replaceSections', () => {
  ...
  async function createSectionsConfig(toml = SEED_TOML) {
    ...
    await storage.write('', 'config.toml', new TextEncoder().encode(toml));
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/pythinker-cfg-replace-sections'));
    ix.stub(IFileSystemStorageService, storage);
    ix.set(IAtomicTomlDocumentStore, new SyncDescriptor(TomlAtomicDocumentStore));
    ix.set(IConfigRegistry, new SyncDescriptor(ConfigRegistry));
    ix.set(IConfigService, new SyncDescriptor(ConfigService));
...
describe('ConfigService persistence guards', () => {
  async function createGuardedConfig(toml: string, env: NodeJS.ProcessEnv = {}) {
    ...
    await storage.write('', 'config.toml', new TextEncoder().encode(toml));
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/pythinker-cfg-guards', env));
    ix.stub(IFileSystemStorageService, storage);
    ix.set(IAtomicTomlDocumentStore, new SyncDescriptor(TomlAtomicDocumentStore));
    ix.set(IConfigRegistry, new SyncDescriptor(ConfigRegistry));
    ix.set(IConfigService, new SyncDescriptor(ConfigService));
  ...
  it('restores env-masked values from the freshly re-read file instead of the stale snapshot', async () => {
    ...
    expect(config.get(DEFAULT_MODEL_SECTION)).toBe('__pythinker_env_model__');

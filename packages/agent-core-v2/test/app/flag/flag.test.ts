...
const exampleFlag: FlagDefinitionInput = {
  ...
  env: 'PYTHINKER_CODE_EXPERIMENTAL_EXAMPLE_FLAG',
  default: true,
  surface: 'core',
};
...
describe('FlagRegistryService', () => {
  it('registers and resolves by id', () => {
    ...
    expect(reg.get('example_flag')?.env).toBe('PYTHINKER_CODE_EXPERIMENTAL_EXAMPLE_FLAG');
  });
...
describe('FlagService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = `/tmp/pythinker-code-flag-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  });
  ...
  it('treats truthy env values case-insensitively', () => {
    const { flags } = makeFlags({ PYTHINKER_CODE_EXPERIMENTAL_EXAMPLE_FLAG: 'YES' });
    expect(flags.enabled('example_flag')).toBe(true);
  });

  it('treats falsy env values case-insensitively', () => {
    const { flags } = makeFlags({ PYTHINKER_CODE_EXPERIMENTAL_EXAMPLE_FLAG: 'off' });
    expect(flags.enabled('example_flag')).toBe(false);
  });

  it('reads only the env name declared in the registry', () => {
    const { flags } = makeFlags({ PYTHINKER_CODE_EXPERIMENTAL_UNKNOWN: 'false' });
    expect(flags.enabled('example_flag')).toBe(true);
  });

  it('ignores garbage env values', () => {
    const { flags } = makeFlags({ PYTHINKER_CODE_EXPERIMENTAL_EXAMPLE_FLAG: 'maybe' });
    expect(flags.enabled('example_flag')).toBe(true);
  });

...
describe('StdioMcpClient', () => {
  ...
  it('resolves relative config.cwd from defaultCwd', async () => {
    const defaultCwd = mkdtempSync(join(tmpdir(), 'pythinker-mcp-relative-cwd-'));
  ...
  it('allows explicit config.cwd outside defaultCwd', async () => {
    const defaultCwd = mkdtempSync(join(tmpdir(), 'pythinker-mcp-default-cwd-'));
    const outsideCwd = mkdtempSync(join(tmpdir(), 'pythinker-mcp-outside-cwd-'));
    const client = createClient(
      {
        transport: 'stdio',
        command: process.execPath,
  ...
  it('forwards configured env to the spawned server', async () => {
    const client = createClient({
      ...
      command: process.execPath,
      args: [stdioFixture],
      env: { PYTHINKER_TEST_ENV: 'forwarded-value' },
    });
    try {
      await client.connect();
      const result = await client.callTool('read_env', { name: 'PYTHINKER_TEST_ENV' });
  ...
  it('inherits parent process env so PATH/HOME survive; config.env overrides on conflict', async () => {
    const parentOnly = `PYTHINKER_TEST_PARENT_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const shared = `PYTHINKER_TEST_SHARED_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  it('captures recent stderr into a snapshot the manager can attach to errors', async () => {
    const banner = `pythinker-test-stderr-${Date.now()}`;
    const client = createClient({
      transport: 'stdio',
      command: process.execPath,
  ...
  it('notifies an unexpected-close listener when the child exits after connect', async () => {
    const banner = `pythinker-test-crash-${Date.now()}`;
    const client = createClient({
      transport: 'stdio',
      command: process.execPath,
  ...
  it('buffers an early close and replays it on listener registration', async () => {
    const banner = `pythinker-test-early-${Date.now()}`;
    const client = createClient({
      transport: 'stdio',
      command: process.execPath,
      args: [crashAfterConnectFixture],
      env: { PYTHINKER_TEST_MCP_STDERR: banner, PYTHINKER_TEST_MCP_EXIT_CODE: '0' },
    });
    try {
      await client.connect();

...
describe('workspace resource sharing (handler chain)', () => {
    ...
    host = createScopedTestHost([
      ...
      stubPair(IProjectLocalConfigService, {
        ...
        readAdditionalDirs: (workDir: string) =>
          Promise.resolve({
            projectRoot: workDir,
            configPath: `${workDir}/.pythinker-code/local.toml`,
            additionalDirs: [],
          }),
  ...
  it('runs one shared MCP manager and one initial connect for two concurrent sessions', async () => {
    const root = await makeRoot('pythinker-ws-mcp-');
    await mkdir(join(root, '.pythinker-code'), { recursive: true });
    await writeFile(
      join(root, '.pythinker-code', 'mcp.json'),
      JSON.stringify({
        mcpServers: {
    ...
    const home = await makeRoot('pythinker-ws-mcp-home-');
  ...
  it('does not rescan skills when a second session of the workspace lists them', async () => {
    const root = await makeRoot('pythinker-ws-skill-');
    await mkdir(join(root, '.pythinker-code', 'skills'), { recursive: true });
    const home = await makeRoot('pythinker-ws-skill-home-');
  ...
  it('refreshes a live session skill list when .agents/skills changes on disk', async () => {
    const root = await makeRoot('pythinker-ws-watch-');
    const home = await makeRoot('pythinker-ws-watch-home-');

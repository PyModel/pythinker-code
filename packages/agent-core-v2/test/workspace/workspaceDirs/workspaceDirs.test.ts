 ...
 * - a persisted `addDir` writes `.pythinker-code/local.toml` and refreshes every
...
describe('workspace add-dir (handler chain)', () => {
  ...
  async function makeProjectRoot(): Promise<string> {
    const root = await makeRoot('pythinker-add-dir-proj-');
  ...
  it('persists addDir to local.toml and every session of the workspace sees the dir', async () => {
    const homeDir = await makeRoot('pythinker-add-dir-home-');
    const root = await makeProjectRoot();
    const extra = await makeRoot('pythinker-add-dir-extra-');
    ...
    expect(result.configPath).toBe(join(root, '.pythinker-code', 'local.toml'));
    expect(result.additionalDirs).toEqual([extra]);
    // local.toml written on disk.
    const toml = await readFile(join(root, '.pythinker-code', 'local.toml'), 'utf8');
  ...
  it('persist: false stays in handler memory, never touches the disk, and is shared', async () => {
    ...
    await expect(readFile(join(root, '.pythinker-code', 'local.toml'), 'utf8')).rejects.toThrow();
  ...
  it('refreshes live session views when another process edits local.toml', async () => {
    ...
    // External write (another process, an editor, `pythinker` in a second CLI).
    await mkdir(join(root, '.pythinker-code'), { recursive: true });
    const writeLocalToml = () =>
      writeFile(join(root, '.pythinker-code', 'local.toml'), `[workspace]\nadditional_dir = ["${extra}"]\n`);
  ...
  it('registers the additional-directory sets into the workspace state container', async () => {
    const homeDir = await makeRoot('pythinker-add-dir-home-');
    const root = await makeProjectRoot();
    const persisted = await makeRoot('pythinker-add-dir-persisted-');
    const ephemeral = await makeRoot('pythinker-add-dir-ephemeral-');
    const host = buildHost(homeDir);
    const handler = await host.app.accessor.get(IWorkspaceLifecycleService).handlerFor({ root });

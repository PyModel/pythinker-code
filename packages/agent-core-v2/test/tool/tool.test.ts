...
describe('Agent tool execution contract', () => {
  ...
  it('rejects spawning the default agent type from the builtin default profile', async () => {
    const lifecycle = createAgentLifecycleStub();
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('Subagent type "agent" is not allowed for this agent');
    expect(result.output).toContain('coder');
    expect(lifecycle.create).not.toHaveBeenCalled();
  });

  it('falls back to the default profile allowlist when the caller binding declares none', async () => {
    const lifecycle = createAgentLifecycleStub();
    const context = createAgentToolContext(
      lifecycle,
      sessionService(ISessionAgentProfileCatalog, allowlistCatalog(['explore'])),
  ...
  it('treats a lone "*" caller allowlist as unrestricted', async () => {
    const lifecycle = createAgentLifecycleStub({
    ...
    const context = createAgentToolContext(
      lifecycle,
      sessionService(ISessionAgentProfileCatalog, allowlistCatalog(['*'])),
    );
  ...
  it('spawns a discovered custom agent type from the main agent', async () => {
    const lifecycle = createAgentLifecycleStub({
      createAgentIds: ['agent-child'],
      runCompletion: async () => ({ summary: 'child result' }),
    });
    ...
    expect(lifecycle.create).toHaveBeenCalledWith(
      expect.objectContaining({
        binding: expect.objectContaining({ profile: 'reviewer' }),
      }),
    );
    expect(result.output).toContain('actual_subagent_type: reviewer');
  });

  it('launches a custom agent discovered after the description froze', async () => {
    const agent = normalizeAgentProfile({
      name: 'agent',
      description: 'Default agent',
      subagents: ['coder'],
    ...
    const profiles: AgentProfile[] = [agent, coder];
    const catalog: ISessionAgentProfileCatalog = {
      _serviceBrand: undefined,
      ready: Promise.resolve(),
      onDidChange: Event.None as ISessionAgentProfileCatalog['onDidChange'],
      ...
      inspect: (name) => {
        ...
        return {
          name,
          profile,
          sourceId: name === 'agent' || name === 'coder' ? 'builtin' : 'workspace',
          priority: 0,
          suppressed: [],
        };
      },
    ...
    context.toolsData();
    await Promise.resolve();
    const frozen = context.toolsData().find((entry) => entry.name === 'Agent')?.description;
    ...
    const after = context.toolsData().find((entry) => entry.name === 'Agent')?.description;
    expect(after).toBe(frozen);

    const result = await executeAgentTool(context, {
      prompt: 'Investigate',
      description: 'Find cause',
      subagent_type: 'latecomer',
    });

    expect(lifecycle.create).toHaveBeenCalledWith(
      expect.objectContaining({
        binding: expect.objectContaining({ profile: 'latecomer' }),
      }),
    );
    expect(result.output).toContain('actual_subagent_type: latecomer');
  });
...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: Agent, AgentDynamicWorkflow, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Lookup, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, WaitFor, Write
        messages:
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "7c8e6fb113f7ed9a8fdba4691959d31bc00006f375e1646904a51d09b22590e2", "messageCount": 4, "turnStep": "0.2", "time": "<time>" }
    ...
    `);
      ...
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: Agent, AgentDynamicWorkflow, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, WaitFor, Write
        messages:
          <last>

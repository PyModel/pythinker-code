...
} from '#/session/dynamic_workflow/agentRunBatch';
import { ISessionDynamicWorkflowService, type SessionDynamicWorkflowSpawnTask, type SessionDynamicWorkflowTask } from '#/session/dynamic_workflow/sessionDynamicWorkflow';
import { Error2 } from '#/_base/errors/errors';
import { ConfigErrors } from '#/app/config/errors';
import { SessionDynamicWorkflowService } from '#/session/dynamic_workflow/sessionDynamicWorkflowService';
...
describe('SessionDynamicWorkflowService metadata compatibility', () => {
  ...
  it('reads dynamic_workflow items from caller-owned v2 labels and legacy v1 metadata', async () => {
    ...
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'main', agentId: 'v2-child' }),
    ).resolves.toBe('src/a.ts');
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'main', agentId: 'legacy-child' }),
    ).resolves.toBe('src/legacy.ts');
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'main', agentId: 'other-child' }),
    ).resolves.toBeUndefined();
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'main', agentId: 'missing' }),
    ).resolves.toBeUndefined();
  });
  ...
  it('prefers labels over legacy metadata fields when both are present', async () => {
    ...
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'main', agentId: 'mixed-child' }),
    ).resolves.toBe('src/labels.ts');
    await expect(
      service.getDynamicWorkflowItem({ callerAgentId: 'other', agentId: 'mixed-child' }),
    ).resolves.toBeUndefined();
  });
  ...
  it('prefers the spawn task binding over the caller model', async () => {
    const service = ix.get(ISessionDynamicWorkflowService);
    const spawnTask: SessionDynamicWorkflowSpawnTask = {
  ...
  it('points at the secondary model config when a spawn task binding is invalid', async () => {
    ...
    await expect(
      ...
      {
        status: 'failed',
        error: expect.stringContaining('comes from [secondary_model].model / PYTHINKER_SECONDARY_MODEL'),
      },
    ]);
...
function resumeSessionTask(agentId: string): SessionDynamicWorkflowTask {
  return {
    kind: 'resume',
    data: {},
    profileName: 'subagent',

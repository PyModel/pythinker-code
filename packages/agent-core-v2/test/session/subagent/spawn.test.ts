...
describe('SessionSubagentService planSpawn and spawn', () => {
  ...
  beforeEach(() => {
    ...
    ix.stub(IAgentLifecycleService, {
      ...
      resolve: (agent: { readonly agentId: string }, definition: unknown) => {
        if (definition === AgentReminder && agent.agentId === 'agent-fork') return createdReminder;
        throw new Error(`unexpected resolve for ${agent.agentId}`);
      },
      remove: removeAgent,

...
describe('AgentLifecycleService', () => {
  ...
  beforeEach(() => {
    ...
    ix.stub(ISessionContext, {
      ...
      sessionId: 'sess_test',
      workspaceId: 'ws_test',
      sessionDir: '/tmp/pythinker-agentLifecycle-test',
      metaScope: 'test',
      scope: (subKey?: string) =>
    ...
    ix.stub(ISessionMetadata, {
      ...
      read: () =>
        Promise.resolve({
          ...
          archived: false,
          agents: {
            'agent-0': { homedir: '/tmp/pythinker-agentLifecycle-test/agents/agent-0', type: 'sub' },
            'agent-1': { homedir: '/tmp/pythinker-agentLifecycle-test/agents/agent-1', type: 'sub' },
          },
        }),
      update: () => Promise.resolve(),
      setTitle: () => Promise.resolve(),
  ...
  it('persists complete agent metadata when creating a child', async () => {
    ...
    expect(registerAgent).toHaveBeenCalledWith('child', {
      homedir: '/tmp/pythinker-agentLifecycle-home/sessions/ws_test/sess_test/agents/child',
      type: 'sub',
      parentAgentId: 'main',
      forkedFrom: 'main',
      labels: { dynamicWorkflowItem: 'dynamic-workflow-item-1' },
  ...
  it('records the bound profile in the subagent metadata at registration', async () => {
    ...
    await svc
      .create({
        agentId: 'child',
        binding: { profile: 'coder', model: 'pythinker-test' },
        labels: { parentAgentId: 'main' },
      })
      .catch(() => undefined);

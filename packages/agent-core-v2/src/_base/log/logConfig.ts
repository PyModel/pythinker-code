 ...
 *
 * Builds the `LoggingConfig` from `PYTHINKER_LOG_*` environment variables plus
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'pythinker-code.log');
}

export function resolveSessionLogPath(sessionDir: string): string {
  return join(sessionDir, 'logs', 'pythinker-code.log');
}

export function resolveLoggingConfig(input: ResolveLoggingInput): LoggingConfig {
  const env = input.env;
  return {
    level: parseLevel(env['PYTHINKER_LOG_LEVEL']) ?? DEFAULT_LOG_LEVEL,
    globalLogPath: resolveGlobalLogPath(input.homeDir),
    globalMaxBytes: parsePositiveInt(env['PYTHINKER_LOG_GLOBAL_MAX_BYTES']) ?? DEFAULT_GLOBAL_MAX_BYTES,
    globalFiles: parsePositiveInt(env['PYTHINKER_LOG_GLOBAL_FILES']) ?? DEFAULT_GLOBAL_FILES,
    sessionMaxBytes:
      parsePositiveInt(env['PYTHINKER_LOG_SESSION_MAX_BYTES']) ?? DEFAULT_SESSION_MAX_BYTES,
    sessionFiles: parsePositiveInt(env['PYTHINKER_LOG_SESSION_FILES']) ?? DEFAULT_SESSION_FILES,
  };
}

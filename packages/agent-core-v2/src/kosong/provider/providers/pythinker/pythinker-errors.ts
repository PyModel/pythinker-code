/**
 * `kosong/provider` domain — Pythinker vendor error classification.
 *
 * This module owns the vendor-specific knowledge of how the PyModel backend
...
export function classifyPythinkerQuotaError(error: unknown): APIProviderQuotaExhaustedError | undefined {
  ...
  const structuredHit = collectErrorCodes(error).some((code) =>
    PYTHINKER_QUOTA_EXHAUSTED_ERROR_CODES.has(code),
  );
  const lowerMessage = message.toLowerCase();
  const wordingHit = PYTHINKER_QUOTA_EXHAUSTED_MESSAGE_PATTERNS.some((pattern) =>

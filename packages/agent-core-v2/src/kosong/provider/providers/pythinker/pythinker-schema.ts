/**
 * `kosong/provider` domain — Pythinker tool-schema dialect normalization.
 *
 ...
 * keys — the schema dialect the Pythinker tool endpoint accepts.
 *
...
function inferTypeFromValues(values: unknown[]): JsonSchemaType {
  ...
  throw new Error2(
    ProtocolErrors.codes.PROVIDER_API_ERROR,
    'Mixed JSON Schema enum or const types are not supported by Pythinker tool schemas.',
  );
}

 ...
 *
 * Resolves each `pythinker-file://` video reference in the projected wire messages
...
import { type PythinkerFileRef, isPythinkerFileUrl, parsePythinkerFileUrl } from './pythinkerFileUrl';
...
export class AgentVideoResolverService implements IAgentVideoResolverService {
  ...
  async resolve(
    ...
    if (!messages.some(hasPythinkerFileVideoPart)) return messages;

    let changed = false;
    const out: Message[] = [];
    for (const message of messages) {
      if (!hasPythinkerFileVideoPart(message)) {
        out.push(message);
        continue;
      }
      ...
      for (const part of message.content) {
        const ref =
          part.type === 'video_url' ? parsePythinkerFileUrl(part.videoUrl.url) : undefined;
...
function hasPythinkerFileVideoPart(message: Message): boolean {
  return message.content.some(
    (part) => part.type === 'video_url' && isPythinkerFileUrl(part.videoUrl.url),
  );
}

function tag(ref: PythinkerFileRef): ContentPart {

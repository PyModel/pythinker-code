 ...
 * (`AGENTS_MD_PLAIN_NAMES` / `dotPythinkerAgentsMdPath` / `findAgentsMdInDir`)
...
export function dotPythinkerAgentsMdPath(dir: string): string {
  return join(dir, '.pythinker-code', 'AGENTS.md');
}

export function agentsMdCandidatePaths(dir: string): string[] {
  return [dotPythinkerAgentsMdPath(dir), ...AGENTS_MD_PLAIN_NAMES.map((name) => join(dir, name))];
}
...
export async function findAgentsMdInDir(
  ...
  const found: string[] = [];
  const dotPythinker = dotPythinkerAgentsMdPath(dir);
  if (await isNonEmptyFile(deps, dotPythinker)) found.push(dotPythinker);
...
export async function loadAgentsMdForRoots(
  ...
  for (const workDir of workDirs) {
    ...
    for (const dir of dirs) {
      await collect(dotPythinkerAgentsMdPath(dir));
      for (const fileName of AGENTS_MD_PLAIN_NAMES) {
...
export async function agentsMdWatchRoots(
  ...
  for (const dir of dirsRootToLeaf(rootWorkDir, projectRoot)) {
    projectCandidates.push(
      join(dir, '.pythinker-code', 'AGENTS.md'),
      join(dir, 'AGENTS.md'),
      join(dir, 'agents.md'),
    );
  }

 ...
 * identically. The dynamic-workflow-mode exit reminder removal is a cross-model fold:
 * `ContextModel` registers a reducer on `dynamic_workflow_mode.exit` (see
 * `popDynamicWorkflowModeReminder`) so the pop replays from the `dynamic_workflow_mode.exit` record
 * itself.
 *
...
export const ContextModel = defineModel<ContextMessage[]>('contextMemory', () => [], {
  ...
  reducers: {
    'dynamic_workflow_mode.exit': popDynamicWorkflowModeReminder,
  },
});

function popDynamicWorkflowModeReminder(state: ContextMessage[], _payload: unknown): ContextMessage[] {
  const last = state[state.length - 1];
  if (last === undefined) return state;
  const origin = last.origin;
  if (origin?.kind !== 'injection' || origin.variant !== 'dynamic_workflow_mode') return state;

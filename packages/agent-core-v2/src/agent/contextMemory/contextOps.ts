...
export function popDynamicWorkflowModeReminder(state: ContextMessage[]): ContextMessage[] {
  const last = state.at(-1);
  if (last?.origin?.kind !== 'injection' || last.origin.variant !== 'dynamic_workflow_mode') return state;

...
export const WATCH_ENV = 'PYTHINKER_CODE_WATCH';
...
function planCandidateWatches(
  ...
  for (const candidate of candidates) {
    ...
    if (watchPathExists(current) && !isPythinkerCodeDir(current)) notify.add(current);
...
function isPythinkerCodeDir(path: string): boolean {
  const name = basename(path);
  return process.platform === 'win32' ? name.toLowerCase() === '.pythinker-code' : name === '.pythinker-code';
}

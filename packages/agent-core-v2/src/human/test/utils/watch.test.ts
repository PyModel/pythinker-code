...
describe('watch chokidar mode', () => {
  ...
  it('does not emit sibling flood names when watching candidate paths', async () => {
    ...
    const pythinker = join(root, '.pythinker-code');
    const sessions = join(pythinker, 'sessions', 's1.json');
    await mkdir(join(pythinker, 'sessions'), { recursive: true });
    await writeFile(join(root, 'ck_0001'), 'x');
    await writeFile(sessions, 'old');
    const events: WatchChange[] = [];
    const file = join(pythinker, 'local.toml');
    ...
    expect(events.some((e) => e.path === pythinker || e.path === root || e.path.includes('sessions'))).toBe(
      false,
    );
  });

...
import { pythinkerAnthropicTrait } from '#/llm-pythinker/trait';
...
describe('anthropic lowering of inline images', () => {
  it('forwards a base64 image the Pythinker policy accepts even though the route id is anthropic', () => {
    const wire = lowerMessage(message, providerImagePolicy('pythinker').acceptedMimes);
  ...
  it('sends the HEIC block on the wire when Pythinker is reached over the Anthropic protocol', async () => {

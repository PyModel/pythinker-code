...
import { PythinkerFiles } from '#/llm-pythinker/files';
import { pythinkerMediaContribution } from '#/llm-pythinker/media';
import { pythinkerProvider } from '#/llm-pythinker/provider';
import {
  PYTHINKER_API_KEY_ENV,
  PYTHINKER_BASE_URL_ENV,
  PYTHINKER_DEFAULT_BASE_URL,
  pythinkerAnthropicTrait,
  pythinkerConnection,
  pythinkerOpenAITrait,
} from '#/llm-pythinker/trait';
import { classifyPythinkerQuotaError } from '#/llm-pythinker/errors';
...
describe('media', () => {
  ...
  it('rejects a non-video mime type', async () => {
    await expect(
      pythinkerMediaContribution.uploadVideo!(
  ...
  it('requires an api key', async () => {
    const files = new PythinkerFiles({ baseUrl: 'https://example.test/v1' });
    await expect(
...
describe('endpoint', () => {
  ...
  it('injects the endpoint from env and connection defaults at request time', async () => {
    vi.stubEnv(PYTHINKER_BASE_URL_ENV, '');
    vi.stubEnv(PYTHINKER_API_KEY_ENV, 'env-key');
    const seen: LlmModel[] = [];
    ...
    await requester.generate(
      { model: pythinkerProvider.resolveModel('kimi-k3') },
      { messages },
      { signal },
    );
    vi.stubEnv(PYTHINKER_BASE_URL_ENV, 'https://example.test/v9');
    await requester.generate(
      { model: pythinkerProvider.resolveModel('kimi-k3') },
      { messages },
      { signal },
    ...
    await requester.generate(
      { model: pythinkerProvider.resolveModel('kimi-k3', { baseUrl: 'https://explicit.test/v1' }) },
      { messages },
      { signal },
    );
  ...
  it('selects protocols by name and rejects undeclared ones', () => {
    expect(pythinkerProvider.protocols).toEqual(['openai', 'anthropic', 'openai_responses']);
    expect(() => pythinkerProvider.createRequester('google-genai')).toThrow(
      "provider 'pythinker' has no protocol 'google-genai'",
    );
    expect(() => pythinkerProvider.resolveModel('kimi-k3', { protocol: 'google-genai' })).toThrow(
      "provider 'pythinker' has no protocol 'google-genai'",
    );
    ...
    const resolved = pythinkerProvider.resolveModel('kimi-k3', {
      baseUrl: 'https://example.test/v1',
      apiKey: 'k',
...
describe('convertTool', () => {
  ...
  it('normalizes tool schemas for pythinker', async () => {
    const client = stubOpenAIClient(chatCompletionChunks);
    const requester = createOpenAIRequester({
      ...pythinkerOpenAI,
      clientFactory: client.clientFactory,
...
describe('message-level tools', () => {
  ...
  it('serializes system message tools for pythinker', async () => {
    const client = stubOpenAIClient(chatCompletionChunks);
    const requester = createOpenAIRequester({
      ...pythinkerOpenAI,
      clientFactory: client.clientFactory,
...
describe('withMaxCompletionTokens', () => {
  it('encodes max completion tokens via the pythinker trait', async () => {
    const client = stubOpenAIClient(chatCompletionChunks);
    const requester = createOpenAIRequester({
      ...pythinkerOpenAI,
      clientFactory: client.clientFactory,
...
describe('anthropic thinking kwargs', () => {
  it('applies the pythinker thinking trait, the anthropic-beta protocol, and thinking echo rules', async () => {

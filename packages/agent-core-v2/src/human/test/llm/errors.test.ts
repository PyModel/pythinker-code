...
describe('classifyPythinkerQuotaError', () => {
  it('classifies by structured error code', () => {
    const classified = classifyPythinkerQuotaError({
      status: 429,
      message: 'quota',
  ...
  it('ignores 429 without quota signals', () => {
    expect(
      classifyPythinkerQuotaError({ status: 429, message: 'slow down', headers: new Headers() }),
    ).toBeUndefined();
  });

  it('ignores non-429 errors', () => {
    expect(
      classifyPythinkerQuotaError({ status: 400, message: 'insufficient balance' }),
    ).toBeUndefined();
  });
});
...
describe('requester error conversion', () => {
  ...
  it('converts a pythinker quota response to quota_exhausted', async () => {
    const requester = createOpenAIRequester({

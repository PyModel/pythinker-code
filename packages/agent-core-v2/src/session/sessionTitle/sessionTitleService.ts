...
export class SessionTitleService implements ISessionTitleService {
  ...
  private async generateAndApply(
    ...
    const runtimeAuth = resolvePythinkerCodeRuntimeAuth({
    ...
    const requestTitle = (accessToken: string) =>
      fetchChatTitle(pythinkerCodeToolsUrl(runtimeAuth.baseUrl), accessToken, chatContent, {
        headers: {
          ...parsePythinkerCodeCustomHeaders(),

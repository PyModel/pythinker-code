/**
 * `sessionDynamicWorkflow` domain — internal concurrency / rate-limit scheduler.
 *
 * Owns the burst-then-throttle launch ramp and the provider-rate-limit recovery
 * loop for dynamic_workflow agent runs; drives each attempt through a
...
export type AgentRunResult<T = unknown> = SessionDynamicWorkflowRunResult<T>;

export type QueuedAgentRunResult<T = unknown> = SessionDynamicWorkflowRunResult<T>;

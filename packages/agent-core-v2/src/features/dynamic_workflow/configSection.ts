...
export const DYNAMIC_WORKFLOW_SECTION = 'dynamic_workflow';
...
export const DYNAMIC_WORKFLOW_TIMEOUT_ENV = 'PYTHINKER_CODE_DYNAMIC_WORKFLOW_TIMEOUT_MS';
...
export const dynamicWorkflowEnvBindings: EnvBindings<DynamicWorkflowConfig> = envBindings(DynamicWorkflowConfigSchema, {
...
export function resolveDynamicWorkflowTimeoutMs(config: IConfigService): number {
  return (
    config.get<DynamicWorkflowConfig | undefined>(DYNAMIC_WORKFLOW_SECTION)?.timeoutMs ?? DEFAULT_DYNAMIC_WORKFLOW_TIMEOUT_MS
  );
}

  ...
  constructor(
        ...
        event.veto(
          denyToolExecution(
            this.toolApproval.formatDenyMessage(
              'The tower experiment is disabled — tower tools are inert. Re-enable the experiment and restart before using the tower protocol.',
            ),
          ),
        );
      }),
    ...
    this._register(
      toolExecutor.onBeforeExecuteTool((event) => {
        ...
        event.veto(
          denyToolExecution(
            this.toolApproval.formatDenyMessage(
              'TodoList is not available while tower mode is active — mission state lives in TowerPlan, TowerMission, and TowerStatus.',
            ),
          ),
        );
      }),
    ...
    this._register(
      toolExecutor.onBeforeExecuteTool(async (event) => {
        ...
        if (event.toolCall.name !== 'Write' && event.toolCall.name !== 'Edit') return;
          ...
          denyToolExecution(
            this.toolApproval.formatDenyMessage(
              `tower workers may only write inside their own worktree (${worktree}) — denied: ` +
                ...
                'Use TowerFinding for out-of-scope changes or TowerSend to ask the tower.',
            ),
          ),
        );
      }),

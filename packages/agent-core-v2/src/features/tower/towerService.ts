  ...
  constructor(
    ...
    this._register(
      toolExecutor.onBeforeExecuteTool((event) => {
        if (!this.flags.enabled(TOWER_FLAG_ID)) return;
        if (!this.isActive) return;
        if (event.toolCall.name !== 'AgentDynamicWorkflow') return;
        event.veto(
          denyToolExecution(
            this.toolApproval.formatDenyMessage(
              'AgentDynamicWorkflow is not available while tower mode is active — dynamic_workflow and tower modes are mutually exclusive, and the tower fleet runs through TowerSpawn, one mission per worker in its own worktree. If the work genuinely needs a dynamic_workflow instead, exit tower mode first.',
            ),
          ),
        );
      }),

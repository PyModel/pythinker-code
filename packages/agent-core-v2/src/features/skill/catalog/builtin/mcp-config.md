...
The pythinker-code runtime resolves the user-global directory as `PYTHINKER_CODE_HOME`
first, falling back to `~/.pythinker-code`. Before touching the user-global file,
resolve the actual directory with Bash so you don't read or write the wrong
one. Check whether `PYTHINKER_CODE_HOME` is set and fall back to `~/.pythinker-code`
when it is empty:
...
rest of this skill, `<PYTHINKER_CODE_HOME>` means that resolved data root —
**never assume `~/.pythinker-code`**.

- User-global: `<PYTHINKER_CODE_HOME>/mcp.json`. Use for servers you want
  everywhere.
...
- Project-local: `<cwd>/.pythinker-code/mcp.json`. Use for Pythinker-specific
...
or the `PYTHINKER_MCP_STARTUP_TIMEOUT_MS` / `PYTHINKER_MCP_TOOL_TIMEOUT_MS` env vars;
...
1. **Pick a scope.** Infer it from the user's words when you can
   ...
   current directory / Pythinker-specific / `.pythinker-code` → project-local). When
...
3. **Write and tell them how to reload MCP servers.** Preserve unrelated
   ...
   restart `pythinker-code` for the change to take effect.

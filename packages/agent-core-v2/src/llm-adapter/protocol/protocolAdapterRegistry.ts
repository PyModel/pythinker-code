...
import { PythinkerFiles } from '#human/llm-pythinker/files';
import { PYTHINKER_DEFAULT_BASE_URL } from '#human/llm-pythinker/trait';
...
const pythinkerMedia: ProviderMediaContribution = {
...
function routeFor(model: Model): AdapterRoute {
  ...
  const routeMedia = definition?.modelSource === 'oauth-catalog' ? pythinkerMedia : undefined;
  const custom =

  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    try {
      if (resolved.kind === 'local-path') {
        ...
        zipTmpDir = await mkdtemp(path.join(tmpdir(), 'pythinker-plugin-zip-'));
      ...
      managedCopy = await copyPluginToManagedRoot(this.pythinkerHomeDir, id, sourceRoot);
    ...
    for (const record of this.records.values()) {
      ...
      for (const hook of record.manifest.hooks ?? []) {
        out.push({
          ...
          cwd: record.root,
          env: {
            PYTHINKER_CODE_HOME: this.pythinkerHomeDir,
            PYTHINKER_PLUGIN_ROOT: record.root,
          },
        });
      }
    }

...
import { pythinkerCdnContentUrl } from '@pymodel/pythinker-code-oauth';
...
const MAC_PLUGIN_ID = 'pythinker-cu';
const WINDOWS_PLUGIN_ID = 'pythinker-cu-win';
const APP_BUNDLE = 'PythinkerCU.app';
const LAUNCHD_LABEL = 'ai.pythinker.cu.service';
...
const WINDOWS_DOCTOR_SCRIPT =
  '$candidates = @($env:PYTHINKER_CU_WINDOWS_EXE); ' +
  "if ($env:PYTHINKER_CU_WINDOWS_HOME) { $candidates += (Join-Path $env:PYTHINKER_CU_WINDOWS_HOME 'pythinker-cu.exe') }; " +
  "if ($env:LOCALAPPDATA) { $candidates += (Join-Path $env:LOCALAPPDATA 'PythinkerCU\\pythinker-cu.exe') }; " +
  "if ($env:ProgramFiles) { $candidates += (Join-Path $env:ProgramFiles 'PythinkerCU\\pythinker-cu.exe') }; " +
...
function macPlugin(): PluginLayerConfig {
  return {
    id: MAC_PLUGIN_ID,
    zipUrl: pythinkerCdnContentUrl('pythinker-computer-use/latest/pythinker-cu-plugin.zip'),
  };
}
...
function windowsPlugin(): PluginLayerConfig {
  return {
    ...
    zipUrl: pythinkerCdnContentUrl('pythinker-computer-use-windows/latest/pythinker-cu-win-plugin.zip'),
  };
}
...
function parseLegacyMcpFile(raw: string, appBin: string): LegacyMcpFile | undefined {
  ...
  const legacy = objectRecord(servers?.['pythinker-cu']);
...
function createMacPythinkerCuEntry(ctx: CapabilityEntryContext): CapabilityEntry {
  const applicationsDir = ctx.applicationsDir ?? '/Applications';
  const appPath = path.join(applicationsDir, APP_BUNDLE);
  const appBin = path.join(appPath, 'Contents', 'MacOS', 'pythinker-cu');
  ...
  const userMcpConfigPath = path.join(ctx.pythinkerHomeDir, 'mcp.json');
  ...
  async function removeLegacyMcpRegistration(
    ...
    delete nextServers['pythinker-cu'];
    const next = { ...legacy.value, mcpServers: nextServers };
    const mode = (await stat(userMcpConfigPath)).mode & 0o777;
    const tempPath = `${userMcpConfigPath}.pythinker-cu-migration-${process.pid}-${Date.now()}`;
    try {
  ...
  async function stopOldProcesses(): Promise<void> {
    ...
    for (const mode of ['service', 'overlay']) {
      await bestEffort('pkill', ['-f', `${APP_BUNDLE}/Contents/MacOS/pythinker-cu[[:space:]]+${mode}`]);
    }
  ...
  async function install(report: CapabilityInstallReporter): Promise<string | undefined> {
    if (!supported) {
      throw new Error(`pythinker-cu is only supported on macOS (current: ${ctx.platform})`);
    }
    ...
    if (installApp) {
      const workDir = await mkdtemp(path.join(tmpdir(), 'pythinker-cu-install-'));
      try {
        report('download', 0);
        const zipPath = path.join(workDir, 'PythinkerCU.app.zip');
        await downloadToFile(
          pythinkerCdnContentUrl('pythinker-computer-use/latest/PythinkerCU.app.zip'),
          zipPath,
          (percent) => {
        ...
        if (unzipped.code !== 0) {
          throw new Error(`Failed to unzip PythinkerCU.app: ${unzipped.stderr || unzipped.stdout}`);
        }
    ...
    if (installApp || stepStates.get('service') !== 'ok') {
      ...
      if (registered.code !== 0) {
        throw new Error(`pythinker-cu install failed: ${registered.stderr || registered.stdout}`);
      }
      await new Promise((resolve) => {
        setTimeout(resolve, 1_000);
      });
      ...
      if (!running) {
        throw new Error('pythinker-cu background service is not running after install');
      }
    }
...
function createWindowsPythinkerCuEntry(ctx: CapabilityEntryContext): CapabilityEntry {
  ...
  async function installerPowerShell(): Promise<string> {
    ...
    throw new Error(
      'Pythinker Computer Use requires Windows PowerShell 5.1 or PowerShell 7 with the commands required by its official installer. ' +
        failures.join('; '),
    );
  }
  ...
  async function install(report: CapabilityInstallReporter): Promise<string | undefined> {
    if (!supported) {
      throw new Error(
        `pythinker-cu is only supported on macOS or Windows x64 (current: ${ctx.platform}/${ctx.arch})`,
      );
    }
    ...
    if (installPowerShell !== undefined) {
      const workDir = await mkdtemp(path.join(tmpdir(), 'pythinker-cu-windows-install-'));
      try {
        const setupPath = path.join(workDir, 'setup_windows.ps1');
        report('download', 0);
        await downloadToFile(
          pythinkerCdnContentUrl('pythinker-computer-use-windows/latest/setup_windows.ps1'),
          setupPath,
          (percent) => {
            report('download', percent);
          },
        ...
        if (installed.code !== 0) {
          throw new Error(
            `pythinker-cu Windows runtime install failed: ${
      ...
      if (runtime.step.state !== 'ok') {
        throw new Error(
          `pythinker-cu Windows runtime is not ready after install: ${runtime.step.detail ?? runtime.step.state}`,
        );
      }
    }
    return undefined;
...
export function createPythinkerCuEntry(ctx: CapabilityEntryContext): CapabilityEntry {
  return ctx.platform === 'win32' ? createWindowsPythinkerCuEntry(ctx) : createMacPythinkerCuEntry(ctx);
}

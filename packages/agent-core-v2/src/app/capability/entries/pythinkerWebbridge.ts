/**
 * `pythinker-webbridge` capability entry (macOS / Linux / Windows).
 *
 * Layers: daemon binary (`~/.pythinker-webbridge/bin/`, local HTTP daemon on
 * 127.0.0.1:10086) + agent wiring (the official `pythinker-webbridge` plugin —
 ...
 *
 * A running daemon is left untouched (start-if-down only, Pythinker Work
...
function binaryAssetName(platform: NodeJS.Platform, arch: string): string | undefined {
  if (platform === 'darwin') {
    if (arch === 'arm64') return 'pythinker-webbridge-darwin-arm64';
    if (arch === 'x64') return 'pythinker-webbridge-darwin-amd64';
    return undefined;
  }
  if (platform === 'linux') {
    if (arch === 'arm64') return 'pythinker-webbridge-linux-arm64';
    if (arch === 'x64') return 'pythinker-webbridge-linux-amd64';
    return undefined;
  }
  if (platform === 'win32' && arch === 'x64') return 'pythinker-webbridge-windows-amd64.exe';
  return undefined;
}

export function createPythinkerWebbridgeEntry(ctx: CapabilityEntryContext): CapabilityEntry {
  const baseUrl = ctx.webbridgeBaseUrl ?? DEFAULT_DAEMON_BASE_URL;
  const binDir = path.join(ctx.userHomeDir, '.pythinker-webbridge', 'bin');
  const binName = ctx.platform === 'win32' ? 'pythinker-webbridge.exe' : 'pythinker-webbridge';
  const binPath = path.join(binDir, binName);
  const userSourceSkillDirs = [
    path.join(ctx.pythinkerHomeDir, 'skills', 'pythinker-webbridge'),
    path.join(ctx.userHomeDir, '.agents', 'skills', 'pythinker-webbridge'),
  ];
  ...
  async function waitForDaemon(): Promise<void> {
    ...
    throw new Error(`WebBridge daemon did not come up on ${baseUrl} — check ~/.pythinker-webbridge/logs`);
  }

  async function install(report: CapabilityInstallReporter): Promise<void> {
    const asset = binaryAssetName(ctx.platform, ctx.arch);
    if (asset === undefined) {
      throw new Error(`pythinker-webbridge is not supported on ${ctx.platform}/${ctx.arch}`);
    }
    ...
    if (status?.running !== true) {
      ...
      if (started.code !== 0) {
        throw new Error(`pythinker-webbridge start failed: ${started.stderr || started.stdout}`);
      }
      await waitForDaemon();
    }
...
export const __pythinkerWebbridgeInternals = { binaryAssetName, renameAcrossDevicesFallback };

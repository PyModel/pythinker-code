...
Help them migrate selected local Claude Code and Codex assets into Pythinker Code.
...
- Do **not** migrate `.agents` content. Pythinker Code already supports `.agents`
...
- Only write under Pythinker Code targets:
  - User-global: `$PYTHINKER_CODE_HOME` if set, otherwise `~/.pythinker-code`.
  - Project instructions/skills: `<project root>/.pythinker-code`, where the project
    root is the nearest parent directory containing `.git`; if no `.git` exists,
    use the current working directory.
  - Project-local MCP: `<cwd>/.pythinker-code/mcp.json`, because Pythinker reads the
    current working directory's Pythinker-specific MCP file, not every project-root
    `.pythinker-code/mcp.json` from subdirectories.
- Preserve existing Pythinker files. Never overwrite existing skills or replace an
...
Resolve paths explicitly; `~` is the real OS home, and Pythinker home follows
`$PYTHINKER_CODE_HOME` before `~/.pythinker-code`.
...
Pythinker-readable, and project-root `.mcp.json` is already read by Pythinker as a
...
- `$PYTHINKER_CODE_HOME/AGENTS.md`, or `~/.pythinker-code/AGENTS.md` if the env var is not
  set.

Map project-level instruction sources to:

- `<project root>/.pythinker-code/AGENTS.md`
...
- `$PYTHINKER_CODE_HOME/skills/`, or `~/.pythinker-code/skills/` if the env var is not set.

Map project-level skill sources to:

- `<project root>/.pythinker-code/skills/`
...
- Read a bundle's `SKILL.md` enough to verify that directory skills have
  frontmatter with non-empty `name` and `description`, because Pythinker requires
...
- If two source entries would write the same target path, keep the first one in
  ...
  3. user Claude
  4. user Codex
- Warn when a source skill uses Claude/Codex-specific fields or syntax that Pythinker
...
For the preview, collect MCP candidates and normalize them into Pythinker's MCP shape
when possible:
...
- Map Codex fields to Pythinker fields:
  - `command` -> `command`
  - `args` -> `args`
  - `env` -> `env`
  - `cwd` -> `cwd`
...
- Do not import project-root `.mcp.json`; Pythinker already reads it.

For each MCP candidate, choose the target scope in the preview:

- User-level source -> user-global MCP target (`$PYTHINKER_CODE_HOME/mcp.json` or
  `~/.pythinker-code/mcp.json`).
- Project-level source -> project-local Pythinker MCP target (`<cwd>/.pythinker-code/mcp.json`). If `<cwd>` is not the project root, call this out in the preview so the user understands when Pythinker will load it.
...
- collision policy: keep existing Pythinker entries on name conflict,
...
- Tell the user to start a new session (for example `/new`) or restart Pythinker Code

...
export interface DynamicWorkflowModeTransitionEvent {
...
export const telemetryEventDefinitions = {
  ...
  turn_interrupted: defineAgentTelemetryEvent<TurnInterruptedEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the most recent LLM request in this turn (the failed request when the turn errored); absent for non-Pythinker protocols',
    },
  }),
  ...
  tool_call: defineAgentTelemetryEvent<ToolCallEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the LLM request that produced this tool call; absent for non-Pythinker protocols',
    },
  }),
  api_error: defineAgentTelemetryEvent<ApiErrorEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the failed request, from its response headers or its error response; absent when the failure happened before any response headers arrived (network errors, local aborts), and for non-Pythinker protocols',
    },
  }),
  ...
  cancel: defineAgentTelemetryEvent<CancelEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the in-flight request, or of the most recent request between steps; absent for non-Pythinker protocols',
    },
  }),
  ...
  permission_approval_result: defineAgentTelemetryEvent<PermissionApprovalResultEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the LLM request that produced the gated tool call; absent for non-Pythinker protocols',
    },
  }),
  ...
  dynamic_workflow_mode_entered: defineAgentTelemetryEvent<DynamicWorkflowModeTransitionEvent>({
    owner: 'pythinker-code',
    comment: 'DynamicWorkflow mode is entered.',
    properties: {
      trigger: 'What triggered dynamic_workflow mode',
    },
  }),
  dynamic_workflow_mode_exited: defineAgentTelemetryEvent<DynamicWorkflowModeTransitionEvent>({
    owner: 'pythinker-code',
    comment: 'DynamicWorkflow mode is exited.',
    properties: {
      trigger: 'What originally triggered the dynamic_workflow mode being exited',
    },
  }),
  ...
  compaction_finished: defineAgentTelemetryEvent<CompactionFinishedEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the final compaction request round; absent for non-Pythinker protocols',
    },
  }),
  compaction_failed: defineAgentTelemetryEvent<CompactionFailedEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the failed compaction request, from its response headers or its error response; absent when the failure happened before any request or before response headers arrived (network errors), and for non-Pythinker protocols',
    },
  }),
  ...
  question_dismissed: defineAgentTelemetryEvent<QuestionDismissedEvent>({
    ...
    properties: {
      trace_id:
        'Trace id of the LLM request that produced the questioning tool call; absent for non-Pythinker protocols',
    },
  }),
  ...
  tool_call_dedup_detected: defineAgentTelemetryEvent<ToolCallDedupDetectedEvent>({
    ...
    properties: {
      ...
      trace_id:
        'Trace id of the LLM request that produced the duplicate tool call; absent for non-Pythinker protocols',
    },
  }),

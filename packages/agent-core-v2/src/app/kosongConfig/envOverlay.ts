/**
 * `kosongConfig` domain — `PYTHINKER_MODEL_*` effective-config overlay.
 *
 * When `PYTHINKER_MODEL_NAME` is set, synthesizes one model id (bound to the
 * reserved `__pythinker_env__` provider whose schema kosong owns) from the
 * `PYTHINKER_MODEL_*` environment variables and overlays it onto the resolved
 ...
 * provider-definition registry, not from a hardcoded vendor table — for Pythinker
 * that is the `PYTHINKER_BASE_URL` → `https://api.moonshot.ai/v1` chain declared
...
export const pythinkerModelEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    const model = trimmed(getEnv('PYTHINKER_MODEL_NAME'));
    const temperature = parseFloatEnv(
      getEnv('PYTHINKER_MODEL_TEMPERATURE'),
      'PYTHINKER_MODEL_TEMPERATURE',
    );
    const topP = parseFloatEnv(getEnv('PYTHINKER_MODEL_TOP_P'), 'PYTHINKER_MODEL_TOP_P');
    const thinkingKeep = trimmed(getEnv('PYTHINKER_MODEL_THINKING_KEEP'));
    const maxCompletionTokens =
      parseCompletionTokens(getEnv('PYTHINKER_MODEL_MAX_COMPLETION_TOKENS')) ??
      parseCompletionTokens(getEnv('PYTHINKER_MODEL_MAX_TOKENS'));
    ...
    const maxContextRaw = trimmed(getEnv('PYTHINKER_MODEL_MAX_CONTEXT_SIZE'));
    const maxContextSize =
      maxContextRaw === undefined
        ? DEFAULT_MAX_CONTEXT_SIZE
        : parsePositiveInt(maxContextRaw, 'PYTHINKER_MODEL_MAX_CONTEXT_SIZE');

    const maxOutputRaw = trimmed(getEnv('PYTHINKER_MODEL_MAX_OUTPUT_SIZE'));
    const maxOutputSize =
      maxOutputRaw === undefined
        ? undefined
        : parsePositiveInt(maxOutputRaw, 'PYTHINKER_MODEL_MAX_OUTPUT_SIZE');
    const capabilities = parseCapabilities(getEnv('PYTHINKER_MODEL_CAPABILITIES')) ?? DEFAULT_CAPABILITIES;
    const displayName = trimmed(getEnv('PYTHINKER_MODEL_DISPLAY_NAME'));
    const reasoningKey = trimmed(getEnv('PYTHINKER_MODEL_REASONING_KEY'));
    const adaptiveThinking = parseBooleanVar(
      getEnv('PYTHINKER_MODEL_ADAPTIVE_THINKING'),
...
registerConfigOverlay(pythinkerModelEnvOverlay);

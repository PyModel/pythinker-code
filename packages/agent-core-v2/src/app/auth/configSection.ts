 ...
 * env bindings.
 *
 * Owns the `[services]` configuration section (`pymodel_search` /
 * `pymodel_fetch`), mirroring v1's `ServicesConfigSchema`: the schema, and the
 ...
 * (`PYTHINKER_WEB_SEARCH_*` / `PYTHINKER_WEB_FETCH_*`, env wins over the file). Its
 ...
 * consumes `pymodel_search`; the `web` domain reads `pymodel_fetch` from the
...
export const PyModelServiceConfigSchema = z.object({
...
export type PyModelServiceConfig = z.infer<typeof PyModelServiceConfigSchema>;

export const ServicesConfigSchema = z
  .object({
    pymodelSearch: PyModelServiceConfigSchema.optional(),
    pymodelFetch: PyModelServiceConfigSchema.optional(),
  })
  .passthrough();

export type ServicesConfig = z.infer<typeof ServicesConfigSchema>;

export const WEB_SEARCH_BASE_URL_ENV = 'PYTHINKER_WEB_SEARCH_BASE_URL';
export const WEB_SEARCH_API_KEY_ENV = 'PYTHINKER_WEB_SEARCH_API_KEY';
export const WEB_FETCH_BASE_URL_ENV = 'PYTHINKER_WEB_FETCH_BASE_URL';
export const WEB_FETCH_API_KEY_ENV = 'PYTHINKER_WEB_FETCH_API_KEY';
...
const pymodelSearchEnvBindings = envBindings(PyModelServiceConfigSchema, {
  baseUrl: { env: WEB_SEARCH_BASE_URL_ENV, parse: nonBlankEnv },
  apiKey: { env: WEB_SEARCH_API_KEY_ENV, parse: nonBlankEnv },
});

const pymodelFetchEnvBindings = envBindings(PyModelServiceConfigSchema, {
...
export const servicesEnvBindings: EnvBindings<ServicesConfig> = envBindings(
  ...
  {
    pymodelSearch: pymodelSearchEnvBindings,
    pymodelFetch: pymodelFetchEnvBindings,
  },
);
...
const servicesCredentialEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    ...
    const pymodelSearch = isolateEnvServiceCredentials(
      services['pymodelSearch'],
      getEnv,
      WEB_SEARCH_BASE_URL_ENV,
      WEB_SEARCH_API_KEY_ENV,
    );
    const pymodelFetch = isolateEnvServiceCredentials(
      services['pymodelFetch'],
      getEnv,
      WEB_FETCH_BASE_URL_ENV,
      WEB_FETCH_API_KEY_ENV,
    );
    if (
      pymodelSearch === services['pymodelSearch'] &&
      pymodelFetch === services['pymodelFetch']
    ) {
      return [];
    }
...
const stripPyModelSearchEnv = stripEnvBoundFields(pymodelSearchEnvBindings);
const stripPyModelFetchEnv = stripEnvBoundFields(pymodelFetchEnvBindings);
...
export const stripServicesEnv: ConfigStripEnv<ServicesConfig> = (value, raw, getEnv) => {
  ...
  for (const [key, strip] of [
    ['pymodelSearch', stripPyModelSearchEnv],
    ['pymodelFetch', stripPyModelFetchEnv],
  ] as const) {
...
export const servicesToToml = (value: unknown, rawSnake: unknown): unknown => {
  ...
  writeService(out, 'pymodel_search', value['pymodelSearch']);
  writeService(out, 'pymodel_fetch', value['pymodelFetch']);
  return out;
};

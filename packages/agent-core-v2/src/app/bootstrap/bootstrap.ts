...
export interface HostArgs {
   ...
   * `X-Msh-*`, built by the host through `createPythinkerDefaultHeaders`).
...
export interface IBootstrapOptions {
  ...
  readonly clientIdentity: PythinkerHostIdentity;
...
export function resolveBootstrapOptions(input: BootstrapInput): IBootstrapOptions {
  ...
  const homeDir = resolvePythinkerHome(input.homeDir, env, osHomeDir);
...
export function resolvePythinkerHome(
  homeDir?: string,
  env: NodeJS.ProcessEnv = process.env,
  osHomeDir: string = homedir(),
): string {
  return homeDir ?? env['PYTHINKER_CODE_HOME'] ?? join(osHomeDir, '.pythinker-code');
}

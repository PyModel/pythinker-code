...
2. **策略异质**。有的查工具名集合，有的数同批 AgentDynamicWorkflow 个数，有的跑 hook，有的检查 plan 状态机——没有统一的 `(sub, obj, act)` 形状。
...
| 2 | 工具批量排他 | `agent-dynamic-workflow-exclusive-deny`、`dynamic-workflow-mode-agent-dynamic-workflow-approve` | 同批工具结构（AgentDynamicWorkflow 须单独）+ dynamic_workflow 模式 |
...
| 工具批量排他 | `dynamic_workflow` domain —— `onBeforeExecuteTool` veto 监听器 | Harness 约束（链外） |
...
1. ~~**Domain 维度下沉**~~（已完成）。plan guard/review、goal-start review、dynamic_workflow 批量排他、btw deny-all 已从链上移出，以 `onBeforeExecuteTool` veto 监听器挂在各自 domain（即时 `veto`/`allow`/`pass` 表态 + cold `waitUntil` factory 承载审批往返）；审批往返提取为共享的 `IAgentToolApprovalService`；`registerPolicy` 机制删除（btw 是唯一生产用例）。链上只剩 12 个危险度判定节点。

...
1. L1 master env `PYTHINKER_CODE_EXPERIMENTAL_FLAG` truthy → every flag on.
2. L2 per-feature `def.env` (e.g. `PYTHINKER_CODE_EXPERIMENTAL_MY_FEATURE`) → forces on/off.

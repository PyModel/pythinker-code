...
1. L1 per-feature `def.env` (e.g. `PYTHINKER_CODE_EXPERIMENTAL_MY_FEATURE`) → forces on/off.
2. L2 `[experimental]` config section per-flag override.
3. L3 master env `PYTHINKER_CODE_EXPERIMENTAL_FLAG` truthy → every flag on.

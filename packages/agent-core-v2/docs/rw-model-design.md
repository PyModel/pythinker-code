...
> agent-core-v2 / server-v2 / TUI（apps/pythinker-code）三方现状的完整调研，
> 所有断言均有 file:line 证据。
>
...
**读路径**
- R1 手写读模型 ~12 处（goal/usage/plan/dynamic_workflow/permission*/turn/task/todo…），
...
- V1 `task.started/terminated` 在 WireRecordMap 和 AgentEvent 双注册，写路径
  ...
  permissionMode 一处使用。
- V2 `agent.status.updated` 是"多域共写的散装快照事件"：plan/dynamic_workflow/usage/
  contextSize/profile 各自手动拼不同字段。
- V3 resume 期 signal 靠 `emitLive` 隐式压制（skill/dynamic_workflow）——"这个 signal 发不发
...
1. **提交队列**：`commit()` 同步折叠所有 view，但 **onChange 通知入队**，
   ...
   equals 去重）。
2. **Effect 注册制**：订阅者回写（L1 的 goal 续跑、dynamic_workflow 自动退出、steer
...
```ts
// dynamic_workflow 自动退出：今天挂在 turn.hooks.onEnded 里直接写（L1）
export class AgentDynamicWorkflowService extends Disposable {
  constructor(@IAgentStream private readonly stream: IAgentStream) {
    super();
    this._register(stream.defineEffect('dynamic-workflow-auto-exit', {

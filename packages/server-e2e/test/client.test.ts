 ...
 * `process.env.PYTHINKER_SERVER_URL ?? http://127.0.0.1:58627`.
 *
...
const BASE_URL = process.env['PYTHINKER_SERVER_URL'] ?? 'http://127.0.0.1:58627';
...
describeLive('DaemonClient (live server required)', () => {
  ...
  it('completes handshake (server_hello + client_hello ack)', async () => {
    ...
    expect(hello.protocol_version).toBe(WS_PROTOCOL_VERSION);
    await client.close();
    log('closed');
  });
...
describe('DaemonClient session action helpers', () => {
  ...
  it('model catalog helpers call the catalog and action-suffix routes', async () => {
    ...
    const calls: FetchCall[] = [];
    const model = testModel({ model: 'pythinker-code/pythinker-for-coding' });
    ...
    await expect(client.getProvider('pythinker')).resolves.toEqual(provider);

    log('fetch calls', calls);
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      ['GET', 'http://server.example.test/api/v1/auth'],
      ['GET', 'http://server.example.test/api/v1/models'],
      ['POST', 'http://server.example.test/api/v1/models/pythinker-code%2Fpythinker-for-coding:set_default'],

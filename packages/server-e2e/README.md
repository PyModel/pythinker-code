...
`PYTHINKER_SERVER_E2E_REPORT_DIR`). The report groups events by case and shows a compact
...
`docker:e2e` builds `pythinker-server:dev` from the root `Dockerfile`, layers
...
`~/.pythinker-code-server-dev/server-e2e-reports/docker/<run-id>/latest/index.html`;
the server log is written beside them as `server.log`.

The Docker workflow uses an isolated PYTHINKER home at
`~/.pythinker-code-server-dev/docker-e2e/<run-id>/pythinker-code-home` to avoid sharing
...
`~/.pythinker-code-server-dev/pythinker-home/pythinker-code-home` when those files exist.
Override the namespace with `PYTHINKER_SERVER_E2E_RUN_ID`, or override paths with
`PYTHINKER_SERVER_E2E_STATE_ROOT`, `PYTHINKER_SERVER_E2E_PYTHINKER_HOME_HOST`,
`PYTHINKER_SERVER_E2E_SEED_PYTHINKER_HOME_HOST`, or `PYTHINKER_SERVER_E2E_REPORT_DIR_HOST`.

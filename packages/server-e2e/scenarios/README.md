...
pnpm --filter @pythoughts/server-e2e test:scenarios

# Custom server URL
PYTHINKER_SERVER_URL=http://127.0.0.1:8080 npx tsx packages/server-e2e/scenarios/02-tool-call-with-approval.ts
```
...
WebSocket frames. Set `PYTHINKER_SERVER_E2E_REPORT_DIR=/tmp/server-e2e-report` to write it
somewhere else.

 ...
 *     with `accepted_subscriptions: []`, `resync_required: []`, and NOT
 ...
 *     durable event (seq 1..N) AFTER the ack lands.
 ...
 *
 * Usage:
 *   PYTHINKER_SERVER_URL=http://127.0.0.1:58627 npx tsx scenarios/03-refresh-replay.ts
 *
 * Exit codes:
 *   0  — pass
...
const PYTHINKER_SERVER_URL = process.env['PYTHINKER_SERVER_URL'] ?? 'http://127.0.0.1:58627';
...
async function openSocketWithHello({
  ...
  const wsUrl = `${PYTHINKER_SERVER_URL.replace(/^http/, 'ws')}${API_PREFIX}/ws`;
  const ws = new WsClient({ url: wsUrl, wsImpl: WsWebSocket, logger: () => {} });
  await ws.open();

  // Capture every frame on its arrival timeline (queue order). Used to
  // distinguish replay events (arrive AFTER ack) from steady-state events.
  const log: AnyFrame[] = [];
  ws.onFrame((f) => log.push(f));

  // 1) server_hello
  const helloFrame = await ws.waitForFrame((f) => f.type === 'server_hello', HANDSHAKE_TIMEOUT_MS);
  const serverHello = helloFrame.payload as { protocol_version?: unknown };
  assert.equal(serverHello.protocol_version, WS_PROTOCOL_VERSION, 'unsupported server protocol version');
  ...
  if (lastSeq === 0) {
    await ws.waitForFrame(
      (f) => typeof f.seq === 'number' && f.session_id === sid && f.seq > 0,
      HANDSHAKE_TIMEOUT_MS,
    );
  }

  // Replay events arrive after the ack. Anything appended later is steady state.
...
async function main() {
  console.log(`▶ server at ${PYTHINKER_SERVER_URL}`);

  // ── Phase 0 ─────────────────────────────────────────────────────────────
  const health = await fetchEnvelope<{ ok: boolean }>(`${PYTHINKER_SERVER_URL}${API_PREFIX}/healthz`);
  assert.equal(health.ok, true, 'healthz did not return ok=true');

  const meta = await fetchEnvelope<MetaResponse>(`${PYTHINKER_SERVER_URL}${API_PREFIX}/meta`);
  ...
  const auth = await fetchEnvelope<{ ready: boolean; providers_count: number }>(
    `${PYTHINKER_SERVER_URL}${API_PREFIX}/auth`,
  );
  ...
  const initial = new DaemonClient({ baseUrl: PYTHINKER_SERVER_URL });
  ...
  try {
    ...
    const meta2 = await fetchEnvelope<MetaResponse>(`${PYTHINKER_SERVER_URL}${API_PREFIX}/meta`);
    ...
    assert.deepEqual(
      ...
      [],
      'expected an empty client_hello acknowledgement',
    );
    assert.deepEqual(
    ...
    console.log('▶ refresh #1: caught-up; hello acknowledged, replayed=0');
    ...
    const tasks = await fetchEnvelope<{ items: unknown[] }>(
      `${PYTHINKER_SERVER_URL}${API_PREFIX}/sessions/${encodeURIComponent(sid)}/tasks`,
    );

...
async function tempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'pythinker-telemetry-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('telemetry bootstrap', () => {
  it('matches the PYTHINKER_DISABLE_TELEMETRY true-value semantics', () => {
    expect(isTelemetryDisabledByEnv({ PYTHINKER_DISABLE_TELEMETRY: '1' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ PYTHINKER_DISABLE_TELEMETRY: 'yes' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ PYTHINKER_DISABLE_TELEMETRY: '0' })).toBe(false);
    expect(isTelemetryDisabledByEnv({ PYTHINKER_DISABLE_TELEMETRY: 'false' })).toBe(false);
  });

  it('disables the singleton without attaching a sink when opted out', async () => {
    const fetchImpl = vi.fn(async () => new Response('', { status: 200 }));
    vi.stubGlobal('fetch', fetchImpl);
    const saved = process.env['PYTHINKER_DISABLE_TELEMETRY'];
    try {
      process.env['PYTHINKER_DISABLE_TELEMETRY'] = 'true';
      initializeTelemetry({
        homeDir: await tempHome(),
        deviceId: 'dev',
        appName: 'pythinker-code-cli',
      ...
      if (saved === undefined) delete process.env['PYTHINKER_DISABLE_TELEMETRY'];
      else process.env['PYTHINKER_DISABLE_TELEMETRY'] = saved;
    }
  ...
  it('resumes intake after a paused start without re-initialization', async () => {
    ...
    const init = requestInitFrom(fetchImpl);
    const payload = JSON.parse(init.body as string) as { events: Array<{ event: string }> };
    expect(payload.events[0]?.['event']).toBe('pfc_sent');
  });

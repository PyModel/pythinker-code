...
import { planKey } from '@pymodel/agent-core-v2/features/plan/planOps';
import { dynamicWorkflowKey } from '@pymodel/agent-core-v2/features/dynamic_workflow/dynamicWorkflowOps';
...
export class SessionProjection {
  ...
  private subscribeAgent(handle: IAgentScopeHandle): void {
    ...
    const dynamicWorkflowMode =
      agentState?.has(dynamicWorkflowKey) === true ? agentState.get(dynamicWorkflowKey) !== null : undefined;
    projector.seedModes({ planMode, dynamicWorkflowMode });
    ...
    if (agentId === MAIN_AGENT_ID) {
      this.seedMainAgent(handle, disposables, { planMode, dynamicWorkflowMode });
    }
  ...
  private seedMainAgent(
    ...
    modes: { planMode?: boolean; dynamicWorkflowMode?: boolean },
  ): void {
    this.aggregator.feedMainStatus({ planMode: modes.planMode, dynamicWorkflowMode: modes.dynamicWorkflowMode });

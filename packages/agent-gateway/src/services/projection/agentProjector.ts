...
export class AgentMessageProjector {
  ...
  private dynamicWorkflowMode = false;
  ...
  seedModes(modes: { planMode?: boolean; dynamicWorkflowMode?: boolean }): void {
    if (modes.planMode !== undefined) this.planMode = modes.planMode;
    if (modes.dynamicWorkflowMode !== undefined) this.dynamicWorkflowMode = modes.dynamicWorkflowMode;
  }
  ...
  private onAgentStatusUpdated(event: {
    ...
    if (event.dynamicWorkflowMode !== undefined && event.dynamicWorkflowMode !== this.dynamicWorkflowMode) {
      this.dynamicWorkflowMode = event.dynamicWorkflowMode;
      ops.push(
        this.systemOp(event.dynamicWorkflowMode ? 'dynamic_workflow.enter' : 'dynamic_workflow.exit', undefined, event.time),
      );
    }
    return ops;
  }

...
const CATALOG = {
  ...
  plugins: [
    ...
    },
    {
      ...
      source: 'https://cdn.example.test/pythinker-cu.zip',
    },
    {
...
describe('server-v2 /api/v1 plugins', () => {
  ...
  async function makePluginDir(id: string, version: string): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), `pythinker-test-plugin-${id}-`));
  ...
  it('maps client-fixable install input errors to 4xx, never 50001', async () => {
    ...
    const noManifest = await mkdtemp(join(tmpdir(), 'pythinker-no-manifest-'));
  ...
  it('serves the marketplace catalog merged with live install state', async () => {
    ...
    expect(before.body.data.entries.map((e) => [e.id, e.tier])).toEqual([
      ...
      ['pythinker-webbridge', 'third-party'],
      ['pythinker-cu', 'third-party'],
    ...
    expect(
      before.body.data.entries.find((e) => e.id === 'pythinker-webbridge')?.capabilityId,
    ).toBeUndefined();
  ...
  it('treats the dev marketplace server as the default catalog', async () => {
    await server?.close();
    vi.stubEnv('PYTHINKER_CODE_PLUGIN_MARKETPLACE_URL', CATALOG_URL);
    vi.stubEnv('PYTHINKER_CODE_PLUGIN_MARKETPLACE_FROM_DEV_SERVER', '1');
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
  ...
  it('reads a local marketplace catalog from disk (plain path or file://)', async () => {
    await server?.close();
    const catalogDir = await mkdtemp(join(tmpdir(), 'pythinker-local-catalog-'));
  ...
  it('expands ~ in local catalog paths like the CLI loader', async () => {
    await server?.close();
    const fakeHome = await mkdtemp(join(tmpdir(), 'pythinker-tilde-home-'));

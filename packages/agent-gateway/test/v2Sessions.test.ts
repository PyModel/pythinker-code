...
import {
  ...
  type FsPullRequest,
  IGitService,
} from '@pymodel/agent-core-v2/app/git/git';
...
describe('server /api/v2/sessions', () => {
  ...
  beforeEach(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-sessions-list-'));
  ...
  it('exposes the live session model in the activity domain', async () => {
    ...
    const page = await getData();
    expect(page.items.find((entry) => entry.id === id)?.activity).toEqual({
      status: 'idle',
      model: 'stub',
    });
  });
...
describe('server /api/v2/sessions batch archive/restore', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'pythinker-server-v2-sessions-batch-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,

...
import * as agentCore from '@pymodel/agent-core-v2';
import type { Scope } from '@pymodel/agent-core-v2';
...
describe('WsConnectionV1 global target registration', () => {
  ...
  it('opts only pythinker-inspect connections into the event.di.* debug feed on client_hello', async () => {
    const socket = new FakeSocket();
    ...
    socket.emit(
      'message',
      JSON.stringify({ type: 'client_hello', id: 'h1', payload: { client_id: 'pythinker-web' } }),
    );
    ...
    socket.emit(
      ...
      JSON.stringify({
        type: 'client_hello',
        id: 'h2',
        payload: { client_id: 'pythinker-inspect' },
      }),
    );

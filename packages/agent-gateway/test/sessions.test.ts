...
import { ISessionMediaStore } from '@pymodel/agent-core-v2/agent/media/sessionMediaStore';
import { mcpResultToExecutableOutput } from '@pymodel/agent-core-v2/agent/mcp/output';
import { renderToolResultForModel } from '@pymodel/agent-core-v2/agent/contextMemory/toolResultRender';
import { IReadTool, ReadInputSchema, type ReadInput } from '@pymodel/agent-core-v2/agent/tools/os/read/read';
...
import { SessionMetaUpdated } from '@pymodel/agent-core-v2/session/sessionMetadata/sessionMetaEvents';
import { TurnSteer } from '@pymodel/agent-core-v2/agent/loop/turnOps';
...
describe('server-v2 /api/v1/sessions', () => {
  ...
  it('downloads a ZIP with the supplied Web log and cleans up its temporary directory', async () => {
    ...
    expect(res.headers.get('content-disposition')).toBe(
      `attachment; filename="pythinker-session-${id}.zip"`,
    );
    ...
    expect(entries.get('logs/pythinker-web.jsonl')?.toString('utf8')).toBe(webLog);
    expect(manifest).toMatchObject({
      sessionId: id,
      pythinkerCodeVersion: TEST_HOST_IDENTITY.version,
      webLogPath: 'logs/pythinker-web.jsonl',
    });
  ...
  it('bundles the on-disk desktop app log when the desktop flag is set', async () => {
    ...
    await writeFile(
      join(home as string, 'logs', 'pythinker-code-desktop.log'),
    ...
    expect(entries.get('logs/pythinker-desktop.log')?.toString('utf8')).toBe(
      '2026-07-27T00:00:00.000Z INFO  [renderer] hello\n',
    );
    expect(manifest.desktopLogPath).toBe('logs/pythinker-desktop.log');
    expect(manifest.pythinkerCodeVersion).toBe(TEST_HOST_IDENTITY.version);
  ...
  it('keeps title generation unavailable without a title backend', async () => {
    const created = await postJson<SessionWire>('/api/v1/sessions', {
      metadata: { cwd: home as string },
    });
    const id = created.body.data.id;
  ...
  it('reflects plan/dynamic_workflow/permission agent_config in GET /status', async () => {
    const cwd = home as string;
    const created = await postJson<SessionWire>('/api/v1/sessions', { metadata: { cwd } });
    const id = created.body.data.id;
    ...
    expect(before.body.data.dynamic_workflow_mode).toBe(false);

    await postJson(`/api/v1/sessions/${id}/profile`, {
      agent_config: { plan_mode: true, dynamic_workflow_mode: true, permission_mode: 'yolo' },
    });
    ...
    expect(after.body.data.dynamic_workflow_mode).toBe(true);
  ...
  it('rejects tower_mode agent_config when the tower feature is unavailable', async () => {
    ...
    expect(on.body.msg).toContain('PYTHINKER_CODE_EXPERIMENTAL_TOWER=1');
    const after = await getJson<{
      tower_mode?: boolean;
    }>(`/api/v1/sessions/${id}/status`);
...
async function listExportTempDirs(sessionId: string): Promise<string[]> {
  const prefix = `pythinker-session-export-${sessionId}-`;

...
import { IModelCatalog } from '@pymodel/agent-core-v2';
import { createActor, setup } from '@pymodel/agent-core-v2/human/xstate2';
...
describe('WS upgrade auth', () => {
  ...
  describe('/api/v1/ws', () => {
    ...
    it('accepts a valid bearer subprotocol and echoes it', async () => {
      const { ws, firstFrame } = await openConn(url(), {
        protocols: [`pythinker-code.bearer.${token()}`],
      });
      sockets.push(ws);
      expect(ws.protocol).toBe(`pythinker-code.bearer.${token()}`);
      expect(firstFrame).toMatchObject({ type: firstType });
    });

    it('rejects a wrong bearer token', async () => {
      await expectRejected(url(), { protocols: ['pythinker-code.bearer.wrong'] });
    });
  ...
  describe('/api/v1/debug/ws', () => {
    it('streams xstate inspection envelopes to an authorized client', async () => {
      const home = await mkdtemp(join(tmpdir(), 'pythinker-kap-debug-ws-'));
  ...
  it('rejects upgrades to a non-WS path', async () => {
    ...
    await expectRejected(badUrl, { protocols: [`pythinker-code.bearer.${token()}`] });
    const debugUrl = `${v1Url().replace('/api/v1/ws', '/api/v1/debug/ws')}`;
    await expectRejected(debugUrl, { protocols: [`pythinker-code.bearer.${token()}`] });
  });
});

...
describe('server-v2 /api/v1 skills', () => {
  ...
  async function seedProjectSkill(root: string, name: string): Promise<void> {
    const dir = join(root, '.pythinker-code', 'skills', name);
    await mkdir(dir, { recursive: true });
    await writeFile(
      join(dir, 'SKILL.md'),
  ...
  describe('GET /api/v1/sessions/{sid}/skills', () => {
    ...
    it('lists the check-pythinker-code-docs builtin skill', async () => {
      const id = await createSession();
      const { body } = await getJson<{ skills: SkillWire[] }>(
        `/api/v1/sessions/${id}/skills`,
      );
      ...
      const docsSkill = skills.find((s) => s.name === 'check-pythinker-code-docs');
  ...
  describe('POST /api/v1/sessions/{sid}/skills/{name}:activate', () => {
    ...
    it('retains rich client metadata and ordered context through a single activation and cold resume', async () => {
      const id = await createSession();
      await createMainAgent(id);
      const metadata = { display_text: 'Example skill · Save button', pythinker_code_composer: { version: 1, doc: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'client-only literal' }] }] }, attachments: [], attachmentOrder: [] } };

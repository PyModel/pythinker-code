...
import { Event2 } from '@pymodel/agent-core-v2/app/event/event2';
import {
  AgentEventBusView,
  EventBusService,
} from '@pymodel/agent-core-v2/app/event/eventBusService';
...
describe('SessionEventBroadcaster', () => {
  ...
  it.each(['prompt.steered', 'prompt.queued', 'prompt.submitted'])(
    ...
    async (type) => {
      ...
      expect(JSON.stringify(live!.payload)).not.toContain('pythinker-file://');
  ...
  describe('global fan-out to unsubscribed connections', () => {
    ...
    it('fans out event.plugin.changed and event.capability.changed to global targets', async () => {
      ...
      eventBus.emit({
        ...
        payload: {
          capability_id: 'pythinker-webbridge',
    ...
    it('drops malformed event.capability.changed payloads', async () => {
      ...
      eventBus.emit({
        type: 'event.capability.changed',
        payload: { capability_id: 'pythinker-cu' },
      });

      eventBus.emit({
        type: 'event.capability.changed',
        payload: { capability_id: 'pythinker-cu', install: { running: false } },
      });

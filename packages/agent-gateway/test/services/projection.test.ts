...
describe('SessionProjection', () => {
  ...
  interface FakeAgent {
    ...
    planActive: boolean;
    dynamicWorkflowTrigger: string | null;
      ...
      accessor: {
        get: (token: unknown) => {
          ...
          if (token === IAgentStateService) {
            return {
              has: (key: { name: string }) => key.name === 'plan' || key.name === 'dynamic_workflow',
              get: (key: { name: string }) =>
                key.name === 'plan' ? { active: agent.planActive } : agent.dynamicWorkflowTrigger,
            };
          }
  ...
  it('streams validated timeline messages and session.state through one sequence', async () => {
    ...
    const dynamicWorkflowChild = makeAgent('agent-2');
    const { projection, received, logger } = makeProjection(agent, child, dynamicWorkflowChild);
    ...
    const dynamicWorkflowState = ofType(received, 'agent.state')
      .filter((m) => m.agent_id === 'agent-2')
      .at(-1)!;
    expect(dynamicWorkflowState).toMatchObject({
      origin: { kind: 'tool-dynamic_workflow', tool_call_id: 'call_b', dynamic_workflow_index: 1, parent_agent_id: 'main' },
      status: 'idle',
    });
  ...
  it('seeds plan and dynamic_workflow modes from agent state at bind without re-emitting enter', () => {
    const agent = makeAgent('main');
    agent.planActive = true;
    agent.dynamicWorkflowTrigger = 'tool';
    const { projection, received } = makeProjection(agent);
    const recovery = projection.recoveryMessages();
    const state = ofType(recovery, 'session.state')[0]!;
    expect(state.modes).toEqual({ plan: {}, dynamic_workflow: {} });
    expect(
      ofType(received, 'system').filter(
        (m) => m.subtype === 'plan.enter' || m.subtype === 'dynamic_workflow.enter',
      ),
    ).toHaveLength(0);

    agent.bus.emit(ev({ type: 'agent.status.updated', agentId: 'main', planMode: false, dynamicWorkflowMode: false }) as Event2<any>);
    const subtypes = ofType(received, 'system').map((m) => m.subtype);
    expect(subtypes).not.toContain('plan.exit');
    expect(subtypes).toContain('dynamic_workflow.exit');

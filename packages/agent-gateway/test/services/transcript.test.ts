...
describe('AgentTranscriptProjector', () => {
  ...
  it('preserves queue metadata through prompt lifecycle updates', () => {
    ...
    const tx = new AgentTranscript('main');
    const feed = (event: ProjectorBusEvent): void => void tx.apply(projector.map(event));
    const metadata = [{ display_text: 'Save button', pythinker_code_composer: { version: 1, doc: { type: 'doc' } } }];
  ...
  it('restores interrupted dynamic_workflow members and exact call associations without starting a session', async () => {
    const home = await mkdtemp(join(tmpdir(), 'transcript-cold-dynamic-workflow-'));
    try {
      const wireDir = join(home, 'sessions', 'ws', 's1', 'agents', 'main');
      await mkdir(wireDir, { recursive: true });
      const records = [
        ...
        { type: 'context.append_message', message: { role: 'assistant', content: [], toolCalls: [
          { type: 'function', id: 'dynamic-workflow-a', name: 'AgentDynamicWorkflow', arguments: '{}' },
          { type: 'function', id: 'dynamic-workflow-b', name: 'AgentDynamicWorkflow', arguments: '{}' },
        ] }, time: 2000 },
        { type: 'subagent.spawned', subagentId: 'child-b', subagentName: 'explore', parentAgentId: 'main', parentToolCallId: 'dynamic-workflow-a', dynamicWorkflowIndex: 1, runInBackground: false, description: 'second', time: 3000 },
        { type: 'subagent.spawned', subagentId: 'child-a', subagentName: 'explore', parentAgentId: 'main', parentToolCallId: 'dynamic-workflow-a', dynamicWorkflowIndex: 0, runInBackground: false, description: 'first', time: 3100 },
        { type: 'subagent.spawned', subagentId: 'child-c', subagentName: 'explore', parentAgentId: 'main', parentToolCallId: 'dynamic-workflow-b', dynamicWorkflowIndex: 0, runInBackground: false, time: 3200 },
      ...
      expect(frames).toEqual(expect.arrayContaining([
        expect.objectContaining({ toolCallId: 'dynamic-workflow-a', agentRefs: [{ agentId: 'child-a', role: 'member' }, { agentId: 'child-b', role: 'member' }] }),
        expect.objectContaining({ toolCallId: 'dynamic-workflow-b', agentRefs: [{ agentId: 'child-c', role: 'member' }] }),
      ]));
  ...
  it('loses a member whose spawning turn ended even while the parent runs a later turn', async () => {
    ...
    try {
      ...
      const records = [
        ...
        { type: 'subagent.spawned', subagentId: 'child-a', subagentName: 'explore', parentAgentId: 'main', parentToolCallId: 'dynamic-workflow-a', dynamicWorkflowIndex: 0, runInBackground: false, time: 3000 },

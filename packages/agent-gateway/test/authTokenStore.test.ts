...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'pythinker-server-v2-auth-token-'));
});
...
describe('password', () => {
  it('resolvePasswordHash returns undefined when env is unset or empty', async () => {
    ...
    expect(await resolvePasswordHash({ PYTHINKER_CODE_PASSWORD: '' })).toBeUndefined();
  });

  it('hashes a set password with bcrypt and verifies correctly', async () => {
    const passwordHash = await resolvePasswordHash({
      PYTHINKER_CODE_PASSWORD: 'correct-horse-battery-staple',
    });

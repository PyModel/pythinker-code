...
async function hasUsableConfiguredDefaultModel(harness: PythinkerHarness): Promise<boolean> {
  if (typeof harness.getConfig !== 'function') return false;
  let config: PythinkerConfig;
  try {
...
function providerHasNonOAuthCredentials(provider: ProviderConfig): boolean {
  ...
  switch (provider.type) {
    ...
    case 'pythinker':
      return hasProviderValue(provider, 'PYTHINKER_API_KEY');
    case 'google-genai':
 ...
 * into {@link PythinkerHarness}; refuses methods that are not yet wired with a
...
export class AcpServer implements Agent {
  ...
  constructor(
    ...
    opts?: {
       ...
       * exploration of `~/.pythinker-code/skills/`.
       */
  ...
  async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    // `packages/node-sdk/src/pythinker-harness.ts:createSession` and
    ...
    // `docs/{zh,en}/reference/pythinker-acp.md` and the changeset for the
   ...
   * {@link PythinkerHarness.listSessions} (optionally filtered by `cwd` —
   ...
   * the pythinker-code app's registry. Phase 11 keeps it as a no-op stub.
   */
  async extMethod(
    method: string,
   ...
   * `PythinkerHarness` shapes that only stub `auth.status` + `createSession`.
 ...
 * {@link PythinkerHarness.close} so an editor terminating the agent process
...
/**
 * Project a Pythinker SDK {@link SessionSummary} into the ACP

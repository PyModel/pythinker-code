...
//      pythinker-cli `acp/server.py:77-96`.
//
 ...
 *
 * Optional inputs:
 *  - `env`: extra env vars forwarded to the spawned `pythinker login`
 *    subprocess (e.g. `{ PYTHINKER_CODE_HOME: '/tmp/sandbox' }` for tests).
  ...
  if (opts.legacyCommand !== undefined && opts.legacyCommand.length > 0) {
    (method as AuthMethod & { _meta: { 'terminal-auth': unknown } })._meta = {
      'terminal-auth': {
        ...
        // `<command> login` runs the top-level `pythinker login` subcommand,
        // skipping the `acp` subprocess entirely. Same behaviour the
        // `pythinker-cli` Python reference advertises.
        args: ['login'],
        env: { ...env },
      },
    };

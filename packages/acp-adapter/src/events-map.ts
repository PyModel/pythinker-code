...
/**
 * Heuristic map from a Pythinker tool's `name` to ACP {@link ToolKind}.
 *
...
/**
 * Translate the pythinker-code TodoList display block into an ACP `plan`
 * session update.
 *
 ...
 *   - We default `priority` to `'medium'` because the pythinker-code
 ...
 * the eventual `plan_removed` story (deferred until pythinker-code grows
 * a clear-plan signal).
 */
 ...
 *
 * The pythinker-code TodoList tool publishes both a structured display
 ...
 * notification. The Pythinker adapter sits at the SDK layer, beneath the
 * TUI slash-command registry (`apps/pythinker-code/src/tui/commands/`),

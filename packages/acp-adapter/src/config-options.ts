 ...
 *     pythinker-code uses a single non-`'off'` level under the hood (default
 ...
 * reference's `_expand_llm_models` (`pythinker-cli/src/pythinker_cli/acp/server.py:441-468`)
 ...
 * capable model (e.g. `pythinker-coder`) to a non-thinking one (e.g.
 * `pythinker-plain`) causes the next `config_option_update` to omit the

/**
 * `AcpPyaos` — a {@link Pyaos} that bridges file reads/writes through the
 * ACP client (e.g. Zed's unsaved-buffer view of the workspace) and
 * delegates every other operation to an `inner` {@link Pyaos} (typically
 * a {@link LocalPyaos}).
 *
 * Why a separate class instead of an `if (acpAvailable) { ... }` branch
 * inside `LocalPyaos`? Because the SDK and the tooling code talk to a
 * single {@link Pyaos} reference, and dependency-inverting the FS bridge
 ...
 * we simply never wrap — tools observe a plain `LocalPyaos` and Phase 6
...
/**
 * `Pyaos` that routes `read*` / `write*` through the ACP reverse-RPC
 ...
 * (mirrors `LocalPyaos._resolvePath`'s public surface). If you need
 * cwd-relative resolution, route through `inner.normpath` first or use
 * `withCwd()` to bind a base.
 */
export class AcpPyaos implements Pyaos {
  constructor(
    private readonly conn: AgentSideConnection,
    private readonly sessionId: string,
    private readonly inner: Pyaos,
  ) {}
  ...
  /**
   * Return a fresh `AcpPyaos` wrapping the inner Pyaos's cwd-derived
   ...
   */
  withCwd(cwd: string): Pyaos {
    return new AcpPyaos(this.conn, this.sessionId, this.inner.withCwd(cwd));
  }

  withEnv(env: Record<string, string>): Pyaos {
    return new AcpPyaos(this.conn, this.sessionId, this.inner.withEnv(env));
  }
   ...
   * should be routed through a non-ACP Pyaos.
   */
  async readText(
    path: string,
    ...
    try {
      ...
      throw wrapPyaosError(`acp: readTextFile failed for ${rpcPath}`, err);
    }
  }
   ...
   * {@link LocalPyaos.readLines} so tools that depend on line terminators
   * (e.g. {@link ReadTool}, which renders CRLF endings) behave identically
   * whether the underlying Pyaos is local or ACP-bridged.
   */
  async *readLines(
    path: string,
   ...
   *
   * Returns `data.length` (chars) to match {@link LocalPyaos.writeText}'s
   * contract.
   */
  async writeText(
    path: string,
  ...
  private async acpWrite(path: string, content: string): Promise<void> {
    ...
    try {
      ...
      throw wrapPyaosError(`acp: writeTextFile failed for ${rpcPath}`, err);
    }
  }
...
/**
 * Build a `PyaosError` wrapping a raw RPC failure. We can't use the
 * `Error(message, { cause })` overload here because {@link PyaosError}'s
 * constructor only accepts `(message: string)` (see
 * `packages/pyaos/src/errors.ts`). Instead we synthesize the message
 ...
 */
function wrapPyaosError(prefix: string, cause: unknown): PyaosError {
  const causeMessage = cause instanceof Error ? cause.message : String(cause);
  const err = new PyaosError(`${prefix}: ${causeMessage}`);
  // Mutating `cause` after construction is the cheapest way to preserve
  // it without touching the pyaos package (denylist forbids edits there).
 ...
 * `PyaosError` by `readText` above. Message substring matching is intentionally

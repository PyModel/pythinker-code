/**
 * ACP → pythinker MCP server conversion.
 *
 * Translates ACP `McpServer[]` (per the ACP schema discriminated by
 * `type: 'http' | 'sse' | 'acp' | 'stdio'`) into pythinker's
 ...
 *
 *  - `http`  → pythinker `transport: 'http'` with headers projected from
 *              `Array<{name, value}>` to `Record<string, string>`.
 *  - `sse`   → pythinker `transport: 'sse'` with headers projected the same way.
 *  - `stdio` → pythinker `transport: 'stdio'` with env projected similarly.

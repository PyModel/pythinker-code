...
/**
 * Adapter-side wrapper around a {@link Session} from the Pythinker node SDK.
 *
...
export class AcpSession {
   ...
   * `~/.pythinker-code/skills/` and incurred permission prompts. Defaults
   ...
   * injecting the `<pythinker-skill-loaded>` block — see
...
/**
 * Map a Pythinker SDK error (raw `Error`, `PythinkerError`, or `PythinkerErrorPayload`)
 ...
 *
 * The pythinker-cli Python reference performs the same mapping at
 * `pythinker-cli/src/pythinker_cli/acp/session.py:218-247`; this is the TS port.
 */
 ...
 * `apps/pythinker-code/src/tui/commands/parse.ts` and `resolve.ts` to
...
/**
 * Inspect a {@link PythinkerErrorPayload} (as carried on `turn.ended`
 ...
 *  - A `PythinkerError` instance with a recognized `code` field.

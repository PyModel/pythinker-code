...
function makeHarnessWithModels(
  entries: ReadonlyArray<{
    ...
    protocol?: 'anthropic';
    providerType?: 'anthropic' | 'pythinker' | 'openai';
    supportEfforts?: readonly string[];
    defaultEffort?: string;
  }>,
): { harness: PythinkerHarness; getConfig: ReturnType<typeof vi.fn> } {
  ...
  return { harness: { getConfig } as unknown as PythinkerHarness, getConfig };
}
...
describe('buildModelOption', () => {
  ...
  it('treats `currentValue` as the bare base model id — Phase 15 keeps the snapshot suffix-free', () => {
    const models: readonly AcpModelEntry[] = [
      { id: 'pythinker-v2', name: 'Pythinker v2', thinkingSupported: true, supportEfforts: [], defaultThinkingEffort: 'on' },
    ];

    const option = buildModelOption(models, 'pythinker-v2');
    if (option.type !== 'select') {
      throw new Error('expected a SessionConfigSelect option');
    }
    expect(option.currentValue).toBe('pythinker-v2');
    expect(option.options.map((o) => ('value' in o ? o.value : ''))).toEqual(['pythinker-v2']);
  });
...
describe('buildSessionConfigOptions', () => {
  it('composes [model, thinking, mode] when current model supports thinking and calls getConfig exactly once', async () => {
    ...
    const { harness, getConfig } = makeHarnessWithModels([
      { id: 'pythinker-coder', model: 'kimi-for-coding', displayName: 'Pythinker Coder' },
    ]);

    const result = await buildSessionConfigOptions(harness, 'pythinker-coder', 'off', 'default');
  ...
  it('hides the thinking control for an unknown model on a Pythinker provider using the Anthropic protocol', async () => {
    const { harness } = makeHarnessWithModels([
      {
        id: 'custom',
        model: 'custom-anthropic-model',
  ...
  it('omits the thinking toggle when current model is non-thinking-supported', async () => {
    const { harness } = makeHarnessWithModels([
      { id: 'pythinker-coder', model: 'kimi-for-coding', displayName: 'Pythinker Coder' },
      { id: 'pythinker-plain', model: 'qwen-2.5-coder', displayName: 'Pythinker Plain' },
    ]);

    const result = await buildSessionConfigOptions(harness, 'pythinker-plain', 'off', 'default');
  ...
  it('reflects the thinking toggle currentValue from the explicit argument', async () => {
    ...
    const result = await buildSessionConfigOptions(harness, 'pythinker-coder', 'on', 'default');
    ...
    const { harness } = makeHarnessWithModels([
      {
        ...
        displayName: 'Pythinker Deep',
        capabilities: ['thinking', 'always_thinking'],
      },
    ]);

    const result = await buildSessionConfigOptions(harness, 'pythinker-deep', 'off', 'default');
  ...
  it('handles missing getConfig (partial-stub harness) by suppressing the toggle and shipping an empty model picker', async () => {
    const harness = {} as unknown as PythinkerHarness;

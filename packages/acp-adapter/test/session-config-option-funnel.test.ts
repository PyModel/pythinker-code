...
function makeHarness(session: Session): PythinkerHarness {
  return {
    ...
    getConfig: async () => ({
      providers: {},
      defaultModel: 'pythinker-coder',
      models: makeModelsMap([
        { id: 'pythinker-coder', name: 'Pythinker Coder', thinkingSupported: false },
        { id: 'pythinker-v2', name: 'Pythinker v2', thinkingSupported: false },
      ]),
    }),
...
describe('config_option_update wire-shape funnel', () => {
  it('unstable_setSessionModel emits one config_option_update with `model` currentValue updated', async () => {
    ...
    await client.unstable_setSessionModel({ sessionId, modelId: 'pythinker-v2' });
  ...
  it('setSessionConfigOption(thinking="on") emits one config_option_update with thinking toggle on', async () => {
    ...
    // is visible in the snapshot; default model resolves to pythinker-coder
    ...
    const harness = {
      ...
      getConfig: async () => ({
        providers: {},
        defaultModel: 'pythinker-coder',
        models: makeModelsMap([{ id: 'pythinker-coder', name: 'Pythinker Coder', thinkingSupported: true }]),
      }),
    } as unknown as PythinkerHarness;
    const { client, capturing, sessionId } = await openSession(harness);
    capturing.notifications.length = 0;
  ...
  it('all three input paths emit the SAME wire envelope (discriminator + option ids + per-option shape)', async () => {
    ...
    const viaModel = await envelopeFromPath((c, sid) =>
      c.unstable_setSessionModel({ sessionId: sid, modelId: 'pythinker-v2' }),
    );

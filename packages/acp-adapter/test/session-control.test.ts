...
function makeHarness(handle: FakeSessionHandle): PythinkerHarness {
  return {
    ...
    getConfig: async () => ({
      ...
      models: makeModelsMap([{ id: 'pythinker-coder', name: 'Pythinker Coder', thinkingSupported: false }]),
    }),
...
describe('AcpServer session/unstable_setSessionModel', () => {
  it('forwards modelId to Session.setModel exactly once + emits one config_option_update', async () => {
    ...
    await client.unstable_setSessionModel({ sessionId, modelId: 'pythinker-v2-something' });
  ...
  it('splits a `,thinking` suffix into a bare setModel + setThinking("high") call; snapshot model carries the base id', async () => {
    ...
    const harness = {
      ...
      getConfig: async () => ({
        providers: {},
        defaultModel: 'pythinker-v2-something',
        models: makeModelsMap([
          { id: 'pythinker-v2-something', name: 'Pythinker v2 something', thinkingSupported: true },
        ]),
      }),
    } as unknown as PythinkerHarness;
    const { client, capturing, sessionId } = await openSession(harness);
  ...
  it('rejects unknown sessionId with invalid_params and does not call setModel or emit notifications', async () => {
    ...
    await expect(
      client.unstable_setSessionModel({ sessionId: 'sess-unknown', modelId: 'pythinker-v2' }),
    ).rejects.toMatchObject({ code: -32602 });

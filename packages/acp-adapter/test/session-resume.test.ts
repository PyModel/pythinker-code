...
function makeHarness(opts: {
  ...
  return {
    ...
    // these to assemble configOptions. `pythinker-coder` opts in to thinking
    // via `capabilities: ['thinking']`; `pythinker-plain` stays off.
    getConfig: async () => ({
      providers: {},
...
describe('AcpServer.resumeSession', () => {
  ...
  it('returns configOptions matching the resumed session model + mode + thinking', async () => {
    ...
    //
    // We use pythinker-coder so the thinking option is rendered (pythinker-plain

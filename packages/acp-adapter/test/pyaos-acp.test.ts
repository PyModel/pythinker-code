/**
 * Unit tests for {@link AcpPyaos}. Uses a hand-rolled mock of
...
import { PyaosError, type Environment, type Pyaos, type PyaosProcess, type StatResult } from '@pymodel/pyaos';
...
/**
 * Minimal stub of an inner {@link Pyaos}. Records delegation; throws if
 * a non-pass-through method is called (defensive — those should never
 * land here in the bridging layer).
 */
interface MockInnerPyaos extends Pyaos {
  __spy: {
...
function makeMockInner(opts?: { pathClass?: 'posix' | 'win32' }): MockInnerPyaos {
  ...
  const inner: MockInnerPyaos = {
    ...
    withCwd: (cwd: string) => {
      ...
      // returned AcpPyaos still bridges through the same conn.
    ...
    readText: async (path: string) => {
      // Used to verify that AcpPyaos.readText does NOT fall back to inner.
...
describe('AcpPyaos', () => {
  describe('readText', () => {
    it('forwards path and sessionId to conn.readTextFile, returning response.content', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', inner);

      const result = await pyaos.readText('/a.ts');
    ...
    it('wraps RPC errors in PyaosError with cause set', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());

      await expect(pyaos.readText('/x.ts')).rejects.toMatchObject({
        name: 'PyaosError',
      });
      await expect(pyaos.readText('/x.ts')).rejects.toBeInstanceOf(PyaosError);
      // Verify cause is preserved.
      try {
        await pyaos.readText('/x.ts');
    ...
    it('uses win32-native separators for ACP file RPC paths', async () => {
      ...
      await pyaos.readText('G:/python-code/render_with_mult_gpu/README.md');
      await pyaos.writeText('G:/python-code/render_with_mult_gpu/README.md', 'updated');
  ...
  describe('readBytes', () => {
    it('delegates to inner.readBytes (binary reads bypass ACP text RPC)', async () => {
      ...
      const buf = await pyaos.readBytes('/img.png', 4);
    ...
    it('forwards omitted n to inner unchanged', async () => {
      ...
      const buf = await pyaos.readBytes('/img.png');
  ...
  describe('readLines', () => {
    ...
    it('yields each line of "a\\nb\\nc" with terminators preserved', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      expect(await collect(pyaos.readLines('/a.ts'))).toEqual(['a\n', 'b\n', 'c']);
    });
    ...
    it('yields nothing for an empty file', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      expect(await collect(pyaos.readLines('/a.ts'))).toEqual([]);
    });
  });
  ...
  describe('writeText', () => {
    it('forwards content to conn.writeTextFile and returns char count', async () => {
      const conn = makeMockConn({});
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      const n = await pyaos.writeText('/a.ts', 'hello');
      expect(n).toBe(5);
    ...
    it('append mode merges with existing content', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      const n = await pyaos.writeText('/a.ts', 'new', { mode: 'a' });
    ...
    it('append mode treats a resourceNotFound read error as empty existing content', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      const n = await pyaos.writeText('/missing.ts', 'fresh', { mode: 'a' });
      expect(n).toBe(5);
      expect(conn.writeCalls).toEqual([
    ...
    it('append mode does not treat a loose "not found" message as missing file', async () => {
      ...
      await expect(pyaos.writeText('/tmp/not found/file.txt', 'fresh', { mode: 'a' }))
        .rejects.toBeInstanceOf(PyaosError);
      expect(conn.writeCalls).toEqual([]);
    });
    ...
    it('append mode rethrows non-not-found read errors and does NOT issue a write', async () => {
      ...
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      await expect(pyaos.writeText('/a.ts', 'new', { mode: 'a' })).rejects.toBeInstanceOf(
        PyaosError,
      );
    ...
    it('wraps writeTextFile RPC errors in PyaosError with cause set', async () => {
      ...
      await expect(pyaos.writeText('/a.ts', 'hello')).rejects.toBeInstanceOf(PyaosError);
      try {
        await pyaos.writeText('/a.ts', 'hello');
      } catch (err) {
        expect((err as Error & { cause?: unknown }).cause).toBe(rpcErr);
  ...
  describe('writeBytes', () => {
    it('forwards utf8-decoded content via conn.writeTextFile, returns byte count', async () => {
      const conn = makeMockConn({});
      const pyaos = new AcpPyaos(conn.asConn(), 's1', makeMockInner());
      const n = await pyaos.writeBytes('/a.ts', Buffer.from('hi'));
      expect(n).toBe(2);
  ...
  describe('withCwd', () => {
    it('returns an AcpPyaos that still bridges through the same conn', async () => {
      const conn = makeMockConn({
        readHandler: async () => ({ content: 'BRIDGED' }),
      });
      const inner = makeMockInner();
      const pyaos = new AcpPyaos(conn.asConn(), 's1', inner);
      const child = pyaos.withCwd('/new/cwd');

      expect(child).toBeInstanceOf(AcpPyaos);
      // Reading on the wrapped child must still hit the mocked ACP conn,
      // NOT the inner Pyaos's local readText.
  ...
  describe('withEnv', () => {
    it('returns an AcpPyaos that delegates env to inner and keeps the ACP bridge', async () => {
      const conn = makeMockConn({
        readHandler: async () => ({ content: 'BRIDGED' }),
      });
      const inner = makeMockInner();
  ...
  describe('pass-through delegation', () => {
    it('delegates pathClass, normpath, gethome, getcwd to inner', () => {
      ...
      expect(pyaos.pathClass()).toBe('posix');
      expect(pyaos.normpath('/foo')).toBe('/foo');
      expect(pyaos.gethome()).toBe('/home/mock');
      expect(pyaos.getcwd()).toBe('/cwd');
    ...
    it('delegates chdir, stat, mkdir to inner', async () => {
      ...
      await pyaos.chdir('/x');
      await pyaos.stat('/y', { followSymlinks: false });
      await pyaos.mkdir('/z', { parents: true });
    ...
    it('delegates iterdir and glob to inner', async () => {
      ...
      for await (const _ of pyaos.iterdir('/d')) {
        // no-op
      }
      for await (const _ of pyaos.glob('/d', '**/*.ts', { caseSensitive: true })) {
        // no-op
      }
    ...
    it('delegates exec and execWithEnv to inner', async () => {
      ...
      await pyaos.exec('ls', '-la');
      await pyaos.execWithEnv(['env'], { FOO: 'bar' });
  ...
  describe('identity', () => {
    it('exposes a wrapping name and the inner osEnv', () => {
      ...
      const inner = makeMockInner();
      const pyaos = new AcpPyaos(conn.asConn(), 's1', inner);
      expect(pyaos.name).toBe('acp(mock-inner)');
      expect(pyaos.osEnv).toBe(inner.osEnv);
    });
  });
});

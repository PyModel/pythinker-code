...
Put TUI product behavior in `apps/pythinker-code/src/tui` (composition, subclassing, existing host callbacks). Change this package only when the public API cannot express the behavior.
...
This package runs `node --test` via `pnpm --filter @pymodel/pi-tui test`. The repo-root vitest run does not execute these tests.

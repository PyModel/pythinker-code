...
//
// Demonstrate the 4 core queries against ~/.pythinker-code with timing.
...
const HOME = path.join(os.homedir(), '.pythinker-code');
...
async function main() {
  ...
  const stats = await store.ingestPythinkerCode(HOME);

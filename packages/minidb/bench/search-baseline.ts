...
//
// "Without minidb" search baseline over the SAME real ~/.pythinker-code sessions
// used by import-pythinker-code.ts. Loads every session's extracted text into an
...
// range). Paired with import-pythinker-code.ts, this shows what minidb's indexes
// buy you on real data.
//
// Run:  node --import tsx bench/search-baseline.ts [--data ~/.pythinker-code]
...
const DATA = path.resolve(arg('data', path.join(os.homedir(), '.pythinker-code')));
const FULL = argv.includes('--full'); // also index full tool results (matches import-pythinker-code --full)
...
async function main() {
  ...
  {
    const { value: res, ms } = med(() => docs.filter((d) => d.workspaceName === 'pythinker-code-dev-1'));
    console.log(`  naive lookup (workspace=pythinker-code-dev-1) -> ${res.length} in ${ms.toFixed(2)} ms`);
  }

// bench/import-pythinker-code.js
//
// Import all workspaces + sessions from ~/.pythinker-code into minidb and build a
// full-text index over the session content, then measure import + search speed.
//
// Run:  node bench/import-pythinker-code.js [--data ~/.pythinker-code] [--out <dir>]
...
const OUT = path.resolve(arg('out', path.join(os.tmpdir(), 'minidb-pythinker-code-' + Date.now())));
...
async function main() {
  ...
  const byWs = db.findEq('byWorkspace', 'pythinker-code-dev-1');
  console.log(`  index lookup (workspace=pythinker-code-dev-1) -> ${byWs.length} in ${(performance.now() - w0).toFixed(2)} ms`);
